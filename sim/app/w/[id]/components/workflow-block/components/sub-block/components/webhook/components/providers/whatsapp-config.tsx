...
export function WhatsAppConfig({
  ...
  return (
    <div className="space-y-4">
      ...
      {testResult?.success && (
        <Alert className="border-green-500/50 text-green-700 dark:border-green-500/60 dark:text-green-400 [&>svg]:text-green-500 dark:[&>svg]:text-green-400">
          <CheckCircle className="h-4 w-4" />
          <AlertTitle className="text-green-800 dark:text-green-300">Webhook Verified!</AlertTitle>
          <AlertDescription>
            WhatsApp successfully verified your webhook endpoint. You should now be able to receive
            messages.
          </AlertDescription>
        </Alert>
      )}
