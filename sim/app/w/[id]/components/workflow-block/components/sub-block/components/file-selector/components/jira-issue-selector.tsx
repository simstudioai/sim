  ...
  const fetchIssues = useCallback(   
    async (searchQuery?: string) => {
      ...
      try {
        ...
        // Simply fetch issues directly using the endpoint
        const response = await fetch('/api/auth/oauth/jira/issues', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        const data = await response.json()
        console.log(`Received ${data.issues?.length || 0} issues from API`)
        setIssues(data.issues || [])

        // If we have a selected issue ID, find the issue info
        if (selectedIssueId) { 
          const issueInfo = data.issues.find((issue: JiraIssueInfo) => issue.id === selectedIssueId)
          if (issueInfo) {
