...
export function TeamManagement() {
  ...
  const handleReduceSeats = async () => {
    if (!session?.user || !activeOrganization || !subscriptionData || subscriptionData.length === 0) return
    ...
    if (totalCount >= currentSeats) {
      setError(`You have ${totalCount} active members/invitations. Please remove members or cancel invitations before reducing seats.`)
      return
    }
            ...
            <div className="rounded-md border p-4">
              ...
              {isLoadingSubscription ? (
                ...
                <>
                  ...
                  <Progress 
                    value={((activeOrganization.members?.length || 0) + 
                          (activeOrganization.invitations?.filter((inv: any) => inv.status === 'pending').length || 0)) / 
                          (subscriptionData[0]?.seats || 1) * 100} 
                    className="h-2"
                  />
                ...
                <div className="text-sm text-muted-foreground space-y-2">
                  ...
                  <p>This might happen if your subscription was created for your personal account
                  but hasn't been properly transferred to the organization.</p>
                  <Button 
                    variant="outline" 
                    size="sm"
                    onClick={() => {
              ...
              <div className="divide-y">
                {activeOrganization.members?.map((member: any) => (
                  <div key={member.id} className="p-4 flex items-center justify-between">
                    ...
                    {isAdminOrOwner && member.role !== 'owner' && member.user?.email !== session?.user?.email && (
                      <Button 
                        variant="outline" 
                        size="sm" 
          ...
          <div className="rounded-md border p-4 space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <div className={`w-2 h-2 rounded-full ${
                        ...
                        subscriptionData.status === 'trialing' ? 'bg-amber-500' : 'bg-red-500'
                      }`}></div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      ...
                      {subscriptionData.periodEnd && (
                        <div>Next billing date: {new Date(subscriptionData.periodEnd).toLocaleDateString()}</div>
                      )}
                      {subscriptionData.trialEnd && (
                        <div>Trial ends: {new Date(subscriptionData.trialEnd).toLocaleDateString()}</div>
                      )}
            ...
            {!isAdminOrOwner && (
              <div>
                ...
                <div className="text-sm">
                  You are a <span className="capitalize font-medium">{userRole}</span> of this workspace.
                  {userRole === 'member' && (
      ...
      >
        <DialogContent>
          ...
          <div className="py-4">
            <div className="flex items-center space-x-2">
              <input 
                ...
                onChange={(e) => setRemoveMemberDialog({ 
                  ...removeMemberDialog, 
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setRemoveMemberDialog({ open: false, memberId: '', memberName: '', shouldReduceSeats: false })}
            >
              Cancel
            </Button>
            <Button
