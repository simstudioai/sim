...
describe('refreshStoredOAuth', () => {
  it('rotates the pair on the server and persists what came back', async () => {
    writeCredentialsProfile('default', {
      kind: 'oauth',
      oauth: { accessToken: 'old-access', refreshToken: 'old-refresh', expiresAt: 1 },
    })
    ...
    expect(readStoredCredential('default')).toEqual({
      kind: 'oauth',
      oauth: { accessToken: 'new-access', refreshToken: 'new-refresh', expiresAt: next.expiresAt },
    })
  })
  ...
  it('adopts a rotation another process already wrote instead of presenting the dead token', async () => {
    writeCredentialsProfile('default', {
      kind: 'oauth',
      oauth: { accessToken: 'winner-access', refreshToken: 'winner-refresh', expiresAt: 99 },
    })
  ...
  it('names the remedy when the server no longer honours the refresh token', async () => {
    ...
    await expect(
      refreshStoredOAuth(PROFILE, { accessToken: 'a', refreshToken: 'r', expiresAt: 1 })
    ).rejects.toThrow('Run: sim login --profile default')
