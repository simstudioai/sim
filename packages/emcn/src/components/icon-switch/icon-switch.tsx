...
export function IconSwitch<T extends string>({
  ...
  return (
    <div
      ...
      aria-label={ariaLabel}
      className={cn(
        'inline-flex w-fit shrink-0 items-center gap-0.5 rounded-lg border border-[var(--border)] bg-[var(--surface-2)] p-[2px]',
        disabled && 'opacity-50',
        className
      )}
    >
      {options.map((option) => {
        ...
        const input = (
          <input
            ...
            className='peer m-0 size-[16px] cursor-pointer appearance-none rounded-[calc(theme(borderRadius.lg)-2px-var(--border-width,1px))] bg-transparent transition-colors checked:bg-[var(--surface-active)] focus-visible:outline focus-visible:outline-1 focus-visible:outline-[var(--text-icon)] disabled:cursor-not-allowed'
          />
        )
