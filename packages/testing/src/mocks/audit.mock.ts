...
/**
 * Faithful copy of `AuditAction` from `@sim/audit` (packages/audit/src/types.ts).
 * Copied rather than imported: `@sim/audit` depends on `@sim/testing`, so an import
 * would make the workspace graph cyclic. Keep it in sync when an action is added.
 */
const AuditAction = {
...
/**
 * Faithful copy of `AuditResourceType` from `@sim/audit` (packages/audit/src/types.ts).
 */
 ...
 *
 * `mockRecordAuditOnce` resolves `undefined` by default, like the real awaited insert.
 *
 * @example
 * ```ts
 * import { auditMockFns } from '@sim/testing/mocks/audit.mock'
 *
 * expect(auditMockFns.mockRecordAudit).toHaveBeenCalledWith(
 *   expect.objectContaining({ action: 'folder.deleted' })
 * )
 * auditMockFns.mockRecordAudit.mockImplementationOnce(() => {
...
export const auditMockFns = {
  ...
  mockRecordAuditOnce: vi.fn(async (_id: string, _params: unknown): Promise<void> => undefined),
}

/**
 * Static mock module for `@sim/audit`. The enums carry the real values, so tests
 * assert the production action strings (`'folder.deleted'`), never invented ones.
 *
 * @example
 * ```ts
