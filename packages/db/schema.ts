...
export const copilotChatMessages = pgTable(
  ...
  (table) => ({
    chatMessageUnique: uniqueIndex('copilot_chat_messages_chat_message_unique').on(
      table.chatId,
      table.messageId
    ),
    chatCreatedAtIdx: index('copilot_chat_messages_chat_created_at_idx')
      .on(table.chatId, table.createdAt, table.id)
      .where(sql`${table.deletedAt} IS NULL`),
    chatStreamIdx: index('copilot_chat_messages_chat_stream_idx')
