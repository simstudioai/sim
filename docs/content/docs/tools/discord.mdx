...
In Sim Studio, the Discord integration enables your agents to access and leverage your organization's Discord servers. Agents can retrieve information from Discord channels, search for specific content, and send messages. This allows your workflows to integrate with your Discord communities, automate notifications, and create interactive experiences.
...
| `embed` | object | No | Optional rich embed for the message |
| `serverId` | string | Yes | The Discord server ID \(guild ID\) |
| `botToken` | string | No | The bot token for authentication \(required if credential not provided\) |
| `credential` | string | No | Discord OAuth credential ID \(required if botToken not provided\) |
