...
Sim supports hosted model access and bring-your-own-key connections for provider accounts. Self-hosted deployments can use [locally configured Ollama models](https://docs.sim.ai/platform/self-hosting/environment-variables). Model and deployment availability can vary by plan and environment, so buyers should verify current terms for their intended setup. The [BYOK and multi-model agent builder guide](https://www.sim.ai/library/byok-multi-model-ai-agent-builder) covers the tradeoffs behind provider choice.
