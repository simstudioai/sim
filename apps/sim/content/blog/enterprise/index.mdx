...
faq:
  ...
  - q: "What identity providers does Sim support for SSO, and what happens when an employee is deprovisioned?"
    a: "Sim integrates with Okta, Microsoft Entra ID (formerly Azure AD), Google Workspace, OneLogin, Auth0, JumpCloud, Ping Identity, ADFS, and other identity providers that support SAML 2.0 or OIDC. With SSO alone, IdP deprovisioning blocks future SSO authentication but does not remove Sim membership or revoke active Sim sessions, so offboarding must also remove or suspend access in Sim."
---
