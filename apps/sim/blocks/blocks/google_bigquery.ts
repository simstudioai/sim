...
Return ONLY the SQL query - no explanations, no quotes, no extra text.`,
...

    {
      ...
      canonicalParamId: 'selected_datasetId',
      serviceId: 'google-bigquery',
...
Return ONLY the JSON array - no explanations, no wrapping, no extra text.`,
  ...
  tools: {
    ...
    config: {
      ...
      params: (params) => {
        const { oauthCredential, rows, maxResults, selected_datasetId, selected_tableId, ...rest } =
          params
        return {
          ...rest,
          oauthCredential,
          datasetId: selected_datasetId,
  ...
  inputs: {
    ...
    selected_datasetId: { type: 'string', description: 'BigQuery dataset ID' },
    selected_tableId: { type: 'string', description: 'BigQuery table ID' },
