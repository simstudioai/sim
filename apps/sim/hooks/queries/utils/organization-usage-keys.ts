...
export const organizationUsageKeys = {
  ...
  overviews: (organizationId: string) =>
    [...organizationUsageKeys.all(organizationId), 'overview'] as const,
  /**
   * `window` last: it is the one segment the overview's `placeholderData` may cross, so
   * the scope's identity — organization and workspace — is a plain prefix that a
   * retained overview can never cross.
   */
  overview: (organizationId: string, window: OrganizationUsageWindowKey, workspaceId?: string) =>
    [...organizationUsageKeys.overviews(organizationId), workspaceId ?? '', window] as const,
