...
import { tableKeys, useDeleteColumn, useRenameTable } from '@/hooks/queries/tables'
...
describe('useDeleteColumn optimistic update', () => {
  it('removes column from schema cache, strips its width, and snapshots rows for rollback', async () => {
...
describe('useRenameTable optimistic update', () => {
  it('updates the cached table name and rolls back on error', async () => {
    const original = { id: TABLE_ID, name: 'old', schema: { columns: [] } }
    setCache(tableKeys.detail(TABLE_ID), original)

    const hook = useRenameTable(WORKSPACE_ID)
    const ctx = await hook.onMutate?.({ tableId: TABLE_ID, name: 'new' })

    expect(getCache<{ name: string }>(tableKeys.detail(TABLE_ID))?.name).toBe('new')

    hook.onError?.(new Error('boom'), { tableId: TABLE_ID, name: 'new' }, ctx)
