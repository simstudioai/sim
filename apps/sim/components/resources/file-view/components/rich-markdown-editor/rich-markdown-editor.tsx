...
export const RichMarkdownEditor = memo(function RichMarkdownEditor({
  ...
  const previewContextKey = streaming?.contextKey
  ...
  const {
    ...
    isAgentEditing: streaming?.isAgentEditing,
    onDirtyChange,
    onSaveStatusChange,
    saveRef,
    discardRef,
  ...
  return (
    <LoadedRichMarkdownEditor
      ...
      userName={userName}
      autoFocus={autoFocus}
      streamIsIncremental={streaming?.isIncremental}
      streamOperation={streaming?.operation}
      disableStreamingAutoScroll={streaming?.disableAutoScroll ?? false}
...
export function LoadedRichMarkdownEditor({
  ...
  const { source, navigate } = useResourceOfKind('file')
  /** Read through a ref so the editor's link handler never re-binds on a host re-render. */
  const navigateRef = useRef(navigate)
  navigateRef.current = navigate
  /** `null` on a share, where there is no workspace to upload into or mention from. */
  const workspaceId = fileWorkspaceId(source)
  const resolveImageSrc = useCallback(
    (src: string | undefined) => fileImageSrc(source, src),
    [source]
  )
  ...
  cloneHostedImageRef.current = (imgSrcs, at) => {
    ...
    const matchedAttrs = findHostedImageAttrs(editor.state.doc, imgSrcs, resolveImageSrc)
  ...
  useEditorMentions(editor, workspaceId ?? undefined, { navigable: true, disableTagging })
