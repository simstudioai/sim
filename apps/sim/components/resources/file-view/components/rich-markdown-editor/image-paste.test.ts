...
import { type ResourceSource, shareSource, workspaceSource } from '@/resources'
...
/** The display-URL rewrite the editor renders embeds through, per scope. */
function imageResolver(source: ResourceSource<'file'>) {
  return { resolveImageSrc: (src: string | undefined) => fileImageSrc(source, src) }
}

const WORKSPACE_RESOLVER = imageResolver(
  workspaceSource({ kind: 'file', workspaceId: 'ws-1', resourceId: 'file-1' })
)
const SHARE_RESOLVER = imageResolver(
  shareSource({
    kind: 'file',
    token: 'tok_1',
    grantId: 'tok_1',
