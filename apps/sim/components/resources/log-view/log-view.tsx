...
export type LogViewTab = 'overview' | 'trace'
...
export interface LogViewProps {
  ...
  host: ResourceHost
  /**
   * The run to render. Supplied rather than resolved from `source` because both
   * consumers already hold it — the logs page from its list query, the panel
   * from `useLogDetail` — and refetching by id would double every fetch.
   */
  log: WorkflowLogRow
  /**
   * Whether this viewer may see a run's execution internals: trace spans, the
   * frozen-canvas snapshot, and workflow input/output payloads. An enterprise
   * permission group can withhold these from someone who may otherwise read the
   * log, so it is not derivable from `grants.write` or `grants.run`.
   *
   * Required, not optional-defaulting-to-true: a consumer that forgets it must
   * fail to compile rather than silently reveal payloads the group hid. It stays
   * a prop instead of a `ResourceGrants` field because it is the only per-kind
   * capability in the tree and no other view has a use for it.
   */
  showExecutionInternals: boolean
  /**
   * Active tab, when the host owns the URL and deep-links it. Ignored unless
   * `hostOwnsUrl(host)` — an embedded host cannot drive (or pollute) this
   * through the address bar even if a consumer passes it.
   */
  tab?: LogViewTab
  /** Tab the viewer selected. Only called for a host that owns the URL. */
  onTabChange?: (tab: LogViewTab) => void
  /** The resolved tab, after a hidden Trace tab folds back to Overview. */
   ...
   * the handoff has nowhere to land and stays inert.
   */
  onNavigate?: (path: string) => void
}

/**
 * Renders one workflow run — status, cost ledger, input/output payloads, files,
 * and the execution trace — from whichever address its {@link ResourceSource}
 * carries. Mounted by the logs page's details panel and by the chat resource
 * panel; neither wraps it.
 */
export function LogView({
  ...
  const setActiveTab = useCallback(
    (next: LogViewTab) => {
      if (urlOwned) onTabChange?.(next)
      else setLocalTab(next)
    },
    [urlOwned, onTabChange]
  )
  ...
  const workspaceId = source.via === 'workspace' ? source.workspaceId : null
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- reset the tab only when switching logs, not when the setter identity changes
  }, [log.id])
  ...
  const showTraceTab = showExecutionInternals && isLikelyExecution
  ...
  const resolvedTab: LogViewTab = activeTab === 'trace' && !showTraceTab ? 'overview' : activeTab
  ...
  const handleTroubleshoot = useCallback(() => {
    ...
    if (!workspaceId || !onNavigate) return
    if (MothershipHandoffStorage.store({ message, contexts: [context] }, workspaceId)) {
      onNavigate(`/workspace/${workspaceId}/home`)
    }
  }, [log.executionId, log.workflow?.name, workspaceId, onNavigate])
    ...
    <>
      <div className='mt-4 flex min-h-0 flex-1 flex-col'>
        <ChipModalTabs
          ...
          value={resolvedTab}
          onChange={(v) => setActiveTab(v as LogViewTab)}
        />
        ...
        {resolvedTab === 'overview' && (
          <div ref={scrollAreaRef} className='mt-4 min-h-0 flex-1 overflow-y-auto'>
            <div className='flex flex-col gap-2.5 pb-4'>
              ...
              {/* Workflow Output */}
              {isWorkflowExecutionLog && workflowOutput && showExecutionInternals && (
                <div className='flex flex-col gap-1.5 rounded-md border border-[var(--border)] bg-[var(--surface-2)] px-2.5 py-2 dark:bg-transparent'>
                  <span
                    className={cn(
                      'text-caption',
              ...
              {log.files && log.files.length > 0 && (
                <FileCards files={log.files} isExecutionFile onNavigate={onNavigate} />
              )}
