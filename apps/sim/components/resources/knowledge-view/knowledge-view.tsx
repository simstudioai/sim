...
import type { BreadcrumbItem, ResourceAction } from '@/components/resource'
...
import {
  ...
  DocumentContextMenu,
  RenameDocumentModal,
} from '@/components/resources/knowledge-view/components'
import {
  DocumentList,
  type KnowledgeDocumentList,
  type KnowledgeEnabledFilter,
} from '@/components/resources/knowledge-view/components/document-list'
import { DocumentTagsModal } from '@/components/resources/knowledge-view/components/document-tags-modal'
import type { TagFilterEntry } from '@/components/resources/knowledge-view/components/tag-filter-panel'
...
import { useKnowledgeListState } from '@/hooks/kb/use-knowledge-list-state'
...
export interface KnowledgeViewProps {
  ...
  host: ResourceHost
  /**
   * The base's name if the host already knows it — a title hint, not an
   * identity. Lets both hosts paint the header before the base loads instead of
   * flashing a placeholder; the fetched name replaces it as soon as it arrives.
   */
...
export function KnowledgeView({
  ...
  const id = knowledgeResourceId(source) ?? ''
   ...
   */
  if (unavailable) {
    return <DocumentList source={source} grants={grants} host={host} list={list} />
  }
