...
/**
 * A parsed segment that interrupts the markdown flow and is drawn as its own
 * card — an `<options>` list, a `<credential>` prompt, a `<question>`. Text and
 * inline resource references are not here: those fold back into the surrounding
 * markdown.
 */
export type SpecialTagSegment = Exclude<
  ContentSegment,
...
interface ChatContentProps {
   ...
   * own. Those cards — credentials, usage upgrades, workspace-resource chips —
   * need workspace identity and permissions, which a chat mounted on an
   * anonymous share page has none of; injecting them keeps ONE markdown
   * renderer serving both surfaces instead of a privileged import that would
   * throw off-workspace.
   *
   * Pass a stable component reference, not an inline arrow: this component is
   * memoized and re-renders on every streamed token.
   *
   * Omitted, the special segments are skipped. That is the correct reading for
   * a surface with no agent protocol behind it — a chat module runs a user's
...
function ChatContentInner({
  ...
  type RenderGroup =
    ...
    | { kind: 'block'; segment: SpecialTagSegment; index: number }
