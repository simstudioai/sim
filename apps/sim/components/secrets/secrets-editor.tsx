...
export interface SecretRowAccess {
  canEdit: boolean
  canReveal: boolean
...
interface SecretsEditorProps {
  ...
  canCreate: boolean
  rowAccess?: ReadonlyMap<string, SecretRowAccess>
  personal?: {
    variables?: Record<string, { key: string; value: string }>
    save: (variables: Record<string, string>) => Promise<unknown>
  }
  save: (changes: SecretChanges) => Promise<unknown>
  isLoading: boolean
  isSaving: boolean
  /** Supports the existing workspace credential-picker deep link. */
  workspaceId?: string
}
...
export function SecretsEditor({
  ...
  sectionLabel,
  variables,
  canCreate: canCreateWorkspaceSecret,
  rowAccess,
  personal,
  save,
  isLoading,
  ...
  const filteredWorkspaceEntries = useMemo(() => {
    ...
    return entries.filter(
      ([key]) =>
        ...
        Boolean(rowAccess?.get(key)?.description?.toLowerCase().includes(term))
    )
  }, [workspaceVars, searchTerm, rowAccess])
  ...
  const handleWorkspaceKeyRename = (currentKey: string, currentValue: string) => {
    ...
    if (validateEnvVarKey(newKey) || Object.hasOwn(workspaceVars, newKey)) {
      toast.error('Use a valid, unique secret name')
      return
    }
  ...
  const handleSave = async () => {
    ...
    for (const row of newWorkspaceRows) {
      if (row.key && row.value) {
        setRecordValue(mergedWorkspaceVars, row.key, row.value)
      }
    }
    ...
    for (const [k, v] of Object.entries(after)) {
      if (!Object.hasOwn(before, k) || before[k] !== v) {
        setRecordValue(toUpsert, k, v)
      }
    }

    for (const k of Object.keys(before)) {
      if (!Object.hasOwn(after, k)) toDelete.push(k)
    }
      ...
      >
        {!isLoading && (
          <div className='flex flex-col gap-7'>
              ...
              <SettingsSection label={sectionLabel}>
                <div className={`${GRID_COLS} gap-y-2`}>
                  {(searchTerm.trim()
                    ...
                    const access = rowAccess?.get(key)
                    const canEditRow = canCreateWorkspaceSecret && (access?.canEdit ?? true)
                    const canRevealRow = access?.canReveal ?? true
                    return (
                      <WorkspaceVariableRow
                        ...
                        hasCredential={Boolean(access?.detailsHref)}
                        canEdit={canEditRow}
                        ...
                        onViewDetails={access?.detailsHref ? handleViewDetails : undefined}
                      />
                    )
                  })}
            ...
            {personal && (!searchTerm.trim() || filteredEnvVars.length > 0) && (
