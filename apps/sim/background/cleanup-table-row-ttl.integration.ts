/**
 * Destructive integration tests against the provisioned, disposable TEST_DATABASE_URL database
 * (the integration setup points DATABASE_URL at it too). All worker SQL, table transactions,
 * schema reads, and row-count triggers run for real.
 */
...
/** `db:push` never installs the migration-only row-count triggers these assertions depend on. */
