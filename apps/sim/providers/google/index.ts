...
/**
 * Creates streaming execution result
 */
  ...
  return {
    ...
    execution: {
      ...
      output: {
        ...
        cost: state?.cost ?? { input: 0, output: 0, total: 0 },
      },
      logs: [],
      metadata: {
        startTime: providerStartTimeISO,
  ...
  executeRequest: async (
    ...
    try {
      ...
      if (request.responseFormat && !tools?.length) {
        ...
        config.responseSchema = cleanSchemaForGemini(request.responseFormat.schema) as Schema
      ...
      if (tools?.length) {
        ...
        if (filteredTools?.length) {
          config.tools = [{ functionDeclarations: filteredTools as FunctionDeclaration[] }]
            ...
            toolConfig = {
              functionCallingConfig: {
                mode:
                  ...
                  }[preparedToolConfig.functionCallingConfig.mode] ??
      ...
      if (shouldStream) {
        ...
        const streamGenerator = await ai.models.generateContentStream({ model, contents, config })
      ...
      if (functionCalls?.length) {
        ...
        while (state.iterationCount < MAX_TOOL_ITERATIONS) {
          ...
          const nextConfig = buildNextConfig(config, state, forcedTools, request)
          ...
          if (request.stream) {
            ...
            // Capture accumulated cost before streaming (like OpenAI pattern)
