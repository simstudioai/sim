  ...
  socket.on('variable-update', async (data) => {
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        if (!workflowRecord) {
          // Workflow was deleted - this is a normal race condition in collaborative editing
          logger.debug(
        ...
        if (!variables[variableId]) {
          // Variable was deleted - this is a normal race condition in collaborative editing
          logger.debug(
      ...
      if (updateSuccessful) {
        ...
        // Variable was deleted - notify client that operation completed (but didn't update anything)
        socket.emit('operation-failed', {
          operationId,
          error: 'Variable no longer exists',
          retryable: false, // No point retrying for deleted variables
        })
      }
    } catch (error) {
      ...
      // Legacy operation-error for backward compatibility
      socket.emit('operation-error', {
