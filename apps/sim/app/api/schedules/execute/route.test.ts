...
describe('Scheduled Workflow Execution API Route', () => {
  ...
  it('reconciles stale database carriers while Trigger.dev is the active backend', async () => {
    const claimedAt = new Date('2025-01-01T00:00:00.000Z')
    orderByLimitMock.mockResolvedValueOnce([
      {
        id: 'old-database-job-id',
        status: 'processing',
        payload: {
          scheduleId: 'schedule-1',
          workflowId: 'workflow-1',
    ...
    dbChainMockFns.returning.mockResolvedValueOnce([{ id: 'old-database-job-id' }])

    await runScheduleTick('test-request-id')

    expect(mockShouldExecuteInline).toHaveReturnedWith(false)
    expect(dbChainMockFns.set).toHaveBeenCalledWith(
  ...
  it('migrates an unclaimed database carrier before Trigger enqueue', async () => {
    const claimedAt = new Date('2025-01-01T00:00:00.000Z')
    const schedule = { ...SINGLE_SCHEDULE[0], lastQueuedAt: claimedAt }
    queueTableRows(mockAsyncJobs, [
      {
        id: 'persisted-job-id',
        status: 'pending',
        attempts: 0,
    ...
    dbChainMockFns.returning
      ...
      .mockResolvedValueOnce([{ id: 'persisted-job-id' }])
      .mockResolvedValueOnce([{ id: 'schedule-1' }])
    ...
    expect(dbChainMockFns.set).toHaveBeenCalledWith(
      expect.objectContaining({
        status: 'cancelled',
        error: expect.stringContaining('Migrated unclaimed schedule carrier'),
      })
    )
    expect(dbChainMockFns.set).toHaveBeenCalledWith(expect.objectContaining({ lastQueuedAt: null }))
    expect(mockEnqueue).not.toHaveBeenCalled()
    expect(mockResolveSystemBillingAttribution).not.toHaveBeenCalled()
    expect(dbChainMockFns.execute).toHaveBeenCalledOnce()
  })

  it('blocks Trigger enqueue when the deterministic database carrier is terminal', async () => {
    const claimedAt = new Date('2025-01-01T00:00:00.000Z')
    const schedule = { ...SINGLE_SCHEDULE[0], lastQueuedAt: claimedAt }
    queueTableRows(mockAsyncJobs, [
      {
  ...
  it('keeps an accounted terminal database carrier as a Trigger fence', async () => {
    const claimedAt = new Date('2025-01-01T00:00:00.000Z')
    const schedule = { ...SINGLE_SCHEDULE[0], lastQueuedAt: claimedAt }
    queueTableRows(mockAsyncJobs, [
      {
        ...
        status: 'completed',
        attempts: 1,
        metadata: { scheduleReconciled: true },
        payload: {
          scheduleId: 'schedule-1',
          workflowId: 'workflow-1',
          executionId: 'execution-1',
  ...
  it('allows Trigger enqueue only for an explicitly migrated database carrier', async () => {
    const claimedAt = new Date('2025-01-01T00:00:00.000Z')
    const schedule = { ...SINGLE_SCHEDULE[0], lastQueuedAt: claimedAt }
    queueTableRows(mockAsyncJobs, [
      {
        ...
        status: 'cancelled',
        attempts: 0,
        metadata: { scheduleReconciled: true, scheduleCarrierMigrated: true },
        payload: {
          scheduleId: 'schedule-1',
          workflowId: 'workflow-1',
          now: claimedAt.toISOString(),
    ...
    expect(mockResolveSystemBillingAttribution).toHaveBeenCalledOnce()
  })
