...
mockSettleClientWorkflowToolExecution.mockResolvedValue(undefined)

executionPayloadStoreMockFns.mockStoreLargeValue.mockImplementation(
  async (_value: unknown, _json: unknown, size: number) => ({
...
const mockCheckHybridAuth = vi.mocked(checkHybridAuth)
...
mockAssertBillingAttributionSnapshot.mockImplementation(<T>(value: T): T => {
...
describe('workflow execute async route', () => {
  ...
  it('queues authenticated workflow input provenance without exposing the private sidecar as input', async () => {
    ...
    const response = await POST(
      createInternalProvenanceRequest({ executionMode: 'async' }),
      createRouteContext({ id: 'workflow-1' })
    )
  ...
  it.each([
    ...
    const response = await POST(request(), createRouteContext({ id: 'workflow-1' }))
  ...
  it.each([true, false])(
    ...
    async (useDraftState) => {
      ...
      const response = await POST(
        createBoundCopilotExecutionRequest({ useDraftState }),
        createRouteContext({ id: 'workflow-1' })
      )
  ...
  it('settles a detached Copilot stream only after its cancelled execution and cleanup end', async () => {
    ...
    try {
      const response = await POST(
        createBoundCopilotExecutionRequest(),
        createRouteContext({ id: 'workflow-1' })
      )
  ...
  it('rejects Copilot workflow bindings outside the interactive SSE surface', async () => {
    const response = await POST(
      createBoundCopilotExecutionRequest({ stream: false }),
      createRouteContext({ id: 'workflow-1' })
    )
  ...
  it('rejects sequential replay of a claimed session execution ID before preprocessing', async () => {
    ...
    const firstResponse = await POST(
      createSessionReplayRequest(requestedExecutionId),
      createRouteContext({ id: 'workflow-1' })
    )
  ...
  it('retries a generated execution ID collision with a fresh server ID', async () => {
    ...
    const response = await POST(
      createCallerExecutionRequest(EXECUTION_CALLERS[0]),
      createRouteContext({ id: 'workflow-1' })
    )
  ...
  it('rejects a workspace API key for another workspace before preprocessing', async () => {
    ...
    const response = await POST(
      createCallerExecutionRequest(caller),
      createRouteContext({ id: 'workflow-1' })
    )
  ...
  it('releases the admission reservation when enqueue proves non-acceptance', async () => {
    ...
    const response = await POST(req, createRouteContext({ id: 'workflow-1' }))
  ...
  it('rejects cross-site session requests before authorization work', async () => {
    ...
    const context = createRouteContext({ id: 'workflow-1' })
