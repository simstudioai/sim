...
import { createErrorResponse } from '../../files/utils'

const logger = createLogger('KnowledgeBasePresignedUploadAPI')
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('Error generating knowledge base presigned URL:', error)
...
async function handleS3PresignedUrl(fileName: string, contentType: string, fileSize: number) {
  // Create a unique key for the file with kb prefix
  const safeFileName = fileName.replace(/\s+/g, '-')
  const uniqueKey = `kb/${Date.now()}-${uuidv4()}-${safeFileName}`
  ...
  // Create the S3 command using KB-specific bucket
  ...
  logger.info(`Generated KB presigned URL for ${fileName} (${uniqueKey})`)
...
async function handleBlobPresignedUrl(fileName: string, contentType: string, fileSize: number) {
  ...
  try {
    ...
    const credential = new StorageSharedKeyCredential(
      BLOB_KB_CONFIG.accountName,
      BLOB_KB_CONFIG.accountKey || ''
    )
    ...
    return NextResponse.json({
      ...
      uploadHeaders: {
        ...
        'x-ms-meta-purpose': 'knowledge-base',
      },
    })
  } catch (error) {
    logger.error('Error generating Azure Blob presigned URL for KB:', error)
    throw error
  }
}
