...
import {
  ...
  InterfaceNotArchivedError,
  InterfaceNotFoundError,
  InterfaceStaleWriteError,
  InterfaceWorkspaceArchivedError,
...
export function interfaceDomainErrorResponse(error: unknown): NextResponse | null {
  ...
  /**
   * The record vanished between the access check and the write. Mirrors the
   * body {@link resolveInterfaceAccess} returns for an unresolvable id, so a
   * client cannot tell the two apart — because it should not have to.
   */
  if (error instanceof InterfaceNotFoundError) {
    return NextResponse.json({ error: error.message }, { status: 404 })
  }
  /**
   * Both are conflicts with the *current state* of the target — the interface
   * is already active, or its workspace is archived — not a malformed body, so
   * 409 rather than 422. They carry their `code` for the same reason the stale
   * write does: 409 alone does not say which condition fired.
   */
  if (error instanceof InterfaceNotArchivedError) {
    return NextResponse.json({ error: error.message, code: error.code }, { status: 409 })
  }
  if (error instanceof InterfaceWorkspaceArchivedError) {
    return NextResponse.json({ error: error.message, code: error.code }, { status: 409 })
  }
  return null
}
