...
describe('POST /api/interfaces/[interfaceId]/modules/[moduleId]/submit', () => {
  ...
  it('lets a read-only member submit, matching what a share link already grants', async () => {
    permissionsMockFns.mockGetUserEntityPermissions.mockResolvedValue('read')

    const response = await callPost(validBody)

    expect(response.status).toBe(200)
    expect(executionPreprocessingMockFns.mockPreprocessExecution).toHaveBeenCalled()
  })
