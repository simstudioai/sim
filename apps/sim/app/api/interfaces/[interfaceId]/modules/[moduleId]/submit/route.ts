 ...
 *
 * Read permission is sufficient. A submit runs the *deployed* workflow and
 * carries none of the execution controls (`useDraftState`,
 * `workflowStateOverride`, `rawRunFromBlock`) that
 * `/api/workflows/[id]/execute` authorizes at `write` — so it is the same
 * capability that route already grants a read-only member, and the same one an
 * anonymous visitor holds through the token-scoped public submit route against
 * this very form.
 */
