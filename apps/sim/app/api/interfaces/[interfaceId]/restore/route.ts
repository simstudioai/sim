...
import { performRestoreInterface } from '@/lib/interfaces'
...
export const POST = withRouteHandler(
  async (request: NextRequest, context: { params: Promise<{ interfaceId: string }> }) => {
    ...
    try {
      ...
      const restored = await performRestoreInterface(
        interfaceId,
        {
          id: authResult.userId,
          name: authResult.userName ?? undefined,
          email: authResult.userEmail ?? undefined,
        },
        { request }
      )
