...
const logger = createLogger('PublicInterfaceOtpAPI')

const rateLimiter = new RateLimiter()

const SHARE_EMAIL_LABEL = 'a shared interface'
...
/**
 * POST /api/interfaces/public/[token]/otp
 * Sends a 6-digit verification code to an allow-listed email for an
 * email-gated interface share.
 */
export const POST = withRouteHandler(
  async (request: NextRequest, context: { params: Promise<{ token: string }> }) => {
    ...
    try {
      ...
      const parsed = await parseRequest(requestPublicInterfaceOtpContract, request, context)
      ...
      if (resolved.share.authType !== 'email') {
        return NextResponse.json(
          { error: 'This interface does not use email authentication' },
          { status: 400 }
        )
      }
      ...
      if (!isEmailAllowed(email, shareAllowedEmails(resolved.share.allowedEmails))) {
        return NextResponse.json(
          { error: 'Email not authorized for this interface' },
          { status: 403 }
        )
      }

      const emailRateLimit = await rateLimiter.checkRateLimitDirect(
        `interface-otp:email:${resolved.share.id}:${email}`,
        OTP_EMAIL_RATE_LIMIT
      )
      ...
      await storeOTP('interface', resolved.share.id, email, otp)
...
/**
 * PUT /api/interfaces/public/[token]/otp
 * Verifies the code and, on success, sets the `interface_auth_{shareId}` cookie.
 * Uses its own IP bucket so failed verifies never throttle a legitimate resend.
 */
export const PUT = withRouteHandler(
  async (request: NextRequest, context: { params: Promise<{ token: string }> }) => {
    ...
    try {
      ...
      const parsed = await parseRequest(verifyPublicInterfaceOtpContract, request, context)
      if (!parsed.success) return parsed.response
      const { token } = parsed.data.params
      ...
      const storedValue = await getOTP('interface', resolved.share.id, email)
      if (!storedValue) {
        return NextResponse.json(
      ...
      if (attempts >= MAX_OTP_ATTEMPTS) {
        await deleteOTP('interface', resolved.share.id, email)
        return NextResponse.json(
