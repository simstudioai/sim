...
import { previewPromoteContract } from '@/lib/api/contracts/apps'
import { getValidationErrorMessage, parseRequest } from '@/lib/api/server'
import { assertAppPermission } from '@/lib/apps/permissions'
import { promotePreviewCandidate } from '@/lib/apps/pins'
...
export const POST = withRouteHandler(
  async (request: NextRequest, context: { params: Promise<{ projectId: string }> }) => {
    ...
    const parsed = await parseRequest(previewPromoteContract, request, context, {
    ...
    const result = await promotePreviewCandidate({
      projectId,
