...
import { rollbackAppReleaseContract } from '@/lib/api/contracts/apps'
import { getValidationErrorMessage, parseRequest } from '@/lib/api/server'
import { assertAppPermission } from '@/lib/apps/permissions'
import { rollbackPublishedRelease } from '@/lib/apps/publish'
...
export const POST = withRouteHandler(
  async (request: NextRequest, context: { params: Promise<{ projectId: string }> }) => {
    ...
    const parsed = await parseRequest(rollbackAppReleaseContract, request, context, {
    ...
    const perm = await assertAppPermission(session.user.id, project.workspaceId, 'rollback')
    if (!perm.ok) return createErrorResponse(perm.message, perm.status)

    const result = await rollbackPublishedRelease({
      projectId,
      targetReleaseId: parsed.data.body.targetReleaseId,
    })
    ...
    if (!result.success) {
      const status =
        ...
        result.code === 'INVALID_ARTIFACT_HASH'
          ? 409
          : result.code === 'MANUAL_REVOKE_NOT_REACTIVATABLE'
            ? 403
            : 400
    ...
    try {
      ...
      recordAudit({
        ...
        action: AuditAction.APP_ROLLED_BACK,
        resourceType: AuditResourceType.APP,
        resourceId: projectId,
        resourceName: project.name,
        description: `Rolled back app to release ${result.publishedReleaseId}`,
        metadata: {
          publishedReleaseId: result.publishedReleaseId,
          revokedVacated: result.revokedVacated,
        },
      })
    } catch {
      // best-effort
