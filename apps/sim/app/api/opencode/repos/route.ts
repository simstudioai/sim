...
const logger = createLogger('OpenCodeReposAPI')
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!authResult.success || !authResult.userId) {
      logger.warn(`[${requestId}] Unauthorized OpenCode repos access attempt`)
    ...
    const routeError = getOpenCodeRouteError(error, 'repositories')
    logger.error(`[${requestId}] Failed to fetch OpenCode repositories`, {
      error,
