...
/**
 * `TOKEN_CACHE` in shared.ts is module-global and survives every test in this file, so
...
describe('SapConcurProxyPath', () => {
  ...
  it.each(accepted)('accepts the ordinary path %s', (path) => {
    expect(SapConcurProxyPath.safeParse(path).success).toBe(true)
  })
  ...
  it.each(rejected)('rejects the traversal-shaped path %s', (path) => {
    expect(SapConcurProxyPath.safeParse(path).success).toBe(false)
  })
  ...
  it('does not reject double-encoded traversal (defense-in-depth, host is pinned elsewhere)', () => {
    expect(SapConcurProxyPath.safeParse('/expensereports/%252e%252e/v4').success).toBe(true)
  })
})

describe('SapConcurProxyRequestSchema accept', () => {
  function parse(overrides: Record<string, unknown>) {
    return SapConcurProxyRequestSchema.parse({
      clientId: 'client-1',
      clientSecret: CLIENT_SECRET,
