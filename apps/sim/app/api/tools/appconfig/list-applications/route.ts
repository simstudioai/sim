...
import { createAppConfigClient, listApplications } from '@/app/api/tools/appconfig/utils'
...
export const POST = withRouteHandler(async (request: NextRequest) => {
  try {
    ...
    logger.info('Listing AppConfig applications')
    ...
    const errorMessage = toError(error).message || 'AppConfig list applications failed'
    logger.error('AppConfig list applications failed:', error)
