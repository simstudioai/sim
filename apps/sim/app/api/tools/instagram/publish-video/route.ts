...
import { instagramPublishVideoContract } from '@/lib/api/contracts/tools/instagram'
...
const logger = createLogger('InstagramPublishVideoAPI')
...
export const POST = withRouteHandler(async (request: NextRequest) => {
  ...
  try {
    ...
    if (!authResult.success || !authResult.userId) {
      logger.warn(`[${requestId}] Unauthorized Instagram publish video: ${authResult.error}`)
    ...
    const parsed = await parseRequest(instagramPublishVideoContract, request, {})
    ...
    logger.error(`[${requestId}] Instagram publish video failed:`, error)
    return NextResponse.json(
      {
        success: false,
        error: getErrorMessage(error, 'Failed to publish video'),
        output: { containerId: null, mediaId: null, statusCode: null },
      },
      { status: 500 }
    )
