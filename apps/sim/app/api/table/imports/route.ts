...
export const POST = withRouteHandler(async (request: NextRequest) => {
  ...
  try {
    const created = await createTableImportResource(parsed.data.body, auth.userId)
    return NextResponse.json({ data: await toV2TableImport(created.record) }, { status: 201 })
  } catch (error) {
