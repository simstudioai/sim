...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const userResponse = await fetch(`${instanceUrl}/api/now/table/sys_user?sysparm_query=user_name=${encodeURIComponent('javascript:gs.getUserName()')}&sysparm_limit=1`, {
      headers: {
    ...
    const now = new Date()
    const expiresIn = expiresInStr ? parseInt(expiresInStr, 10) : 3600 // Default to 1 hour
    ...
    const effectiveScope = scope && scope.trim() ? scope : 'useraccount'
