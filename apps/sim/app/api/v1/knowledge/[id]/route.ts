...
export const GET = withRouteHandler(async (request: NextRequest, { params }: KnowledgeRouteParams) => {
...
export const PUT = withRouteHandler(async (request: NextRequest, { params }: KnowledgeRouteParams) => {
  const auth = await authenticateRequest(request, 'knowledge-detail')
  if (auth instanceof NextResponse) return auth
  const { requestId, userId, rateLimit } = auth
...
export const DELETE = withRouteHandler(async (request: NextRequest, { params }: KnowledgeRouteParams) => {
  const auth = await authenticateRequest(request, 'knowledge-detail')
  if (auth instanceof NextResponse) return auth
  const { requestId, userId, rateLimit } = auth
