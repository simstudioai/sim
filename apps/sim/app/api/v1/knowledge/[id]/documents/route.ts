...
export const GET = withRouteHandler(async (request: NextRequest, { params }: DocumentsRouteParams) => {
...
export const POST = withRouteHandler(async (request: NextRequest, { params }: DocumentsRouteParams) => {
  const auth = await authenticateRequest(request, 'knowledge-detail')
  if (auth instanceof NextResponse) return auth
  const { requestId, userId, rateLimit } = auth
