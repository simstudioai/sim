...
export const GET = withRouteHandler(async (request: NextRequest, { params }: DocumentDetailRouteParams) => {
...
export const DELETE = withRouteHandler(async (request: NextRequest, { params }: DocumentDetailRouteParams) => {
  const auth = await authenticateRequest(request, 'knowledge-detail')
  if (auth instanceof NextResponse) return auth
  const { requestId, userId, rateLimit } = auth
