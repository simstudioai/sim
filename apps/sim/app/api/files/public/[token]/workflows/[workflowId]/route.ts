...
function publicWorkflowRoute(run: boolean) {
  return withRouteHandler(
    async (
      ...
      const contract = run ? runPublicFileWorkflowContract : readPublicFileWorkflowContract
      const parsed = await parseRequest(contract, request, context, { maxBodyBytes: 1024 })
      if (!parsed.success) {
...
export const GET = publicWorkflowRoute(false)
export const POST = publicWorkflowRoute(true)
