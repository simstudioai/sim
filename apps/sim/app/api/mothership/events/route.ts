...
const logger = createLogger('MothershipEventsSSE')
...
export async function GET(request: NextRequest) {
  ...
  const cleanup = () => {
    ...
    decrementSSEConnections('mothership-events')
    logger.info(`SSE connection closed for workspace ${workspaceId}`)
  }

  const stream = new ReadableStream({
    start(controller) {
      incrementSSEConnections('mothership-events')
      ...
      if (taskPubSub) {
        const unsub = taskPubSub.onStatusChanged((event) => {
