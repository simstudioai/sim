...
export const GET = withRouteHandler(async (req: NextRequest, { params }: { params: Promise<{ token: string }> }) => {
...
export const POST = withRouteHandler(async (req: NextRequest, { params }: { params: Promise<{ token: string }> }) => {
  const { token } = await params
