...
import { db } from '@/db'
import { workflow, workspace, workspaceMember, workflowBlocks } from '@/db/schema'
...
async function createWorkspace(userId: string, name: string) {
  ...
  try {
    ...
    await tx.insert(workflowBlocks).values({
      ...
      console.log(`✅ Created workspace ${workspaceId} with initial workflow ${workflowId} for user ${userId}`)
    })
  } catch (error) {
