 ...
 * Tests GET (details + token acceptance), PUT (API acceptance), DELETE (cancellation)
 *
...
describe('Workspace Invitation [invitationId] API Route', () => {
  ...
  describe('PUT /api/workspaces/invitations/[invitationId]', () => {
    it('should accept invitation with valid status', async () => {
    ...
    it('should reject invitation with valid status', async () => {
      const { PUT } = await import('./route')
    ...
    it('should return 400 for already processed invitation', async () => {
      ...
      expect(response.status).toBe(400)
      expect(data).toEqual({ error: 'Invitation already processed' })
    })
  })
