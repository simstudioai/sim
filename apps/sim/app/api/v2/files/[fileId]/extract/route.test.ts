...
import { POST } from '@/app/api/v2/files/[fileId]/extract/route'
...
function extractRequest(body: Record<string, unknown> = { workspaceId: WORKSPACE_ID }) {
  return new NextRequest(`http://localhost:3000/api/v2/files/${FILE_ID}/extract`, {
    method: 'POST',
...
describe('POST /api/v2/files/[fileId]/extract', () => {
  beforeEach(() => {
    vi.clearAllMocks()
   ...
   */
  it('extracts for a workspace API key', async () => {
    const response = await POST(extractRequest(), context)
  ...
  it('extracts for a personal API key', async () => {
  ...
  /**
   * Counts and a path only. Returning the extracted files would materialize a
   * large archive's whole contents into one response body.
   */
  it('does not return the extracted files', async () => {
    const response = await POST(extractRequest(), context)
  ...
  it('rejects a body with an undeclared key', async () => {
    const response = await POST(
      extractRequest({ workspaceId: WORKSPACE_ID, destination: '/elsewhere' }),
      context
    )
  ...
  it('surfaces a concurrent extraction as 409', async () => {
    mocks.extract.mockRejectedValueOnce(
