...
import { POST } from '@/app/api/v2/files/bulk-archive/route'
...
const callArchive = (body: unknown) =>
  POST(
    new NextRequest('http://localhost:3000/api/v2/files/bulk-archive', {
      method: 'POST',
...
describe('POST /api/v2/files/bulk-archive', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 404 when the v2 API surface flag is off', async () => {
    ...
    const res = await callArchive({ workspaceId: WS, fileIds: ['wf_1'] })
  ...
  it('400s when the selection is empty', async () => {
    const res = await callArchive({ workspaceId: WS, fileIds: [], folderIds: [] })
  ...
  it('archives the selection and reports the full cascade', async () => {
    const res = await callArchive({ workspaceId: WS, fileIds: ['wf_1'], folderIds: ['fold_1'] })
    const body = await res.json()

    expect(res.status).toBe(200)
    expect(body.data).toEqual({ deletedItems: { files: 3, folders: 1 } })
  ...
  it('maps a not_found errorCode to 404', async () => {
    ...
    const res = await callArchive({ workspaceId: WS, fileIds: ['wf_missing'] })
