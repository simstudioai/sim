...
import { POST } from '@/app/api/v2/tables/[tableId]/rows/batch-update/route'
...
function call(body: unknown) {
  const request = new NextRequest('http://localhost/api/v2/tables/table-1/rows/batch-update', {
    method: 'POST',
...
describe('POST /api/v2/tables/[tableId]/rows/batch-update', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('rejects an empty batch before delegation', async () => {
  ...
  it('rejects a batch naming the same row twice', async () => {
  ...
  it('answers 400 when the batch names a row this table does not have', async () => {
