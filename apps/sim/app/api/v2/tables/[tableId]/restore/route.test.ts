...
const { mockRestoreTableUseCase } = tableApplicationTablesMockFns
...
describe('POST /api/v2/tables/[tableId]/restore', () => {
  beforeEach(() => {
    ...
    mockRestoreTableUseCase.mockResolvedValue({ table: TABLE, folderPath: '/' })
  })

  it('conceals a table in another workspace as a 404', async () => {
    mockRestoreTableUseCase.mockRejectedValueOnce(new NoWorkspaceAccessError())
