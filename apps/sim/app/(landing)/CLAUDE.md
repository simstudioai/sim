...
This route group owns `/` — Sim's public landing page, being rebuilt from scratch. Read this file in full before adding or changing anything here. Positioning and language rules live in `.claude/rules/constitution.md`; SEO/GEO rules in `.claude/rules/landing-seo-geo.md`. Both apply to every file in this directory.

## What this is (and what `(home)` is)

- `app/(landing)/` — the NEW landing page. Currently a blank slate: `page.tsx` (route + metadata) and `landing.tsx` (empty root component).
- `app/(home)/` — the OLD landing page and the remaining marketing subroutes (`/contact`, `/privacy`, `/terms`, `/blog`, `/models`, `/integrations`, `/partners`).

### Reference policy for `(home)`

Use the old landing as an **idea reference only** — section concepts, content, copy that worked, structural lessons (its TSDoc on `landing.tsx` documents a sound SEO/GEO architecture). But:

- **Never import from `@/app/(home)/**`.** No shared components, no shared utils, no shared data files.
- **Never copy components or styles wholesale.** Re-author with the practices in this file. If old code survives a rewrite unchanged, that's a smell — it means you copied instead of re-deriving.
- The old landing's dark `--landing-*` tokens, Martian Mono accents, and bespoke chrome are explicitly **not** carried over.
...
- **No responsiveness yet.** Do not use breakpoint variants (`sm:`, `md:`, `lg:`, `xl:`, `2xl:`, `max-*:`) anywhere in this directory — the page is desktop-only for now. A responsive pass will happen later as a deliberate, separate effort.
