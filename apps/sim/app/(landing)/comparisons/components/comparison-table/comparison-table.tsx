...
export interface ComparisonTableProps {
  ...
  /** Render one group when provided; older callers can still render the complete comparison. */
...
export function ComparisonTable({
  ...
  const sections = selectedSection ? [selectedSection] : COMPARISON_SECTIONS
  return (
    <div className='w-full overflow-x-auto rounded-xl border border-[var(--border-1)]'>
      <div
        ...
        aria-label={
          selectedSection
            ? `Sim vs ${competitor.name}: ${selectedSection.title}`
            : `Sim vs ${competitor.name} feature comparison`
        }
                ...
                return (
                  <div key={row.key} className='contents' role='row'>
                    ...
                    >
                      ...
                      <FactValue fact={simFact} label={`${sim.name}: ${row.label}`} />
                    </div>
                    <div
                      role='cell'
                      className={cn(
                    ...
                    >
                      ...
                      <FactValue fact={competitorFact} label={`${competitor.name}: ${row.label}`} />
                    </div>
                  </div>
                )
              })}
