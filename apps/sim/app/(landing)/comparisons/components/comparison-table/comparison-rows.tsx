...
/**
 * Each native sticky label is contained by a rail ending at the next category cell.
  ...
  return COMPARISON_SECTIONS.map((section, sectionIndex) => {
    ...
    return (
      ...
      >
        ...
        >
          <div role='rowheader' className='flex items-center bg-[var(--surface-1)] px-4 py-2'>
            <h2
              id={`${section.id}-heading`}
              className='font-medium text-[var(--text-primary)] text-base leading-5'
            >
              {section.title}
            </h2>
          </div>
        ...
        <div role='row' className={intro ? 'contents' : 'hidden lg:contents'}>
          <div
            ...
            className={cn(
              'relative z-30 col-start-1 hidden lg:block',
              isFirstSection && 'lg:row-start-1'
            )}
          >
          ...
          <div
            ...
            aria-colspan={2}
            className={cn(
              'flex min-w-0 scroll-mt-[calc(var(--comparison-header-height)+var(--comparison-section-height))] items-center bg-[var(--comparison-column-bg)] px-4 py-4 text-[var(--text-body)] text-sm leading-relaxed lg:col-span-2 lg:col-start-2 lg:scroll-mt-[calc(var(--comparison-header-height)-0.5px)] lg:border-[var(--border)] lg:border-x lg:border-t lg:px-6 xl:px-12',
              isFirstSection && !intro && 'lg:row-start-1'
            )}
          >
            {intro ? (
              <p>
        ...
        {section.rows.map((row, index) => (
          <div key={row.key} role='row' className='contents'>
            <div
              ...
              role='rowheader'
              className='flex scroll-mt-[calc(var(--comparison-header-height)+var(--comparison-section-height))] items-center bg-[var(--surface-1)] px-4 py-2 font-normal text-[var(--text-body)] text-small leading-relaxed lg:min-h-16 lg:scroll-mt-[calc(var(--comparison-header-height)-0.5px)] lg:justify-end lg:bg-[var(--comparison-column-bg)] lg:px-6 lg:text-right lg:text-sm'
            >
              {row.label}
            </div>
            <div
              role='cell'
              className='flex min-w-0 flex-col justify-center gap-0.5 bg-[var(--surface-2)] px-4 py-1 lg:gap-2 lg:border-[var(--border)] lg:border-x lg:border-t lg:bg-[var(--comparison-column-bg)] lg:px-6 lg:py-2 xl:px-12'
            >
            ...
            <div
              role='cell'
              className='flex min-w-0 flex-col justify-center gap-0.5 border-[var(--border)] border-b bg-[var(--surface-2)] px-4 pt-1 pb-3 lg:gap-2 lg:border-t lg:border-r lg:border-b-0 lg:bg-[var(--comparison-column-bg)] lg:px-6 lg:py-2 xl:px-12'
            >
              <span className='font-medium text-[var(--text-muted)] text-caption lg:hidden'>
                {competitor.name}
              </span>
