 ...
 * `<PlatformPage config={…} />`.
 *
 * This component owns the entire `<main>`: the one horizontal gutter
 * (`PLATFORM_SPACING.gutter`, matching the navbar and landing hero) and the
 * inter-section vertical rhythm (`PLATFORM_SPACING.sectionRhythm`, the `<main>`
 * flex gap). The hero, the logos row, and every card row carry no gutter and no
 * inter-section margin of their own, so spacing is uniform and unreachable from
 * a consumer page — the config is pure content (strings + `ReactNode` visual
 * slots), with no layout knob anywhere in its tree.
 *
...
export function PlatformPage({ config }: PlatformPageProps) {
  return (
    <>
      ...
      <main
        id='main-content'
        className={cn('flex flex-col', PLATFORM_SPACING.sectionRhythm, PLATFORM_SPACING.gutter)}
      >
