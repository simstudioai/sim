...
function Testimonials() {
  ...
  return (
    <section className='animation-container relative flex w-full flex-col gap-16 overflow-hidden border-border border-b py-10 sm:py-12 md:py-16'>
      <div className='flex flex-col gap-5 px-8 sm:px-8 md:px-12 lg:px-40'>
        <div className='flex items-center justify-between'>
          ...
          <Button
            variant='ghost'
            className='gap-2 font-base text-base text-neutral-900 hover:bg-neutral-200/50 '
          >
            <XIcon className='h-4 w-4' /> Follow Sim <ArrowUpRight className='h-4 w-4' />
          </Button>
        </div>
        <p className='font-normal text-[#515151] text-lg'>People loved Sim, see it yourself.</p>
      </div>
        ...
        <div className='animation-container flex w-full animate-fade-up flex-col text-foreground opacity-0 will-change-[opacity,transform] [animation-delay:400ms]'>
          <Marquee className='flex w-full [--duration:40s]' pauseOnHover={true}>
            {firstRowTestimonials.map((card, index) => (
              <motion.div
                key={`first-row-${index}`}
                className='mx-0.5 flex min-w-[280px] max-w-[340px] cursor-pointer flex-col gap-2 rounded-[10px] border border-border bg-card p-2 sm:min-w-[320px] sm:max-w-[380px] sm:p-3'
              ...
              >
                <div className='flex flex-col gap-1'>
                  <p className='font-medium text-foreground text-sm sm:text-base'>{card.text}</p>
                </div>
