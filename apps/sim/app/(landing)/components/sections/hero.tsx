...
const MODES = [
  { value: 'agent', label: 'Agent', icon: <BookIcon className='h-3 w-3 text-muted-foreground' /> },
  { value: 'ask', label: 'Ask', icon: <MessageSquare className='h-3 w-3 text-muted-foreground' /> },
] as const
...
function Hero() {
  ...
  React.useEffect(() => {
    ...
    const runHideEdgesAndGroup = () => {
      ...
      const edgePaths = root
        ? Array.from(root.querySelectorAll<SVGPathElement>('.react-flow__edge-path'))
        : []
    ...
    const runHideBlocks = () => {
      ...
      const nodeInners = root
        ? Array.from(root.querySelectorAll<HTMLElement>('.landing-node .landing-node-inner'))
        : []
    ...
    PROMPTS.forEach((entry, idx) => {
      ...
      tl.addLabel(`prompt-${idx}-start`)
        ...
        .call(() => {
          ...
          const layout = computeLayout(
            entry.blocks as unknown as WorkflowManualBlock[],
            entry.groupGrid
          )
          ...
          if (flowWrapRef.current)
            gsap.to(flowWrapRef.current, { autoAlpha: 1, duration: 0.3, ease: 'power2.out' })
        })
      ...
      tl.to({}, { duration: 0.02 })
        ...
        .call(() => {
          ...
          showControls()
          if (flowWrapRef.current)
            gsap.set(flowWrapRef.current, { clearProps: 'opacity,transform' })
        })
        .to({}, { duration: 0.05 })
        .call(panReset)
      ...
      tl.to([sendBtnRef.current, modeTagRef.current], {
        autoAlpha: 1,
        scale: 1,
        duration: 0.35,
        ease: 'power2.out',
  ...
  return (
    <div className="relative flex min-h-[calc(100vh-6rem)] w-full flex-col items-center overflow-hidden bg-center bg-cover px-4 pt-32 [background-image:url('/static/bg.png')] sm:px-8 sm:pt-48 md:px-12 lg:px-20 lg:pt-52 xl:px-32">
      <div className='z-10 flex w-full max-w-6xl flex-col items-center gap-12 lg:gap-16'>
        <div className='flex flex-col items-center gap-4 text-center'>
          ...
          <p className='text-[#484848] text-base leading-6 sm:text-lg md:text-xl'>
            Build and deploy AI agent workflows
          </p>
        </div>
        {/* CHAT WITH AUTOPLAY */}
        <div className='chat flex w-full flex-col items-center gap-2 md:w-auto'>
          ...
          <div
            ref={textareaWrapRef}
            className={`relative block w-full overflow-hidden rounded-[10px] ${isCondensed ? 'h-fit' : ''}`}
          >
            {/* Visual input */}
            <Textarea
              ...
              rows={isMobile ? 2 : 1}
              className={`!block !align-middle !text-base md:!text-base placeholder:!text-base resize-none overflow-hidden rounded-[10px] border-input px-4 py-3 pr-16 leading-[1.25] shadow-sm placeholder:text-muted-foreground placeholder:leading-[1.25] focus-visible:border-[#A46FFF] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#A46FFF] focus-visible:ring-offset-0 ${isCondensed ? '!min-h-0' : '!min-h-[7rem]'}`}
              value={prompt}
            ...
            <div
              ...
              className='pointer-events-none invisible absolute top-0 left-0 w-full whitespace-pre-wrap break-words px-4 py-4 pr-16 text-base leading-[1.25] md:text-base'
            >
              {prompt || ' '}
            </div>
            <Button
        ...
        <div
          ref={previewRef}
          className='preview relative mt-4 flex h-[36rem] w-full overflow-hidden rounded-t-[10px] border-border border-x border-t bg-background/80 shadow-sm'
        >
...
const WorkflowBlock = React.memo(function WorkflowBlock({
  icon,
  color,
  name,
  tags,
