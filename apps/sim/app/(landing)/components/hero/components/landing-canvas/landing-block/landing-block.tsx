...
export const LandingBlock = React.memo(function LandingBlock({
  ...
  return (
    <div
      className={`z-10 flex w-[250px] flex-col rounded-lg border border-[var(--surface-6)] bg-white ${className ?? ''}`}
    >
      {/* Header - matches workflow-block.tsx header styling */}
      <div
        className={`flex items-center justify-between p-2 ${hasContentBelowHeader ? 'border-[var(--surface-6)] border-b' : ''}`}
      >
