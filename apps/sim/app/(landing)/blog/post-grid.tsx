    ...
    <div className='grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 lg:grid-cols-3'>
      {posts.map((p, index) => (
        <Link key={p.slug} href={`/blog/${p.slug}`} className='group flex flex-col'>
          <div className='flex h-full flex-col overflow-hidden rounded-xl border border-[var(--surface-4)] transition-colors duration-300 hover:border-[var(--border-1)]'>
            ...
            <div className='flex flex-1 flex-col p-4'>
              <div className='mb-2 text-[var(--text-subtle)] text-xs'>
              ...
              <div className='flex items-center gap-2'>
                <div className='-space-x-1.5 flex'>
                  {(p.authors && p.authors.length > 0 ? p.authors : [p.author])
                    .slice(0, 3)
                    .map((author, idx) => (
                      <Avatar key={idx} className='size-4 border border-[var(--text-primary)]'>
                        <AvatarImage src={author?.avatarUrl} alt={author?.name} />
                        <AvatarFallback className='border border-[var(--text-primary)] bg-[var(--surface-4)] text-[var(--text-subtle)] text-micro'>
