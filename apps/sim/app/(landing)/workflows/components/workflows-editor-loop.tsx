...
/** Sidebar content for the workflows hero - a builder's workspace. */
const SIDEBAR_CHATS = [
  'Support bot revamp',
  'Lead scoring tweaks',
...
/**
 * The complete support-routing workflow on the editor canvas - a trigger, an
 ...
 * blocks, brand tiles only for real third-party marks.
 *
...
const EDITOR_CANVAS = { width: 1360, height: 910 } as const
...
/** The agent block gets its selection ring this long after the last block. */
...
const SELECTED_BLOCK_ID = 'agent'

/**
 * The workflows hero's editor loop - the chat-free sibling of the enterprise
 * platform loop. Same architecture (fixed 1280x735 design-space layer scaled
 * to the window via ResizeObserver + `transform: scale`, a parent-owned clock
 * driving a presentational stage, reduced-motion showing the finished frame)
 * and the same live {@link EnterpriseSidebar}, but the workspace pane is the
 * editor canvas itself: the support-routing workflow assembles block by block
 * (edges stroke-draw as endpoints land), then the agent block picks up the
 * real editor's selection ring - the "being edited" beat - before the scene
