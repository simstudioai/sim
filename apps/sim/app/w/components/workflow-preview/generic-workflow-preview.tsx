 ...
 */
function prepareSubBlocks(
  blockSubBlocks: Record<string, any>,
  blockConfig: any
) {
  ...
  return Object.entries(blockSubBlocks)
    .map(([id, subBlock]) => {
      const matchingConfig = configSubBlocks.find(
...
function PreviewSubBlock({ config }: { config: ExtendedSubBlockConfig }) {
        ...
        return (
          <div className="flex h-7 items-center justify-between rounded-md border border-input bg-background px-3 py-1 text-muted-foreground text-xs">
            <span>
              {config.value
                ? 'Connected account'
                : config.placeholder || 'Select account'}
            </span>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="12"
        ...
        return (
          <div className="flex h-7 items-center justify-between rounded-md border border-input bg-background px-3 py-1 text-muted-foreground text-xs">
            <span>
              {config.value
                ? 'File selected'
                : config.placeholder || 'Select file'}
            </span>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="12"
        ...
        return (
          <div className="flex h-7 items-center justify-between rounded-md border border-input bg-background px-3 py-1 text-muted-foreground text-xs">
            <span>
              {config.value
                ? 'Folder selected'
                : config.placeholder || 'Select folder'}
            </span>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="12"
        ...
        return (
          <div className="flex h-7 items-center justify-between rounded-md border border-input bg-background px-3 py-1 text-muted-foreground text-xs">
            <span>
              {config.value
                ? 'Project selected'
                : config.placeholder || 'Select project'}
            </span>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="12"
  ...
  return (
    <div className="space-y-1">
      {config.type !== 'switch' && (
        <Label className="text-xs">{config.title}</Label>
      )}
      ...
      >
        ...
        {type !== 'starter' && (
          <Handle
            type="target"
            position={
              blockState?.horizontalHandles ? Position.Left : Position.Top
            }
            id="target"
            className={cn(
              '!w-3 !h-3',
        ...
        {type !== 'condition' && (
          <Handle
            type="source"
            position={
              blockState?.horizontalHandles ? Position.Right : Position.Bottom
            }
            id="source"
            className={cn(
              '!w-3 !h-3',
