...
type OrganizationRoute = keyof Omit<ReturnType<typeof organizationRoutes>, 'root' | 'chat'>
        ...
        <ExpandableContent className={cn(!animationsEnabled && 'animate-none!')}>
          ...
          <div className='flex flex-col pt-1.5'>
            {STEPS.map((step, i) => {
              ...
              const className = cn(ROW_CLASS, i > 0 && 'border-t')
              const content = (
                <>
              ...
              return step.route ? (
                <Link key={step.id} href={routes[step.route]} className={className}>
                  {content}
                </Link>
              ) : (
                <button key={step.id} type='button' className={className}>
                  {content}
                </button>
              )
            })}
