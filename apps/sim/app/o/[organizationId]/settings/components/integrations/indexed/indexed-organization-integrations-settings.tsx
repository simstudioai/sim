...
import { OrganizationIntegrationsSetup } from '@/app/o/[organizationId]/settings/components/integrations/indexed/organization-integrations-setup'
import { OrganizationSourcePeople } from '@/app/o/[organizationId]/settings/components/integrations/indexed/organization-source-people'
import { OrganizationSourceStats } from '@/app/o/[organizationId]/settings/components/integrations/indexed/organization-source-stats'
import {
...
/**
 * Organization Integrations settings for indexed organization search: the Sources, People, and
 * Stats tabs over the connectors that crawl the search index. Rendered only while the deployment
 * serves indexed search (`features.liveEnterpriseSearch === false`).
 */
export function IndexedOrganizationIntegrationsSettings() {
    ...
    <div className='flex flex-col gap-6'>
      {tab === 'providers' && (
        <div className='flex items-center justify-between gap-4'>
          {tabs}
          {!accounts.error && group && group.options.length > 0 && (
            <RowActionsMenu
