...
export default function PersonaPage() {
  ...
  const handleCreatePersona = async (form: {
    ...
    try {
      ...
      setPersonas((prev) => [...prev, { ...data.persona, workflows: [], connectedPersonas: [] }])
    } catch (err) {
              ...
              <div className='px-6 pb-6'>
                ...
                {error && (
                  <div className='mb-4 rounded-md border border-red-200 bg-red-50 p-4'>
                    <p className='text-red-800 text-sm'>Error loading personas: {error}</p>
                  </div>
                )}
                {/* Content Area */}
                {loading ? (
                    ...
                    {filteredPersonas.length === 0 ? (
                      personas.length === 0 ? (
                        <EmptyStateCard
                          ...
                          icon={<AgentIcon className='h-4 w-4 text-muted-foreground' />}
                        />
                      ) : (
