...
export function useIdentifierValidation(
  ...
  useEffect(() => {
    ...
    if (!IDENTIFIER_PATTERN.test(identifier)) {
      setError(t('form_deploy.errors.identifier_invalid'))
      return
    }
    ...
    timeoutRef.current = setTimeout(async () => {
      try {
        ...
        if (!response.ok) {
          setError(t('form_deploy.errors.identifier_check_error'))
          setIsValid(false)
        } else if (!data.available) {
          setError(data.error || t('form_deploy.errors.identifier_unavailable'))
          setIsValid(false)
        } else {
          setError(null)
          setIsValid(true)
