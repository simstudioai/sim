...
function getToolDisplayNameByState(toolCall: any): string {
  ...
  if (isInterruptTool && (state === 'executing' || state === 'pending')) {
    ...
    if (toolName === COPILOT_TOOL_IDS.SET_ENVIRONMENT_VARIABLES) {
      ...
      if (params.variables && typeof params.variables === 'object') {
        ...
        if (varNames.length > 0) {
          ...
          const truncatedName =
            firstVarName.length > 15 ? `${firstVarName.substring(0, 15)}...` : firstVarName
    ...
    if (toolName === COPILOT_TOOL_IDS.RUN_WORKFLOW) {
      ...
      if (params.description) {
        const truncatedDescription =
          params.description.length > 30
            ? `${params.description.substring(0, 30)}...`
            : params.description
      ...
      if (params.workflowId) {
        const truncatedId =
          params.workflowId.length > 20
            ? `${params.workflowId.substring(0, 20)}...`
            : params.workflowId
  ...
  if (state === 'error' && toolCall.error) {
    const errorMessage =
      typeof toolCall.error === 'string' ? toolCall.error : toolCall.error.message || ''
    if (
...
function InlineToolCall({ tool, stepNumber }: { tool: ToolCallState | any; stepNumber?: number }) {
  ...
  const showInterruptConfirmation =
    requiresInterrupt && (tool.state === 'executing' || tool.state === 'pending') && !buttonsHidden
    ...
    switch (tool.state) {
      ...
      case 'completed':
        return isWorkflowTool ? (
          ...
          getToolIcon()
        ) // Use the actual tool icon instead of Search
      ...
      case 'rejected':
        ...
        return isInterruptTool ? (
          <div className='flex h-3 w-3 items-center justify-center rounded-full border border-gray-400'>
      ...
      if (tool.name === COPILOT_TOOL_IDS.RUN_WORKFLOW) {
        ...
        try {
          ...
          if (!backgroundConfirmationSent) {
            ...
            console.log(
              `Tool ${toolCallId} execution completed - confirmation already sent via background move`
            )
          }
        } catch (workflowError) {
  ...
  return (
    <div className='flex items-center justify-between gap-2 py-1'>
      ...
      {tool.name === COPILOT_TOOL_IDS.RUN_WORKFLOW &&
        isWorkflowExecuting &&
        !backgroundConfirmationSent && (
          <div className='flex items-center gap-1.5'>
