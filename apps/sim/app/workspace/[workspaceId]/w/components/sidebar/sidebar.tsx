...
export const Sidebar = memo(function Sidebar({
  ...
  const { data: tableFolderMap = EMPTY_FOLDER_MAP } = useFolderMap(workspaceId, 'table')
  const { data: knowledgeFolderMap = EMPTY_FOLDER_MAP } = useFolderMap(
    workspaceId,
    'knowledge_base'
  )
  ...
  const searchModalTables = useMemo(
    () =>
      permissionConfig.hideTablesTab
        ? []
        : fetchedTables.map((t) => {
            const folderPath = t.folderId
              ? getFolderPath(tableFolderMap, t.folderId).map((folder) => folder.name)
              : []
            return {
              id: t.id,
              name: t.name,
  ...
  const searchModalKnowledgeBases = useMemo(
    () =>
      permissionConfig.hideKnowledgeBaseTab
        ...
        : fetchedKnowledgeBases.map((kb) => {
            const folderPath = kb.folderId
              ? getFolderPath(knowledgeFolderMap, kb.folderId).map((folder) => folder.name)
              : []
            return {
              id: kb.id,
              name: kb.name,
    ...
    [fetchedKnowledgeBases, knowledgeFolderMap, workspaceId, permissionConfig.hideKnowledgeBaseTab]
  )
  ...
  const searchModalPageContext = useMemo((): PageActionContext | null => {
    ...
    if (pathname === `${base}/scheduled-tasks`) return 'scheduledTasks'
    return null
