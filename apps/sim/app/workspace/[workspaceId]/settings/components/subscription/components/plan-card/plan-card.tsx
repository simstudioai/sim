...
export function PlanCard({
  ...
  const renderPrice = () => {
    if (typeof price === 'string') {
      return (
        <>
          <span className='font-medium text-[15px] text-[var(--text-primary)]'>{price}</span>
          {priceSubtext && (
            <span className='ml-[4px] text-[13px] text-[var(--text-secondary)]'>
              {priceSubtext}
            </span>
          )}
        </>
      ...
      >
        <div className='flex items-center justify-between gap-[8px] px-[14px] py-[10px]'>
          <div className='flex items-baseline gap-[8px]'>
            <span className='font-medium text-[15px] text-[var(--text-primary)]'>{name}</span>
