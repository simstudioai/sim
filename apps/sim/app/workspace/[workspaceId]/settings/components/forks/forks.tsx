...
import { ForkSyncView } from '@/app/workspace/[workspaceId]/settings/components/forks/components/fork-sync/fork-sync-view'
import {
...
import { useForkLineage, useRollbackFork, useUnlinkFork } from '@/hooks/queries/workspace-fork'
              ...
              {parentVisible && parent !== null && (
                <SettingsSection label='Parent'>
                  <ForkListRow
                    name={parent.name}
                    actions={[
                      { label: 'Edit mappings', onSelect: () => openForkMappings(parent.id) },
                  ...
                  <div className='flex flex-col gap-2'>
                    {filteredForks.map((fork) => (
                      <ForkListRow
                        ...
                        name={fork.name}
                        actions={[
                          { label: 'Open workspace', onSelect: () => openForkWorkspace(fork.id) },
                          {
                            label: 'Disconnect',
                            destructive: true,
