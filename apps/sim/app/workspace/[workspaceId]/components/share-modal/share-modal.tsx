...
/**
 * THE share dialog — one component for every shareable resource, so the chrome,
 * state machine, and save flow cannot drift as further resource families gain a
 * public route. Everything resource-specific comes in through props: the
 * endpoints and policy gates branch on `resourceType`, the hint copy reads from
 * {@link ACCESS_COPY}, and callers that already know the share state pass
 * `initialShare` to avoid a loading flicker. `file` is the only family today.
 *
