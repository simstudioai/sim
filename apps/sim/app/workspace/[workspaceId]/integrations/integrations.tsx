...
export function Integrations() {
  ...
  const integrationByLowerName = useMemo(
    () => new Map(allIntegrations.map((i) => [i.name.toLowerCase(), i])),
    [allIntegrations]
  )
  ...
  const connectedItems = useMemo<ConnectedDisplayItem[]>(() => {
    return oauthCredentials.flatMap((credential) => {
      ...
      if (!service) return []
      const integration = integrationByLowerName.get(service.name.toLowerCase())
      return [
        {
          credential,
  ...
  }, [oauthCredentials, integrationByLowerName])
  ...
  const categoryOptions = useMemo(
    () => [
      ...
      FEATURED_LABEL,
      ...allCategorySections.map((section) => section.label),
    ],
    [allCategorySections, connectedItems.length]
  )
  ...
  const filteredCategorySections = useMemo(() => {
    ...
    if (isFeaturedSelected) {
      const items = featuredIntegrations.filter(matchesSearch)
    ...
    const featured = normalizedSearch ? [] : featuredIntegrations.filter(matchesCategory)
    const featuredSection =
      featured.length > 0 ? [{ label: FEATURED_LABEL, integrations: featured }] : []

    if (isAllCategorySelected) {
      const rest = allCategorySections
        .map((section) => ({
          label: section.label,
