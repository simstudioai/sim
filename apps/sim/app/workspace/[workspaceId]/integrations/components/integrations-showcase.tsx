...
export function IntegrationTile({ blockType, icon: Icon, framed = false }: IntegrationTileProps) {
  ...
  if (!framed) {
    return (
      <div className='h-9 w-9 flex-shrink-0'>
        <div
          className='flex h-full w-full items-center justify-center rounded-xl border border-[var(--border-1)] bg-[var(--bg)]'
  ...
  return (
    <div className='h-11 w-11 flex-shrink-0 rounded-xl border border-[var(--border-muted)] bg-[var(--surface-4)] p-[3px] shadow-sm dark:bg-[var(--surface-5)]'>
      <div
        className='flex h-full w-full items-center justify-center rounded-[9px] border border-[var(--border-1)] bg-[var(--bg)]'
        style={brandBg ? { background: brandBg } : undefined}
      >
