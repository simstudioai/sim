 ...
 * an existing row.
 */
const formatInterfaceName = (attempt: number) =>
  attempt === 0 ? BASE_INTERFACE_NAME : `${BASE_INTERFACE_NAME} ${attempt + 1}`

/**
 * Interfaces list. An interface is a grid canvas of chat/form/table/file modules;
...
export function Interfaces() {
  ...
  const handleCreateInterface = useCallback(async () => {
    const name = generateUniqueName(
      interfaces.map((definition) => definition.name),
      formatInterfaceName,
  ...
  /** Derived from the URL, never mirrored into state — the modal mounts fresh per open. */
  const shareInterface = shareInterfaceId
    ? (interfaces.find((definition) => definition.id === shareInterfaceId) ?? null)
    : null
    ...
    <>
      ...
      {shareInterface ? (
        <ShareModal
          open
          onOpenChange={(open) =>
            !open && setInterfaceFilters({ shareInterfaceId: null }, { history: 'replace' })
          }
          workspaceId={workspaceId}
          resourceType='interface'
          resourceId={shareInterface.id}
          resourceName={shareInterface.name}
        />
      ) : null}
    </>
  )
