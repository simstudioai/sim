...
import { createInterfaceModule, DEFAULT_MODULE_SPAN } from '@/lib/interfaces/constants'
import { moveModuleToCell, overlappingModules, placementFitsGrid } from '@/lib/interfaces/geometry'
import type {
  InterfaceCell,
  InterfaceLayout,
  InterfaceModule,
 ...
 * helpers in `@/lib/interfaces/geometry` and PATCHed as a whole-layout
 * replace, which is what the update contract expects (the granular service
 * operations exist for the Sim agent, not the editor).
 *
 * Those helpers are the same ones the service runs under its row lock, so the
 * optimistic layout is byte-identical to what gets persisted — there is no
 * client-side mirror of the move rules to keep in step.
 *
...
export function useInterfaceLayout({
  ...
  }, [layout, updatedAt, interfaceId, onModuleAdded])
  ...
  const addModule = useCallback(
    (type: InterfaceModuleType, cell: InterfaceCell) => {
      ...
      const placement: InterfacePlacement = { ...cell, ...DEFAULT_MODULE_SPAN }
      /**
       * Both checks the server would apply, so an add the contract would
       * reject never becomes an optimistic patch that has to roll back.
       */
      if (!placementFitsGrid(placement, base.grid)) return
      if (overlappingModules(base, placement).length > 0) return
      const module = createInterfaceModule(generateId(), type, placement)
  ...
  const moveModule = useCallback(
    (moduleId: string, cell: InterfaceCell) => {
      const base = currentLayout()
      const outcome = moveModuleToCell(base, moduleId, cell)
      if (outcome.status !== 'moved') return
      commit(outcome.layout)
    },
    [commit, currentLayout]
  )
