...
import { GripVertical, ListFilter, Plus, Rows3, TypeBoolean, TypeText, X } from '@sim/emcn/icons'
import { omit } from '@sim/utils/object'
import { INTERFACE_LAYOUT_LIMITS } from '@/lib/interfaces/constants'
import { deriveFormFieldErrors } from '@/lib/interfaces/form-config'
...
export interface FormFieldRowProps {
  ...
  /** One field is expanded at a time, so a long form stays scannable. */
  collapsed: boolean
 ...
 *
 * Rendered as the shared {@link CollapsibleCard}, the same card the workflow
 * editor's input-mapping rows use, so a field row here and a mapping row there
 * are one piece of chrome rather than two look-alikes. Collapsed by default:
 * the properties a builder reaches for most — the label, the order, the field
 * list itself — are now edited on the module, and this panel is where the rest
 * lives.
 *
 * Reordering is a drag on the row, exactly as on the canvas, so the section
 * above owns it and this component only paints the grip.
 */
export function FormFieldRow({
  ...
  /**
   * The card header doubles as the collapse toggle, so remove stops the click
   * from reaching it — deleting a field must not also fold a sibling open.
   */
  const actions = (
    /** `ml-auto`: the card's badge slot sits right after the title, not at the end of the row. */
    <span className={cn('ml-auto flex shrink-0 items-center', chipContentGap)}>
      <GripVertical aria-hidden className={cn(chipContentIconClass, 'cursor-grab')} />
      <Button
        type='button'
        variant='ghost'
        size='sm'
        ...
        className='!p-1 -my-0.5 size-6 shrink-0 cursor-pointer'
  ...
  return (
    ...
    >
      {/**
       * `gap-4` — EMCN's modal-body rhythm. `CollapsibleCard` stacks its own
       * children at `gap-2`, which sat too close to a field's internal
       * `gap-[9px]` for the rows to read as separate; with one child that gap
       * is inert and this wrapper sets the spacing instead.
       */}
      <div className='flex flex-col gap-4'>
        <InspectorField flush title='Type'>
          <ChipDropdown
            value={field.type}
            onChange={(next) => {
        ...
        <InspectorField flush title='Label' required error={errors.label}>
          {(control) => (
            <ChipInput
              value={field.label}
        ...
        {/** The one hint kept: "Name" vs "Label" is the panel's only genuinely ambiguous pair. */}
        <InspectorField
          flush
          title='Name'
          required
          error={errors.name}
          hint='Start-block input key.'
        >
          {(control) => (
            <ChipInput
              value={field.name}
        ...
        {field.type !== 'switch' && (
          <InspectorField flush title='Placeholder'>
            {(control) => (
              <ChipInput
        ...
        <InspectorField flush title='Hint'>
          {(control) => (
            <ChipInput
              value={field.hint ?? ''}
        ...
        <InspectorField flush title='Required'>
          {(control) => (
            <Switch
        ...
        {field.type === 'dropdown' && (
          <InspectorField flush title='Options' required error={errors.options}>
        ...
        {field.type === 'switch' ? (
          <InspectorField flush title='Default on'>
            {(control) => (
              <Switch
          ...
          <InspectorField flush title='Default'>
            <ChipSelect
