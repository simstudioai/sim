...
import type { FileModuleConfig, InterfaceMode, InterfaceModule } from '@/lib/interfaces'
import { INTERFACE_MODULE_META } from '@/lib/interfaces/module-meta'
...
function ModuleInspectorBody({
  ...
  /**
   * The workflow editor's own scroll well, class for class — the panel opens
   * straight onto its first field, with no standing description above it.
   */
  return (
...
/**
 * Config section for a file module: the one workspace file it previews. A
 * file binding has no invalid intermediate state — an unresolvable id is
 * tolerated by design — so the section always reports `isValid: true`.
 */
function FileModuleFields({
  workspaceId,
  value,
  onChange,
  disabled,
}: ModuleConfigSectionProps<FileModuleConfig>) {
  ...
  return (
    <ResourcePickerField
      ...
      value={value.fileId}
      onChange={(next) => onChange({ fileId: next }, true)}
      disabled={disabled}
    />
  )
}
