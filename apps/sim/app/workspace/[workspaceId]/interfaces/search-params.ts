 ...
 * - `shareInterfaceId` deep-links an interface's share dialog open. The modal
 *   opens when the id resolves to a loaded interface; closing it clears the
 *   param. Opening and closing pass an explicit `{ history: 'replace' }` so the
 *   dialog toggle never lands in the back/forward stack (a deep link still
 *   opens it on load).
 *
...
export const interfacesParsers = {
  ...
  shareInterfaceId: parseAsString,
} as const
