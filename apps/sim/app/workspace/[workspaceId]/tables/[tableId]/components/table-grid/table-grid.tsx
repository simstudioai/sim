...
export function TableGrid({
  ...
  const hasReferenceColumns = useMemo(
    () =>
      referenceColumnsEnabled &&
      columns.some((column) => Boolean(columnTypeOf(column).referencePreview)),
    [columns, referenceColumnsEnabled]
  )
  const { data: workspaceTables } = useTableNames(workspaceId, {
    enabled: hasReferenceColumns,
  })
  ...
  const referencePreviewState = referencePreviewQuery.isError
    ...
    : referencePreviewQuery.isFetching || !referencePreviewQuery.data
      ...
      : ({ status: 'ready', ...referencePreviewQuery.data } as const)
