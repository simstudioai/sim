 ...
 *  client-only until their server row exists. Other job types are excluded — deletes/backfills
 *  reflect directly in the grid, and exports surface as toasts (`useExportJobToasts`). */
...
export function useWorkspaceImports(
  ...
  return useMemo(() => {
    ...
    for (const table of tables ?? []) {
      ...
      // live-fill cells via SSE, and exports surface as toasts (`useExportJobToasts`).
