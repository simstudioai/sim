 ...
 *
 * Guards every Google Drive tool that interpolates an LLM-writable identifier
 ...
 * `/drive/v3/files/..` would still satisfy).
 */
import { describe, expect, it } from 'vitest'
import * as serviceTools from '@/tools/google_drive/index'
...
const PATH_PREFIX = '/drive/v3/files/'
...
function isServiceTool(value: unknown): value is AnyTool {
  return (
    ...
    (value as AnyTool).id.startsWith('google_drive_') &&
...
describe('Google Drive path traversal safety', () => {
  it('covers every tool that interpolates an identifier into its path', () => {
    expect(PATH_TOOLS.length).toBeGreaterThanOrEqual(14)
  })
