...
export const watermarkApplyTool: ToolConfig<ResembleWatermarkParams, ResembleResponse> = {
  ...
  description: 'Apply an invisible Resemble provenance watermark and return the watermarked media (audio-first).',
  version: '1.0.0',
  params: {
    ...
    strength: { type: 'number', required: false, visibility: 'user-only', description: 'Watermark strength 0.0–1.0 (image/video only)' },
    customMessage: { type: 'string', required: false, visibility: 'user-only', description: 'Message to embed (image/video only)' },
  ...
  transformResponse: async (response: Response, params?: ResembleWatermarkParams) => {
    ...
    if (!(it.watermarked_media || it.url) && it.uuid && params) {
      try {
        data = await pollResource(baseOf(params), `/watermark/apply/${it.uuid}/result`, authHeaders(params), params.maxWaitSeconds || 120)
      } catch {
        /* fall through */
      }
    }
