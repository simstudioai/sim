import {
  createProcurementTool,
  procurementParamFields,
} from '@/tools/oracle_fusion_procurement/shared'
import { PROCUREMENT_STRING_ACTION_OUTPUTS } from '@/tools/oracle_fusion_procurement/types'

export const oracleFusionProcurementRemovePurchaseOrderHoldTool = createProcurementTool({
  id: 'oracle_fusion_procurement_remove_purchase_order_hold',
  name: 'Oracle Fusion Procurement Remove Purchase Order Hold',
  description:
    'Explicitly remove an approved purchase-order hold and check the Oracle action result.',
  params: {
    purchaseOrderKey: procurementParamFields.purchaseOrderKey,
    removeHoldReason: { ...procurementParamFields.removeHoldReason, required: false },
  },
  outputs: PROCUREMENT_STRING_ACTION_OUTPUTS,
})
