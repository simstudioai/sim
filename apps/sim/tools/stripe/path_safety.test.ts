 ...
 *
 * Guards every Stripe tool against path traversal through an LLM-writable
 * identifier that gets interpolated into the request path.
 *
 * These identifiers are `visibility: 'user-or-llm'`, so prompt injection
 * controls them. Interpolating one raw let a value like `../../v1/customers/cus_live`
 * escape its API prefix once `fetch` normalized the URL, re-aiming the request
 * — and the user's Stripe credential — at a sibling endpoint, including on
 * DELETE.
 *
...
import * as serviceTools from '@/tools/stripe/index'
import type { ToolConfig } from '@/tools/types'

const ORIGIN = 'https://api.stripe.com'
const ID_PREFIX = 'stripe_'
const MIN_PATH_TOOLS = 28
...
describe('Stripe path-parameter traversal safety', () => {
  it('covers every Stripe tool that interpolates a param into its path', () => {
    expect(PATH_TOOLS.length).toBeGreaterThanOrEqual(MIN_PATH_TOOLS)
  })

  /**
   * Every Stripe path here carries exactly one LLM-writable id, so there is no
   * second param a first guard could mask. Asserted rather than assumed: a
   * future nested-resource tool (`/v1/customers/{id}/sources/{sourceId}`) must
   * make this fail so the independence block below is revisited.
   */
  it('has only single-parameter paths', () => {
    expect(MULTI_PARAM_TOOLS).toEqual([])
  })
