  ...
  request: {
    url: (params) => {
      ...
      if (!params.filter?.trim()) {
        throw new Error(
          'filter is required for update operations. It must be present and must not contain "#", but it is not checked for selectivity — a broad filter still updates every row it matches.'
        )
      }
