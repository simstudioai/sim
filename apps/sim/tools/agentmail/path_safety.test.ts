...
import * as toolModule from '@/tools/agentmail/index'
...
const LEGITIMATE = [
  'yourinbox@agentmail.to',
  'agent+tag@sub.example.co.uk',
  'msg_01HQ8ZK4N2XW9V',
  '..foo',
  'foo..',
] as const
...
describe('AgentMail path-parameter traversal safety', () => {
  it('covers every tool that interpolates a parameter into its path', () => {
    expect(PATH_TOOLS.length).toBe(19)
    expect(TOTAL_PATH_PARAMS).toBe(30)
  })
