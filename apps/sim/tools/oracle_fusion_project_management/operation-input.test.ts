...
describe('Oracle Project Management tool input declarations', () => {
  it('declares 53 unique internal operations with credential-authoritative authentication', () => {
    ...
    for (const tool of declarations) {
      ...
      expect(tool.oauth).toEqual({ required: true, provider: 'oracle_fusion_project_management', credentialKind: 'service-account', authoritativeParams: ['instanceUrl'] })
  ...
  it('passes semantic references and large string IDs unchanged, removing executor scope', () => {
    const input = { oauthCredential: 'credential-1', projectId: '999999999999999999', taskName: '<agent.output>', taskNumber: 'T1', taskLevel: 1, parentTaskId: '<previous.task.TaskId>', _context: { secret: 'canary' } }
    const result = oracleFusionProjectManagementCreateTaskTool.operation.input(input)
    expect(result).toEqual({ oauthCredential: 'credential-1', projectId: '999999999999999999', taskName: '<agent.output>', taskNumber: 'T1', taskLevel: 1, parentTaskId: '<previous.task.TaskId>' })
  })
  ...
  it('models documented budget lines instead of an arbitrary JSON body and excludes immutable team fields', () => {
    ...
    expect(resources).toMatchObject({ type: 'array', maxItems: 100, items: { type: 'object', required: ['RbsElementId', 'TaskId'], additionalProperties: false } })
    expect(resources.items?.properties?.RbsElementId.type).toBe('string')
    expect(oracleFusionProjectManagementUpdateProjectTeamMemberTool.params).not.toHaveProperty('personEmail')
    expect(oracleFusionProjectManagementUpdateProjectTeamMemberTool.params).not.toHaveProperty('projectRole')
