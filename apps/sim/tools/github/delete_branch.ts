...
export const deleteBranchTool: ToolConfig<DeleteBranchParams, DeleteBranchResponse> = {
  ...
  request: {
    url: (params) =>
      `https://api.github.com/repos/${strictUrlPathSegment(params.owner, 'owner')}/${strictUrlPathSegment(params.repo, 'repo')}/git/refs/heads/${safeUrlPath(params.branch, 'branch')}`,
    method: 'DELETE',
    headers: (params) => ({
