...
export const getWorkflowTool: ToolConfig<GetWorkflowParams, WorkflowResponse> = {
  ...
  request: {
    url: (params) =>
      `https://api.github.com/repos/${safeUrlPathSegment(params.owner, 'owner')}/${safeUrlPathSegment(params.repo, 'repo')}/actions/workflows/${safeUrlPathSegment(params.workflow_id, 'workflow_id')}`,
    method: 'GET',
    headers: (params) => ({
