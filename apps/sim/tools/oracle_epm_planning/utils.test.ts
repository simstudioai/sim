...
describe('Planning public metadata and late input normalization', () => {
  it('reuses bounded job discovery for data maps and preserves manual job names', () => {
    const values = {
      operation: 'oracle_epm_planning_run_data_map', credential: 'credential-1',
      applicationSelector: 'Vision', objectType: 'PLAN_TYPE_MAP', jobType: 'Stale',
    }
    const selector = OracleEpmPlanningBlock.subBlocks.find((field) => field.id === 'dataMapNameSelector')!
    expect(buildSelectorContextFromValues({
    ...
    })).toMatchObject({ projectId: 'Vision', objectType: 'PLAN_TYPE_MAP', oauthCredential: 'credential-1' })
    expect(mapped({ ...values, dataMapName: 'Manual Map', clearData: false })).toMatchObject({ jobName: 'Manual Map', clearData: false })
    expect(OracleEpmPlanningBlock.subBlocks.find((field) => field.id === 'clearData')?.defaultValue).toBe(false)
  })
  it('parses nested data-map overrides late without leaking generic job fields', () => {
    const result = mapped({
      operation: 'oracle_epm_planning_run_data_map', dataMapName: 'Reporting', clearData: 'false',
      overrideMembersMap: '{"Period":"ILvl0Descendants(Q1)"}',
      overrideExclusionMembersMap: { Period: 'Jan' }, parameters: { stale: true },
    })
    expect(result).toMatchObject({ jobName: 'Reporting', clearData: false, overrideMembersMap: { Period: 'ILvl0Descendants(Q1)' }, overrideExclusionMembersMap: { Period: 'Jan' } })
    expect(result.parameters).toBeUndefined()
    expect(() => mapped({ operation: 'oracle_epm_planning_run_data_map', clearData: 'yes' })).toThrow()
  })
  it('discards inactive summary inputs before parsing stale JSON or recompute settings', () => {
    const ids = mapped({ operation: 'oracle_epm_planning_summarize_insights', summaryInputMode: 'ids', insightIds: '["426"]', insightSlice: '{invalid', cube: 'Stale', retrievalMode: 'FORCE_RECOMPUTE', calendar: 'Stale' })
    expect(ids.insightIds).toEqual(['426'])
    for (const field of ['insightSlice', 'cube', 'retrievalMode', 'calendar']) expect(ids[field]).toBeUndefined()
    const slice = mapped({ operation: 'oracle_epm_planning_summarize_insights', summaryInputMode: 'slice', insightSlice: '{}', insightIds: '{invalid', retrievalMode: 'USE_EXISTING', calendar: 'Stale' })
  ...
  it('shows and requires insight inputs only for their active summary mode', () => {
    ...
    const visible = (id: string, values: Record<string, unknown>) => evaluateSubBlockCondition(fields.find((field) => field.id === id)!.condition, values)
    const ids = { operation: 'oracle_epm_planning_summarize_insights', summaryInputMode: 'ids', retrievalMode: 'FORCE_RECOMPUTE' }
    for (const field of ['cubeSelector', 'cubeManual', 'insightSlice', 'retrievalMode', 'calendar']) expect(visible(field, ids)).toBe(false)
    expect(visible('insightIds', ids)).toBe(true)
    const slice = { ...ids, summaryInputMode: 'slice' }
    for (const field of ['cubeSelector', 'cubeManual', 'insightSlice', 'retrievalMode', 'calendar']) expect(visible(field, slice)).toBe(true)
    expect(visible('insightIds', slice)).toBe(false)
    expect(visible('calendar', { ...slice, retrievalMode: 'USE_EXISTING' })).toBe(false)
    expect(visible('insightSlice', { ...ids, operation: 'oracle_epm_planning_get_insights' })).toBe(true)
    for (const field of ['cubeSelector', 'cubeManual', 'insightSlice']) {
      const required = fields.find((item) => item.id === field)!.required
      expect(typeof required === 'function' ? evaluateSubBlockCondition(required, slice) : required).toBe(true)
    }
  })
  it('coerces approval numbers without changing compound identifier text', () => {
    const puhIdentifier = 'Forecast::"Working"'
    expect(mapped({ operation: 'oracle_epm_planning_change_planning_unit_status', puhIdentifier, actionId: '6', pmMembers: 'Sales & Marketing' })).toMatchObject({ puhIdentifier, actionId: 6, pmMembers: 'Sales & Marketing' })
    expect(mapped({ operation: 'oracle_epm_planning_get_planning_unit_history', annotSeq: '-1', logSeq: '2' })).toMatchObject({ annotSeq: -1, logSeq: 2 })
    expect(() => mapped({ operation: 'oracle_epm_planning_get_planning_unit_actions', approvalOptions: '2' })).toThrow()
  })
