...
import {
  oracleFusionRecruitingListCandidatesTool,
  ...
  oracleFusionRecruitingListCandidatePhonesTool,
  ...
  oracleFusionRecruitingListCandidateSkillsTool,
  ...
  oracleFusionRecruitingListApplicationsTool,
...
describe('Recruiting block and tools', () => {
  it('exposes exactly the agreed 28 operations and maps each to its tool', () => {
    ...
    const options = OracleFusionRecruitingBlock.subBlocks.find((field) => field.id === 'operation')?.options
    if (!Array.isArray(options)) throw new Error('Expected operation choices')
    expect(options.map(({ id }) => OracleFusionRecruitingBlock.tools.config.tool({ operation: id })).sort())
      .toEqual(tools.map((tool) => tool.id).sort())
    expect([...OracleFusionRecruitingBlock.tools.access].sort()).toEqual(tools.map((tool) => tool.id).sort())
  })
  it.each(tools)('$id uses the saved credential and one in-process boundary', (tool) => {
    expect(tool.oauth).toMatchObject({ provider: 'oracle_fusion_recruiting', credentialKind: 'service-account', authoritativeParams: ['instanceUrl'] })
    expect(tool.operation).toBeDefined()
    expect(tool.request).toBeUndefined()
    expect(tool.params).toMatchObject({ accessToken: { visibility: 'hidden' }, instanceUrl: { visibility: 'hidden' }, oauthCredential: { required: true, visibility: 'user-only' } })
  })
  it('does not coerce unresolved references during tool selection', () => {
    ...
    expect(OracleFusionRecruitingBlock.tools.config.tool(params)).toBe('oracle_fusion_recruiting_get_offer')
  ...
  it('keeps canonical picker and manual IDs distinct from their input keys', () => {
    ...
    for (const canonical of ['candidateNumber', 'phoneId', 'requisitionId', 'applicationId', 'offerId', 'scheduleId']) {
  ...
  it('normalizes empty optional values after the executor merges inputs', () => {
    ...
    const raw = { operation: 'list_candidates', accessToken: 'token', instanceUrl: 'https://example.fa.ocs.oraclecloud.com', search: null, limit: '', offset: null }
    expect(listCandidatesSchema.parse({ ...raw, ...map(raw) })).toMatchObject({ search: undefined, limit: undefined, offset: undefined })
  ...
  it('parses write bodies at runtime without rounding string IDs', () => {
    const map = OracleFusionRecruitingBlock.tools.config.params
    if (!map) throw new Error('Expected mapping')
    expect(map({ operation: 'update_requisition', requisitionBody: '{"RecruiterId":"9007199254740993"}' })).toMatchObject({ body: { RecruiterId: '9007199254740993' } })
  })
})
