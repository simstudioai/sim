 ...
 * controls them. Interpolating one raw let a value like `../../zones/prod-zone`
 * escape its API prefix once `fetch` normalized the URL, re-aiming the request
 * — and the user's Cloudflare credential — at a sibling endpoint, including on
 * DELETE.
 *
 ...
 * segments, never a `startsWith` on the prefix: `/client/v4/x/..` still
 * starts with `/client/v4/`, so a prefix check passes while the hole is live.
 */
import { describe, expect, it } from 'vitest'
import * as serviceTools from '@/tools/cloudflare/index'
