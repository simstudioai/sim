 ...
 *
 * Guards every Google Vault tool that interpolates an LLM-writable identifier
 ...
 * `/v1/matters/..` would still satisfy).
 */
import { describe, expect, it } from 'vitest'
import * as serviceTools from '@/tools/google_vault/index'
...
describe('Google Vault path traversal safety', () => {
  it('covers every tool that interpolates an identifier into its path', () => {
    expect(PATH_TOOLS.length).toBeGreaterThanOrEqual(19)
  })
