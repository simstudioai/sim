 ...
 *
 * Guards every Tailscale tool against path traversal through an LLM-writable ID
 ...
 * decoding. `new URL('https://api.tailscale.com/api/v2/tailnet/example.com/keys/' + encodeURIComponent('..'))`
 * resolves to `/api/v2/tailnet/example.com/`, popping a segment on a fixed host with the
...
const HOST = 'https://api.tailscale.com'
...
const ALL_TOOLS = Object.values(tailscaleTools).filter(isTailscaleTool)
...
describe('tailscale path-ID traversal safety', () => {
  it('covers every Tailscale tool that interpolates an ID into its path', () => {
    expect(PATH_TOOLS.length).toBeGreaterThanOrEqual(24)
  })

  it('finds Tailscale tools that interpolate more than one ID', () => {
    expect(MULTI_PARAM_TOOLS.length).toBeGreaterThanOrEqual(2)
  })

  describe.each(PATH_TOOLS)('$name', ({ tool }) => {
    const baseline = segmentsOf(buildUrl(tool, SAFE_ID))

    it('keeps its baseline under the Tailscale API prefix', () => {
      expect(buildUrl(tool, SAFE_ID).pathname.startsWith('/api/v2/')).toBe(true)
