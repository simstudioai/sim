...
export const riskParamFields = {
  ...
  activityKey: {
    type: 'string', required: true, visibility: 'user-or-llm',
    description: 'Opaque key returned by the corresponding list action; retain the same parent identifiers',
  },
  advancedControlId: {
    type: 'string', required: true, visibility: 'user-or-llm',
  ...
  provisioningInfo: {
    type: 'json', required: true, visibility: 'user-or-llm',
    description: 'JSON object mapping role codes to arrays of data-access scope strings. Supported scope kinds: BUSINESS_UNIT, LEDGER_SET, DATA_ACCESS_SET, ASSET_BOOK, REFERENCE_DATA_SET. Maximum 100 roles and 100 scopes per role. This runs analysis only; it grants no access and creates no incidents.',
  },
  relationshipKey: {
    type: 'string', required: true, visibility: 'user-or-llm',
    description: 'Opaque key returned by the corresponding list action; retain the same parent identifiers',
  ...
  requestId: {
    type: 'string', required: true, visibility: 'user-or-llm',
    description: 'Simulation request identifier returned by Run Access Simulation, preserved as a decimal string',
  },
  ...
  userName: {
    type: 'string', required: true, visibility: 'user-or-llm',
    description: 'Exact Oracle Security Console username whose existing and proposed access will be simulated',
  },
} satisfies ToolConfig['params']

export const riskListParams = {
  q: { type: 'string', required: false, visibility: 'user-or-llm', description: 'Oracle q filter for this resource; maximum 4096 characters' },
  orderBy: { type: 'string', required: false, visibility: 'user-or-llm', description: 'Comma-separated Oracle sort fields with optional :asc or :desc' },
  limit: { type: 'number', required: false, visibility: 'user-or-llm', default: 100, description: 'One page of 1-100 records; default 100' },
  offset: { type: 'number', required: false, visibility: 'user-or-llm', default: 0, description: 'Zero-based offset, at most 1000000' },
  totalResults: { type: 'boolean', required: false, visibility: 'user-or-llm', description: "Request Oracle's estimated total count" },
} satisfies ToolConfig['params']
...
export function createRiskTool(input: {
  ...
  if (definition.kind === 'list') {
    Object.assign(params, riskListParams)
    if (definition.resource === 'simulation_result') delete params.q
  }
