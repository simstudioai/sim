...
import type {
  OciDevopsListBuildPipelineStagesParams,
  OciDevopsResponse,
} from '@/tools/oci_devops/types'
import type { InternalToolConfig } from '@/tools/types'

export const ociDevopsListBuildPipelineStagesTool: InternalToolConfig<
  OciDevopsListBuildPipelineStagesParams,
  OciDevopsResponse
> = {
  id: 'oci_devops_list_build_pipeline_stages',
  name: 'OCI DevOps List Build Pipeline Stages',
  description: 'List Build Pipeline Stages in OCI DevOps',
  version: '1.0.0',
  params: {
    oauthCredential: {
      type: 'string',
    ...
    buildPipelineId: {
      ...
      required: true,
      visibility: 'user-or-llm',
      description: 'The OCID of the parent build pipeline.',
    },
    compartmentId: {
      type: 'string',
      required: false,
    ...
    lifecycleState: {
      ...
      visibility: 'user-or-llm',
      description:
        'A filter to return the stages that matches the given lifecycle state. Allowed: CREATING, UPDATING, ACTIVE, DELETING, DELETED, FAILED.',
    },
    displayName: {
      type: 'string',
      required: false,
