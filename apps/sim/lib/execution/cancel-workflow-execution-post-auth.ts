...
import type { CancelWorkflowExecutionResponse } from '@/lib/api/contracts/workflows'
...
/**
 * Builds the single success shape this handler returns. It hand-builds its
...
export interface CancelWorkflowExecutionPostAuthInput {
  workflowId: string
  executionId: string
  userId: string
  assertedWorkspaceId?: string
  abortSignal?: AbortSignal
  workspaceApiKeyScope?: { workspaceId?: string }
}

function cancellationAbortedResponse(abortSignal?: AbortSignal): NextResponse | null {
...
}: CancelWorkflowExecutionPostAuthInput): Promise<NextResponse> {
  try {
    ...
    if (
      workspaceApiKeyScope &&
      workflowAuthorization.workflow?.workspaceId !== workspaceApiKeyScope.workspaceId
    ) {
    ...
    if (
      ...
      workflowAuthorization.workflow?.workspaceId !== assertedWorkspaceId
    ) {
      return NextResponse.json({ error: 'Execution not found' }, { status: 404 })
    }

    const postAuthorizationAbort = cancellationAbortedResponse(abortSignal)
    if (postAuthorizationAbort) return postAuthorizationAbort
    ...
    const postLookupAbort = cancellationAbortedResponse(abortSignal)
    if (postLookupAbort) return postLookupAbort
    ...
    logger.info('Cancel execution requested', { workflowId, executionId, userId })
