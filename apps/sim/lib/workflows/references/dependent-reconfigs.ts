 ...
 * source->target on sync) whose target swap clears its dependents.
 */
const PARENT_ANCHORS: ReadonlyArray<{
  ...
  {
    subBlockType: 'mcp-server-selector',
    parentKind: 'mcp-server',
    parentContextKey: 'mcpServerId',
  },
]
...
interface EmitAnchoredParams {
  ...
  includeUnconfiguredRequired?: boolean
}
    ...
    for (const anchorCfg of scanSubBlocks) {
      ...
      for (const clear of getTransitiveSubBlockDependents(scanSubBlocks, [anchorCfg.id])) {
        ...
        const rawSourceValue =
          typeof rawDependentValue === 'string'
            ? rawDependentValue
            : Array.isArray(rawDependentValue) &&
                rawDependentValue.every((value) => typeof value === 'string')
              ? rawDependentValue.join(',')
              : ''
        ...
        out.push({
          ...
          ...(dependent.multiSelect ? { multiSelect: true } : {}),
          ...(toolName ? { toolName } : {}),
          ...
          required:
            configuredRequired &&
            (params.includeUnconfiguredRequired ||
              dependent.selectorKey === 'mcp.tools' ||
              isNonEmptyValue(rawDependentValue)),
          providesContextKey,
          consumesContextKeys,
 ...
 * this promote (see {@link buildWorkflowBlockIdResolver}); otherwise the modal would key a
...
export function collectForkDependentReconfigs(
  ...
  resolveTargetBlockId: WorkflowBlockIdResolver,
  /**
  ...
  includeUnconfiguredRequired = false
      ...
      for (const cfg of config.subBlocks) {
        ...
        for (let index = 0; index < tools.length; index++) {
          ...
          if (
            ...
            const value = typeof tool.params.toolName === 'string' ? tool.params.toolName : ''
            out.push({
              parentKind: 'mcp-server',
              parentSourceId: tool.params.serverId,
              parentContextKey: 'mcpServerId',
              targetWorkflowId: item.targetWorkflowId,
              targetBlockId: resolveBlockId(),
              blockName: block.name,
              subBlockKey: `${cfg.id}[${index}].toolName`,
              selectorKey: 'mcp.tools',
              title: 'Tool',
              toolName: typeof tool.title === 'string' ? tool.title : 'MCP Tool',
              dependencyScope: `${cfg.id}[${index}]`,
              currentValue: value,
              sourceValue: value,
              required: true,
              consumesContextKeys: [],
