...
/** Preserves every file occurrence, including repeated keys and serialized single objects. */
export function fileUploadReferencePositions(value: unknown, key: string): number[] {
  const parsed = parseMaybeJson(value).value
  const items = Array.isArray(parsed) ? parsed : parsed ? [parsed] : []
  return items.flatMap((item, index) => (fileItemKeyField(item)?.key === key ? [index] : []))
}
