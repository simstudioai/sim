  ...
  const query = executor
    ...
    .where(
      and(
        ...
        page?.after ? gt(userTableDefinitions.id, page.after) : undefined,
        ids ? inArray(userTableDefinitions.id, ids) : undefined
      )
    )
  return page
    ? query.orderBy(asc(userTableDefinitions.id)).limit(page.limit + 1)
    : ids
      ? query
      : query.limit(CANDIDATE_LIMIT)
}

const knowledgeBaseCandidatesQuery = (
  executor: DbOrTx,
  workspaceId: string,
  ids?: string[],
  page?: { after?: string; limit: number }
  ...
  const query = executor
    ...
    .where(
      and(
        ...
        page?.after ? gt(knowledgeBase.id, page.after) : undefined,
        ids ? inArray(knowledgeBase.id, ids) : undefined
      )
    )
  return page
    ? query.orderBy(asc(knowledgeBase.id)).limit(page.limit + 1)
    : ids
      ? query
      : query.limit(CANDIDATE_LIMIT)
}

const customToolCandidatesQuery = (
  executor: DbOrTx,
  workspaceId: string,
  ids?: string[],
  page?: { after?: string; limit: number }
  ...
  const query = executor
    ...
    .where(
      and(
        ...
        page?.after ? gt(customTools.id, page.after) : undefined,
        ids ? inArray(customTools.id, ids) : undefined
      )
    )
  return page
    ? query.orderBy(asc(customTools.id)).limit(page.limit + 1)
    : ids
      ? query
      : query.limit(CANDIDATE_LIMIT)
}

const skillCandidatesQuery = (
  executor: DbOrTx,
  workspaceId: string,
  ids?: string[],
  page?: { after?: string; limit: number }
  ...
  const query = executor
    ...
    .where(
      and(
        ...
        page?.after ? gt(skill.id, page.after) : undefined,
        ids ? inArray(skill.id, ids) : undefined
      )
    )
  return page
    ? query.orderBy(asc(skill.id)).limit(page.limit + 1)
    : ids
      ? query
      : query.limit(CANDIDATE_LIMIT)
}
  ...
  const query = executor
    ...
    .where(
      and(
        ...
        page?.after ? gt(mcpServers.id, page.after) : undefined,
        ids ? inArray(mcpServers.id, ids) : undefined
      )
    )
  return page
    ? query.orderBy(asc(mcpServers.id)).limit(page.limit + 1)
    : ids
      ? query
      : query.limit(CANDIDATE_LIMIT)
}
  ...
  const query = executor
    ...
    .where(
      and(
        ...
        keys ? undefined : fileDiscoveryCondition(),
...
const fileCandidatesWithFolderQuery = (
  executor: DbOrTx,
  workspaceId: string,
  options: { keys?: string[]; page?: { after?: string; limit: number } } = {}
) => {
  const { keys, page } = options
  const query = executor
    .select({
  ...
  return page
    ? query.orderBy(asc(workspaceFiles.id)).limit(page.limit + 1)
    : keys
      ? query
      : query.limit(CANDIDATE_LIMIT)
}
 ...
 */
async function sandboxCandidatesQuery(executor: DbOrTx, workspaceId: string, ids?: string[]) {
  const query = executor
    .select({ id: workspaceSandbox.id, label: workspaceSandbox.name })
    .from(workspaceSandbox)
    .where(
      and(
        eq(workspaceSandbox.workspaceId, workspaceId),
        ids ? inArray(workspaceSandbox.id, ids) : undefined
      )
    )
...
async function loadForkResourceRows(
  ...
  if (sandboxIds.length > 0) result.sandbox = sandboxes
...
export async function classifyCredentialResourceType(
  ...
  credentialId: string,
  workspaceId: string
): Promise<'oauth_credential' | 'service_account_credential'> {
  const [row] = await executor
...
/** Keyset pages reuse the canonical eligibility queries without the browser picker ceiling. */
export async function listForkCopyableResourcePage(
  ...
  Array<
    ForkResourceCandidate & { key?: string; folderId?: string | null; folderName?: string | null }
  >
> {
  switch (kind) {
    case 'files':
      return fileCandidatesWithFolderQuery(executor, workspaceId, { page })
    case 'tables':
      return tableCandidatesQuery(executor, workspaceId, undefined, page)
    case 'knowledgeBases':
      return knowledgeBaseCandidatesQuery(executor, workspaceId, undefined, page)
    case 'customTools':
      return customToolCandidatesQuery(executor, workspaceId, undefined, page)
    case 'skills':
      return skillCandidatesQuery(executor, workspaceId, undefined, page)
    case 'mcpServers':
      return mcpServerCandidatesQuery(executor, workspaceId, undefined, page)
    case 'workflowMcpServers':
      return executor
        .select({ id: workflowMcpServer.id, label: workflowMcpServer.name })
        .from(workflowMcpServer)
        .where(
          and(
            ...
            page.after ? gt(workflowMcpServer.id, page.after) : undefined
          )
        )
