 ...
 * running and stack walks of the same directory. The idempotency key is the
 * sync interval the tick falls in, so a wrapper retry — which lands seconds
 * later, in the same interval — dispatches nothing new. Without Trigger.dev
 * the refresh runs in-process and the request returns without waiting for it.
 */
export async function dispatchDirectorySync(
  ...
  if (isTriggerAvailable()) {
    const idempotencyKey = await idempotencyKeys.create(
      `${DIRECTORY_SYNC_TASK_ID}:${connectorId}:${syncIntervalIndex(options.tickAt)}`,
      { scope: 'global' }
    )
