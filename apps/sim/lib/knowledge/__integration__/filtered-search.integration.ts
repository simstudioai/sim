...
describe.each([384, 768, 1024, 1536, 3072] as const)(
  'bounded filtered vector retrieval at %i dimensions',
  (dimensions) => {
    const ids = createKnowledgeAclFixtureIds()
    const secondBaseId = generateId()
    const embeddingModel = dimensions === 384 ? 'ollama/all-minilm' : 'text-embedding-3-large'
    ...
    beforeAll(async () => {
      ...
      for (const fixture of fixtures) {
        ...
        await db.insert(embedding).values({
          ...
          endOffset: 12,
          embeddingModel,
          ...embeddingVectorValues(dimensions, [
            1,
            fixture.y,
            ...Array<number>(dimensions - 2).fill(0),
          ]),
          tag1: tag,
