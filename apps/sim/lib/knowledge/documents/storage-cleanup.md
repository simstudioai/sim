...
Each event binds the file ID, storage key, owner, and content version. New events use the canonical workspace or organization. Previously queued events may still carry an explicit user for a legacy personal KB, so the handler retains that payload compatibility. The handler only acts on the same active metadata version, holds that row's exclusive lock during bounded, abortable object deletion, then soft-deletes metadata. A failed object deletion leaves the metadata active for retry. An ambiguous successful object deletion is safe to repeat because missing objects count as success. A restored or replacement version is retained. Document creation and active metadata registration take a shared lock on the same binding, and the cleanup handler checks the indexed `document.storage_key` before deleting; shared and concurrently attached files are retained. New document attachments require a live binding owned by their workspace.
