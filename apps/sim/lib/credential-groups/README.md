...
An organization has at most one account pool. Owners and admins manage it in **Organization settings → Connected accounts** or the normal **Settings → Organization → Connected accounts** page, with Providers, People, and Workspace access tabs. Both render the same org-scoped manager. The initial workspace allowlist is empty. Workspace admins without organization admin rights see sharing status; account settings show the signed-in person’s own contributions, including contributions to organizations they have not joined.
...
The Providers tab lists only added providers. **Add provider** opens a searchable catalog with the remaining providers. Required configuration is completed before adding a provider; **Configure** reopens its settings later. Providers without required setup are added directly. Indexable providers show **Indexing on / off**, with **Remove** in their actions menu. Individual account connections remain in People.
...
Supported OAuth providers show an **Indexing Off / On** switch inside **Configure**. Existing sources can be edited through **Source settings** in that view. Support comes from the existing Search connector registry's permission-scoped OAuth ingestion capabilities: Gmail, Google Drive, Google Calendar, GitHub repositories, Jira, Confluence, and Slack. Turning indexing on for the first time opens source setup and creates the organization's Search knowledge base and a member-scoped source. Repository, domain, space, or other required settings are collected there. Closing source setup returns to provider configuration; the switch becomes On only after the source is saved. Later changes resume or pause all live member-scoped Search sources bound to that provider option in one transaction. A running sync must finish before its setting can change; pending jobs can be cancelled. Pausing retains indexed documents and their document permissions. Reconnecting an OAuth account queues the existing active sources for that option; it never enables paused indexing.
...
| --- | --- |
| Connected accounts settings, provider setup, personal contributions, workspace access to the pool | `credential-groups` |
