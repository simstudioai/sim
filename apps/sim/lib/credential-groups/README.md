...
An organization has at most one account pool. Owners and admins manage it in **Organization settings → Credential Groups**, with Providers, People, and Workspace access tabs. The initial workspace allowlist is empty. Personal account settings show the signed-in person’s own contributions, including contributions to organizations they have not joined.

Credential Groups remains available whenever its organization feature is enabled, independently of Search. Search administrators use **Settings → Sources**; members use **Integrations**. The live Sources page has no credential-pool setup or People tabs. These surfaces reuse current grants without changing credential-group API access or stored workspace policies.

**Credential Groups → People → Request connections** retains manual email invitations, Resend, and Revoke actions. If no org credential group exists, People directs the admin to provider setup first. Search source creation does not replace required provider app configuration.
...
Search availability and mode are managed through **Organization settings → Sources**. Member mode has no resource filters; service mode uses the configured source’s resource boundary. Live search/read adapters are registered under `lib/sim-search/live/`; they resolve only the acting member’s current grants. OAuth completion may invoke the shared dispatch helper, but live Search sources are rejected by queue and worker guards before indexing. Ordinary workspace KB sources and the explicit `SIM_SEARCH_LIVE=false` backend retain ingestion behavior.
...
| Legacy Search content/member sync and persisted directory maintenance | Search features above and explicit `SIM_SEARCH_LIVE=false` |
