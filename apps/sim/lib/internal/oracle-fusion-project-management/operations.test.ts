...
const reads: Array<
  [OracleFusionProjectManagementOperation, Record<string, unknown>, string, Record<string, unknown>]
> = [
  ...
  [
    'get_task',
    { projectId: '101', taskId: '202' },
    'projectPlanDetails/101/child/Tasks/202',
    { TaskId: 202 },
  ],
  [
    ...
    {},
    'projectStatusesLOV',
    { ProjectStatusCode: 'APPROVED', StatusObjectCode: 'PROJECT' },
  ],
  [
    'list_project_status_history',
    { projectId: '101' },
    'projects/101/child/ProjectStatusHistory',
    { StatusHistoryId: 303 },
  ],
  [
    'list_task_status_history',
    { projectId: '101', taskId: '202' },
    'projectPlans/101/child/Tasks/202/child/StatusHistory',
    { StatusHistoryId: 303 },
  ],
  ...
  [
    ...
    { deliverableId: '404' },
    'deliverables/404/child/ProjectTaskAssociation',
    { ObjectAssociationId: 505 },
  ],
  [
    'get_deliverable_task_association',
    { deliverableId: '404', associationId: '505' },
    'deliverables/404/child/ProjectTaskAssociation/505',
    { ObjectAssociationId: 505 },
  ],
  [
    'list_project_team_members',
    { projectId: '101' },
    'projects/101/child/ProjectTeamMembers',
    { TeamMemberId: 606 },
  ],
  [
    'get_project_team_member',
    { projectId: '101', teamMemberId: '606' },
    'projects/101/child/ProjectTeamMembers/606',
    { TeamMemberId: 606 },
  ],
  [
    'list_task_labor_resource_assignments',
    { projectId: '101' },
    'projectPlans/101/child/TaskLaborResourceAssignments',
    { TaskLaborResourceAssignmentId: 707 },
  ],
  [
    'get_task_labor_resource_assignment',
    { projectId: '101', assignmentId: '707' },
    'projectPlans/101/child/TaskLaborResourceAssignments/707',
    { TaskLaborResourceAssignmentId: 707 },
  ],
...
describe('Oracle Project Management documented operations', () => {
  ...
  it.each(reads)(
    '%s uses its documented read resource',
    async (operation, params, path, record) => {
      mocks.json.mockResolvedValue(operation.startsWith('list_') ? page([record]) : record)
      const controller = new AbortController()
      expect((await execute(operation, { ...auth, ...params }, controller.signal)).success).toBe(
        true
      )
  ...
  it('preserves nullable fields and exact identifiers, while excluding unrequested payloads', async () => {
    mocks.json.mockResolvedValue({
      ProjectId: '9.99999999999999999E17',
      ProjectName: 'Implementation',
      ProjectDescription: null,
      ProjectDFF: [{ secret: 'canary' }],
      links: [],
    })
  ...
  it('returns the actual page and combines a project filter without fetching subsequent pages', async () => {
    mocks.json.mockResolvedValue(
      page([{ PlanVersionId: 909, PCRawCostAmounts: '9007199254740993' }], {
        limit: 25,
        offset: 50,
        hasMore: true,
        totalResults: 70,
    ...
    const result = await execute('list_project_budgets', {
      ...auth,
      projectId: '101',
      q: "PlanVersionStatus='Current Working'",
      limit: 25,
      offset: 50,
      totalResults: true,
    })
    ...
    expect(mocks.json.mock.calls[0][1].query).toMatchObject({
      q: "ProjectId=101 and (PlanVersionStatus='Current Working')",
      totalResults: true,
    })
  })
  ...
  it('serializes body IDs as exact numeric JSON tokens, never quoted strings or rounded numbers', async () => {
    ...
    await execute('create_deliverable_task_association', {
      ...auth,
      deliverableId: '404',
      projectId: '999999999999999999',
  ...
  it.each([
    [
      'create_project',
      { projectName: 'Implementation', organizationName: 'Consulting' },
      'projects',
      { ProjectId: 101 },
      { ProjectName: 'Implementation', OwningOrganizationName: 'Consulting' },
    ],
    [
      'update_project',
      { projectId: '101', projectDescription: null },
      'projects/101',
      { ProjectId: 101 },
      { ProjectDescription: null },
    ],
    [
      'create_task',
      { projectId: '101', taskName: 'Design', taskNumber: 'T1', taskLevel: 1 },
      'projectPlans/101/child/Tasks',
      { TaskId: 202 },
      { Name: 'Design', TaskNumber: 'T1', TaskLevel: 1 },
    ],
    [
      'update_task',
      {
      ...
      'projectPlans/101/child/Tasks/202',
      { TaskId: 202 },
      { PhysicalPercentComplete: 50, TaskStatusCode: 'IN_PROGRESS' },
    ],
    [
      'create_deliverable',
      {
        ...
        shortName: 'DES',
        deliverableTypeId: '1',
        deliverablePriorityCode: 'MEDIUM',
        deliverableStatusCode: 'NEW',
      },
      'deliverables',
      { DeliverableId: 404 },
      {
    ...
    [
      'update_deliverable',
      { deliverableId: '404', needByDate: '2026-09-30' },
      'deliverables/404',
      { DeliverableId: 404 },
    ...
    [
      ...
      { deliverableId: '404', associationId: '505', taskId: '203' },
      'deliverables/404/child/ProjectTaskAssociation/505',
      { ObjectAssociationId: 505 },
      { TaskId: 203 },
    ],
    [
      ...
      { projectId: '101', personEmail: 'staff@example.test', projectRole: 'Project Manager' },
      'projects/101/child/ProjectTeamMembers',
      { TeamMemberId: 606 },
      { PersonEmail: 'staff@example.test', ProjectRole: 'Project Manager' },
    ],
    [
      'update_project_team_member',
      {
        ...
        teamMemberId: '606',
        trackTimeFlag: false,
        personEmail: 'ignored@example.test',
        projectRole: 'Ignored',
      },
      'projects/101/child/ProjectTeamMembers/606',
      { TeamMemberId: 606 },
    ...
    [
      'create_task_labor_resource_assignment',
      {
        projectId: '101',
        taskId: '202',
        resourceEmail: 'staff@example.test',
        plannedEffortinHours: 8,
      },
      'projectPlans/101/child/TaskLaborResourceAssignments',
      { TaskLaborResourceAssignmentId: 707 },
      { TaskId: 202, ResourceEmail: 'staff@example.test', PlannedEffortinHours: 8 },
    ],
    [
      'update_task_labor_resource_assignment',
      { projectId: '101', assignmentId: '707', laborResourceId: '808', primaryResourceFlag: false },
      'projectPlans/101/child/TaskLaborResourceAssignments/707',
      { TaskLaborResourceAssignmentId: 707 },
      { LaborResourceId: 808, PrimaryResourceFlag: false },
    ],
    [
      'update_project_budget',
      { planVersionId: '909', lockedFlag: false },
      'projectBudgets/909',
      { PlanVersionId: 909 },
      { LockedFlag: false },
    ],
    [
      ...
      { invoiceId: '110', invoiceComment: null },
      'projectContractInvoices/110',
      { InvoiceId: 110 },
      { InvoiceComment: null },
    ],
  ] as const)(
    '%s sends only its documented writable fields',
    async (operation, params, path, response, body) => {
  ...
  it('creates a budget with documented nested resource amounts and synchronous creation by default', async () => {
      ...
      planningResources: [
        {
          ...
          TaskId: '202',
          PlanningAmounts: [{ Currency: 'USD', RawCostAmounts: 125.5, Quantity: 2 }],
        },
      ],
    })
    ...
    expect(JSON.parse(sentBody())).toMatchObject({
      ...
      PlanningAmounts: 'Cost',
      PlanningResources: [
        { TaskId: 202, PlanningAmounts: [{ Currency: 'USD', RawCostAmounts: 125.5, Quantity: 2 }] },
      ],
    })
  })

  it.each(['Y', null])(
    'rejects deferred or ambiguous budget creation (%s) before sending a request',
    async (deferFinancialPlanCreation) => {
      await expect(
        execute('create_project_budget', {
          ...auth,
          projectId: '101',
          projectName: 'Implementation',
          projectNumber: 'P1',
  ...
  it('keeps project cost self-link keys distinct from CostId for reads and rate updates', async () => {
    const record = {
      CostId: '9007199254740993',
      ExternalBillRate: 125.5,
      '@context': {
        links: [
          {
            rel: 'self',
            href: `${auth.instanceUrl}/fscmRestApi/resources/11.13.18.05/projectCosts/opaque%2Bkey`,
          },
        ],
      },
    }
    ...
    mocks.json.mockResolvedValueOnce(record)
    await execute('update_project_cost', {
      ...auth,
      costKey: 'opaque+key',
      externalBillRate: 125.5,
      billableFlag: true,
  ...
  it.each([
    [
      'adjust_project_cost',
      { costKey: 'opaque+key', adjustmentTypeCode: 'TENANT_CODE', justification: 'Approved' },
      'projectCosts/opaque%2Bkey/action/adjustProjectCosts',
      { AdjustmentTypeCode: 'TENANT_CODE', Justification: 'Approved' },
    ],
    [
      'adjust_project_budget',
      {
        ...
        adjustmentType: 'TENANT_TYPE',
        createNewWorkingVersion: 'Y',
      },
      'projectBudgets/909/action/adjust',
      { adjustmentPercentage: 5, adjustmentType: 'TENANT_TYPE', createNewWorkingVersion: 'Y' },
    ],
    [
      'refresh_project_budget_rates',
      { planVersionId: '909', retainRateOverride: 'Y' },
      'projectBudgets/909/action/refreshRates',
      { retainRateOverride: 'Y' },
    ],
  ] as const)(
    '%s preserves the documented action result',
    async (operation, params, path, body) => {
  ...
  it.each([
    ...
    [
      ...
      { invoiceDate: '2026-09-04', receivablesNumber: 'INV-1' },
    ],
    ...
    'dispatches the fixed %s invoice action without chaining other transitions',
    async (action, endpoint, params) => {
      mocks.json.mockResolvedValue({ result: 'Action completed' })
      await execute('transition_project_contract_invoice', {
        ...auth,
        invoiceId: '110',
        action,
        ...(params as object),
      ...
      expect(mocks.json.mock.calls[0][1].address.relativePath).toBe(
        `projectContractInvoices/110/action/${endpoint}`
      )
  ...
  it.each([
    ...
    [
      'update_task_labor_resource_assignment',
      { projectId: '101', assignmentId: '707', resourceAllocation: 50 },
    ],
    [
      'create_task_labor_resource_assignment',
      { projectId: '101', taskId: '202', resourceEmail: 'a@example.test', laborResourceId: '808' },
    ],
  ...
  it('does not invent empty resource or action shapes, and marks uncertain mutations non-retryable', async () => {
    ...
    const action = await execute('transition_project_contract_invoice', {
      ...auth,
      invoiceId: '110',
      action: 'approve',
    })
  ...
  it('preserves safe foundation errors and cancellation without leaking unexpected errors', async () => {
    ...
    await expect(
      execute('get_project', { ...auth, projectId: '101' }, controller.signal)
