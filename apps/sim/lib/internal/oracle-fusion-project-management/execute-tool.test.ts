...
describe('Oracle Project Management internal dispatch', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.execute.mockResolvedValue({ success: true, output: { result: 'ok' } })
  })

  it.each(OPERATIONS)(
    'registers and dispatches %s without changing semantic inputs',
    async (operation) => {
  ...
  it('rejects unsupported routes without falling back to an arbitrary Oracle request', async () => {
    expect(
      await getInternalToolOperationHandler('oracle_fusion_project_management_arbitrary')
    ).toBeNull()
    const response = await executeOracleFusionProjectManagementTool(
      request('oracle_fusion_project_management_arbitrary')
    )
    expect(response.status).toBe(400)
    expect(mocks.execute).not.toHaveBeenCalled()
  })

  it.each([null, [], 'not an object', { projectId: '../unsafe' }])(
    'preserves operation validation failures as bad input',
    async (input) => {
      mocks.execute.mockRejectedValue(
        new OracleFusionProjectManagementInputError('projectId is invalid')
      )
      const response = await executeOracleFusionProjectManagementTool(
        request('oracle_fusion_project_management_get_project', input)
      )
      expect(response.status).toBe(400)
  ...
  it('propagates cancellation and sanitizes unexpected dispatch errors', async () => {
    ...
    await expect(
      executeOracleFusionProjectManagementTool({
        ...request('oracle_fusion_project_management_list_projects'),
