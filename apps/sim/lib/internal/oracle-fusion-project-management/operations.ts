...
const responseIds = {"project":"ProjectId","plan":"ProjectId","task":"TaskId","deliverable":"DeliverableId","association":"ObjectAssociationId","teamMember":"TeamMemberId","laborAssignment":"TaskLaborResourceAssignmentId","resource":"ResourceId","cost":"CostId","budget":"PlanVersionId","invoice":"InvoiceId","status":"ProjectStatusCode","history":"StatusHistoryId"} as const
...
const operations = {
  ...
  create_project: {
    ...
    "body": {
      ...
      "organizationName": {
        "field": "OwningOrganizationName",
        "kind": "string",
        "maxLength": 240
      },
      "projectCurrencyCode": {
        "field": "ProjectCurrencyCode",
        "kind": "string",
        "maxLength": 15
      },
      "sourceTemplateId": {
  ...
  create_task_labor_resource_assignment: {
    ...
    "body": {
      ...
      "laborResourceId": {
        "field": "LaborResourceId",
        "kind": "id",
        "nullable": true
      },
      "plannedEffortinHours": {
        "field": "PlannedEffortinHours",
        "kind": "number",
        "nullable": true
      },
      "actualEffortinHours": {
        "field": "ActualEffortinHours",
        "kind": "number",
        "nullable": true
      },
      "remainingEffortinHours": {
        "field": "RemainingEffortinHours",
        "kind": "number",
        "nullable": true
      },
      "percentComplete": {
      ...
      "effectiveBillRate": {
        "field": "EffectiveBillRate",
        "kind": "number",
        "nullable": true
      },
      "effectiveCostRate": {
        "field": "EffectiveCostRate",
        "kind": "number",
        "nullable": true
      }
    }
  ...
  adjust_project_cost: {
    ...
    "body": {
      ...
      "capitalizableFlag": {
        "field": "CapitalizableFlag",
        "kind": "boolean",
        "nullable": true
      },
      "holdInvoiceFlag": {
        "field": "HoldInvoiceFlag",
        "kind": "boolean",
        "nullable": true
      },
      "holdRevenueFlag": {
        "field": "HoldRevenueFlag",
        "kind": "boolean",
        "nullable": true
      },
      "targetProjectId": {
  ...
  create_project_budget: {
    ...
    "body": {
      ...
      "planVersionStatus": {
        "field": "PlanVersionStatus",
        "kind": "string",
        "nullable": true
      },
      "budgetCreationMethod": {
        "field": "BudgetCreationMethod",
        "kind": "string",
        "nullable": true
      },
      "budgetGenerationSource": {
        "field": "BudgetGenerationSource",
        "kind": "string",
        "nullable": true
      },
      "planningAmounts": {
        "field": "PlanningAmounts",
        "kind": "string",
        "nullable": true,
        "maxLength": 30
      },
      "sourcePlanType": {
        "field": "SourcePlanType",
        "kind": "string",
        "nullable": true
      },
      "sourcePlanVersionId": {
        "field": "SourcePlanVersionId",
        "kind": "id",
        "nullable": true
      },
...
function parseValue(value: unknown, rule: FieldRule, field: string): unknown {
  ...
  if (rule.kind === 'string') {
    ...
    if (rule.format) {
      ...
      if (!validDate || (rule.format === 'date' ? value !== date :
  ...
  if (field === 'taskLevel' && (value < 1 || value > 999)) invalid(field, 'must be between 1 and 999; level 0 is reserved for the project rollup')
  return value
}
...
function parseBudgetResources(value: unknown): unknown[] {
  if (!Array.isArray(value) || value.length > 100) invalid('planningResources', 'must be an array of at most 100 lines')
  ...
  return value.map((line) => {
    ...
    if (Object.keys(line).some((key) => !allowed.includes(key))) invalid('planningResources', 'contains an unsupported line field')
    ...
    for (const key of ['PlanningStartDate', 'PlanningEndDate']) {
      if (line[key] !== undefined) result[key] = parseValue(line[key], { kind: 'string', format: 'date-time', nullable: true }, key)
    }
    if (line.PlanningAmounts !== undefined) {
      if (!Array.isArray(line.PlanningAmounts) || line.PlanningAmounts.length > 100) invalid('PlanningAmounts', 'must be an array of at most 100 amounts')
      result.PlanningAmounts = line.PlanningAmounts.map((amount) => {
        if (!isPlainRecord(amount) || Object.keys(amount).some((key) => !Object.hasOwn(amountFields, key))) invalid('PlanningAmounts', 'contains an unsupported amount field')
        if (typeof amount.Currency !== 'string' || !amount.Currency.trim()) invalid('Currency', 'is required for each planning amount')
        return Object.fromEntries(Object.entries(amount).map(([key, entry]) => [key, parseValue(entry, amountFields[key], key)]))
      })
    }
    return result
  })
...
function optionalQuery(value: unknown, name: string): string | undefined {
  ...
  if (typeof value !== 'string' || value.length > 4000 || /[\u0000-\u001f\u007f]/.test(value)) invalid(name, 'must be a bounded query string')
  return value
}

function projectRecord(family: keyof typeof responseFields, value: unknown, credential: OracleFusionResolvedCredential): Record<string, unknown> {
  ...
  for (const [field, rule] of Object.entries(fields)) {
    ...
    if (entry === null) {
      ...
      if (typeof entry === 'number' ? !Number.isFinite(entry) || (Number.isInteger(entry) && !Number.isSafeInteger(entry)) :
        typeof entry !== 'string' || entry.length > 128 || !/^-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?$/.test(entry)) throw new Error('Invalid Oracle numeric measure')
      result[field] = String(entry)
    } else if (rule.kind === 'integer') {
      if (typeof entry !== 'number' || !Number.isSafeInteger(entry)) throw new Error('Invalid Oracle integer')
      result[field] = entry
    } else {
  ...
  if (family === 'cost') result.costKey = extractOracleFusionOpaqueKey(value, credential.instanceUrl, { family: 'fscm', relativePath: 'projectCosts' })
  return result
}
...
export async function executeOracleFusionProjectManagementOperation(
  ...
  for (const name of spec.required) {
    if (input[name] === undefined || input[name] === null || (typeof input[name] === 'string' && !input[name].trim())) invalid(name, 'is required')
  }
  if (typeof input.accessToken !== 'string' || !input.accessToken || typeof input.instanceUrl !== 'string' || !input.instanceUrl) invalid('Credential', 'must provide resolved authentication and an application origin')
  ...
  let path = spec.path.replace(/\{([^}]+)\}/g, (_, field: string) => {
    ...
    try { return encodeOracleFusionPathSegment(input[field]) } catch { return invalid(field, 'must be a safe opaque resource key') }
  })
  ...
  if (spec.method === 'PATCH' && Object.keys(body).length === 0) invalid('Update', 'requires at least one editable field')
  if (operation === 'create_project_budget' && body.DeferFinancialPlanCreation === undefined) body.DeferFinancialPlanCreation = 'N'
  if (operation === 'create_task_labor_resource_assignment' || operation === 'update_task_labor_resource_assignment') {
    ...
    if (hasEmail === hasId || input.resourceEmail === null || input.laborResourceId === null) invalid('Resource', 'requires exactly one of resourceEmail and laborResourceId')
  }
  if (operation === 'transition_project_contract_invoice') {
    if (typeof input.action !== 'string' || !Object.hasOwn(INVOICE_ACTIONS, input.action)) invalid('action', 'is not a supported invoice transition')
    const action = input.action as keyof typeof INVOICE_ACTIONS
    const allowed = action === 'release' ? ['receivablesNumber', 'creditMemoReasonCode', 'invoiceDate', 'creditMemoReasonMeaning'] : action === 'unrelease' ? ['unreleaseComments'] : []
    if (Object.keys(body).some((name) => !allowed.includes(name))) invalid('Invoice transition', 'contains fields that do not apply to the selected action')
  ...
  if (spec.list) {
    ...
    if (input.totalResults !== undefined) query.totalResults = parseValue(input.totalResults, { kind: 'boolean' }, 'totalResults') as boolean
    const filters: string[] = spec.filter ? [spec.filter] : []
    if (['cost', 'budget', 'invoice'].includes(spec.family) && input.projectId !== undefined && input.projectId !== '') filters.push(`ProjectId=${requireProjectManagementId(input.projectId, 'projectId')}`)
    if (spec.family === 'laborAssignment' && input.taskId !== undefined && input.taskId !== '') filters.push(`TaskId=${requireProjectManagementId(input.taskId, 'taskId')}`)
    if (spec.family === 'status' && input.statusObjectCode !== undefined && input.statusObjectCode !== '') {
  ...
  try {
    if (spec.method === 'DELETE') {
      ...
      return { success: true, output: { deleted: true, id: idField ? requireProjectManagementId(input[idField], idField) : undefined } }
    }
    const request: OracleFusionRequest = spec.method === 'GET' ? { address, method: 'GET', query } : {
      address, method: spec.method, body,
    ...
    signal?.throwIfAborted()
    if (spec.action) {
      if (!isPlainRecord(data) || typeof data.result !== 'string') throw new Error('Invalid Oracle action result')
      return { success: true, output: { result: data.result } }
    }
    if (spec.list) {
      const page = parseOracleFusionCollection(data, (item) => projectRecord(spec.family, item, credential), { expectedOffset: query.offset as number, maxItems: query.limit as number })
    ...
    return {
      success: false,
      output: {},
      error: error instanceof OracleFusionProviderError ? error.message : spec.method === 'GET'
