...
import type { FccsAddMemberParams } from '@/tools/oracle_epm_fccs/types'
...
export const executeFccsAddMemberOperation: InternalToolOperationImplementation<
  FccsAddMemberParams
> = async (params, signal, context) => {
  ...
  return fccsResult(
    projectFccsResponse(
      fccsMemberSchema,
      await ctx.client.request(fccsEndpoints.addMember, {
        pathParams: { application: input.application, dimension: input.dimension },
        json: { memberName: input.member, parentName: input.parentName },
        signal,
      })
    )
  )
