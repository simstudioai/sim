...
describe('Learning request contracts', () => {
  ...
  it('preserves PATCH omission and explicit null without materializing Oracle defaults', () => {
    expect(schemas.recordPatchSchema.parse({ completedDate: null })).toEqual({
      completedDate: null,
    })
    expect(schemas.selfPatchSchema.parse({ learningItemDescription: null })).toEqual({
      learningItemDescription: null,
    })
    expect(schemas.profilePatchSchema.parse({ completionComments: 'Verified' })).toEqual({
      completionComments: 'Verified',
    })
  ...
  it('rejects creation-only IDs, read-only fields, nested mutations, and upload material', () => {
    ...
    expect(
      schemas.contentPostSchema.safeParse({
        Title: 'Package',
    ...
    expect(
      schemas.update_completion_detailSchema.safeParse({
        ...auth,
        personId: '1',
        recordId: '2',
  ...
  it('accepts only the documented scalar completion-detail updates', () => {
    const body = {
      ...
      activityAttemptCompletionReasonCode: null,
    }
    expect(schemas.completionPatchSchema.parse(body)).toEqual(body)
    expect(
      schemas.completionPatchSchema.safeParse({ completedDate: '2026-09-05T00:00:00Z' }).success
    ).toBe(false)
  })
  ...
  it('validates calendar dates, timestamp offsets, required authoring fields, and page bounds', () => {
    ...
    expect(
      schemas.recordPatchSchema.safeParse({ completedDate: '2026-09-05T10:00:00-07:00' }).success
    ).toBe(true)
    expect(
      schemas.selfPostSchema.safeParse({ learningItemTitle: 'Incomplete draft' }).success
    ).toBe(false)
  ...
  it('rejects oversized serialized bodies before JSON parsing', () => {
    const result = schemas.update_content_itemSchema.safeParse({
      ...auth,
      contentId: '1',
      body: ' '.repeat(MAX_INLINE_MATERIALIZATION_BYTES + 1),
    })
  ...
  it('parses JSON bodies while retaining string IDs and rejecting malformed JSON', () => {
    ...
    expect(schemas.create_learning_recordSchema.parse(input).body.learningItemId).toBe(
      '9007199254740993'
    )
    expect(schemas.create_learning_recordSchema.safeParse({ ...input, body: '{' }).success).toBe(
      false
    )
    expect(
      schemas.create_learning_recordSchema.safeParse({
        ...input,
        body: { learningItemId: 9007199254740992 },
      }).success
    ).toBe(false)
  })
})
