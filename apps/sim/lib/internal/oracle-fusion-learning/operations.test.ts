...
describe('Learning operations through the Fusion client', () => {
  ...
  it('targets root completion details for updates and never sends effectiveDate to that child', async () => {
    ...
    expect(mocks.json.mock.calls[2][1]).toMatchObject({
      method: 'PATCH',
      address: {
        relativePath: 'learnerLearningRecords/record-key/child/completionDetails/detail-key',
      },
      body: { activityAttemptStatus: 'ORA_ASSN_TASK_COMPLETED' },
    })
  })
  ...
  it('reads completion evidence nested under the selected offering without claiming a catalog', async () => {
    ...
    mocks.json.mockResolvedValueOnce(
      page([
        linked('learnerLearningRecords/record-key/child/selectedCourseOfferings/offering-key', {
          assignmentRecordId: '4',
        }),
      ])
    )
    mocks.json.mockResolvedValueOnce(
  ...
  it('processes profiles using the ADF action contract and preserves the numeric result', async () => {
    mocks.json
      .mockResolvedValueOnce(
        page([linked('learningAssignmentProfiles/profile-key', { assignmentProfileId: '5' })])
      )
      .mockResolvedValueOnce({ result: 123 })
    expect(await operations.executeProcessAssignmentProfile({ ...auth, profileId: '5' })).toEqual({
      success: true,
      output: { result: 123 },
    })
    expect(mocks.json.mock.calls[1][1]).toEqual({
      address: {
        family: 'hcm',
        relativePath: 'learningAssignmentProfiles/profile-key/action/process',
      },
      method: 'POST',
  ...
  it('uses bodyless deletion acknowledgements and does not manufacture Oracle fields', async () => {
    mocks.json.mockResolvedValue(
      page([linked('learningSelfPacedItems/draft-key', { learningItemId: '6' })])
    )
    mocks.empty.mockResolvedValue(undefined)
    expect(await operations.executeDeleteSelfPacedItem({ ...auth, learningItemId: '6' })).toEqual({
      success: true,
      output: { deleted: true },
    })
    expect(mocks.empty.mock.calls[0][1]).toEqual({
      address: { family: 'hcm', relativePath: 'learningSelfPacedItems/draft-key' },
      method: 'DELETE',
    })
  })
  ...
  it('creates URL content and excludes upload credentials from returned metadata', async () => {
    mocks.json.mockResolvedValue({
      ...
      UploadLocation: 'https://upload.example.com',
    })
    const result = await operations.executeCreateWebLinkContent({
      ...auth,
      body: { Title: 'Web course', URL: 'https://example.com/course' },
    })
    expect(mocks.json.mock.calls[0][1].body).toEqual({
      Title: 'Web course',
  ...
  it('creates event activities under their validated event parent', async () => {
    mocks.json.mockResolvedValueOnce(
      page([linked('learningEvents/event-key', { learningItemId: '8' })])
    )
    mocks.json.mockResolvedValueOnce({
      activityId: '9',
      activityNumber: 'ACT-1',
      status: 'TEST_STATUS',
    })
    const body = {
      activityNumber: 'ACT-1',
    ...
    expect(mocks.json.mock.calls[1][1]).toMatchObject({
      address: { relativePath: 'learningEvents/event-key/child/activities' },
      method: 'POST',
      body,
    })
  })
  ...
  it('selects an offering beneath the course assignment with the person bound by context', async () => {
    ...
    await operations.executeSelectCourseOffering({
      ...auth,
      personId: '1',
      recordId: '2',
      body: { learningItemId: '5' },
    })
  ...
  it('adds profile criteria using the documented child name and exact source ID', async () => {
    ...
    await operations.executeAddAssignmentProfileCriterion({
      ...auth,
      profileId: '5',
      body: { assignmentProfileCriteriaTypeId: '9007199254740993' },
    })
    expect(mocks.json.mock.calls[1][1].address.relativePath).toBe(
      'learningAssignmentProfiles/profile-key/child/learningAssignmentProfileCriteria'
    )
    expect(serializeOracleFusionJsonBody(mocks.json.mock.calls[1][1].body)).toBe(
      '{"assignmentProfileCriteriaTypeId":9007199254740993}'
    )
  })
  ...
  it('does not retry uncertain mutations and returns fixed provider errors', async () => {
    ...
    ).rejects.toEqual(new OracleFusionProviderError('Oracle Fusion Learning request failed', 504))
    expect(mocks.json).toHaveBeenCalledTimes(1)
  })
})
