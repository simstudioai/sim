...
export const decimalIdSchema = z.string().trim().regex(/^[1-9]\d*$/, 'ID must be a positive decimal string').max(19).refine(
  (value) => value.length <= 19 && /^[1-9]\d*$/.test(value) && BigInt(value) <= 9_223_372_036_854_775_807n,
...
function jsonBody<S extends z.ZodType>(schema: S) {
  return z.preprocess((value, context) => {
    ...
    if (value.length > MAX_INLINE_MATERIALIZATION_BYTES || new TextEncoder().encode(value).byteLength > MAX_INLINE_MATERIALIZATION_BYTES) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: 'Learning writable fields exceed the inline payload limit' })
      return z.NEVER
    }
...
export const selfPostSchema = z.object({
...
export const selfPatchSchema = z.object({
  ...
  learningItemProvider: z.string().max(32).nullable().optional(),
  learningItemProviderType: z.string().max(32).nullable().optional(),
}).strict().refine((value) => Object.keys(value).length > 0, 'Provide at least one writable field')
...
export const offeringPostSchema = z.object({
  learningItemId: decimalIdSchema,
...
export const criterionPostSchema = z.object({
