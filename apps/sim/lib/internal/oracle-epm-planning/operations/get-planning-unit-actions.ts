...
import { type PlanningOperationContext, parsePlanningResponse, planningUnitActionsSchema } from '@/lib/internal/oracle-epm-planning/schema'
import type { OracleEpmPlanningGetPlanningUnitActionsParams, OracleEpmPlanningResponse } from '@/tools/oracle_epm_planning/types'
...
export async function executeOracleEpmPlanningGetPlanningUnitActions(
  ...
  const parsed = parsePlanningResponse(planningUnitActionsSchema,
    await context.client.request(planningEndpoints.planningUnitActions, {
      ...
      stream: new TextEncoder().encode(new URLSearchParams({ pmMembers: input.pmMembers }).toString()),
