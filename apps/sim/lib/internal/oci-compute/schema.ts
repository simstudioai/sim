...
}).strict().refine((value) => value.ocpus === undefined || value.vcpus === undefined, {
  message: 'Specify ocpus or vcpus, not both',
})

const ipv6Pair = z.object({ ipv6Address: z.string().max(46).optional(), ipv6SubnetCidr: z.string().max(64).optional() }).strict()
const vnic = z.object({
...
const imageSourceFields = {
  ...
  bootVolumeVpusPerGB: z.number().int().refine((n) => n === 10 || n === 20 || (n >= 30 && n <= 120), 'Unsupported boot volume performance').optional(),
  kmsKeyId: id.optional(),
}
const configurationSource = z.discriminatedUnion('sourceType', [
  z.object({
...
]).refine((source) => source.sourceType !== 'image' || !source.imageId || !source.instanceSourceImageFilterDetails, {
...
const launchFields = {
  ...
  displayName: name.optional(),
  shape: name.optional(),
  shapeConfig: shapeConfig.optional(),
...
export const configurationDetailsSchema = z.object({
  ...
  blockVolumes: z.array(z.object({ volumeId: id, attachDetails: attachVolume }).strict()).max(32).optional(),
...
export const ociComputeSchemas = {
  ...
  change_instance_compartment: common.extend({ ...instance, ...match, compartmentId: id }),
  ...
  update_image: common.extend({ ...image, ...resourceTags, ...match }),
  delete_image: common.extend({ ...image, ...match }),
  change_image_compartment: common.extend({ ...image, ...match, compartmentId: id }),
  ...
  update_instance_configuration: common.extend({ ...configuration, ...resourceTags, ...match }),
  ...
  change_instance_configuration_compartment: common.extend({ ...configuration, ...match, compartmentId: id }),
  ...
  update_instance_pool: common.extend({ ...pool, ...resourceTags, ...match, instanceConfigurationId: id.optional(), size: z.number().int().min(0).optional(), placementConfigurations: json(z.array(placement).min(1).max(20)).optional(), instanceDisplayNameFormatter: z.string().max(255).optional(), instanceHostnameFormatter: z.string().max(255).optional() }),
  instance_pool_action: common.extend({ ...pool, ...match, action: z.enum(['START', 'STOP', 'SOFTSTOP', 'RESET', 'SOFTRESET']) }),
  terminate_instance_pool: common.extend({ ...pool, ...match }),
  change_instance_pool_compartment: common.extend({ ...pool, ...match, compartmentId: id }),
  list_instance_pool_instances: common.extend({ ...pool, ...namedListing }),
  get_instance_pool_instance: common.extend({ ...pool, ...instance }),
  attach_instance_pool_instance: common.extend({ ...pool, ...instance }),
  detach_instance_pool_instance: common.extend({ ...pool, ...instance, isAutoTerminate: z.boolean().default(false), isDecrementSize: z.boolean().default(true) }),
