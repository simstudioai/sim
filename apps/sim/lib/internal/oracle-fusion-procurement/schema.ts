...
export const procurementIdentifierSchema = z.unknown().transform((value, context) => {
  ...
  if (
    ...
    context.addIssue({
      ...
      message: 'Expected an exact non-negative int64 ID; supply large identifiers as decimal strings',
    })
    return z.NEVER
  }
  return normalized
...
export const procurementKeySchema = z.string().min(1).max(2048).refine((value) => {
  try {
...
const supplierSiteAssignmentBodySchema = z.object({
...
const requisitionLineBodySchema = z.object({
...
const draftPurchaseOrderScheduleBodySchema = z.object({
...
const draftPurchaseOrderLineBodySchema = z.object({
  LineNumber: z.number().finite(),
...
const negotiationLineBodySchema = z.object({
...
const negotiationSupplierBodySchema = z.object({
...
export const procurementWriteSchemas = {
  ...
  createPurchaseRequisition: z.object({
  ...
  updatePurchaseRequisition: z.object({
  ...
  createDraftPurchaseOrder: z.object({
  ...
  updateDraftPurchaseOrder: z.object({
  ...
  createSupplierNegotiation: z.object({
    ProcurementBUId: requestInteger,
  ...
  updateSupplierNegotiation: z.object({
    NegotiationTitle: z.string().max(80).nullable().optional(),
    CurrencyCode: z.string().max(15).nullable().optional(),
    OpenDate: requestDateTime.nullable().optional(),
    CloseDate: requestDateTime.nullable().optional(),
