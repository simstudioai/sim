...
describe('Recruiting provider operations', () => {
  ...
  it('lists posting records through publishedJobs', async () => {
    ...
    const result = await operations.executeListRequisitionPostings({
      ...auth,
      ...{ requisitionId: '1' },
    })
    expect(mocks.json).toHaveBeenLastCalledWith(
      ...
      expect.objectContaining({
        address: {
          family: 'hcm',
          relativePath: 'recruitingJobRequisitions/opaque-key/child/publishedJobs',
        },
        query: expect.objectContaining({ limit: 20, offset: 0, onlyData: true }),
      }),
      undefined
  ...
  it('lists application records through recruitingJobApplications', async () => {
    ...
    expect(mocks.json).toHaveBeenLastCalledWith(
      expect.anything(),
      expect.objectContaining({
        address: { family: 'hcm', relativePath: 'recruitingJobApplications' },
        query: expect.objectContaining({ limit: 20, offset: 0, onlyData: true }),
      }),
      undefined
    )
  ...
  it('lists offer records through recruitingJobOffers', async () => {
    ...
    expect(mocks.json).toHaveBeenLastCalledWith(
      expect.anything(),
      expect.objectContaining({
        address: { family: 'hcm', relativePath: 'recruitingJobOffers' },
        query: expect.objectContaining({ limit: 20, offset: 0, onlyData: true }),
      }),
      undefined
    )
  ...
  it('lists interview_schedule records through recruitingInterviewSchedulesLOV', async () => {
    ...
    expect(mocks.json).toHaveBeenLastCalledWith(
      expect.anything(),
      expect.objectContaining({
        address: { family: 'hcm', relativePath: 'recruitingInterviewSchedulesLOV' },
        query: expect.objectContaining({ limit: 20, offset: 0, onlyData: true }),
      }),
      undefined
    )
  ...
  it('lists requisition_template records through recruitingJobRequisitionTemplatesLOV', async () => {
    ...
    expect(mocks.json).toHaveBeenLastCalledWith(
      expect.anything(),
      expect.objectContaining({
        address: { family: 'hcm', relativePath: 'recruitingJobRequisitionTemplatesLOV' },
        query: expect.objectContaining({ limit: 20, offset: 0, onlyData: true }),
      }),
      undefined
    )
  ...
  it('lists representative records through recruitingRepresentativesLOV', async () => {
    ...
    expect(mocks.json).toHaveBeenLastCalledWith(
      expect.anything(),
      expect.objectContaining({
        address: { family: 'hcm', relativePath: 'recruitingRepresentativesLOV' },
        query: expect.objectContaining({ limit: 20, offset: 0, onlyData: true }),
      }),
      undefined
    )
  ...
  it('preserves zero years of candidate skill experience', async () => {
    mocks.json
      ...
      .mockResolvedValueOnce(page([{ SkillId: '1', YearsOfExperience: 0 }]))
  ...
  it('preserves bounded pagination even when the estimated total is smaller', async () => {
    mocks.json.mockResolvedValue(
      page([{ CandidateNumber: '1' }], { hasMore: true, offset: 20, totalResults: 0 })
    )
  ...
  it('rejects oversized pages instead of returning unbounded results', async () => {
    ...
    await expect(operations.executeListCandidates({ ...auth, limit: 1 })).rejects.toMatchObject({
      status: 502,
    })
  })
  ...
  it.each([
    ...
    page([
      { CandidateNumber: '1', links: [{ rel: 'self', href: 'https://other.example/candidate' }] },
    ]),
  ])('rejects missing, ambiguous, mismatched, or foreign candidate lookups', async (raw) => {
    mocks.json.mockResolvedValue(raw)
    await expect(
      operations.executeDeleteCandidate({ ...auth, candidateNumber: '1' })
  ...
  it('finds an offer by exact primary key without treating its ID as a path key', async () => {
    mocks.json.mockResolvedValue(
      page([item('recruitingJobOffers', { OfferId: '9007199254740993' })])
    )
  ...
  it('preserves null and omits undefined candidate updates', async () => {
    mocks.json.mockResolvedValueOnce(page([candidate()])).mockResolvedValueOnce(candidate())
    await operations.executeUpdateCandidate({
      ...auth,
      candidateNumber: '1',
      body: { FirstName: 'Taylor', Email: null, LastName: undefined },
    })
  ...
  it('serializes requisition IDs as exact numeric JSON tokens', async () => {
    ...
    await operations.executeCreateRequisition({
      ...auth,
      body: {
        Title: 'Engineer',
  ...
  it('deletes a phone only under the resolved candidate and phone keys', async () => {
    mocks.json
      ...
      .mockResolvedValueOnce(
        page([item('recruitingCandidates/opaque-key/child/candidatePhones', { PhoneId: '2' })])
      )
    mocks.empty.mockResolvedValue(undefined)
    const result = await operations.executeDeleteCandidatePhone({
      ...auth,
      candidateNumber: '1',
      phoneId: '2',
    })
    ...
    expect(mocks.empty).toHaveBeenCalledWith(
      expect.anything(),
      {
        address: {
          family: 'hcm',
          relativePath: 'recruitingCandidates/opaque-key/child/candidatePhones/opaque-key',
        },
        method: 'DELETE',
      },
      undefined
  ...
  it('does not repeat failed writes', async () => {
    ...
    await expect(
      operations.executeCreateCandidate({ ...auth, body: { FirstName: 'Taylor' } })
  ...
  it('forwards cancellation and stops before a second request', async () => {
    ...
    await expect(
      operations.executeGetCandidate({ ...auth, candidateNumber: '1' }, controller.signal)
    ).rejects.toThrow()
    expect(mocks.json).toHaveBeenCalledOnce()
