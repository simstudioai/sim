...
export const decimalIdSchema = z
  ...
  .refine((value) => !/^[1-9]\d{0,18}$/.test(value) || BigInt(value) <= 9_223_372_036_854_775_807n, 'ID exceeds int64 range')

export const stringIdSchema = z.string().trim().min(1).max(256)
  .regex(/^[a-zA-Z0-9_.-]+$/, 'ID must contain only letters, digits, underscores, periods, or hyphens')
...
export const createCandidateFieldsSchema = z.object({
...
}).strict().refine((body) => Object.values(body).some((value) => value !== undefined), 'Provide at least one field')

/** Documented scalar fields for recruitingcandidates-candidatenumber-patch. */
export const updateCandidateFieldsSchema = z.object({
  FirstName: z.string().max(150).nullable().optional(),
  LastName: z.string().max(150).nullable().optional(),
  MiddleNames: z.string().max(80).nullable().optional(),
  Email: z.string().max(240).nullable().optional(),
...
export const createPhoneFieldsSchema = z.object({
...
export const updatePhoneFieldsSchema = z.object({
  PhoneNumber: z.string().max(60).nullable().optional(),
  CountryCodeNumber: z.string().max(30).nullable().optional(),
  AreaCode: z.string().max(30).nullable().optional(),
  LegislationCode: z.string().max(4).nullable().optional(),
...
export const createRequisitionFieldsSchema = z.object({
...
export const updateRequisitionFieldsSchema = z.object({
