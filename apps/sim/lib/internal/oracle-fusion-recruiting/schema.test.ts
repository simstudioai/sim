...
describe('Recruiting schemas', () => {
  it.each(['0', '-1', '1.5', '1e3', '9223372036854775808', '1;Other=2'])('rejects invalid numeric ID %s', (id) => {
  ...
  it.each(['1;Other=2', '1,Other=2', '../1', 'a/b', 'a%2fb'])('rejects unsafe string ID %s', (id) => {
  ...
  it('requires the documented requisition creation fields', () => {
    expect(schemas.createRequisitionFieldsSchema.safeParse({ Title: 'Engineer' }).success).toBe(false)
  })
  it('rejects unknown fields and nested mutations', () => {
    expect(schemas.createCandidateFieldsSchema.safeParse({ candidatePhones: [] }).success).toBe(false)
  ...
  it('preserves omitted versus explicitly cleared fields', () => {
    ...
    expect(schemas.updateCandidateFieldsSchema.parse({ FirstName: 'Taylor' })).not.toHaveProperty('Email')
  })
  it('rejects numeric JS values for int64 write fields', () => {
    expect(schemas.updateRequisitionFieldsSchema.safeParse({ RecruiterId: 9007199254740992 }).success).toBe(false)
  })
})
