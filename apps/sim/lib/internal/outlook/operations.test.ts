...
describe('Outlook attachment downloads', () => {
  ...
  it('preserves zero-byte file attachments', async () => {
    mocks.json.mockResolvedValue({ ...ATTACHMENT_METADATA, size: 0 })

    const result = await executeOutlookGetAttachment(ATTACHMENT_INPUT)

    if (!isInternalToolFileResult(result)) throw new Error('Expected a file result')
    expect(result.files[0]?.buffer.byteLength).toBe(0)
    expect(mocks.buffer).toHaveBeenCalledOnce()
  })

  it.each(['#microsoft.graph.itemAttachment', '#microsoft.graph.referenceAttachment'])(
    'preserves %s metadata without trying to download raw content',
    async (attachmentType) => {
      mocks.json.mockResolvedValue({ ...ATTACHMENT_METADATA, '@odata.type': attachmentType })
      ...
      expect(result).toMatchObject({
        success: true,
        output: {
          results: { attachmentType, name: 'report.xlsx' },
          attachments: [],
        },
      })
  ...
  it('accepts the exact metadata size limit and bounds the raw body independently', async () => {
    mocks.json.mockResolvedValue({ ...ATTACHMENT_METADATA, size: MAX_BUFFERED_TRANSFER_BYTES })
    mocks.buffer.mockRejectedValue(
      new PayloadSizeLimitError({
  ...
  it.each(['metadata', 'download'])('preserves Graph %s errors and HTTP status', async (phase) => {
    const error = new OutlookOperationError('Attachment not found', 404)
    if (phase === 'metadata') mocks.json.mockRejectedValueOnce(error)
    else mocks.buffer.mockRejectedValueOnce(error)

    await expect(executeOutlookGetAttachment(ATTACHMENT_INPUT)).rejects.toBe(error)
    if (phase === 'metadata') expect(mocks.buffer).not.toHaveBeenCalled()
  })

  it('uses response MIME and a fallback filename when metadata omits them', async () => {
    mocks.json.mockResolvedValue({ '@odata.type': '#microsoft.graph.fileAttachment' })
    mocks.buffer.mockResolvedValue({ buffer: Buffer.from('text'), contentType: 'text/plain' })

    const result = await executeOutlookGetAttachment(ATTACHMENT_INPUT)

    if (!isInternalToolFileResult(result)) throw new Error('Expected a file result')
    expect(result.files[0]?.name).toBe('attachment')
    expect(result.files[0]?.mimeType).toBe('text/plain')
  })

  it('stops between metadata and raw downloads when cancelled', async () => {
    const controller = new AbortController()
    mocks.json.mockImplementationOnce(async () => {
      controller.abort(new DOMException('cancelled', 'AbortError'))
      return ATTACHMENT_METADATA
    })

    await expect(
      executeOutlookGetAttachment(ATTACHMENT_INPUT, controller.signal)
    ).rejects.toMatchObject({ name: 'AbortError' })
    expect(mocks.buffer).not.toHaveBeenCalled()
  })

  it('does not produce a file result if cancellation occurs during download', async () => {
    const controller = new AbortController()
    mocks.buffer.mockImplementationOnce(async () => {
      controller.abort(new DOMException('cancelled', 'AbortError'))
      return { buffer: Buffer.alloc(0), contentType: null }
    })
