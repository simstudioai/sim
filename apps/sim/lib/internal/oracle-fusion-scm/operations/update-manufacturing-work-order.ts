...
/** Oracle 26C contract: op-workorders-workorderid-patch.html */
const bodySchema = z
  .object({
...
const inputSchema = oracleFusionScmMutationInputSchema(bodySchema, ['manufacturingWorkOrderKey'])

/** Updates the documented manufacturing work order fields without numeric identifier rounding. */
export async function executeOracleFusionScmUpdateManufacturingWorkOrder(
  rawInput: unknown,
  signal?: AbortSignal
): Promise<ToolResponse> {
  const input = inputSchema.parse(rawInput)
  const collection = oracleFusionScmCollectionPath('manufacturingWorkOrders', input)
  const resourceKey = oracleFusionScmOpaqueKeySchema.parse(input.manufacturingWorkOrderKey)
  ...
  if (item.manufacturingWorkOrderKey !== resourceKey) {
