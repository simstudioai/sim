 ...
 */
function createSimpleLinearLayout(
  nodes: LayoutNode[],
  options: LayoutOptions
): LayoutResult {
...
function calculatePositions(
  ...
  if (isHorizontal) {
    ...
    layers.forEach((layer, layerIndex) => {
      ...
      const totalHeight = layer.reduce((sum, node) => sum + node.height, 0) + 
        (layer.length - 1) * spacing.vertical
      
      // Starting Y position based on alignment
      let startY: number
      switch (alignment) {
        case 'start': startY = padding.y; break
        case 'end': startY = -totalHeight + padding.y; break
        case 'center':
        default: startY = -totalHeight / 2 + padding.y; break
      }
    ...
    layers.forEach((layer, layerIndex) => {
      ...
      const totalWidth = layer.reduce((sum, node) => sum + node.width, 0) + 
        (layer.length - 1) * spacing.horizontal
      
      // Starting X position based on alignment
      let startX: number
      switch (alignment) {
        case 'start': startX = padding.x; break
        case 'end': startX = -totalWidth + padding.x; break
        case 'center':
        default: startX = -totalWidth / 2 + padding.x; break
      }
