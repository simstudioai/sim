...
export class EditWorkflowClientTool extends BaseTool {
  ...
  async execute(toolCall: CopilotToolCall, options?: ToolExecutionOptions): Promise<ToolExecuteResult> {
    ...
    try {
      ...
      if (!Array.isArray(operations) || !workflowId) { options?.onStateChange?.('errored'); return { success:false, error:'operations and workflowId are required' } }

      const body = { methodId: 'edit_workflow', params: { operations, workflowId, ...(currentUserWorkflow?{currentUserWorkflow}:{}) }, toolCallId: toolCall.id, toolId: toolCall.id }
      ...
      if (!response.ok) { const e = await response.json().catch(()=>({})); options?.onStateChange?.('errored'); return { success:false, error: e?.error || 'Failed to edit workflow' } }
      ...
      try {
        ...
        if (yamlContent && typeof yamlContent === 'string') {
          ...
          logger.info('Diff store updated from edit_workflow result', { yamlLength: yamlContent.length })
        }
      } catch (e) {
        logger.warn('Failed to update diff store from edit_workflow result', { error: e instanceof Error ? e.message : String(e) })
      }
