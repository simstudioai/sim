...
describe('view operations', () => {
  ...
  it('create_view sends the config through the use case and echoes names back', async () => {
    ...
    expect(result.success).toBe(true)
    // This layer forwards the model's column NAMES verbatim, with strictRefs so
    // the service rejects unknown refs rather than silently storing them.
    // Name -> storage-id normalization is `normalizeViewConfigForStorage`'s job
    // inside `createTableView`, which is mocked here and covered by its own tests.
    const created = mockCreateTableView.mock.calls[0][0]
    expect(created.name).toBe('Mine')
    expect(created.strictRefs).toBe(true)
    expect(created.config).toMatchObject({
      filter: { all: [{ field: 'Owner', op: 'eq', value: 'me' }] },
      hiddenColumns: ['Status'],
    })
    // The tool projects the stored (id-keyed) result back to names for the model.
  ...
  it('surfaces a service-side unknown-column rejection as a safe tool failure', async () => {
    // strictRefs makes the service reject refs that aren't columns; the tool must
    // turn that into a clean result, never a raw error handed to the model.
    mockCreateTableView.mockRejectedValueOnce(
      new OrchestrationError('invalid', 'Unknown column(s): Nope')
    )
    const result = await userTableServerTool.execute(
      {
        operation: 'create_view',
  ...
  it('update_view patches only the provided slices, null clears', async () => {
    mockGetTableView.mockResolvedValueOnce({
      id: 'view_1',
      name: 'Open items',
  ...
  it('delete_view surfaces a missing view as a safe failure', async () => {
  ...
  it('refuses view mutations when the table-views flag is off', async () => {
    mockIsFeatureEnabled.mockResolvedValue(false)
    const result = await userTableServerTool.execute(
  ...
  it('query_rows applies the view predicate and order, explicit args override', async () => {
    mockGetTableView.mockResolvedValue({
      id: 'view_1',
      name: 'Open items',
      config: {
