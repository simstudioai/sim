    ...
    try {
      switch (operation) {
        ...
        case 'query_rows': {
          ...
          // A saved view supplies predicate/order when the model passes
          // `viewId`. Read through its own use case, then converted back to
          // column NAMES so it enters queryTableRows on the same validated path
          // as wire args — select option ids survive that round-trip because the
          // inbound resolver accepts an id as readily as a name. An explicit
          // filter or order wins, so the model can narrow a view without
          // redefining it.
          let viewPredicate: TablePredicate | undefined
          let viewOrder: SortSpec | undefined
          if (args.viewId) {
            const { view, columns } = await executeCopilotTableUseCase(
              context,
              readTableViewUseCase,
              { tableId: args.tableId, workspaceId, viewId: args.viewId },
              { tableId: args.tableId }
            )
            const nameById = buildNameById({ columns })
            if (view.config.filter)
              viewPredicate = predicateIdsToNames(view.config.filter, nameById)
            if (view.config.sort?.length) viewOrder = sortSpecIdsToNames(view.config.sort, nameById)
          }
          ...
          const result = await executeCopilotTableUseCase(
            ...
            queryTableRows,
            {
              ...
              predicate: args.filter
                ...
                : viewPredicate,
              sort: (args.order as SortSpec | undefined) ?? viewOrder,
        ...
        case 'delete_view': {
          ...
          // Product gate, not authorization — the use cases own authorization.
          // Keyed on the workspace's HOST org, matching the table page, so an
          // agent can't create views the user can't see or present view chrome
          // the workspace doesn't have. A slice-less show_view merely opens the
          // table and stays ungated.
          ...
          /** Wire args are column-NAME keyed; the use cases validate and
           *  translate them to storage exactly as the HTTP surface does. */
          const viewConfigFromArgs = (): TableViewConfig => {
            ...
            if ('order' in args) {
              const orderSpec = args.order as SortSpec | undefined
              config.sort = orderSpec?.length ? orderSpec : null
            }
            if ('hiddenColumns' in args && Array.isArray(args.hiddenColumns)) {
              config.hiddenColumns = args.hiddenColumns as string[]
            }
            return config
          }

          /** Stored configs are id-keyed; the wire speaks names. Select option
           *  ids in predicate values round-trip unchanged, since the inbound
           *  resolver accepts an id as well as a name. */
          const toWireView = (view: TableView, columns: ColumnDefinition[]) => {
            const nameById = buildNameById({ columns })
            return {
              id: view.id,
              name: view.name,
              filter: view.config.filter ? predicateIdsToNames(view.config.filter, nameById) : null,
              order: view.config.sort ? sortSpecIdsToNames(view.config.sort, nameById) : null,
          ...
          try {
            if (operation === 'show_view') {
              // Persists nothing. Reads the table through its use case so the
              // draft is validated against the real schema, then hands the
              // agent back exactly what it asked for, name-keyed — the client
              // opens the tab and seeds it as applied-but-unsaved state.
              const { table } = await executeCopilotTableUseCase(
                context,
                readTableUseCase,
                { tableId: args.tableId, workspaceId },
              ...
              const columns = (table.schema as TableSchema).columns ?? []
              const draftConfig = viewConfigFromArgs()
              if (draftConfig.filter) validatePredicate(draftConfig.filter, columns)
              if (draftConfig.sort?.length) validateSortSpec(draftConfig.sort, columns)
              const unknownHidden = (draftConfig.hiddenColumns ?? []).filter(
                (name) => !columns.some((col) => columnMatchesRef(col, name))
              )
              if (unknownHidden.length > 0) {
                return {
                  success: false,
                  message: `Unknown column(s): ${unknownHidden.join(', ')}`,
                }
              }
              return {
                success: true,
                message: `Showing ${table.name} with the requested view applied (not saved)`,
                data: {
                  tableId: table.id,
                  title: table.name,
                  draft: {
                    filter: draftConfig.filter ?? null,
                    order: draftConfig.sort ?? null,
                    hiddenColumns: draftConfig.hiddenColumns ?? [],
                  },
                },
              }
            }
            ...
            if (operation === 'create_view') {
              ...
              return {
                ...
                data: { view: toWireView(view, columns) },
              }
            }
            ...
            const configPatch = viewConfigFromArgs()
            const viewName = args.viewName as string | undefined
            if (!viewName && Object.keys(configPatch).length === 0) {
              return {
                success: false,
                message:
                  'update_view needs at least one of viewName, filter, order, hiddenColumns (pass null to clear filter or order)',
              }
            }
            const { view, columns } = await executeCopilotTableUseCase(
              context,
            ...
            const viewMessage = messageForCopilotTableError(error)
            if (viewMessage) return { success: false, message: viewMessage }
            throw error
          }
        }
