...
/**
 * Pure leaves rather than the barrel: they carry no database dependency, and
...
/**
 * Reads the `cell` argument as a grid coordinate.
 *
 * Only the *shape* is checked here — whether the coordinate exists on the
 * target interface's grid is a question about that interface, so it is left to
 * the service, which knows the layout and reports the real dimensions. That
 * also keeps this parser correct for any grid the product ships.
 *
 * The wire key stays `cell` because the published tool schema is generated
 * from the copilot contracts repo; the domain calls the resulting rectangle a
 * placement (see `@/lib/interfaces/types`).
 */
function requiredCell(args: ToolArgs, operation: string): InterfaceCell {
  const value = args.cell
  if (!isRecord(value)) {
    throw new InterfaceToolInputError(`cell is required for ${operation} and must be { row, col }`)
  }
  const { row, col } = value
  if (!isTrackIndex(row) || !isTrackIndex(col)) {
    throw new InterfaceToolInputError('cell.row and cell.col must each be a non-negative integer')
  }
  return { row, col }
}
...
function buildAddModuleData(
  ...
  switch (type) {
    case 'chat':
      return { type, placement, config: hasConfig ? normalizeChatConfig(raw) : undefined }
    case 'table':
      return { type, placement, config: hasConfig ? normalizeTableConfig(raw) : undefined }
    case 'file':
      return { type, placement, config: hasConfig ? normalizeFileConfig(raw) : undefined }
    case 'form':
      return { type, placement, config: hasConfig ? normalizeFormConfig(raw) : undefined }
  }
}
...
export const userInterfaceServerTool: BaseServerTool<UserInterfaceArgs, UserInterfaceResult> = {
  ...
  async execute(
    ...
    try {
      ...
      const level = USER_INTERFACE_WRITE_OPERATION_SET.has(operation) ? 'write' : 'read'
      ...
      switch (operation) {
        ...
        case 'restore': {
          ...
          assertNotAborted()
          const definition = await performRestoreInterface(
            interfaceId,
            { id: actorId },
        ...
        case 'move_module': {
          ...
          const existing = await requireInterfaceInWorkspace(interfaceId, workspaceId)
          const module = requireModule(existing, moduleId)
          const [displaced] = overlappingModules(
            existing.layout,
            { ...cell, rowSpan: module.placement.rowSpan, colSpan: module.placement.colSpan },
            moduleId
          )
      ...
      if (
        ...
        error instanceof InterfaceNotFoundError ||
        error instanceof InterfaceNotArchivedError ||
        error instanceof InterfaceWorkspaceArchivedError
      ) {
