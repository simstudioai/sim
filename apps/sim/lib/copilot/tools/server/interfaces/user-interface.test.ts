...
const {
  ...
  mockPerformRestoreInterface: vi.fn(),
...
const {
  MockConflictError,
  MockLayoutError,
  MockNotArchivedError,
  MockNotFoundError,
  MockReferenceError,
  MockWorkspaceArchivedError,
} = vi.hoisted(() => ({
  MockConflictError: class InterfaceConflictError extends Error {},
  MockLayoutError: class InterfaceLayoutError extends Error {},
  MockNotArchivedError: class InterfaceNotArchivedError extends Error {},
  MockNotFoundError: class InterfaceNotFoundError extends Error {},
  MockReferenceError: class InvalidModuleReferenceError extends Error {},
  MockWorkspaceArchivedError: class InterfaceWorkspaceArchivedError extends Error {},
}))
...
vi.mock('@/lib/interfaces', () => ({
  ...
  performRestoreInterface: mockPerformRestoreInterface,
  ...
  InterfaceNotArchivedError: MockNotArchivedError,
  InterfaceNotFoundError: MockNotFoundError,
  InterfaceWorkspaceArchivedError: MockWorkspaceArchivedError,
...
describe('userInterfaceServerTool', () => {
  ...
  describe('get', () => {
    it('returns the full module layout', async () => {
      ...
      mockGetInterfaceById.mockResolvedValue(
        buildDefinition({ layout: { version: 1, grid: { rows: 2, cols: 2 }, modules } })
      )
  ...
  describe('list', () => {
    it('summarizes the workspace interfaces', async () => {
      mockListInterfaces.mockResolvedValue([
        buildDefinition({
          layout: { version: 1, grid: { rows: 2, cols: 2 }, modules: [chatModule('mod_1', 0, 0)] },
        }),
  ...
  describe('delete and restore', () => {
    ...
    it('restores an archived interface', async () => {
      ...
      mockPerformRestoreInterface.mockResolvedValue(buildDefinition())
      ...
      expect(mockPerformRestoreInterface).toHaveBeenCalledWith(
        'int_1',
        { id: USER_ID },
        { auditMetadata: { source: 'tool_input' } }
      )
    ...
    it('reports the suffixed name when the original was taken', async () => {
      ...
      mockPerformRestoreInterface.mockResolvedValue(
        buildDefinition({ name: 'Support desk (restored)' })
      )
    ...
    it('refuses to restore an active interface', async () => {
      ...
      expect(mockPerformRestoreInterface).not.toHaveBeenCalled()
    })

    it('reports a restore lost to a concurrent one as a soft failure, not a fault', async () => {
      mockGetInterfaceById.mockResolvedValue(
        buildDefinition({ archivedAt: '2024-02-01T00:00:00.000Z' })
      )
      mockPerformRestoreInterface.mockRejectedValue(
        new MockNotArchivedError('Interface is not archived')
      )

      const result = await run('restore', { interfaceId: 'int_1' })

      expect(result).toEqual({ success: false, message: 'Interface is not archived' })
    })

    it('reports an archived workspace as a soft failure', async () => {
      mockGetInterfaceById.mockResolvedValue(
        buildDefinition({ archivedAt: '2024-02-01T00:00:00.000Z' })
      )
      mockPerformRestoreInterface.mockRejectedValue(
        new MockWorkspaceArchivedError('Cannot restore interface into an archived workspace')
      )
      ...
      expect(result).toEqual({
        success: false,
        message: 'Cannot restore interface into an archived workspace',
      })
    })
  })
  ...
  describe('add_module', () => {
    it('adds a module with the type defaults when no config is sent', async () => {
      mockAddModule.mockResolvedValue({
        definition: buildDefinition({
          layout: { version: 1, grid: { rows: 2, cols: 2 }, modules: [tableModule('mod_1', 0, 1)] },
        }),
        moduleId: 'mod_1',
      })
    ...
    it.each([
      ['a negative row', { row: -1, col: 0 }],
      ['a fractional column', { row: 0, col: 0.5 }],
      ['a non-numeric coordinate', { row: '0', col: 0 }],
    ])('rejects %s', async (_label, cell) => {
      const result = await run('add_module', {
        interfaceId: 'int_1',
        moduleType: 'chat',
        cell,
      ...
      expect(result).toEqual({
        success: false,
        message: 'cell.row and cell.col must each be a non-negative integer',
      })
      expect(mockAddModule).not.toHaveBeenCalled()
    })

    /**
     * Whether a coordinate exists is a question about the target interface's
     * grid, so the tool forwards it and the layout validator answers — which is
     * what keeps this parser correct for any grid the product ships.
     */
    it('forwards a well-formed cell the grid may still reject', async () => {
      mockAddModule.mockResolvedValue({ definition: buildDefinition(), moduleId: 'mod_1' })
      ...
      expect(mockAddModule).toHaveBeenCalledWith(
        'int_1',
        expect.objectContaining({
          placement: { row: 5, col: 5, rowSpan: 1, colSpan: 1 },
        })
      )
    })
  ...
  describe('update_module', () => {
    beforeEach(() => {
      mockGetInterfaceById.mockResolvedValue(
        buildDefinition({
          layout: { version: 1, grid: { rows: 2, cols: 2 }, modules: [tableModule('mod_1', 0, 0)] },
        })
      )
    })
    ...
    it('replaces the config using the module type on record', async () => {
      ...
      expect(mockUpdateModuleConfig).toHaveBeenCalledWith('int_1', 'mod_1', {
        tableId: 'tbl_1',
      })
