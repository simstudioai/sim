...
async function applyOperationsToYaml(currentYaml: string, operations: EditWorkflowOperation[]): Promise<string> {
						...
						Object.keys(block.connections).forEach((key) => {
							...
							if (typeof value === 'string') {
								...
								block.connections[key] = value.filter((item: any) =>
									typeof item === 'string' ? !allDeleted.includes(item) : !allDeleted.includes(item?.block)
								)
					...
					if (params?.removeEdges && Array.isArray(params.removeEdges)) {
						params.removeEdges.forEach(({ targetBlockId, sourceHandle = 'default' }) => {
							...
							if (typeof value === 'string') {
								...
								if ((block.connections as any)[sourceHandle].length === 0) delete (block.connections as any)[sourceHandle]
							} else if (typeof value === 'object' && value?.block) {
...
async function getCurrentWorkflowStateFromDb(workflowId: string): Promise<{ workflowState: any; subBlockValues: Record<string, Record<string, any>> }> {
  const [workflowRecord] = await db.select().from(workflowTable).where(eq(workflowTable.id, workflowId)).limit(1)
