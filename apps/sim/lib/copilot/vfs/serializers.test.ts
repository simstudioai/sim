...
describe('serializeInterfaceMeta', () => {
  ...
  const baseInterface: InterfaceDefinition = {
    id: 'iface-1',
    workspaceId: 'ws-1',
    name: 'Support Desk',
  ...
  const emptyLayout = { version: 1, grid: { rows: 2, cols: 2 }, modules: [] } as const
  ...
  it('omits an empty description and keeps an empty grid as an empty array', () => {
    const nulled = JSON.parse(
      serializeInterfaceMeta({ ...baseInterface, description: null, layout: emptyLayout })
    )
    const blank = JSON.parse(
      serializeInterfaceMeta({ ...baseInterface, description: '', layout: emptyLayout })
    )
  ...
  // The definition's timestamps are already ISO strings (toDefinition
  // serializes the row), so they publish verbatim.
  it('publishes the definition timestamps verbatim', () => {
    const metadata = JSON.parse(serializeInterfaceMeta(baseInterface))

    expect(metadata.createdAt).toBe('2026-07-01T00:00:00.000Z')
    expect(metadata.updatedAt).toBe('2026-07-09T12:34:56.000Z')
  })
})
