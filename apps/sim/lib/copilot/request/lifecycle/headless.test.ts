...
describe("runHeadlessCopilotLifecycle", () => {
  ...
  it("passes an OTel context to the lifecycle and trace report", async () => {
    ...
    runCopilotLifecycle.mockImplementationOnce(async (_payload, options) => {
      const { traceHeaders } =
        await import("@/lib/copilot/request/go/propagation");
      lifecycleTraceparent =
        traceHeaders({}, options.otelContext).traceparent ?? "";
    ...
    expect(lifecycleTraceparent).toMatch(
    ...
    expect(headers.traceparent.split("-")[2]).not.toBe(
      lifecycleTraceparent.split("-")[2],
    );
  });
