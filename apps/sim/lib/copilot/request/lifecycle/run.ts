  ...
  try {
    ...
    const err =
      error instanceof Error
        ? error
        : new Error("Copilot orchestration failed");
...
async function runCheckpointLoop(
  ...
  for (;;) {
        ...
        `${SIM_AGENT_API_URL}${route}`,
        {
          ...
          headers: {
            ...
            ...(options.simRequestId
              ? { "X-Sim-Request-ID": options.simRequestId }
              : {}),
          },
    ...
    if (context.pendingToolPromises.size > 0) {
      const waitSpan = context.trace.startSpan(
        "Wait for Tools",
        "lifecycle.wait_tools",
        {
    ...
    const undispatchedToolIds = continuation.pendingToolCallIds.filter(
      (toolCallId) => {
    ...
    if (undispatchedToolIds.length > 0) {
      logger.warn(
        "Checkpointed tools were never dispatched; executing before resume",
        {
          checkpointId: continuation.checkpointId,
    ...
    for (const toolCallId of continuation.pendingToolCallIds) {
      ...
      if (!tool || !tool.result) {
        ...
        throw new Error(
          `Cannot resume: missing result for pending tool call ${toolCallId}`,
        );
      }
      results.push({
        callId: toolCallId,
...
async function buildExecutionContext(
  ...
  const userTimezone =
    typeof requestPayload?.userTimezone === "string"
      ? requestPayload.userTimezone
      : undefined;
  const requestMode =
    typeof requestPayload?.mode === "string" ? requestPayload.mode : undefined;
...
async function ensureHeadlessRunIdentity(input: {
  ...
  try {
    await createRunSegment({
      ...
      model:
        typeof input.requestPayload?.model === "string"
          ? input.requestPayload.model
          : null,
      provider:
        typeof input.requestPayload?.provider === "string"
          ? input.requestPayload.provider
          : null,
      requestContext: {
