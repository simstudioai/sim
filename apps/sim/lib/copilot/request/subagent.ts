...
export interface SubagentOrchestratorOptions extends Omit<
  OrchestratorOptions,
  "onComplete"
> {
  userId: string;
...
async function orchestrateSubagentStreamInner(
  ...
  let resolvedWorkflowName =
    typeof requestPayload.workflowName === "string"
  ...
  if (!resolvedWorkspaceContext && resolvedWorkspaceId) {
    try {
      resolvedWorkspaceContext = await generateWorkspaceContext(
        resolvedWorkspaceId,
        userId,
      );
    } catch (error) {
  ...
  try {
      ...
      execContext,
      {
        ...
        onBeforeDispatch: (event: StreamEvent, ctx: StreamingContext) => {
          if (
            ...
            (event.payload.kind ===
              MothershipStreamV1SpanPayloadKind.structured_result ||
              event.payload.kind ===
                MothershipStreamV1SpanPayloadKind.subagent_result)
          ) {
    ...
    const err =
      error instanceof Error
        ? error
        : new Error("Subagent orchestration failed");
...
function normalizeStructuredResult(
  data: unknown,
): SubagentOrchestratorResult["structuredResult"] {
