...
import {
  ...
  TraceCollector,
} from "@/lib/copilot/request/trace";
import type {
  ExecutionContext,
  StreamingContext,
...
describe("copilot go stream helpers", () => {
  ...
  it("emits full snapshots for append (sidebar viewer uses replace mode; no delta merge)", () => {
    expect(
      buildPreviewContentUpdate("hello", "hello world", 100, 200, "append"),
    ).toEqual({
      content: "hello world",
  ...
  it("emits deltas for update when the preview extends the previous text", () => {
    expect(
      buildPreviewContentUpdate("hello", "hello world", 100, 200, "update"),
    ).toEqual({
      content: " world",
      contentMode: "delta",
      lastSnapshotAt: 100,
  ...
  it("falls back to snapshots for patches and divergent content", () => {
    expect(
      buildPreviewContentUpdate("hello", "goodbye", 100, 200, "update"),
    ).toEqual({
      content: "goodbye",
      contentMode: "snapshot",
      lastSnapshotAt: 200,
    ...
    expect(
      buildPreviewContentUpdate("hello", "hello world", 100, 200, "patch"),
    ).toEqual({
      content: "hello world",
      contentMode: "snapshot",
      lastSnapshotAt: 200,
  ...
  it("drops duplicate tool_result events before forwarding them", async () => {
    ...
    vi.mocked(fetch).mockResolvedValueOnce(
      createSseResponse([toolResult, toolResult, complete]),
    );
  ...
  it("fails closed when the shared stream receives malformed JSON", async () => {
    ...
    expect(
      context.errors.some((message) =>
        message.includes("Failed to parse SSE event JSON"),
      ),
    ).toBe(true);
  });
