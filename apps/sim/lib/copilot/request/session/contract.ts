...
export type SyntheticFilePreviewPhase =
  (typeof FILE_PREVIEW_PHASE)[keyof typeof FILE_PREVIEW_PHASE];
...
export type ContractStreamEvent =
  EnvelopeToStreamEvent<MothershipStreamV1EventEnvelope>;
...
function isValidEnvelopeShell(value: unknown): value is JsonRecord & {
  ...
  if (typeof value.type !== "string" || !KNOWN_EVENT_TYPES.has(value.type))
    return false;
...
function isSyntheticEnvelopeBase(
  value: unknown,
): value is Omit<SyntheticFilePreviewEventEnvelope, "payload"> & {
  payload?: unknown;
} {
  return (
    isRecord(value) &&
...
function isSyntheticFilePreviewTarget(
  value: unknown,
): value is SyntheticFilePreviewTarget {
  return (
    isRecord(value) &&
...
function isSyntheticFilePreviewPayload(
  value: unknown,
): value is SyntheticFilePreviewPayload {
  if (!isRecord(value)) {
    return false;
  }
  ...
  switch (value.previewPhase) {
    ...
    case FILE_PREVIEW_PHASE.complete:
      return (
        isOptionalString(value.fileId) &&
        isOptionalFiniteNumber(value.previewVersion)
      );
    default:
      return false;
  }
...
export function isSyntheticFilePreviewEventEnvelope(
  ...
  return (
    isSyntheticEnvelopeBase(value) &&
    isSyntheticFilePreviewPayload(value.payload)
  );
}
...
export function isToolCallStreamEvent(
...
export function isToolArgsDeltaStreamEvent(
  ...
  return (
    event.type === "tool" &&
    isRecord(event.payload) &&
    event.payload.phase === "args_delta"
  );
}

export function isToolResultStreamEvent(
  event: SessionStreamEvent,
...
export function parsePersistedStreamEventEnvelope(
  value: unknown,
): ParseStreamEventEnvelopeResult {
  ...
  if (!isRecord(value)) {
    ...
    if (typeof value.type !== "string") hints.push("missing type");
      ...
      hints.push(`unknown type="${value.type}"`);
...
export function parsePersistedStreamEventEnvelopeJson(
  raw: string,
): ParseStreamEventEnvelopeResult {
  let parsed: unknown;
  try {
