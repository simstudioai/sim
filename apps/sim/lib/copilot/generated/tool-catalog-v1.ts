      ...
      args: {
        ...
        properties: {
          ...
          rows: {
            type: 'array',
            description:
              'Array of row data objects (required for batch_insert_rows). TTL cells take ISO timestamp strings with Z or an explicit UTC offset (for example, 2026-09-07T14:30:00-07:00); up to 6 fractional second digits are accepted and values normalize to UTC; a missing or null TTL means no expiration.',
            items: { type: 'object' },
          },
          tableId: { type: 'string', description: 'Table ID (required for every operation)' },
          updates: {
            type: 'array',
            description:
              "Array of per-row updates: [{ rowId, data: { col: val } }] (batch_update_rows format a). TTL values are ISO timestamp strings with Z or an explicit UTC offset (for example, 2026-09-07T14:30:00-07:00); up to 6 fractional second digits are accepted and values normalize to UTC; omit a row's TTL key to preserve it or set it to null to clear the expiration.",
            items: {
              type: 'object',
          ...
          values: {
            type: 'object',
            description:
              "Map of rowId → value for single-column batch update (batch_update_rows format b, with columnName). For a TTL column, values are ISO timestamp strings with Z or an explicit UTC offset (for example, 2026-09-07T14:30:00-07:00); up to 6 fractional second digits are accepted and values normalize to UTC; set a row's value to null to clear its expiration, and omit the row from the map to leave it unchanged.",
          },
        },
        required: ['tableId'],
      },
      ...
      args: {
        ...
        properties: {
          ...
          data: {
            type: 'object',
            description:
              'Row data as key-value pairs (required for insert_row, update_row). TTL cells take ISO timestamp strings with Z or an explicit UTC offset (for example, 2026-09-07T14:30:00-07:00); up to 6 fractional second digits are accepted and values normalize to UTC. On insert_row, a missing or null TTL means no expiration. On update_row, omit the TTL to preserve its current value or set it to null to clear the expiration.',
          },
          dependencies: {
            type: 'object',
            description:
          ...
          filter: {
            type: 'object',
            description:
              'Predicate filter object for query_rows, update_rows_by_filter, delete_rows_by_filter. A predicate is a tree: {"all":[...]} (AND) or {"any":[...]} (OR); members are leaves {field, op, value} or nested groups. Ops: eq, ne, gt, gte, lt, lte, in, nin, like, ilike (use * as the wildcard), nlike, nilike, contains, ncontains, startsWith, endsWith, isNull, isNotNull, isEmpty, isNotEmpty. in/nin take a non-empty array value. TTL filter values are ISO timestamp strings with Z or an explicit UTC offset (for example, 2026-09-07T14:30:00-07:00); up to 6 fractional second digits are accepted and values normalize to UTC. Examples: {"all":[{"field":"status","op":"eq","value":"active"}]}; {"all":[{"field":"wins","op":"gte","value":18},{"field":"status","op":"eq","value":"pending"}]}; {"any":[{"field":"status","op":"eq","value":"active"},{"field":"status","op":"eq","value":"pending"}]}; {"all":[{"field":"name","op":"ilike","value":"*jo*"}]}; {"all":[{"field":"slack_user_id","op":"in","value":["U1","U2"]}]}.',
          },
          groupId: {
            type: 'string',
            description:
