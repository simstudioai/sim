...
/**
 * Credentials a Credential Group collected on behalf of someone outside the workspace.
 *
 * Neither kind is user-managed: they are created by an enrollment, authorized through the
 * group's resource policy, and read only by the execution paths that own them.
 */
export type ManagedCredentialType = Extract<CredentialType, 'managed_oauth' | 'managed_api_key'>
