  ...
  facts: {
    ...
    integrations: {
      ...
      codeSandboxRuntime: {
        ...
        shortValue: 'Yes, self-hosted only: requirements.txt, uv, or a custom Docker image',
        confidence: 'verified',
        sources: [
          {
    ...
    security: {
      ...
      sessionPolicy: {
        value:
          'Yes: on self-hosted deployments only, via environment variables. Langflow issues short-lived JWTs and exposes `LANGFLOW_ACCESS_TOKEN_EXPIRE_SECONDS` (default 3600, one hour) and `LANGFLOW_REFRESH_TOKEN_EXPIRE_SECONDS` (default 604800, seven days), so an operator sets the absolute ceiling on how long a signed-in session survives before re-authentication. No inactivity or idle timeout setting is documented, and no admin-console UI for setting session policy across an organization is documented for Langflow Cloud.',
        detail:
        ...
        shortValue: 'Yes, self-hosted only, via token-expiry env variables',
        confidence: 'verified',
        sources: [
          {
