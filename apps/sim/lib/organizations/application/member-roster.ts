...
/** permission-group-exempt: members are checked below; admins retain access to manage the directory setting. */
export const readOrganizationRosterOperation = defineOrganizationOperation({
  id: 'organization.members.roster',
  minimumRole: 'member',
  capability: 'none',
...
export const readOrganizationRoster: OperationUseCase<
  typeof readOrganizationRosterOperation,
  { organizationId: string },
  OrganizationRoster
> = {
  operation: readOrganizationRosterOperation,
  async execute({ principal, input }) {
    const callerMembership = await authorizeOrganizationOperation(
      principal,
      readOrganizationRosterOperation,
      input
    )
    const { organizationId } = callerMembership
    if (
      !isOrgAdminRole(callerMembership.role) &&
      (await isOrganizationCapabilityWithheld(organizationId, 'organization.member_directory'))
    )
      throw new OrchestrationError('forbidden', capabilityRefusal('organization.member_directory'))
