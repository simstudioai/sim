...
export const v2KnowledgeSearchResultSchema = z
  .object({
    ...
    rankScore: z
      .number()
      .describe(
        'Retrieval score before optional recency boosting. In `vector` mode it equals `similarity`; in `hybrid` mode it is the reciprocal-rank-fusion score (a sum of 1/(60 + rank) over the lexical and vector legs). When a reranker orders the results it is `rerankerScore`. Use `rank` for the final order.'
      )
