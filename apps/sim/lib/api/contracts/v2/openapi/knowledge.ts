...
const declaredRoutes = [
  ...
  defineOpenApiRoute(
    ...
    knowledgeOperation({
      ...
      description: `Get one connector and its ten most recent synchronization attempts. Stored API keys and encrypted secret material are never returned. ${WORKSPACE_API_KEY_DENIED}`,
      errors: RESOURCE_ERRORS,
      success: {
  ...
  defineOpenApiRoute(
    ...
    knowledgeOperation({
      ...
      description: `Rename a document, change search availability, update tag slots, or requeue processing. Omitted fields remain unchanged; indexing state is read-only. Use List Tags to resolve names to slots and Get Document for connector provenance, which this response omits. ${WORKSPACE_API_KEY_DENIED}`,
      errors: [...RESOURCE_ERRORS, 'PayloadTooLarge'],
      success: {
