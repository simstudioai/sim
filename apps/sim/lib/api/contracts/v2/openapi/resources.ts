...
const routes = [
  ...
  defineOpenApiRoute(
    ...
    resourceOperation('MCP Servers', {
      ...
      description: `Connect to a registered MCP server and return the tools it exposes. This is also what populates \`connectionStatus\`, \`toolCount\`, \`lastError\`, and \`lastToolsRefresh\` on the server resource, so registering a server and then calling this completes onboarding without opening the Sim UI. Results are served from a short-lived cache; pass \`refresh=true\` to reconnect and pick up tools added since the last pass, at the cost of a live round trip to the server. The set is bounded by discovery itself — at most 1,000 tools and 5 MB of tool payload per server — so it is returned as a single page with \`nextCursor\` always null. An unreachable, slow, or cooling-down server is a \`503\`; a server whose stored OAuth grant no longer works is a \`401\` and must be reauthorized in Sim. ${WORKSPACE_API_KEY_DENIED} Discovery resolves the calling user's own OAuth credentials for the server, which a workspace key cannot supply — so a workspace key that can register a server cannot list its tools.`,
      errors: [...WORKSPACE_ERRORS, 'NotFound'],
