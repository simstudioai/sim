...
const declaredRoutes = [
  ...
  defineOpenApiRoute(
    ...
    tableOperation({
      ...
      description:
        'Start workflow or enrichment groups across all rows or selected rows. Poll Get Run Dispatch until `complete` or `canceled`. A null `dispatchId` means execution settled inline. Use Cancel Run Dispatch to stop further scheduling.',
      errors: RESOURCE_ERRORS,
  ...
  defineOpenApiRoute(
    ...
    tableOperation({
      ...
      description:
        'Start one workflow or enrichment group for a table row. Poll Get Run Dispatch using the returned `dispatchId`; null means execution settled inline.',
      errors: RESOURCE_ERRORS,
  ...
  defineOpenApiRoute(
    ...
    tableOperation({
      ...
      description: 'Get progress and terminal state for a durable table export.',
      errors: RESOURCE_ERRORS,
  ...
  defineOpenApiRoute(
    ...
    tableOperation({
      ...
      description:
        'Archive up to 100 tables and recursively delete selected folders. Items succeed or fail independently, with `skipped`, `notFound`, and `failed` outcomes. `deletedItems` includes all descendants. Use Restore Table to recover archived tables.',
