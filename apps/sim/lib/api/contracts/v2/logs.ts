...
export const v2ListLogsQuerySchema = v1ListLogsQuerySchema
  ...
  .extend({
    ...
    includeTraceSpans: booleanQueryFlagSchema
      .describe(
        'Whether to include block-level trace spans. Implies `details=full`, so the `workflow` summary is present regardless of what `details` is set to. Spans are stored apart from the log row and pruned on their own retention schedule, so a run whose spans have aged out returns `traceSpans: []` rather than an error — an empty array does not mean the run recorded none.'
      )
      .optional()
      .default(false),
