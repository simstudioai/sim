...
export class LoggingSession {
  ...
  async completeWithError(error?: any): Promise<void> {
    try {
      ...
      if (this.requestId) {
        logger.debug(
          `[${this.requestId}] Completed logging with error for execution ${this.executionId}`
        )
      }
    } catch (error) {
      if (this.requestId) {
        logger.error(`[${this.requestId}] Failed to complete logging for error:`, error)
      }
    }
  }
  ...
  async safeStart(params: SessionStartParams = {}): Promise<boolean> {
    try {
      ...
      if (this.requestId) {
        logger.error(`[${this.requestId}] Logging start failed, continuing execution:`, error)
      }
      return false
    }
  }
  ...
  async safeCompleteWithError(error?: any): Promise<void> {
    try {
      ...
      if (this.requestId) {
        logger.error(`[${this.requestId}] Logging error completion failed:`, error)
      }
    }
  }
}
