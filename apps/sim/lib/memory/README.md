...
New tool exchanges append to `memory_item`; they do not grow the legacy `memory.data` array. A serialized result record or rendered exchange above 100 KiB goes through the existing large-value storage service. Artifact plaintext is bounded to 8 MiB; the database retains ownership metadata and bounded history previews. An oversized result that cannot be retained marks the session degraded and records an explicit unavailable-detail notice in its in-memory recovery state.

Checkpoint encoding limits plaintext to just under 2 MiB and encrypted database state to 4 MiB. The repository also rejects an individual history item's data above 1 MiB before writing. These are per-value bounds, not a total conversation-storage quota. The checkpoint currently includes accumulated invocation steps and is rewritten on advancement, so long invocations can still incur substantial write overhead or exhaust the checkpoint budget. Exceeding a durability limit degrades the session instead of allowing an unbounded database write.

Stored history and model context have separate limits. Message-count windows do not count tool exchanges as additional conversational messages. Provider-attempt entry budgets the selected history against the actual model, including tool schemas, attachments, and output reserve; subsequent generations inside that provider's tool loop do not rerun this budget. There is currently no automatic summary or model-facing retrieval tool for omitted artifact content. The public [Agent block documentation](../../../docs/content/docs/workflows/blocks/agent.mdx) describes these user-visible limits.
