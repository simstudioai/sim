...
vi.mock('@/lib/table/application/tables', () => ({
  readTableDefinitionUseCase: { execute: readTableDefinition },
}))
...
describe('processContextsServer - workflow references', () => {
  ...
  it('attaches a canonical CLI folder path including its parents', async () => {
    listFolders.mockResolvedValueOnce([
      { folderId: 'root', folderName: 'Sales team', parentId: null },
      { folderId: 'folder-1', folderName: 'Leads / new', parentId: 'root' },
    ])
    expect(
    ...
    expect(listFolders).toHaveBeenCalledWith('workspace-1')
    listFolders.mockResolvedValueOnce([])
    expect(
      await resolveActiveResourceContext('folder', 'folder-2', 'workspace-1', 'reader')
    ).toBeNull()
  })
})
...
describe('processContextsServer - skill contexts', () => {
  ...
  it('resolves a tagged workspace skill to its full body without an unreadable file path', async () => {
...
describe('processContextsServer - table_selection contexts', () => {
  ...
  it('distinguishes which row was selected when visible cell values are identical', async () => {
    ...
    getRowsByIds.mockImplementation(async (_tableId: string, ids: string[]) =>
      ids.map((id) => ({ id, data: { c_status: 'new' } }))
    )
  ...
  it('retains an oversized selected row by ID without forcing its cells into the preview', async () => {
    ...
    expect(result[0].content).toContain('omitted for length')
...
describe('table view context', () => {
  ...
  it('accepts an unfiltered panel without a saved view and still authorizes the table', async () => {
    readTableDefinition.mockResolvedValueOnce({ table: { id: 'table-1', name: 'Leads' } })
    ...
    expect(readTableDefinition).toHaveBeenCalledOnce()
  ...
  it('does not replace a denied or missing saved view with the whole table', async () => {
    ...
    expect(readTableDefinition).not.toHaveBeenCalled()
  })
})
