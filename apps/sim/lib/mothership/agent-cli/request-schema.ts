 ...
 * before executing it — no re-parsing, no routing decisions. Slicing never arrives here:
 * the worker applies pipes to whatever this side returns.
 */
export const agentCliRequestSchema = z.object({
  invocation: z.discriminatedUnion('kind', [
    ...
    z.object({ kind: z.literal('stdout'), stdout: z.string().max(50_000_000) }),
  ]),
