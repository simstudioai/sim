      ...
      field.type !== 'tool-input'
        ...
        : {
            ...
            toolBinding: {
              ...
              access: isMothership
                ? 'Selections add MCP operations to available integration access; this is not an integration allowlist. Empty tools does not disable integrations. Fixed MCP arguments are not supported. Each selection supports usageControl (auto/force/none) or usageControlExpression resolving to those values. Send only the chosen property; Sim selects its mode. none removes the selection from discovery and execution; force requires an invocation of each selected operation before completion. These controls do not disable ambient integrations. Actual calls remain subject to execution permissions and MCP policies.'
