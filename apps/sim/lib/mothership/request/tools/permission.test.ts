...
vi.mock('@/lib/mothership/persistence/tool-permission', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@/lib/mothership/persistence/tool-permission')>()
  return { ...actual, waitForToolPermissionDecision }
})

import { MothershipStreamV1ToolExecutor } from '@/lib/mothership/generated/mothership-stream-v1'
...
import type { StreamEvent, ToolCallState } from '@/lib/mothership/request/types'
...
describe('gated tools are askable', () => {
  it('every tool requiring approval renders a row the user can answer on', async () => {
    ...
    const { getHiddenToolNames } = await vi.importActual<
      typeof import('@/lib/mothership/tools/client/hidden-tools')
    >('@/lib/mothership/tools/client/hidden-tools')
