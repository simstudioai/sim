 ...
 * why share modals used to re-derive the public link by hand — and would hand a
 * new resource family the wrong prefix the moment one is added. Both sides
 * import this module instead, so a share's link has exactly one definition.
 */
...
/** Public share URL for a token, e.g. `{baseUrl}/f/{token}`. */
