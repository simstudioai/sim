...
describe('providerText', () => {
  ...
  it('keeps each table row on one line', () => {
    expect(providerText('<table><tr><td>Widget</td><td>2</td></tr></table>', 'html')).toContain(
      'Widget 2'
    )
  })
  it('separates a definition term from its definition', () => {
    expect(providerText('<dl><dt>Term</dt><dd>Def</dd></dl>', 'html')).toMatch(/Term\s+Def/)
  })
  ...
  it('keeps a zero-width non-joiner that shapes a Persian word', () => {
    const word = String.fromCodePoint(0x0645, 0x06cc, 0x200c, 0x062e, 0x0648, 0x0627, 0x0645)
    expect(providerText(word)).toBe(word)
  })
