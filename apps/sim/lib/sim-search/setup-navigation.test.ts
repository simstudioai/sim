...
describe('source setup detours', () => {
  it('keeps organization Slack setup and its continuation in the same organization', () => {
    ...
    expect(searchSetupReturnHref(scope, 'slack')).toBe('/o/org-1/integrations?addConnector=slack')
    expect(searchSetupReturnHref(scope, 'search')).toBe('/o/org-1/integrations')
  })
