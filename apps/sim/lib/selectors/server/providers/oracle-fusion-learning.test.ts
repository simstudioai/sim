...
import type { ExecuteServerSelectorArgs, ServerSelectorAttachment } from '@/lib/selectors/server/types'
...
const bound = { instanceUrl: 'https://acme.fa.ocs.oraclecloud.com', accessToken: 'test-access-token' }
function args(overrides: Partial<ExecuteServerSelectorArgs> = {}): ExecuteServerSelectorArgs {
  return {
    ...
    credential: {
      ...
      access: { resolvedCredentialId: 'credential', credentialType: 'service_account', credentialOwnerUserId: 'owner' } as never,
    },
    references: new Map(),
...
describe('Learning selector permissions and scoping', () => {
  ...
  it('declares exactly seven Learning-only credential policies and prepared destinations', () => {
    ...
    for (const [key, attachment] of entries) {
      expect(attachment.credential).toEqual({ kind: 'stored', field: 'oauthCredential', serviceIds: ['oracle_fusion_learning'] })
      ...
      expect(selectorManifest[key as keyof typeof oracleFusionLearningSelectorAttachments]).toMatchObject({ listMode: 'paginated', staleTime: 0, supportsDetail: true })
    }
  })
  ...
  it('resolves the trusted Fusion provider and returns only safe option fields', async () => {
    mocks.executeListSelfPacedItems.mockResolvedValue({
      output: { items: [{ learningItemId: '9007199254740993', learningItemTitle: 'Safety', learningItemNumber: 'SAF-1', UploadAuthToken: 'secret' }], hasMore: true, nextOffset: 51 },
    })
    const input = args({ request: { kind: 'list', cursor: '50', search: 'Safety' } })
    const result = await run(oracleFusionLearningSelectorAttachments['oracle_fusion_learning.selfPacedItems'], input)
    expect(mocks.credential).toHaveBeenCalledWith(expect.objectContaining({
      credential: input.credential, protectedValues: input.protectedValues, providerId: 'oracle-fusion-service-account',
    }))
    expect(mocks.executeListSelfPacedItems).toHaveBeenCalledWith(expect.objectContaining({ ...bound, limit: 50, offset: 50, search: 'Safety' }), undefined)
    expect(result).toEqual({ kind: 'list', items: [{ id: '9007199254740993', label: 'Safety', meta: { number: 'SAF-1' } }], nextCursor: '51' })
    expect(JSON.stringify(result)).not.toContain('secret')
  })

  it('requires parent context and keeps detail resolution under the selected person and assignment', async () => {
    const attachment = oracleFusionLearningSelectorAttachments['oracle_fusion_learning.selectedCourseOfferings']
    ...
    mocks.getSelectedCourseOfferingForSelector.mockResolvedValue({ output: { item: { assignmentRecordId: '3', assignmentRecordNumber: 'A3', learningItemTitle: 'Morning offering' } } })
    await run(attachment, args({ context: { oauthCredential: 'credential', personId: '1', recordId: '2' }, request: { kind: 'detail', id: '3' } }))
    expect(mocks.getSelectedCourseOfferingForSelector).toHaveBeenCalledWith(expect.objectContaining({ ...bound, personId: '1', recordId: '2', offeringRecordId: '3' }), undefined)
    expect(selectorManifest['oracle_fusion_learning.selectedCourseOfferings'].context.readiness).toEqual({ all: ['oauthCredential', 'personId', 'recordId'] })
  })
  ...
  it('rejects mismatched detail IDs and maps missing details to null', async () => {
    ...
    mocks.executeGetLearningEvent.mockResolvedValueOnce({ output: { item: { learningItemId: '99' } } })
    await expect(run(attachment, input)).rejects.toBeInstanceOf(SelectorOptionsUnavailableError)
    mocks.executeGetLearningEvent.mockRejectedValueOnce(new OracleFusionProviderError('private', 404))
  ...
  it('uses the credential-bound origin and rejects absent credentials or malformed destinations', async () => {
    const attachment = oracleFusionLearningSelectorAttachments['oracle_fusion_learning.events']
    await expect(run(attachment, args({ credential: undefined }))).rejects.toBeInstanceOf(SelectorConnectionUnavailableError)
