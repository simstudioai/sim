...
vi.mock('@/lib/internal/oracle-fusion-financials/client', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@/lib/internal/oracle-fusion-financials/client')>()
  return { ...actual, requestOracleFusionJson: mocks.request }
})
...
function prepared() {
  return { accessToken: 'server-only-token', instanceUrl: ORIGIN }
}
...
describe('Oracle Fusion Financials invoice selector', () => {
  ...
  it('hydrates one invoice by its opaque key with the same fixed projection', async () => {
    ...
    expect(mocks.request).toHaveBeenCalledWith(
      prepared(),
      {
        path: '/fscmRestApi/resources/11.13.18.05/invoices/opaque%20key',
