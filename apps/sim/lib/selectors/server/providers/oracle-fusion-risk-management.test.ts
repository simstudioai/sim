...
vi.mock('@/lib/internal/oracle-fusion/client', () => ({ requestOracleFusionJson: mocks.request, requestOracleFusionEmpty: vi.fn() }))
...
function args(): ExecuteServerSelectorArgs {
  return {
    ...
    credential: {
      ...
      access: { ok: true, resolvedCredentialId: 'credential', credentialOwnerUserId: 'user', credentialType: 'service_account' },
    },
  }
}
...
beforeEach(() => {
  vi.resetAllMocks()
  mocks.account.mockResolvedValue({ credentialType: 'service_account', providerId: 'oracle-fusion-service-account' })
...
describe('Risk Management selectors', () => {
  it('selects group navigation keys instead of business IDs', async () => {
    ...
    mocks.request.mockResolvedValue({ items: [{ GroupId: 'business-id', Name: 'Reviewers', '@context': { links: [{ rel: 'self', href: `${ORIGIN}/fscmRestApi/resources/11.13.18.05/userAssignmentGroups/group-hash` }] } }], count: 1, hasMore: false, limit: 100, offset: 0 })
    expect(await groupAttachment.execute(input, { instanceUrl: ORIGIN, accessToken: 'hidden-token' })).toEqual({ kind: 'list', items: [{ id: 'group-hash', label: 'Reviewers' }] })
  })
  ...
  it('prepares only a credential-bound Oracle destination', async () => {
    ...
    expect(await attachment.destination.prepare(input)).toEqual({ instanceUrl: ORIGIN, accessToken: 'hidden-token' })
    mocks.account.mockResolvedValue({ credentialType: 'service_account', providerId: 'other-service' })
  ...
  it('projects safe exact-ID options from one page and preserves cancellation', async () => {
    ...
    mocks.request.mockResolvedValue({ items: [{ ProcessId: ID, Name: 'Quarterly review', secret: 'unprojected' }], count: 1, hasMore: true, limit: 100, offset: 100 })
    const result = await attachment.execute(input, { instanceUrl: ORIGIN, accessToken: 'hidden-token' })
    expect(result).toEqual({ kind: 'list', items: [{ id: ID, label: 'Quarterly review' }], nextCursor: '101' })
  ...
  it('rejects invalid cursors before any request', async () => {
    ...
    await expect(attachment.execute(input, { instanceUrl: ORIGIN, accessToken: 'hidden-token' })).rejects.toThrow()
    expect(mocks.request).not.toHaveBeenCalled()
  })
  ...
  it('returns null for an unavailable detail and sanitizes other provider errors', async () => {
    ...
    expect(await attachment.execute(input, { instanceUrl: ORIGIN, accessToken: 'hidden-token' })).toEqual({ kind: 'detail', item: null })
    mocks.request.mockRejectedValue(new OracleFusionProviderError('private provider text', 500))
    await expect(attachment.execute(input, { instanceUrl: ORIGIN, accessToken: 'hidden-token' })).rejects.not.toThrow('private provider text')
  })
})
