...
describe('Oracle Project Management credential-bound selectors', () => {
  ...
  it('prepares only the authorized Oracle Fusion service-account destination', async () => {
    ...
    expect(attachments[key].credential).toEqual({
      kind: 'stored',
  ...
  it.each([
    [
      'projects',
      'projects',
      { ProjectId: '999999999999999999', ProjectName: 'Delivery', ProjectNumber: 'P1' },
      '999999999999999999',
      'Delivery',
    ],
    [
      ...
      { OrganizationName: 'Consulting', OrganizationId: 101 },
      'Consulting',
      'Consulting',
    ],
    [
      ...
      {
        ResourceId: 808,
        ResourceDisplayName: 'Staff Member',
    ...
    [
      'roles',
      'projectRolesLOV',
      { ProjectRoleId: 1, ProjectRoleName: 'Project Manager' },
      'Project Manager',
      'Project Manager',
    ],
    [
      'deliverableTypes',
      'deliverableTypesLOV',
      { DeliverableTypeId: 1, Name: 'General', DeliverableTypeClass: 'DOCUMENT' },
      '1',
      'General',
    ],
    [
      ...
      'projectStatusesLOV',
      {
        ...
        StatusObjectCode: 'SYNTHETIC_PROJECT_OBJECT',
      },
      'APPROVED',
      'Approved (SYNTHETIC_PROJECT_OBJECT)',
    ],
  ] as const)(
    'projects safe %s options with the persisted write value',
    async (name, path, record, id, label) => {
      mocks.json.mockResolvedValue(
        page([
          {
            ...record,
            accessToken: 'secret-canary',
            links: [{ href: 'https://not-a-public-option.test' }],
          },
        ])
      )
  ...
  it('escapes provider-side search and preserves the server paging offset', async () => {
    mocks.json.mockResolvedValue(
      page([{ ProjectId: 101, ProjectName: "O'Neil", ProjectNumber: 'P1' }], {
        offset: 50,
        hasMore: true,
      })
    )
    const result = await execute('oracleFusionProjectManagement.projects', {
      request: { kind: 'list', search: "O'Neil", cursor: '50' },
    })
    expect(result).toMatchObject({
      kind: 'list',
      nextCursor: '51',
      items: [{ id: '101', label: "O'Neil" }],
    })
    expect(mocks.json.mock.calls[0][1].query).toMatchObject({
      offset: 50,
      limit: 50,
      q: "ProjectName like '%O''Neil%'",
      fields: 'ProjectId,ProjectName,ProjectNumber',
    })
  ...
  it('hydrates name-valued organizations with a query rather than treating the name as a numeric path ID', async () => {
    mocks.json.mockResolvedValue(
      page([{ OrganizationName: 'Consulting', OrganizationId: 101 }], { limit: 2 })
    )
    expect(
      await execute('oracleFusionProjectManagement.organizations', {
        request: { kind: 'detail', id: 'Consulting' },
      })
    ).toEqual({
      kind: 'detail',
    ...
    expect(
      await execute('oracleFusionProjectManagement.projects', {
        request: { kind: 'detail', id: '101' },
      })
  ...
  it('requires active parents and queries task assignments within both the project and task', async () => {
    await expect(execute('oracleFusionProjectManagement.tasks')).rejects.toBeInstanceOf(
      SelectorContextUnavailableError
    )
    await expect(
      execute('oracleFusionProjectManagement.laborAssignments', { context: { projectId: '101' } })
    ...
    const result = await execute('oracleFusionProjectManagement.laborAssignments', {
      context: { projectId: '101', taskId: '202' },
      request: { kind: 'detail', id: '707' },
    })
    expect(result).toMatchObject({ kind: 'detail', item: { id: '707', label: 'Staff' } })
    expect(mocks.json.mock.calls[1][1]).toMatchObject({
      address: {
        family: 'fscm',
        relativePath: 'projectPlans/101/child/TaskLaborResourceAssignments',
      },
      query: { q: 'TaskId=202 and TaskLaborResourceAssignmentId=707' },
    })
  })
  ...
  it('uses project-specific membership IDs, not person IDs', async () => {
    ...
    expect(
      await execute('oracleFusionProjectManagement.teamMembers', { context: { projectId: '101' } })
    ).toMatchObject({ kind: 'list', items: [{ id: '606', label: 'Staff' }] })
  })

  it('skips resources without an email while advancing the original provider page', async () => {
    mocks.json.mockResolvedValue(
      page([{ ResourceId: 808, ResourceDisplayName: 'No email', ResourceEmail: null }], {
        hasMore: true,
      })
    )
    expect(await execute('oracleFusionProjectManagement.resources')).toMatchObject({
      kind: 'list',
      items: [],
      nextCursor: '1',
    })
  ...
  it('reports the provider cap and rejects malformed or over-budget cursors', async () => {
    mocks.json.mockResolvedValue(
      page([{ ProjectId: 101, ProjectName: 'Last', ProjectNumber: 'P1' }], {
        limit: 1,
        offset: MAX_SELECTOR_OPTIONS - 1,
        hasMore: true,
      })
    )
    const result = await execute('oracleFusionProjectManagement.projects', {
      request: { kind: 'list', cursor: String(MAX_SELECTOR_OPTIONS - 1) },
    })
    ...
    for (const cursor of ['-1', '1.5', String(MAX_SELECTOR_OPTIONS)]) {
      await expect(
        execute('oracleFusionProjectManagement.projects', { request: { kind: 'list', cursor } })
      ).rejects.toBeInstanceOf(SelectorContextUnavailableError)
    }
  })
  ...
  it('rejects malformed collections and preserves safe status and cancellation behavior', async () => {
    ...
    controller.abort()
    await expect(
      execute('oracleFusionProjectManagement.projects', { signal: controller.signal })
