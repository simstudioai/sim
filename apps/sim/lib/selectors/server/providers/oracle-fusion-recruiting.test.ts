...
describe('Recruiting selectors', () => {
  beforeEach(() => {
    ...
    mocks.list.mockResolvedValue({
      success: true,
      output: {
        candidates: [{ candidateNumber: '1', displayName: 'Taylor', accessToken: 'private-token' }],
        hasMore: true,
        nextOffset: 51,
      },
    })
  ...
  it('projects safe options and forwards bounded pagination', async () => {
    const result = await attachments['oracle_fusion_recruiting.candidates'].execute(
      args({ request: { kind: 'list', cursor: '50' } })
    )
    expect(result).toEqual({
      kind: 'list',
      items: [{ id: '1', label: 'Taylor' }],
      nextCursor: '51',
    })
    ...
    expect(mocks.bundle).toHaveBeenCalledWith(
      expect.objectContaining({ providerId: 'oracle-fusion-service-account' })
    )
  })
  ...
  it.each(['-1', '1e3', '9007199254740992'])('rejects invalid cursor %s', async (cursor) => {
    await expect(
      attachments['oracle_fusion_recruiting.candidates'].execute(
  ...
  it('represents a missing detail without inventing an option', async () => {
    ...
    expect(
      await attachments['oracle_fusion_recruiting.candidates'].execute(
        args({ request: { kind: 'detail', id: '1' } })
      )
  ...
  it('does not expose raw errors or credentials', async () => {
    ...
    await expect(
      attachments['oracle_fusion_recruiting.candidates'].execute(args())
