...
function makeRow(overrides?: Record<string, unknown>) {
  return {
    ...
    name: 'Support',
    description: null,
    layout: { version: 1, grid: { rows: 2, cols: 2 }, modules: [] } satisfies InterfaceLayout,
    archivedAt: null,
    createdBy: 'user-1',
...
describe('interface service', () => {
  ...
  describe('createInterface', () => {
    it('creates with an empty versioned layout', async () => {
      ...
      expect(inserted.layout).toEqual({ version: 1, grid: { rows: 2, cols: 2 }, modules: [] })
      ...
      dbChainMockFns.limit.mockResolvedValueOnce([
        makeRow({
          layout: {
            version: 1,
            grid: { rows: 2, cols: 2 },
            modules: [chatModule('existing', 0, 0)],
          },
        }),
      ])
  ...
  describe('moveModule', () => {
    it('swaps cells when the target cell is occupied', async () => {
      dbChainMockFns.limit.mockResolvedValueOnce([
        makeRow({
          layout: {
            version: 1,
            grid: { rows: 2, cols: 2 },
            modules: [chatModule('a', 0, 0), chatModule('b', 1, 1)],
          },
        }),
      ])
      dbChainMockFns.returning.mockResolvedValueOnce([makeRow()])
      ...
      expect(byId.get('a')?.placement).toEqual({ row: 1, col: 1, rowSpan: 1, colSpan: 1 })
      expect(byId.get('b')?.placement).toEqual({ row: 0, col: 0, rowSpan: 1, colSpan: 1 })
    })
    ...
    it('moves onto an empty cell without touching other modules', async () => {
      ...
      const layout = lastWrittenLayout()
      const byId = new Map(layout.modules.map((m) => [m.id, m]))
      expect(byId.get('a')?.placement).toEqual({ row: 0, col: 1, rowSpan: 1, colSpan: 1 })
      expect(byId.get('b')?.placement).toEqual({ row: 1, col: 1, rowSpan: 1, colSpan: 1 })
    })
  ...
  describe('updateModuleConfig', () => {
    it('fully replaces the module config', async () => {
      dbChainMockFns.limit.mockResolvedValueOnce([
        makeRow({
          layout: { version: 1, grid: { rows: 2, cols: 2 }, modules: [chatModule('a', 0, 0)] },
        }),
      ])
      dbChainMockFns.returning.mockResolvedValueOnce([makeRow()])
      ...
      dbChainMockFns.limit.mockResolvedValueOnce([
        makeRow({
          layout: {
            version: 1,
            grid: { rows: 2, cols: 2 },
            modules: [chatModule('a', 0, 0), chatModule('b', 0, 1)],
          },
        }),
      ])
      dbChainMockFns.returning.mockResolvedValueOnce([makeRow()])
      ...
      dbChainMockFns.limit.mockResolvedValueOnce([
        makeRow({
          layout: {
            version: 1,
            grid: { rows: 2, cols: 2 },
            modules: [chatModule('committed', 0, 0)],
          },
        }),
      ])
      dbChainMockFns.returning.mockResolvedValueOnce([makeRow()])
  ...
  describe('transaction executor discipline', () => {
    ...
    it('keeps the global pool idle on a granular module operation', async () => {
      ...
      dbChainMockFns.limit.mockResolvedValueOnce([
        makeRow({
          layout: { version: 1, grid: { rows: 2, cols: 2 }, modules: [tableModule('t', null)] },
        }),
      ])
      dbChainMockFns.returning.mockResolvedValueOnce([makeRow()])
      mockGetTableById.mockResolvedValueOnce({ id: 'tbl-1', workspaceId: WORKSPACE_ID })
    ...
    it('still rejects a cross-workspace reference resolved through the transaction', async () => {
      ...
      await expect(
        updateInterfaceLayout('iface-1', {
          version: 1,
          grid: { rows: 2, cols: 2 },
          modules: [tableModule('t', 'tbl-1')],
        })
    ...
    it('grandfathers a stored reference without querying at all', async () => {
      ...
      const stored = {
        version: 1,
        grid: { rows: 2, cols: 2 },
        modules: [tableModule('t', 'tbl-archived')],
      } as InterfaceLayout
