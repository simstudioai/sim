/**
 * Whether a form module's config is safe to persist, and why not when it isn't.
 *
 * Lives in the domain rather than beside the inspector because both authoring
 * surfaces need it: the inspector's field rows, and the form module itself,
 * which edits its own field labels on the canvas. The module is part of a
 * canonical resource view that anonymous share pages mount, so it cannot reach
 * into the workspace route tree for this.
 *
 * Database-free by construction (constants and types only), so it is safe on
