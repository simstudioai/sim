...
interface BuildUsageBreakdownArgs {
  ...
  workspaceId?: string
  limit: number
  /** The public API's bound on how many groups a window may have. */
  maxGroupedRows?: number
}

/**
 * A dimension's ranked list for a resolved window: grouped, re-keyed onto what the
 * panel displays, named, folded to `limit` rows plus a remainder, and apportioned so
 * the rows and the remainder add up. The one pipeline every ranked list uses, the
 * overview's member card included.
 */
export async function buildUsageBreakdown({
  entity,
  window,
  timezone,
  dimension,
  ...
  limit,
  maxGroupedRows,
}: BuildUsageBreakdownArgs): Promise<OrganizationUsageBreakdownResult> {
  const raw = await readUsageGroups({
    entity,
    window,
    timezone,
    dimension,
    workspaceId,
    maxRows: maxGroupedRows,
  })
  if (maxGroupedRows !== undefined && raw.length > maxGroupedRows) {
  ...
  const isModelDimension = dimension === 'model' || dimension === 'byok'
...
export const getOrganizationUsageBreakdown = defineAuthorizedOrganizationUsageUseCase({
  ...
  async execute({ input, context }): Promise<OrganizationUsageBreakdownResult> {
    ...
    return buildUsageBreakdown({
      ...
      limit: input.limit,
      maxGroupedRows: input.maxGroupedRows,
    })
  },
})
