...
vi.mock('@/lib/scim/identity/end-directory-membership', () => ({
  endDirectoryMembershipTx: mocks.endDirectoryMembership,
}))
vi.mock('@/lib/scim/repository/users', () => ({
  findScimUserById: mocks.findScimUserById,
}))
vi.mock('@/lib/scim/application/audit', () => ({
  recordScimAuditEntries: mocks.recordAudit,
}))
vi.mock('@/lib/scim/base-url', () => ({ scimBaseUrl: () => 'https://sim.test/api/scim/v2' }))
