...
describe('moveWorkspaceToOrganization retries', () => {
  ...
  it('does not fence entitlements when they are not subscription-backed', async () => {
    queueMoveSelects(organizationWorkspace)
    /**
     * `resolveOrganizationEnterprisePlan` grants entitlement by deployment
     * configuration in two modes — billing disabled, and self-hosted with
     * access control enabled — so `sourceIsEnterprise` is true while no
     * `subscription` rows exist. A fence that treats a missing row as a failure
     * would then reject EVERY organization-to-organization move in both.
     */
    resolveMoveEntitlements.mockResolvedValueOnce({
