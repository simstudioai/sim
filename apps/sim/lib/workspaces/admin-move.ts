    ...
    try {
      result = await db.transaction(async (tx) => {
        ...
        if (sourceOrganizationId) {
           ...
           */
          if (isSubscriptionBackedEntitlement()) {
            const entitledRows = await tx
            ...
            /**
             * Billing-blocked organizations are NOT entitled, even holding an
             * active subscription — `resolveOrganizationEnterprisePlan` returns
             * false for them via `isOrganizationBillingBlocked`, so a fence
             * ignoring it would call a destination entitled that every gate it
             * protects treats as disabled. Blocked state lives on the
             * organization owner's `user_stats`, which is what
             * `getBillingEntityBlockStatus` resolves an organization to.
             */
            ...
            if (
              entitledOrganizationIds.has(sourceOrganizationId) &&
              !entitledOrganizationIds.has(params.destinationOrganizationId)
            ) {
              throw new WorkspaceMoveError(
                'The destination organization does not hold a paid organization plan, so this workspace would lose the capabilities gated on one.',
                'destination-entitlement-downgrade'
              )
            }
          } else if (entitlements.capabilitiesLost.length > 0) {
            throw new WorkspaceMoveError(
              `The destination organization is not on Enterprise, so this workspace would lose ${entitlements.capabilitiesLost.join(', ')}`,
              'destination-entitlement-downgrade'
            )
          }
