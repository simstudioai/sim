...
const ENTERPRISE_GATED_CAPABILITIES = [
  'permission groups',
  'SSO domains',
...
export async function resolveMoveEntitlements(
  ...
  const [sourceIsEnterprise, destinationIsEnterprise] = await Promise.all([
    sourceOrganizationId ? isOrganizationOnEnterprisePlan(sourceOrganizationId) : false,
    isOrganizationOnEnterprisePlan(destinationOrganizationId),
  ])
  return {
    sourceIsEnterprise,
...
export async function willBrandingChange(
  ...
  const source = rows.find((row) => row.id === sourceOrganizationId)?.whitelabelSettings ?? null
  const destination =
    rows.find((row) => row.id === destinationOrganizationId)?.whitelabelSettings ?? null
  return JSON.stringify(source ?? null) !== JSON.stringify(destination ?? null)
}
