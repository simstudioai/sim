...
import {
  isOrgAdminRole,
  ORG_ADMIN_ROLES,
  type PermissionType as WorkspacePermission,
...
import type { WorkspacePermissionUpdate } from '@/lib/workspaces/permissions/input'
import {
...
export class WorkspacePermissionError extends HttpError {
  constructor(
...
/** Updates existing grants atomically and returns the authoritative transitions. */
export async function updateWorkspacePermissionRecords(
  workspaceId: string,
  actorId: string,
  ...
  if (!workspaceRow.length) {
    throw new WorkspacePermissionError(404, 'Workspace not found')
  }
  ...
  const targetUserIds = updates.map((update) => update.userId)
  ...
  if (nonMemberUserIds.length > 0) {
    ...
    throw new WorkspacePermissionError(
      400,
      'Only existing workspace members can have their permissions updated'
    )
  }

  if (updates.some((update) => update.userId === actorId && update.permissions !== 'admin')) {
    throw new WorkspacePermissionError(400, 'Cannot remove your own admin permissions')
  }

  const preflightViolation = findInheritedRoleViolation(updates, {
    ownerId,
    billedAccountUserId,
    orgAdminUserIds,
  })
  if (preflightViolation) {
    throw new WorkspacePermissionError(400, preflightViolation)
  }
  ...
  const { previousRoles, changedUserIds } = await withTransactionRetry(async (tx) => {
     ...
     */
    const lockUserIds = [...new Set([actorId, ...targetUserIds])]
    ...
    if (organizationId) {
      const lockedMembers = await tx
        ...
        .from(member)
        .where(and(eq(member.organizationId, organizationId), inArray(member.userId, lockUserIds)))
    ...
    const lockedViolation = findInheritedRoleViolation(updates, {
  ...
  return updates
    .filter((update) => changedUserIds.has(update.userId))
    .map((update) => ({
      targetUserId: update.userId,
      targetEmail: emailByUserId.get(update.userId),
      previousRole: previousRoles.get(update.userId) ?? null,
      newRole: update.permissions,
    }))
}

/** Reconciles credential memberships after the audited permission commit. */
export async function reconcileWorkspacePermissionCredentials(
  workspaceId: string,
  actorId: string
) {
  /**
