 ...
 *
 * Two lists are pinned, because the single-row paths now signal from inside their application use
 * case rather than from the route. The call itself is no longer the decision: the use case is
 * shared with `/api/v2` and Copilot, and it degrades to a broadcast whenever no actor is named.
 * What actually selects the behavior is which surface supplies `actorClientId`, so that is pinned
 * too and is the list to scrutinise.
 *
...
const ACTOR_SUPPLYING_SURFACES = [] as const
...
describe('signalTableRowsChangedByActor call sites', () => {
  ...
  it('is given an actor only by surfaces whose client hook reconciles locally', async () => {
    const suppliers = await filesContaining(
      'actorClientId:',
      // Declares the field rather than supplying one.
      (relative) => relative === 'lib/table/application/rows.ts'
    )
