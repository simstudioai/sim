...
export const WORDMARK_FLAT_INK_CLASS = '[color:#e6e6e6] dark:[color:#3b3b3b]'

/** Resting mark geometry within the morph's 100-unit stage. */
const WORDMARK_MORPH_WIDTH = 91.1
const WORDMARK_MORPH_SCALE = WORDMARK_MORPH_WIDTH / WORDMARK_VIEW_BOX.width

/** Shared resting position, preserving the animation's coordinate precision. */
export const WORDMARK_MORPH_TRANSFORM = `translate(${((100 - WORDMARK_MORPH_WIDTH) / 2).toFixed(3)} ${((100 - WORDMARK_VIEW_BOX.height * WORDMARK_MORPH_SCALE) / 2).toFixed(3)}) scale(${WORDMARK_MORPH_SCALE.toFixed(5)})`
