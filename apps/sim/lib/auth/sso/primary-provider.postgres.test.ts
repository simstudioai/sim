...
/** Admission is per address and is not what this suite proves. */
vi.mock('@/lib/core/rate-limiter', () => ({ enforceIpRateLimit: vi.fn(async () => null) }))

const databaseUrl = process.env.OAUTH_TOKEN_FAMILY_TEST_DATABASE_URL

async function loadRuntime() {
  const [{ db }, schema, { eq, inArray }, resolveRoute, listRoute, providerRoute, registerRoute] =
    await Promise.all([
      import('@sim/db'),
  ...
  return {
    ...
    eq,
    inArray,
    resolve: resolveRoute.POST,
    list: listRoute.GET,
    patch: providerRoute.PATCH,
    remove: providerRoute.DELETE,
    register: registerRoute.POST,
  }
}
...
describe.skipIf(!databaseUrl)('Primary SSO provider per organization domain in PostgreSQL', () => {
  ...
  async function insertDomain(
    ...
    const { db, schema } = runtime
    const verified = (options.status ?? 'verified') === 'verified'
  ...
  /** The providers the settings page labels Primary. */
  ...
  it("refuses to let an admin of one organization change another's primary", async () => {
    ...
    expect(await makePrimary('foreign')).toBe(403)
