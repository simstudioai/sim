...
/**
 * Process generic webhook files - handles both base64 ('file' type) and URL pass-through ('url' type)
 */
async function processGenericWebhookFile(
  ...
  if (file.type === 'file' && file.data && file.name) {
    ...
    logger.debug(`[${requestId}] Uploading webhook file: ${file.name} (${buffer.length} bytes)`)
    ...
    logger.debug(`[${requestId}] Successfully uploaded webhook file ${file.name}`)
    return userFile
  }
...
/**
 * Process all files for a given field in the generic webhook input
 */
export async function processGenericWebhookFiles(
  fieldValue: any,
  ...
  for (const file of files) {
    try {
      const userFile = await processGenericWebhookFile(file, fullContext, requestId)

      if (userFile) {
        uploadedFiles.push(userFile)
      }
    } catch (error) {
      logger.error(`[${requestId}] Failed to process webhook file ${file.name}:`, error)
