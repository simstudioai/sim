...
import { ExecuteResponseSuccessSchema, BuildWorkflowResult } from '@/lib/copilot-new/tools/shared/schemas'
...
export class EditWorkflowClientTool extends BaseClientTool {
	...
	async handleAccept(): Promise<void> {
		...
		logger.info('handleAccept called', { toolCallId: this.toolCallId, state: this.getState(), hasResult: this.lastResult !== undefined })
      ...
      if (!currentUserWorkflow) {
        try {
          ...
          logger.warn('Failed to build currentUserWorkflow from stores; proceeding without it', e as any)
			...
			logger.info('server result parsed', { hasYaml: !!result?.yamlContent, yamlLength: (result?.yamlContent || '').length })
