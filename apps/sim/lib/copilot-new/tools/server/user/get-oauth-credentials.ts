...
interface GetOAuthCredentialsParams { userId?: string; workflowId?: string }
...
export const getOAuthCredentialsServerTool: BaseServerTool<GetOAuthCredentialsParams, any> = {
	...
	async execute(params: GetOAuthCredentialsParams): Promise<any> {
		...
		logger.info('Fetching OAuth credentials for user', { hasDirectUserId: !!directUserId, hasWorkflowId: !!params?.workflowId })
		...
		const credentials: Array<{ id: string; name: string; provider: string; lastUsed: string; isDefault: boolean; accessToken: string | null }> = []
		...
		for (const acc of accounts) {
			...
			credentials.push({ id: acc.id, name: displayName, provider: providerId, lastUsed: acc.updatedAt.toISOString(), isDefault: featureType === 'default', accessToken })
		}
