...
import { GetBlocksAndToolsInput, GetBlocksAndToolsResult } from '@/lib/copilot-new/tools/shared/schemas'

export const getBlocksAndToolsServerTool: BaseServerTool<ReturnType<typeof GetBlocksAndToolsInput.parse>, ReturnType<typeof GetBlocksAndToolsResult.parse>> = {
	...
	async execute() {
		...
		const tools: any[] = Object.entries(toolsRegistry).map(([toolId, toolConfig]: any) => ({ id: toolId, type: toolId, name: toolConfig?.name || toolId }))
