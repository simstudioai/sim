...
export const makeApiRequestServerTool: BaseServerTool<MakeApiRequestParams, any> = {
	...
	async execute(params: MakeApiRequestParams): Promise<any> {
		...
		const toTableRows = (obj?: Record<string, any>): TableRow[] | null => {
			...
			return Object.entries(obj).map(([key, value]) => ({ id: key, cells: { Key: key, Value: value } }))
		}
		const headersTable = toTableRows(headers)
		const queryParamsTable = toTableRows(queryParams as Record<string, any> | undefined)

		const result = await executeTool('http_request', { url, method, params: queryParamsTable, headers: headersTable, body }, true)
		...
		const stripHtml = (html: string): string => {
			try { return html.replace(/<script[\s\S]*?<\/script>/gi, '').replace(/<style[\s\S]*?<\/style>/gi, '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() } catch { return html }
		}
		...
		if (totalChars > CAP) {
			...
			logger.warn('API response truncated by character cap', { url, method, totalChars, previewChars: preview.length, cap: CAP })
			return { data: preview, status, headers: respHeaders, truncated: true, totalChars, previewChars: preview.length, note: `Response truncated to ${CAP} characters to avoid large payloads` }
		}
