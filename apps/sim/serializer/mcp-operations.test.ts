...
describe('MCP action serialization and runtime resolution', () => {
  it.each(['run', 'list'])(
    ...
    async (operation) => {
      ...
      expect(block.config.params.server).toBe('<source.connection>')
  ...
  it.each(['basic', 'advanced'] as const)('serializes only the active %s values', (mode) => {
    const mcp = createBlock({
      id: 'mcp',
      type: 'mcp',
      data: { canonicalModes: { server: mode, tool: mode } },
      subBlocks: {
        ...
        toolSelector: { id: 'toolSelector', type: 'mcp-tool-selector', value: 'basic-operation' },
        toolReference: { id: 'toolReference', type: 'short-input', value: '<source.operation>' },
        arguments: { id: 'arguments', type: 'mcp-dynamic-args', value: '{}' },
      },
    })
    const [block] = new Serializer().serializeWorkflow({ mcp }, [], {}, {}, true).blocks
    expect(block.config.tool).toBe('mcp_run_operation')
    expect(block.config.params.server).toBe(
      mode === 'basic' ? 'basic-server' : '<source.connection>'
    )
    expect(block.config.params.tool).toBe(
      mode === 'basic' ? 'basic-operation' : '<source.operation>'
    )
    expect(block.config.params.serverSelector).toBeUndefined()
    expect(block.config.params.toolReference).toBeUndefined()
  })
})
