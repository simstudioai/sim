...
vi.mock('@/ee/scim/identity/account-identity', () => ({
  syncAccountIdentityTx: mocks.syncIdentity,
}))
vi.mock('@/ee/scim/identity/resolve-user', () => ({
  assertDomainOwned: mocks.assertDomainOwned,
}))
vi.mock('@/ee/scim/projection/reconcile-user', () => ({
  reconcileUserProjection: mocks.reconcile,
}))
vi.mock('@/ee/scim/repository/users', () => ({
...
vi.mock('@/ee/scim/application/audit', () => ({
  recordScimAuditEntries: mocks.recordAudit,
}))
vi.mock('@/ee/scim/base-url', () => ({ scimBaseUrl: () => 'https://sim.test/api/scim/v2' }))
