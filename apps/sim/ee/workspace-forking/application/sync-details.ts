...
import { readTargetDraftDependentValue } from '@/lib/workflows/references/remap-references'
import { listForkResourceCandidates } from '@/lib/workflows/references/resources'
...
interface SyncDetailsInput {
  workspaceId: string
  otherWorkspaceId: string
  direction: 'push' | 'pull'
}

export const getWorkspaceSyncDetails = defineForkUseCase({
  ...
  async execute({
    ...
    const { workspaceId: id, direction } = input
    const auth = {
      edge: context.edge!,
