...
const SHAREABLE_KINDS = ['file'] as const
...
describe('workspaceSource', () => {
  ...
  it('resolves a self link to its own workspace route', () => {
    const source = workspaceSource({ kind: 'table', workspaceId: 'ws_1', resourceId: 'tbl_1' })

    expect(source.hrefFor({ to: 'self' })).toBe('/workspace/ws_1/tables/tbl_1')
  })
