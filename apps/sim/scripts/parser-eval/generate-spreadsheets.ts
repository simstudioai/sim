    ...
    sheets: [
      {
        name: 'Employees',
        rows: [
          ['Employee ID', 'Full name', 'Department', 'Office', 'Salary', 'Start date'],
          ...Array.from({ length: 60 }, (_, i) => [
            `E-${2000 + i}`,
            `${pick(['Ana', 'Bjørn', 'Chen', 'Dmitri', 'Eszter', 'Fatima', 'Gustavo', 'Hana'])} ${pick(['Araújo', 'Nakamura', 'Okafor', 'Svensson', 'Müller', 'Patel'])}`,
            pick(['Platform', 'Finance', 'Security', 'Support']),
            pick(cities),
            48000 + Math.floor(rand() * 90000),
    ...
    sheets: [
      {
        ...
        rows: [
          ['Metric', 'Q1', 'Q2'],
          ['Active workspaces', 1240, 1398],
          ['Churned workspaces', 31, 27],
          ['Net revenue retention', '104%', '109%'],
        ],
      },
      {
        name: 'Notes',
        rows: [
          ['Prepared by the Finance team on 4 July.'],
          [],
          ['Figures exclude the Northwind pilot.'],
        ],
      },
      {
        name: 'Raw',
        rows: [
          ['Export generated 2026-07-04'],
          [],
          ['Workspace', 'Plan', 'Seats', 'MRR'],
          ...Array.from({ length: 25 }, (_, i) => [
            `ws-${3000 + i}`,
            pick(['Team', 'Enterprise', 'Pro']),
            3 + Math.floor(rand() * 200),
            Math.round(rand() * 20000) / 100,
          ]),
        ],
      },
    ],
    ...
    sheets: [
      {
        name: 'Matrix',
        rows: [
          ['Row'].concat(Array.from({ length: 39 }, (_, c) => `Col ${c + 1}`)),
          ...Array.from({ length: 30 }, (_, r): (string | number)[] => [
            `R${r + 1}`,
            ...Array.from({ length: 39 }, (_, c) =>
              c % 7 === 0 ? `note, with "quotes" ${r}-${c}` : r * 100 + c
            ),
          ]),
        ],
      },
  ...
  sheets: [
    {
      name: 'Ledger',
      rows: [
        ['Invoice', 'Issued', 'Due', 'Amount', 'Tax rate', 'Paid', 'Total'],
...
function buildTypedSheet(): XLSX.WorkSheet {
  ...
  const rows = [
    [1, new Date(Date.UTC(2026, 2, 4)), new Date(Date.UTC(2026, 3, 3)), 1250, 0.2, true],
    [2, new Date(Date.UTC(2026, 4, 17)), new Date(Date.UTC(2026, 5, 16)), 980.5, 0.085, false],
    [3, new Date(Date.UTC(2026, 6, 29)), new Date(Date.UTC(2026, 7, 28)), 12000, 0, true],
  ]
  rows.forEach((r, i) => {
    const n = i + 2
    ...
    ws[`E${n}`] = { t: 'n', v: r[4], z: Number.isInteger((r[4] as number) * 100) ? '0%' : '0.0%' }
    ws[`F${n}`] = { t: 'b', v: r[5] }
    ws[`G${n}`] = {
      t: 'n',
      f: `D${n}*(1+E${n})`,
      v: (r[3] as number) * (1 + (r[4] as number)),
      z: '"$"#,##0.00',
    }
  })
  ws['!ref'] = 'A1:G4'
...
for (const book of books) {
  ...
  const cells = book.sheets
    .flatMap((s) => s.rows.flatMap((r) => r.map(String)))
    .filter((c) => c.length >= 2)
  const adjacency = book.sheets.flatMap((s) =>
    s.rows.flatMap((r) =>
      r
        .slice(0, -1)
        .map((c, i) => [String(c), String(r[i + 1])])
        .filter(([a, b]) => a.length >= 2 && b.length >= 2 && a !== b)
    )
  )
  const firstSheetCells = book.sheets[0].rows
    .flatMap((r) => r.map(String))
    .filter((c) => c.length >= 2)
  writeFileSync(
    path.join(SPEC, `${book.name}.json`),
    JSON.stringify(
      {
        name: book.name,
        kind: 'spreadsheet',
        ...
        sentinels: cells,
        firstSheetSentinels: firstSheetCells,
        table_adjacency: adjacency,
        order_pairs: [],
        headings: [],
        paragraphs: [],
        list_items: [],
  ...
  writeFileSync(
    path.join(SPEC, `${book.name}.gt.txt`),
    book.sheets.map((s) => s.rows.map((r) => r.join('\t')).join('\n')).join('\n\n')
  )
}
