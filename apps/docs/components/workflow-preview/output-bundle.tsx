...
const BADGE_COLORS: Record<ValueType, { bg: string; text: string }> = {
  string: { bg: '#1d3528', text: '#63d489' },
  number: { bg: '#232c4d', text: '#93a8f7' },
  boolean: { bg: '#3a2a16', text: '#f0a35e' },
  array: { bg: '#2e2347', text: '#b794f6' },
  object: { bg: '#2e2e2e', text: '#b3b3b3' },
  null: { bg: '#2e2e2e', text: '#b3b3b3' },
}
  ...
  return (
    <div className='flex min-w-0 flex-col'>
      <div className='flex min-h-[26px] items-center gap-2 rounded-[6px] px-1'>
        <span className='text-[13px]' style={{ color: node.highlight ? '#33b4ff' : '#e6e6e6' }}>
          {node.key}
        </span>
        <TypeBadge type={type} />
        <ChevronDown
          className='h-[7px] w-[9px] flex-shrink-0 text-[#7a7a7a]'
          style={expanded ? undefined : { transform: 'rotate(-90deg)' }}
        />
      </div>
      {expanded && (node.children || node.value !== undefined) && (
        <div className='mt-0.5 ml-[5px] flex min-w-0 flex-col gap-0.5 border-[#2e2e2e] border-l pl-[10px]'>
          {node.children
            ? node.children.map((child) => (
                <TreeNode key={child.key} node={child} depth={depth + 1} />
              ))
            : node.value !== undefined && (
                <div className='py-0.5 text-[#cfcfcf] text-[13px]'>{node.value}</div>
              )}
        </div>
      )}
    </div>
...
export function OutputBundle({
  ...
  return (
    <div className='not-prose my-6 flex w-full max-w-[640px] flex-col gap-3'>
      <div className='flex overflow-hidden rounded-xl border border-[#2a2a2a] bg-[#141414]'>
        <div className='flex w-[210px] flex-shrink-0 flex-col border-[#2a2a2a] border-r px-2 py-2'>
          <div className='px-2 pb-2 text-[#7a7a7a] text-[12px]'>Logs</div>
          {logRows.map((row) => {
            ...
            return (
              <div
                ...
                style={row.selected ? { background: '#262626' } : undefined}
              >
                <div
                  className='flex size-[18px] flex-shrink-0 items-center justify-center rounded-[5px]'
                  style={{ background: row.color ?? '#3d3d3d' }}
                >
                  {Icon && <Icon className='size-[10px] text-white' />}
                </div>
                <span className='truncate text-[#e6e6e6] text-[13px]'>{row.name}</span>
                {row.duration && (
                  <span className='ml-auto text-[#9a9a9a] text-[12px]'>{row.duration}</span>
                )}
              </div>
            )
          })}
        ...
        <div className='flex min-w-0 flex-1 flex-col px-3 py-2'>
          <div className='flex items-center gap-3 pb-2'>
            <span className='text-[#e6e6e6] text-[13px]'>Output</span>
            <span className='text-[#7a7a7a] text-[13px]'>Input</span>
            <span className='ml-auto flex items-center gap-2 text-[#5a5a5a]'>
