...
エディタでの手動実行は`value`列を使用するため、リクエストを送信せずにテストできます。実行中、リゾルバは`<api.fieldName>`と`<api.input.fieldName>`の両方に値を設定します。
...
リアルタイムストリーミングを有効にすると、ワークフロー出力が生成されるたびに文字単位で受信できます。これはAIレスポンスをユーザーに段階的に表示するのに役立ちます。
...
```json
{
  "files": [{
    "name": "document.pdf",
    "content": "JVBERi0xLjMKJcTl8uXrp/Og0MTGCjQgMCBvY..."
  }]
}
```
