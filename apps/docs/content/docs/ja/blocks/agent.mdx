...
- **システムプロンプトは具体的に記述する**: エージェントの役割、トーン、制限事項を明確に定義してください。指示が具体的であればあるほど、エージェントは意図された目的をより適切に果たすことができます。
- **適切な温度設定を選択する**: 精度が重要な場合は低い温度設定（0～0.3）を使用し、より創造的または多様な応答が必要な場合は温度を上げてください（0.7～2.0）。
- **ツールを効果的に活用する**: エージェントの目的を補完し、その機能を強化するツールを統合してください。エージェントに過度な負担をかけないよう、提供するツールは慎重に選択してください。重複の少ないタスクには、最良の結果を得るために別のエージェントブロックを使用してください。
