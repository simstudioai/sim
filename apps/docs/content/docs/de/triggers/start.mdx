...
- **Eingabeformat-Felder** — Jedes Feld, das Sie hinzufügen, wird als <code>&lt;start.feldName&gt;</code> verfügbar. Zum Beispiel erscheint ein `customerId` Feld als <code>&lt;start.customerId&gt;</code> in nachgelagerten Blöcken und Vorlagen.
- **Nur-Chat-Felder** — Wenn der Workflow über das Chat-Seitenpanel oder eine bereitgestellte Chat-Erfahrung ausgeführt wird, stellt Sim auch <code>&lt;start.input&gt;</code> (neueste Benutzernachricht), <code>&lt;start.conversationId&gt;</code> (aktive Sitzungs-ID) und <code>&lt;start.files&gt;</code> (Chat-Anhänge) bereit.
...
<Tabs items={['Editor-Ausführung', 'API-Bereitstellung', 'Chat-Bereitstellung']}>
  ...
  <Tab>
    <div className="space-y-3">
      <p>
        Die Bereitstellung für eine API verwandelt das Eingabeformat in einen JSON-Vertrag für Clients. Jedes Feld wird Teil des Anforderungskörpers, und Sim erzwingt primitive Typen bei der Aufnahme. Dateifelder erwarten Objekte, die auf hochgeladene Dateien verweisen; verwenden Sie den Ausführungs-Datei-Upload-Endpunkt, bevor Sie den Workflow aufrufen.
      </p>
      <p>
        API-Aufrufer können zusätzliche optionale Eigenschaften einbeziehen. Diese werden
        innerhalb der <code>&lt;start.feldName&gt;</code> Ausgaben beibehalten, sodass Sie experimentieren
...
- Halte <code>&lt;start.conversationId&gt;</code> bereit, wenn du Ausgaben gruppieren, Konversationsverlauf aktualisieren oder die Chat-API erneut aufrufen musst.
