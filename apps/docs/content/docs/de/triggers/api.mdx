...

{
  ...
  "blockType": "code",
  "language": "javascript",
  "code": "console.log('Dies ist ein Code-Block');"
}
```
...
const response = await fetch('/api/v1/execute', {
  ...
  body: JSON.stringify({
    input: { prompt: "Erkläre Quantencomputing" }
  })
});
...
while (true) {
  ...
  for (const line of lines) {
    if (line.startsWith('data: ')) {
      ...
      // Verarbeite den Block basierend auf data.type, data.blockType, etc.
    }
  }
}
```
...
```json
{
  "input": {
    "document": {
      "type": "url",
      "url": "https://beispiel.com/dokument.pdf"
    }
  }
}
```

- Datei wird nicht hochgeladen, URL wird direkt weitergegeben
- Nützlich für Verweise auf bestehende Dateien
