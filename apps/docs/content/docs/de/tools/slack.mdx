...
Slack in den Workflow integrieren. Kann Nachrichten senden, Canvases erstellen und Nachrichten lesen. Benötigt im erweiterten Modus ein Bot-Token anstelle von OAuth. Kann im Trigger-Modus verwendet werden, um einen Workflow auszulösen, wenn eine Nachricht an einen Kanal gesendet wird.
