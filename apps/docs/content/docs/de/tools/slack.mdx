...
Slack in den Workflow integrieren. Kann Nachrichten senden, Leinwände erstellen und Nachrichten lesen. Erfordert Bot-Token anstelle von OAuth im erweiterten Modus. Kann im Trigger-Modus verwendet werden, um einen Workflow auszulösen, wenn eine Nachricht an einen Kanal gesendet wird.
