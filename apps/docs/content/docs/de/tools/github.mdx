...
Github in den Workflow integrieren. Kann PR-Details abrufen, PR-Kommentare erstellen, Repository-Informationen erhalten und den neuesten Commit abrufen. Kann im Trigger-Modus verwendet werden, um einen Workflow auszulösen, wenn ein PR erstellt wird, kommentiert wird oder ein Commit gepusht wird.
