---
title: Telegram
description: Mit Telegram interagieren
---
...
Integriert Telegram in den Workflow. Kann Nachrichten senden und löschen. Kann im Trigger-Modus verwendet werden, um einen Workflow auszulösen, wenn eine Nachricht an einen Chat gesendet wird.
...
Löscht Nachrichten in Telegram-Kanälen oder Chats über die Telegram Bot API. Erfordert die Nachrichten-ID der zu löschenden Nachricht.
...
| `botToken` | string | Ja | Ihr Telegram Bot API-Token |
| `chatId` | string | Ja | Ziel-Telegram-Chat-ID |
| `messageId` | string | Ja | Zu löschende Nachrichten-ID |
...
Sendet Fotos an Telegram-Kanäle oder Benutzer über die Telegram Bot API.
...
| `caption` | string | Nein | Audiobeschreibung \(optional\) |
...
Sende Animationen (GIFs) an Telegram-Kanäle oder Benutzer über die Telegram Bot API.
...
| `caption` | string | Nein | Beschriftung der Animation \(optional\) |
