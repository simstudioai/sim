...
Poll the [Get Job Status](/api-reference/workflow-runs/getWorkflowRunV2) endpoint until the status is `completed` or `failed`:
...
<Callout type="info">
  Use the [Get Usage Limits](/api-reference/billing/getBillingStatus) endpoint to check your current rate limit status and billing usage at any time.
</Callout>
