...
Официальный SDK для Sim позволяет вам выполнять рабочие процессы программно из ваших приложений на Python, используя официальный SDK для Python.
...
**Параметры:**
- `workflow_id` (строка): Идентификатор рабочего процесса для выполнения- `input` (словарь, необязательный параметр): Входные данные для передачи в рабочий процесс.- `timeout` (число с плавающей точкой, необязательный параметр): Время ожидания в секундах (по умолчанию: 30.0)- `stream` (логическое значение, необязательное): Включить потоковую передачу ответов (по умолчанию: False)- `selected_outputs` (список строк, необязательный): Отключает вывод в поток в формате `blockName.attribute` (например, `["agent1.content"]`)- `async_execution` (логическое значение, необязательное): Выполнять асинхронно (по умолчанию: False)
**Возвращаемые значения:** `WorkflowExecutionResult | AsyncExecutionResult`

Когда выполняется условие `async_execution=True`, функция немедленно возвращается с результатом и данными для повторного запроса. В противном случае, она ожидает завершения операции.
...
Получите информацию о текущем ограничении скорости запросов из последнего ответа API.
...
## Примеры
### Базовая реализация рабочих процессов
<Steps>
