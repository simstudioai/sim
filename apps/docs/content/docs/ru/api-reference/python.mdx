...
Официальный SDK для Sim позволяет вам выполнять рабочие процессы программно из ваших приложений на языке Python, используя официальный SDK для Python.
...
**Параметры:**
- `api_key` (строка): Ваш ключ API для Sim- `base_url` (строка, необязательно): Базовый URL для API Sim
#### Методы
##### выполнить рабочий процесс()
Выполните рабочий процесс с необязательными входными данными.
...
**Параметры:**
- `workflow_id` (строка): Идентификатор рабочего процесса для выполнения.- `input` (словарь, необязательный параметр): Входные данные для передачи в рабочий процесс.- `timeout` (число с плавающей точкой, необязательный параметр): Время ожидания в секундах (по умолчанию: 30.0)- `stream` (логическое значение, необязательный параметр): Включить потоковую передачу ответов (по умолчанию: False)- `selected_outputs` (список строк, необязательный): Отключает вывод в поток в формате `blockName.attribute` (например, `["agent1.content"]`)- `async_execution` (логическое значение, необязательное): Выполнять асинхронно (по умолчанию: False)
**Возвращаемые значения:** `WorkflowExecutionResult | AsyncExecutionResult`

Когда выполняется условие `async_execution=True`, функция немедленно возвращается с результатом и данными для повторного запроса. В противном случае, она ждет завершения операции.

##### получить статус рабочего процесса
Получите информацию о статусе рабочего процесса (статус развертывания и т.д.).
...
**Параметры:**
- `workflow_id` (строка): Идентификатор рабочего процесса
**Возвращаемые значения:** `WorkflowStatus`

##### проверить рабочий процесс()
Убедитесь, что рабочий процесс готов к выполнению.
...
Получите статус выполнения асинхронной задачи.
...
**Параметры:**
- `task_id` (строка): Идентификатор задачи, возвращаемый после асинхронного выполнения.
**Возвращаемые значения:** `Dict[str, Any]`

**Поля ответа:**
- `success` (логическое значение): Указывает, был ли запрос успешным.- `taskId` (строка): Идентификатор задачи- `status` (строка): Один из `'queued'`, `'processing'`, `'completed'`, `'failed'`, `'cancelled'`- `metadata` (словарь): Содержит `startedAt`, `completedAt` и `duration`.- `output` (любой, необязательный): Результат выполнения рабочего процесса (после завершения)- `error` (любой, необязательный): Подробности об ошибке (в случае неудачи)- `estimatedDuration` (целое число, необязательный параметр): Ориентировочная продолжительность в миллисекундах (при обработке или добавлении в очередь).
##### Выполнить с повторной попыткой
Выполните рабочий процесс с автоматической повторной попыткой в случае ошибок, связанных с ограничением скорости, используя экспоненциальное увеличение задержки.
...
**Параметры:**
- `workflow_id` (строка): Идентификатор рабочего процесса для выполнения- `input` (словарь, необязательный параметр): Входные данные для передачи в рабочий процесс.- `timeout` (число с плавающей точкой, необязательный параметр): Время ожидания в секундах.- `stream` (логическое значение, необязательный параметр): Включить потоковую передачу ответов.- `selected_outputs` (список, необязательный параметр): Отключает вывод в поток.- `async_execution` (логическое значение, необязательный параметр): Выполнять асинхронно- `max_retries` (целое число, необязательный параметр): Максимальное количество повторных попыток (по умолчанию: 3)- `initial_delay` (число с плавающей точкой, необязательный параметр): Начальная задержка в секундах (по умолчанию: 1.0)- `max_delay` (число с плавающей точкой, необязательный параметр): Максимальная задержка в секундах (по умолчанию: 30.0)- `backoff_multiplier` (число с плавающей точкой, необязательный параметр): Множитель для повторных попыток (по умолчанию: 2.0)
**Возвращаемые значения:** `WorkflowExecutionResult | AsyncExecutionResult`

Логика повторных попыток использует экспоненциальное уменьшение задержки (1с → 2с → 4с → 8с...) с отклонением ±25%, чтобы предотвратить ситуацию, когда одновременно отправляется большое количество запросов. Если API предоставляет заголовок `retry-after`, он будет использован вместо этого.

##### получить информацию о лимитах использования
Получите информацию о текущем ограничении использования API из последнего ответа API.
...
**Возвращаемые значения:** `RateLimitInfo | None`

##### получить лимиты использования
Узнайте текущие ограничения и информацию о лимитах для вашего аккаунта.
...
**Возвращаемые значения:** `UsageLimits`
...
- `UNAUTHORIZED`: Неверный ключ API- `TIMEOUT`: Запрос не был выполнен в течение установленного времени.- `RATE_LIMIT_EXCEEDED`: Превышен лимит запросов.- `USAGE_LIMIT_EXCEEDED`: Превышен лимит использования.- `EXECUTION_ERROR`: Не удалось выполнить рабочий процесс.
## Примеры
### Основная схема выполнения задач
<Steps>
...
### Обработка ошибок
Обрабатывать различные типы ошибок, которые могут возникнуть во время выполнения рабочего процесса:
...
Используйте клиент как менеджер контекста для автоматической обработки освобождения ресурсов.
...
### Выполнение задач в пакетном режиме
...
Выполняйте рабочие процессы асинхронно для задач продолжительной длительности:
...
Автоматически обрабатывайте ограничения скорости с помощью экспоненциального повторного запроса.
...
Отслеживайте использование и лимиты вашего аккаунта:
...
### Выполнение рабочего процесса потокового видео
Выполняйте рабочие процессы с мгновенными откликами в режиме реального времени:
...
Ответ при потоковой передаче соответствует формату Server-Sent Events (SSE):
...
**Пример использования Flask для потоковой передачи данных:**
...
### Настройка окружения
Настройте клиента с помощью переменных окружения:
...
## Требования
- Python 3.8 и более поздние версии- `requests` версии 2.25.0 или более поздней
## Лицензия
Apache-2.0
