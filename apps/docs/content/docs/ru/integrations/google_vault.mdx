---
title: Google Vault
description: Поиск, экспорт и управление блокировками/экспортом для дел Vault
---
...
| --------- | ---- | -------- | ----------- |
| `matterId` | string | Да | The matter ID \(e.g., "12345678901234567890"\) |
| `exportName` | string | Да | Name for the export \(avoid special characters\) |
| `corpus` | string | Да | Data corpus to export \(MAIL, DRIVE, GROUPS, HANGOUTS_CHAT, VOICE\) |
| `accountEmails` | string | Нет | Comma-separated list of user emails to scope export \(e.g., "user1@example.com, user2@example.com"\) |
| `orgUnitId` | string | Нет | Organization unit ID to scope export \(e.g., "id:03ph8a2z1enx5q0", alternative to emails\) |
| `startTime` | string | Нет | Start time for date filtering \(ISO 8601 format, e.g., "2024-01-01T00:00:00Z"\) |
| `endTime` | string | Нет | End time for date filtering \(ISO 8601 format, e.g., "2024-12-31T23:59:59Z"\) |
| `terms` | string | Нет | Search query terms to filter exported content \(e.g., "from:sender@example.com subject:invoice"\) |
...
| --------- | ---- | -------- | ----------- |
| `matterId` | string | Да | The matter ID \(e.g., "12345678901234567890"\) |
| `pageSize` | number | Нет | Number of exports to return per page |
| `pageToken` | string | Нет | Token for pagination |
| `exportId` | string | Нет | Optional export ID to fetch a specific export \(e.g., "exportId123456"\) |
...
| --------- | ---- | -------- | ----------- |
| `matterId` | string | Да | The matter ID \(e.g., "12345678901234567890"\) |
| `bucketName` | string | Да | GCS bucket name from cloudStorageSink.files.bucketName |
| `objectName` | string | Да | GCS object name from cloudStorageSink.files.objectName |
| `fileName` | string | Нет | Optional filename override for the downloaded file |
...
| --------- | ---- | -------- | ----------- |
| `matterId` | string | Да | The matter ID \(e.g., "12345678901234567890"\) |
| `holdName` | string | Да | Name for the hold |
| `corpus` | string | Да | Data corpus to hold \(MAIL, DRIVE, GROUPS, HANGOUTS_CHAT, VOICE\) |
| `accountEmails` | string | Нет | Comma-separated list of user emails to put on hold \(e.g., "user1@example.com, user2@example.com"\) |
| `orgUnitId` | string | Нет | Organization unit ID to put on hold \(e.g., "id:03ph8a2z1enx5q0", alternative to accounts\) |
| `terms` | string | Нет | Search terms to filter held content \(e.g., "from:sender@example.com subject:invoice", for MAIL and GROUPS corpus\) |
| `startTime` | string | Нет | Start time for date filtering \(ISO 8601 format, e.g., "2024-01-01T00:00:00Z", for MAIL and GROUPS corpus\) |
| `endTime` | string | Нет | End time for date filtering \(ISO 8601 format, e.g., "2024-12-31T23:59:59Z", for MAIL and GROUPS corpus\) |
| `includeSharedDrives` | boolean | Нет | Include files in shared drives \(for DRIVE corpus\) |
...
| --------- | ---- | -------- | ----------- |
| `matterId` | string | Да | The matter ID \(e.g., "12345678901234567890"\) |
| `pageSize` | number | Нет | Number of holds to return per page |
| `pageToken` | string | Нет | Token for pagination |
| `holdId` | string | Нет | Optional hold ID to fetch a specific hold \(e.g., "holdId123456"\) |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `name` | string | Да | Name for the new matter |
| `description` | string | Нет | Optional description for the matter |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `pageSize` | number | Нет | Number of matters to return per page |
| `pageToken` | string | Нет | Token for pagination |
| `matterId` | string | Нет | Optional matter ID to fetch a specific matter \(e.g., "12345678901234567890"\) |
