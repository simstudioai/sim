...
Learn how to use the Telegram Tool in Sim to seamlessly automate message delivery to any Telegram group. This tutorial walks you through integrating the tool в ваш рабочий процесс, configuring group messaging, and triggering automated updates in real-time.
...
Интегрируйте Telegram в рабочий процесс. Can send and delete messages. Can be used in trigger mode to trigger a workflow when a message is sent to a chat.
...
| --------- | ---- | -------- | ----------- |
| `botToken` | string | Да | Your Telegram Bot API Token |
| `chatId` | string | Да | Telegram chat ID \(numeric, can be negative for groups\) |
| `text` | string | Да | Message text to send |
...
|     ↳ `id` | number | Chat ID |
|     ↳ `is_bot` | boolean | Указывает,  chat is a bot or not |
...
| `botToken` | string | Да | Your Telegram Bot API Token |
| `chatId` | string | Да | Telegram chat ID \(numeric, can be negative for groups\) |
| `messageId` | string | Да | Telegram message ID \(numeric identifier of the message to delete\) |
...
|   ↳ `ok` | boolean | API response статус успешности |
|   ↳ `deleted` | boolean | Указывает,  message was successfully deleted |
...
| `botToken` | string | Да | Your Telegram Bot API Token |
| `chatId` | string | Да | Telegram chat ID \(numeric, can be negative for groups\) |
| `photo` | string | Да | Photo to send. Pass a file_id or HTTP URL |
| `caption` | string | Нет | Photo caption \(optional\) |
...
|   ↳ `photo` | array | Получение списка of photos included in the message |
|     ↳ `file_id` | string | Unique file ID of the photo |
|     ↳ `file_unique_id` | string | Уникальный идентификатор for this file across different bots |
...
| `botToken` | string | Да | Your Telegram Bot API Token |
| `chatId` | string | Да | Telegram chat ID \(numeric, can be negative for groups\) |
| `video` | string | Да | Video to send. Pass a file_id or HTTP URL |
| `caption` | string | Нет | Video caption \(optional\) |
...
| `botToken` | string | Да | Your Telegram Bot API Token |
| `chatId` | string | Да | Telegram chat ID \(numeric, can be negative for groups\) |
| `audio` | string | Да | Audio file to send. Pass a file_id or HTTP URL |
| `caption` | string | Нет | Audio caption \(optional\) |
...
|     ↳ `file_unique_id` | string | Уникальный идентификатор across different bots for this file |
...
| `botToken` | string | Да | Your Telegram Bot API Token |
| `chatId` | string | Да | Telegram chat ID \(numeric, can be negative for groups\) |
| `animation` | string | Да | Animation to send. Pass a file_id or HTTP URL |
| `caption` | string | Нет | Animation caption \(optional\) |
...
| `botToken` | string | Да | Your Telegram Bot API Token |
| `chatId` | string | Да | Telegram chat ID \(numeric, can be negative for groups\) |
| `files` | file[] | Нет | Document file to send \(PDF, ZIP, DOC, etc.\). Max size: 50MB |
| `caption` | string | Нет | Document caption \(optional\) |
...
Запуск рабочего процесса from Telegram bot messages and events

#### Конфигурация

| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `botToken` | string | Да | Your Telegram Bot Token from BotFather |
...
|       ↳ `is_bot` | boolean | Указывает,  sender is a bot |
...
|   ↳ `isBot` | boolean | Указывает,  sender is a bot |
