---
title: Embeddings
description: Генерация эмбеддингов Open AI
---
...
| --------- | ---- | -------- | ----------- |
| `input` | string | Да | Text to generate embeddings for |
| `model` | string | Нет | Model to use for embeddings |
| `encodingFormat` | string | Нет | The format to return the embeddings in |
| `apiKey` | string | Да | OpenAI API key |
