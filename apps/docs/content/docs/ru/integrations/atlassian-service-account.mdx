...
### 1. Создайте учетную запись для сервиса
<Steps>
  <Step>
...
### 2. Создайте токен API с ограниченным доступом
<Steps>
  <Step>
...
### 3. Найдите доменное имя вашего сайта
Ваш домен сайта Atlassian – это URL, который вы используете для доступа к Jira или Confluence в своем браузере. Например, `your-team.atlassian.net`. Откройте Jira или Confluence, посмотрите на адресную строку и скопируйте часть перед первым `/`.

## Добавление учетной записи сервиса в Sim
<Steps>
  <Step>
...
Токен, домен и найденный идентификатор облака шифруются перед сохранением.

## Использование учетной записи сервиса в рабочих процессах
Добавьте блок для Jira или Confluence в свой рабочий процесс. В выпадающем списке учетных данных будет отображаться ваш аккаунт Atlassian вместе с любыми учетными данными OAuth. Выберите его и настройте блок так, как обычно делаете.

<div className="flex justify-center">
<Изображение
src="/static/credentials/atlassian/sim-jira-block-credential.png"
«Блок Jira в рабочем процессе с выбранным аккаунтом сервиса Atlassian в качестве учетных данных»
ширина = 1000
высота = {500}
className="my-4"
  />
...
Этот блок использует API-шлюз Atlassian (`api.atlassian.com/ex/jira/{cloudId}/...`) с помощью токена учетной записи сервиса. Здесь нет шага имитации – учетная запись сервиса действует как сама по себе, используя те разрешения, которые вы ей предоставили в admin.atlassian.com.
