---
title: Greptile
description: Поиск по кодовой базе и вопросы-ответы на базе ИИ
---
...
Запросы и поиск по кодовым базам на естественном языке с помощью Greptile. Get AI-generated answers about your code, find relevant files, and understand complex codebases.
...
| --------- | ---- | -------- | ----------- |
| `query` | string | Да | Natural language question about the codebase. Example: "How does authentication work?" or "Where is the payment processing logic?" |
| `repositories` | string | Да | Comma-separated list of repositories. Format: "github:branch:owner/repo" or just "owner/repo" \(defaults to github:main\). Example: "facebook/react" or "github:main:facebook/react,github:main:facebook/relay" |
| `sessionId` | string | Нет | Session ID for conversation continuity. Use the same sessionId across multiple queries to maintain context. Example: "session-abc123" |
| `genius` | boolean | Нет | Enable genius mode for more thorough analysis \(slower but more accurate\) |
| `apiKey` | string | Да | Greptile API key |
| `githubToken` | string | Да | GitHub Personal Access Token with repo read access |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `query` | string | Да | Natural language search query to find relevant code. Example: "authentication middleware" or "database connection handling" |
| `repositories` | string | Да | Comma-separated list of repositories. Format: "github:branch:owner/repo" or just "owner/repo" \(defaults to github:main\). Example: "facebook/react" or "github:main:facebook/react,github:main:facebook/relay" |
| `sessionId` | string | Нет | Session ID for conversation continuity. Use the same sessionId across multiple searches to maintain context. Example: "session-abc123" |
| `genius` | boolean | Нет | Enable genius mode for more thorough search \(slower but more accurate\) |
| `apiKey` | string | Да | Greptile API key |
| `githubToken` | string | Да | GitHub Personal Access Token with repo read access |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `remote` | string | Да | Git remote type: github or gitlab |
| `repository` | string | Да | Repository in owner/repo format. Example: "facebook/react" or "vercel/next.js" |
| `branch` | string | Да | Branch to index \(e.g., "main" or "master"\) |
| `reload` | boolean | Нет | Force re-indexing even if already indexed |
| `notify` | boolean | Нет | Send email notification when indexing completes |
| `apiKey` | string | Да | Greptile API key |
| `githubToken` | string | Да | GitHub Personal Access Token with repo read access |
...
| `remote` | string | Да | Git remote type: github or gitlab |
| `repository` | string | Да | Repository in owner/repo format. Example: "facebook/react" or "vercel/next.js" |
| `branch` | string | Да | Branch name \(e.g., "main" or "master"\) |
| `apiKey` | string | Да | Greptile API key |
| `githubToken` | string | Да | GitHub Personal Access Token with repo read access |
