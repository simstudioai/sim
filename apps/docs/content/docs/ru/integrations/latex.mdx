---
title: LaTeX
description: Компиляция документов LaTeX в PDF
---
...
| --------- | ---- | -------- | ----------- |
| `content` | string | Да | LaTeX source of the main document, from \\documentclass to \\end\{document\} |
| `compiler` | string | Нет | LaTeX compiler: pdflatex \(default\), xelatex, lualatex, platex, uplatex, or context |
| `fileName` | string | Нет | Name for the generated PDF file \(default: document.pdf\) |
| `resources` | array | Нет | Supporting files for the compilation. Each entry has a "path" plus exactly one of "content" \(plain text\), "file" \(base64\), or "url" \(remote file\), e.g. \[\{"path": "refs.bib", "content": "..."\}, \{"path": "logo.png", "url": "https://..."\}\] |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `query` | string | Да | Search terms matched against package names and descriptions |
| `maxResults` | number | Нет | Maximum number of packages to return \(default: 25, max: 100\) |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `name` | string | Да | Exact package name, e.g. amsmath, tikz, or biblatex |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `query` | string | Нет | Filter matched against font family and full font name, e.g. "Noto Serif" |
| `maxResults` | number | Нет | Maximum number of fonts to return \(default: 50, max: 200\) |
