---
title: Neo4j
description: Подключение к графовой базе данных Neo4j
---
...
| --------- | ---- | -------- | ----------- |
| `host` | string | Да | Neo4j server hostname or IP address |
| `port` | number | Да | Neo4j server port \(default: 7687 for Bolt protocol\) |
| `database` | string | Да | Database name to connect to \(e.g., "neo4j", "movies", "social"\) |
| `username` | string | Да | Neo4j username |
| `password` | string | Да | Neo4j password |
| `encryption` | string | Нет | Connection encryption mode \(enabled, disabled\) |
| `cypherQuery` | string | Да | Cypher query to execute \(e.g., "MATCH \(n:Person\) RETURN n LIMIT 10", "MATCH \(a\)-\[r\]-&gt;\(b\) WHERE a.name = $name RETURN a, r, b"\) |
| `parameters` | object | Нет | Parameters for the Cypher query as a JSON object. Use for any dynamic values including LIMIT \(e.g., query: "MATCH \(n\) RETURN n LIMIT $limit", parameters: \{limit: 100\}\). |
...
| `password` | string | Да | Neo4j password |
| `encryption` | string | Нет | Connection encryption mode \(enabled, disabled\) |
| `cypherQuery` | string | Да | Cypher CREATE statement to execute \(e.g., "CREATE \(n:Person \{name: $name, age: $age\}\)", "CREATE \(a\)-\[:KNOWS\]-&gt;\(b\)"\) |
| `parameters` | object | Нет | Parameters for the Cypher query as a JSON object \(e.g., \{"name": "Alice", "age": 30\}\) |
...
| `password` | string | Да | Neo4j password |
| `encryption` | string | Нет | Connection encryption mode \(enabled, disabled\) |
| `cypherQuery` | string | Да | Cypher MERGE statement to execute \(e.g., "MERGE \(n:Person \{name: $name\}\) ON CREATE SET n.created = timestamp\(\)", "MERGE \(a\)-\[r:KNOWS\]-&gt;\(b\)"\) |
| `parameters` | object | Нет | Parameters for the Cypher query as a JSON object \(e.g., \{"name": "Alice", "email": "alice@example.com"\}\) |
...
| `password` | string | Да | Neo4j password |
| `encryption` | string | Нет | Connection encryption mode \(enabled, disabled\) |
| `cypherQuery` | string | Да | Cypher query with MATCH and SET statements to update properties \(e.g., "MATCH \(n:Person \{name: $name\}\) SET n.age = $age", "MATCH \(n\) WHERE n.id = $id SET n += $props"\) |
| `parameters` | object | Нет | Parameters for the Cypher query as a JSON object \(e.g., \{"name": "Alice", "age": 31, "props": \{"city": "NYC"\}\}\) |
...
| `password` | string | Да | Neo4j password |
| `encryption` | string | Нет | Connection encryption mode \(enabled, disabled\) |
| `cypherQuery` | string | Да | Cypher query with MATCH and DELETE/DETACH DELETE statements \(e.g., "MATCH \(n:Person \{name: $name\}\) DELETE n", "MATCH \(n\) DETACH DELETE n"\) |
| `parameters` | object | Нет | Parameters for the Cypher query as a JSON object \(e.g., \{"name": "Alice", "id": 123\}\) |
| `detach` | boolean | Нет | Whether to use DETACH DELETE to remove relationships before deleting nodes |
...
| `password` | string | Да | Neo4j password |
| `encryption` | string | Нет | Connection encryption mode \(enabled, disabled\) |
| `cypherQuery` | string | Да | Cypher query to execute \(e.g., "CALL db.labels\(\)", "MATCH \(n\) RETURN count\(n\)", "CREATE INDEX FOR \(n:Person\) ON \(n.name\)"\) |
| `parameters` | object | Нет | Parameters for the Cypher query as a JSON object \(e.g., \{"name": "Alice", "limit": 100\}\) |
