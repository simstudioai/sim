---
title: Redis
description: Операции ключ-значение с Redis
---
...
Получение the value of a key from Redis.

#### Вход

| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `url` | string | Да | Redis connection URL (например, redis://user:password@host:port\) |
...
Получение списка keys matching a pattern in Redis. Avoid using on large databases in production; use the Redis Command tool with SCAN for large key spaces.
...
| --------- | ---- | -------- | ----------- |
| `url` | string | Да | Redis connection URL (например, redis://user:password@host:port\) |
| `pattern` | string | Нет | Pattern to match keys \(по умолчанию: * for all keys\) |
...
| --------- | ---- | -------- | ----------- |
| `url` | string | Да | Redis connection URL (например, redis://user:password@host:port\) |
| `command` | string | Да | Redis command as a JSON array (например, \["SET", "key", "value"\]\) |
...
Получение the value of a field in a hash stored at a key in Redis.
...
Получение all fields and values of a hash stored at a key in Redis.
...
Удаление field from a hash stored at a key in Redis.
...
Получение the remaining time to live (in seconds) of a key in Redis.
...
Получение the length of a list stored at a key in Redis.
...
Получение a range of elements from a list stored at a key in Redis.
...
| --------- | ---- | ----------- |
| `key` | string | The list key |
| `values` | array | Получение списка elements in the specified range |
