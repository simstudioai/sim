---
title: Kalshi
description: Доступ к рынкам прогнозов и торговля на Kalshi
---
...
| --------- | ---- | -------- | ----------- |
| `status` | string | Нет | Filter by market status: "unopened", "open", "closed", or "settled" |
| `seriesTicker` | string | Нет | Filter by series ticker \(e.g., "KXBTC", "INX", "FED-RATE"\) |
| `eventTicker` | string | Нет | Filter by event ticker \(e.g., "KXBTC-24DEC31", "INX-25JAN03"\) |
| `minCreatedTs` | number | Нет | Minimum created timestamp in Unix seconds \(e.g., 1704067200\) |
| `maxCreatedTs` | number | Нет | Maximum created timestamp in Unix seconds \(e.g., 1704153600\) |
| `minUpdatedTs` | number | Нет | Minimum updated timestamp in Unix seconds \(e.g., 1704067200\) |
| `minCloseTs` | number | Нет | Minimum close timestamp in Unix seconds \(e.g., 1704067200\) |
| `maxCloseTs` | number | Нет | Maximum close timestamp in Unix seconds \(e.g., 1704153600\) |
| `minSettledTs` | number | Нет | Minimum settled timestamp in Unix seconds \(e.g., 1704067200\) |
| `maxSettledTs` | number | Нет | Maximum settled timestamp in Unix seconds \(e.g., 1704153600\) |
| `tickers` | string | Нет | Comma-separated list of tickers \(e.g., "KXBTC-24DEC31,INX-25JAN03"\) |
| `mveFilter` | string | Нет | Multivariate event filter: "only" or "exclude" |
| `limit` | string | Нет | Number of results to return \(1-1000, default: 100\) |
| `cursor` | string | Нет | Pagination cursor from previous response for fetching next page |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `ticker` | string | Да | Market ticker identifier \(e.g., "KXBTC-24DEC31", "INX-25JAN03-T4485.99"\) |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `status` | string | Нет | Filter by event status: "open", "closed", or "settled" |
| `seriesTicker` | string | Нет | Filter by series ticker \(e.g., "KXBTC", "INX", "FED-RATE"\) |
| `withNestedMarkets` | string | Нет | Include nested markets in response: "true" or "false" |
| `withMilestones` | string | Нет | Include milestones in response: "true" or "false" |
| `minCloseTs` | number | Нет | Minimum close timestamp in Unix seconds \(e.g., 1704067200\) |
| `limit` | string | Нет | Number of results to return \(1-200, default: 200\) |
| `cursor` | string | Нет | Pagination cursor from previous response for fetching next page |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `eventTicker` | string | Да | Event ticker identifier \(e.g., "KXBTC-24DEC31", "INX-25JAN03"\) |
| `withNestedMarkets` | string | Нет | Include nested markets in response \(true/false\) |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `keyId` | string | Да | Your Kalshi API Key ID |
| `privateKey` | string | Да | Your RSA Private Key \(PEM format\) |
...
| `keyId` | string | Да | Your Kalshi API Key ID |
| `privateKey` | string | Да | Your RSA Private Key \(PEM format\) |
| `ticker` | string | Нет | Filter by market ticker \(e.g., "KXBTC-24DEC31"\) |
| `eventTicker` | string | Нет | Filter by event ticker, max 10 comma-separated \(e.g., "KXBTC-24DEC31,INX-25JAN03"\) |
| `countFilter` | string | Нет | Restrict to positions with non-zero values for the given fields \(comma-separated\): "position", "total_traded" |
| `subaccount` | string | Нет | Subaccount identifier to get positions for |
| `limit` | string | Нет | Number of results to return \(1-1000, default: 100\) |
| `cursor` | string | Нет | Pagination cursor from previous response for fetching next page |
...
| `ticker` | string | Нет | Filter by market ticker \(e.g., "KXBTC-24DEC31"\) |
| `eventTicker` | string | Нет | Filter by event ticker, max 10 comma-separated \(e.g., "KXBTC-24DEC31,INX-25JAN03"\) |
| `status` | string | Нет | Filter by order status: "resting", "canceled", or "executed" |
| `minTs` | string | Нет | Minimum timestamp filter \(Unix timestamp, e.g., "1704067200"\) |
| `maxTs` | string | Нет | Maximum timestamp filter \(Unix timestamp, e.g., "1704153600"\) |
| `subaccount` | string | Нет | Subaccount identifier to filter orders |
| `limit` | string | Нет | Number of results to return \(1-1000, default: 100\) |
| `cursor` | string | Нет | Pagination cursor from previous response for fetching next page |
...
| `keyId` | string | Да | Your Kalshi API Key ID |
| `privateKey` | string | Да | Your RSA Private Key \(PEM format\) |
| `orderId` | string | Да | Order ID to retrieve \(e.g., "abc123-def456-ghi789"\) |
...
| --------- | ---- | -------- | ----------- |
| `ticker` | string | Да | Market ticker identifier \(e.g., "KXBTC-24DEC31", "INX-25JAN03-T4485.99"\) |
| `depth` | number | Нет | Number of price levels to return \(e.g., 10, 20\). Default: all levels |
...
| --------- | ---- | -------- | ----------- |
| `ticker` | string | Нет | Filter by market ticker \(e.g., "KXBTC-24DEC31"\) |
| `minTs` | number | Нет | Minimum timestamp in Unix seconds \(e.g., 1704067200\) |
| `maxTs` | number | Нет | Maximum timestamp in Unix seconds \(e.g., 1704153600\) |
| `limit` | string | Нет | Number of results to return \(1-1000, default: 100\) |
| `cursor` | string | Нет | Pagination cursor from previous response for fetching next page |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `seriesTicker` | string | Да | Series ticker identifier \(e.g., "KXBTC", "INX", "FED-RATE"\) |
| `ticker` | string | Да | Market ticker identifier \(e.g., "KXBTC-24DEC31", "INX-25JAN03-T4485.99"\) |
| `startTs` | number | Да | Start timestamp in Unix seconds \(e.g., 1704067200\) |
| `endTs` | number | Да | End timestamp in Unix seconds \(e.g., 1704153600\) |
| `periodInterval` | number | Да | Period interval: 1 \(1 minute\), 60 \(1 hour\), or 1440 \(1 day\) |
...
| `privateKey` | string | Да | Your RSA Private Key \(PEM format\) |
| `ticker` | string | Нет | Filter by market ticker \(e.g., "KXBTC-24DEC31"\) |
| `orderId` | string | Нет | Filter by order ID \(e.g., "abc123-def456-ghi789"\) |
| `minTs` | number | Нет | Minimum timestamp in Unix seconds \(e.g., 1704067200\) |
| `maxTs` | number | Нет | Maximum timestamp in Unix seconds \(e.g., 1704153600\) |
| `subaccount` | string | Нет | Subaccount identifier to get fills for |
| `limit` | string | Нет | Number of results to return \(1-1000, default: 100\) |
| `cursor` | string | Нет | Pagination cursor from previous response for fetching next page |
...
| `privateKey` | string | Да | Your RSA Private Key \(PEM format\) |
| `ticker` | string | Нет | Filter by market ticker \(e.g., "KXBTC-24DEC31"\) |
| `eventTicker` | string | Нет | Filter by event ticker \(e.g., "KXBTC-24DEC31"\) |
| `minTs` | number | Нет | Minimum settled timestamp in Unix seconds \(e.g., 1704067200\) |
| `maxTs` | number | Нет | Maximum settled timestamp in Unix seconds \(e.g., 1704153600\) |
| `subaccount` | string | Нет | Subaccount number \(0 for primary, 1-63 for subaccounts\) |
| `limit` | string | Нет | Number of results to return \(1-1000, default: 100\) |
| `cursor` | string | Нет | Pagination cursor from previous response for fetching next page |
...
| --------- | ---- | -------- | ----------- |
| `seriesTicker` | string | Да | Series ticker identifier \(e.g., "KXBTC", "INX", "FED-RATE"\) |
| `includeVolume` | string | Нет | Include volume data in response \(true/false\) |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `category` | string | Нет | Filter by category \(e.g., "Economics", "Politics", "Crypto"\) |
| `tags` | string | Нет | Filter by comma-separated tags |
| `includeProductMetadata` | string | Нет | Include product metadata in response \(true/false\) |
| `includeVolume` | string | Нет | Include volume data in response \(true/false\) |
| `minUpdatedTs` | number | Нет | Minimum updated timestamp in Unix seconds \(e.g., 1704067200\) |
...
| `privateKey` | string | Да | Your RSA Private Key \(PEM format\) |
| `ticker` | string | Да | Market ticker identifier \(e.g., "KXBTC-24DEC31", "INX-25JAN03-T4485.99"\) |
| `side` | string | Да | Side of the order: "yes" or "no" |
| `action` | string | Да | Action type: "buy" or "sell" |
| `count` | string | Нет | Number of contracts to trade \(e.g., "10", "100"\). Provide count or countFp |
| `type` | string | Нет | Order type: "limit" or "market" \(default: "limit"\) |
| `yesPrice` | string | Нет | Yes price in cents \(1-99\) |
| `noPrice` | string | Нет | No price in cents \(1-99\) |
| `yesPriceDollars` | string | Нет | Yes price in dollars \(e.g., "0.56"\) |
| `noPriceDollars` | string | Нет | No price in dollars \(e.g., "0.56"\) |
| `clientOrderId` | string | Нет | Custom order identifier |
| `expirationTs` | string | Нет | Unix timestamp for order expiration |
| `timeInForce` | string | Нет | Time in force: 'fill_or_kill', 'good_till_canceled', 'immediate_or_cancel' |
| `buyMaxCost` | string | Нет | Maximum cost in cents \(auto-enables fill_or_kill\) |
| `postOnly` | string | Нет | Set to 'true' for maker-only orders |
| `reduceOnly` | string | Нет | Set to 'true' for position reduction only |
| `selfTradePreventionType` | string | Нет | Self-trade prevention: 'taker_at_cross' or 'maker' |
| `orderGroupId` | string | Нет | Associated order group ID |
| `countFp` | string | Нет | Count in fixed-point for fractional contracts |
| `cancelOrderOnPause` | string | Нет | Set to 'true' to cancel order on market pause |
| `subaccount` | string | Нет | Subaccount to use for the order |
...
| `keyId` | string | Да | Your Kalshi API Key ID |
| `privateKey` | string | Да | Your RSA Private Key \(PEM format\) |
| `orderId` | string | Да | Order ID to cancel \(e.g., "abc123-def456-ghi789"\) |
...
| `keyId` | string | Да | Your Kalshi API Key ID |
| `privateKey` | string | Да | Your RSA Private Key \(PEM format\) |
| `orderId` | string | Да | Order ID to amend \(e.g., "abc123-def456-ghi789"\) |
| `ticker` | string | Да | Market ticker identifier \(e.g., "KXBTC-24DEC31", "INX-25JAN03-T4485.99"\) |
| `side` | string | Да | Side of the order: "yes" or "no" |
| `action` | string | Да | Action type: "buy" or "sell" |
| `clientOrderId` | string | Нет | Original client-specified order ID |
| `updatedClientOrderId` | string | Нет | New client-specified order ID after amendment |
| `count` | string | Нет | Updated quantity for the order \(e.g., "10", "100"\) |
| `yesPrice` | string | Нет | Updated yes price in cents \(1-99\) |
| `noPrice` | string | Нет | Updated no price in cents \(1-99\) |
| `yesPriceDollars` | string | Нет | Updated yes price in dollars \(e.g., "0.56"\) |
| `noPriceDollars` | string | Нет | Updated no price in dollars \(e.g., "0.56"\) |
| `countFp` | string | Нет | Count in fixed-point for fractional contracts |
