---
title: Loops
description: Управление контактами и отправка писем с Loops
---
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Loops API key for authentication |
| `email` | string | Да | The email address for the new contact |
| `firstName` | string | Нет | The contact first name |
| `lastName` | string | Нет | The contact last name |
| `source` | string | Нет | Custom source value replacing the default "API" |
| `subscribed` | boolean | Нет | Whether the contact receives campaign emails \(defaults to true\) |
| `userGroup` | string | Нет | Group to segment the contact into \(one group per contact\) |
| `userId` | string | Нет | Unique user identifier from your application |
| `mailingLists` | json | Нет | Mailing list IDs mapped to boolean values \(true to subscribe, false to unsubscribe\) |
| `customProperties` | json | Нет | Custom contact properties as key-value pairs \(string, number, boolean, or date values\) |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Loops API key for authentication |
| `email` | string | Нет | The contact email address \(at least one of email or userId is required\) |
| `userId` | string | Нет | The contact userId \(at least one of email or userId is required\) |
| `firstName` | string | Нет | The contact first name |
| `lastName` | string | Нет | The contact last name |
| `source` | string | Нет | Custom source value replacing the default "API" |
| `subscribed` | boolean | Нет | Whether the contact receives campaign emails \(sending true re-subscribes unsubscribed contacts\) |
| `userGroup` | string | Нет | Group to segment the contact into \(one group per contact\) |
| `mailingLists` | json | Нет | Mailing list IDs mapped to boolean values \(true to subscribe, false to unsubscribe\) |
| `customProperties` | json | Нет | Custom contact properties as key-value pairs \(send null to reset a property\) |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Loops API key for authentication |
| `email` | string | Нет | The contact email address to search for \(at least one of email or userId is required\) |
| `userId` | string | Нет | The contact userId to search for \(at least one of email or userId is required\) |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Loops API key for authentication |
| `email` | string | Нет | The email address of the contact to delete \(at least one of email or userId is required\) |
| `userId` | string | Нет | The userId of the contact to delete \(at least one of email or userId is required\) |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Loops API key for authentication |
| `email` | string | Да | The email address of the recipient |
| `transactionalId` | string | Да | The ID of the transactional email template to send |
| `dataVariables` | json | Нет | Template data variables as key-value pairs \(string or number values\) |
| `addToAudience` | boolean | Нет | Whether to create the recipient as a contact if they do not already exist \(default: false\) |
| `attachments` | json | Нет | Array of file attachments. Each object must have filename \(string\), contentType \(MIME type string\), and data \(base64-encoded string\). |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Loops API key for authentication |
| `email` | string | Нет | The email address of the contact \(at least one of email or userId is required\) |
| `userId` | string | Нет | The userId of the contact \(at least one of email or userId is required\) |
| `eventName` | string | Да | The name of the event to trigger |
| `eventProperties` | json | Нет | Event data as key-value pairs \(string, number, boolean, or date values\) |
| `mailingLists` | json | Нет | Mailing list IDs mapped to boolean values \(true to subscribe, false to unsubscribe\) |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Loops API key for authentication |
| `perPage` | string | Нет | Number of results per page \(10-50, default: 20\) |
| `cursor` | string | Нет | Pagination cursor from a previous response to fetch the next page |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Loops API key for authentication |
| `name` | string | Да | The property name in camelCase format \(e.g., "favoriteColor"\) |
| `type` | string | Да | The property data type \(e.g., "string", "number", "boolean", "date"\) |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Loops API key for authentication |
| `list` | string | Нет | Filter type: "all" for all properties \(default\) or "custom" for custom properties only |
