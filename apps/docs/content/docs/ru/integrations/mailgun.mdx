---
title: Mailgun
description: Отправка писем и управление списками рассылки с Mailgun
---
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Mailgun API key |
| `domain` | string | Да | Mailgun sending domain \(e.g., mg.example.com\) |
| `from` | string | Да | Sender email address \(e.g., sender@example.com or "Name &lt;sender@example.com&gt;"\) |
| `to` | string | Да | Recipient email address \(e.g., user@example.com\). Use comma-separated values for multiple recipients |
| `subject` | string | Да | Email subject line |
| `text` | string | Нет | Plain text body of the email |
| `html` | string | Нет | HTML body of the email \(e.g., "&lt;h1&gt;Hello&lt;/h1&gt;&lt;p&gt;Message content&lt;/p&gt;"\) |
| `cc` | string | Нет | CC recipient email address \(e.g., cc@example.com\). Use comma-separated values for multiple recipients |
| `bcc` | string | Нет | BCC recipient email address \(e.g., bcc@example.com\). Use comma-separated values for multiple recipients |
| `tags` | string | Нет | Tags for the email \(comma-separated\) |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Mailgun API key |
| `domain` | string | Да | Mailgun domain for retrieving messages \(e.g., mg.example.com\) |
| `messageKey` | string | Да | Message storage key |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Mailgun API key |
| `domain` | string | Да | Mailgun domain for listing events \(e.g., mg.example.com\) |
| `event` | string | Нет | Filter by event type \(accepted, delivered, failed, opened, clicked, etc.\) |
| `limit` | number | Нет | Maximum number of events to return \(default: 100\) |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Mailgun API key |
| `address` | string | Да | Mailing list address \(e.g., newsletter@mg.example.com\) |
| `name` | string | Нет | Mailing list name |
| `description` | string | Нет | Mailing list description |
| `accessLevel` | string | Нет | Access level: readonly, members, or everyone |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Mailgun API key |
| `address` | string | Да | Mailing list address to retrieve \(e.g., newsletter@mg.example.com\) |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Mailgun API key |
| `listAddress` | string | Да | Mailing list address \(e.g., list@mg.example.com\) |
| `address` | string | Да | Member email address to add \(e.g., user@example.com\) |
| `name` | string | Нет | Member name |
| `vars` | string | Нет | JSON string of custom variables |
| `subscribed` | boolean | Нет | Whether the member is subscribed |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Mailgun API key |
| `domain` | string | Да | Domain name to retrieve details for \(e.g., mg.example.com\) |
