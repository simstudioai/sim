---
title: Google Maps
description: Геокодирование, маршруты, места и расчёт расстояний
---
...
Интеграция API Google Maps Platform в ваш рабочий процесс. Поддерживает геокодирование адресов в координаты, обратное геокодирование, получение маршрутов между точками, расчёт матриц расстояний, поиск мест, получение сведений о местах, данные о высоте и информацию о часовых поясах.
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Google Maps API key with Air Quality API enabled |
| `lat` | number | Да | Latitude coordinate |
| `lng` | number | Да | Longitude coordinate |
| `languageCode` | string | Нет | Language code for the response \(e.g., "en", "es"\) |
| `pricing` | per_request | Нет | No description |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Google Maps API key |
| `origin` | string | Да | Starting location \(address or lat,lng\) |
| `destination` | string | Да | Destination location \(address or lat,lng\) |
| `mode` | string | Нет | Travel mode: driving, walking, bicycling, or transit |
| `avoid` | string | Нет | Features to avoid: tolls, highways, or ferries |
| `waypoints` | json | Нет | Array of intermediate waypoints |
| `units` | string | Нет | Unit system: metric or imperial |
| `language` | string | Нет | Language code for results \(e.g., en, es, fr\) |
| `pricing` | per_request | Нет | No description |
| `rateLimit` | string | Нет | No description |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Google Maps API key |
| `origin` | string | Да | Origin location \(address or lat,lng\) |
| `destinations` | json | Да | Array of destination locations |
| `mode` | string | Нет | Travel mode: driving, walking, bicycling, or transit |
| `avoid` | string | Нет | Features to avoid: tolls, highways, or ferries |
| `units` | string | Нет | Unit system: metric or imperial |
| `language` | string | Нет | Language code for results \(e.g., en, es, fr\) |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Google Maps API key |
| `address` | string | Да | The address to geocode |
| `language` | string | Нет | Language code for results \(e.g., en, es, fr\) |
| `region` | string | Нет | Region bias as a ccTLD code \(e.g., us, uk\) |
| `pricing` | per_request | Нет | No description |
| `rateLimit` | string | Нет | No description |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Google Maps API key with Geolocation API enabled |
| `homeMobileCountryCode` | number | Нет | Home mobile country code \(MCC\) |
| `homeMobileNetworkCode` | number | Нет | Home mobile network code \(MNC\) |
| `radioType` | string | Нет | Radio type: lte, gsm, cdma, wcdma, or nr |
| `carrier` | string | Нет | Carrier name |
| `considerIp` | boolean | Нет | Whether to use IP address for geolocation \(default: true\) |
| `cellTowers` | array | Нет | Array of cell tower objects with cellId, locationAreaCode, mobileCountryCode, mobileNetworkCode |
| `wifiAccessPoints` | array | Нет | Array of WiFi access point objects with macAddress \(required\), signalStrength, etc. |
| `pricing` | per_request | Нет | No description |
| `rateLimit` | string | Нет | No description |
...
| `fields` | string | Нет | Comma-separated list of fields to return |
| `language` | string | Нет | Language code for results \(e.g., en, es, fr\) |
| `pricing` | per_request | Нет | No description |
| `rateLimit` | string | Нет | No description |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Google Maps API key |
| `query` | string | Да | Search query \(e.g., "restaurants in Times Square"\) |
| `location` | json | Нет | Location to bias results towards \(\{lat, lng\}\) |
| `radius` | number | Нет | Search radius in meters |
| `type` | string | Нет | Place type filter \(e.g., restaurant, cafe, hotel\) |
| `language` | string | Нет | Language code for results \(e.g., en, es, fr\) |
| `region` | string | Нет | Region bias as a ccTLD code \(e.g., us, uk\) |
| `pricing` | per_request | Нет | No description |
| `rateLimit` | string | Нет | No description |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Google Maps API key with Pollen API enabled |
| `lat` | number | Да | Latitude coordinate |
| `lng` | number | Да | Longitude coordinate |
| `days` | number | Нет | Number of forecast days to return \(1-5, defaults to 1\) |
| `languageCode` | string | Нет | Language code for the response \(e.g., "en", "es"\) |
| `plantsDescription` | boolean | Нет | Include detailed plant descriptions \(defaults to true\) |
| `pricing` | per_request | Нет | No description |
| `rateLimit` | string | Нет | No description |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Google Maps API key with Roads API enabled |
| `path` | string | Да | Pipe-separated list of lat,lng coordinates \(e.g., "60.170880,24.942795\|60.170879,24.942796"\) |
| `interpolate` | boolean | Нет | Whether to interpolate additional points along the road |
| `pricing` | per_request | Нет | No description |
| `rateLimit` | string | Нет | No description |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Google Maps API key with Solar API enabled |
| `lat` | number | Да | Latitude coordinate |
| `lng` | number | Да | Longitude coordinate |
| `requiredQuality` | string | Нет | Minimum imagery quality to accept \(HIGH, MEDIUM, or BASE\) |
| `pricing` | per_request | Нет | No description |
| `rateLimit` | string | Нет | No description |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Google Maps API key with Roads API enabled |
| `path` | string | Нет | Pipe-separated list of lat,lng coordinates \(required if placeIds not provided\) |
| `placeIds` | array | Нет | Array of Place IDs for road segments \(required if path not provided\) |
...
| `lat` | number | Да | Latitude coordinate |
| `lng` | number | Да | Longitude coordinate |
| `timestamp` | number | Нет | Unix timestamp to determine DST offset \(defaults to current time\) |
| `language` | string | Нет | Language code for timezone name \(e.g., en, es, fr\) |
| `pricing` | per_request | Нет | No description |
| `rateLimit` | string | Нет | No description |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Да | Google Maps API key with Address Validation API enabled |
| `address` | string | Да | The address to validate \(as a single string\) |
| `regionCode` | string | Нет | ISO 3166-1 alpha-2 country code \(e.g., "US", "CA"\) |
| `locality` | string | Нет | City or locality name |
| `enableUspsCass` | boolean | Нет | Enable USPS CASS validation for US addresses |
| `pricing` | per_request | Нет | No description |
| `rateLimit` | string | Нет | No description |
