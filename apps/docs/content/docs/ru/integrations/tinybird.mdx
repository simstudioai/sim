---
title: Tinybird
description: Отправка событий, запрос данных и управление источниками данных с Tinybird
---
...
- **Monitor and debug:** Получение insights into pipeline performance, query latencies, and ingestion health with real-time monitoring.
...
| --------- | ---- | -------- | ----------- |
| `base_url` | string | Да | Tinybird API base URL (например, https://api.tinybird.co or https://api.us-east.tinybird.co\) |
| `datasource` | string | Да | Name of the Tinybird Data Source to send events to. Example: "events_raw", "user_analytics" |
| `data` | string | Да | Data to send as NDJSON \(newline-delimited JSON\) or JSON string. Each event should be a valid JSON object. Example NDJSON: \{"user_id": 1, "event": "click"\}\\n\{"user_id": 2, "event": "view"\} |
| `wait` | boolean | Нет | Wait for database acknowledgment before responding. Enables safer retries but introduces latency. Defaults to false. |
| `format` | string | Нет | Format of the events data: "ndjson" \(default\) or "json" |
| `compression` | string | Нет | Compression format: "none" \(default\) or "gzip" |
| `token` | string | Да | Tinybird API Token with DATASOURCE:APPEND or DATASOURCE:CREATE scope |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `base_url` | string | Да | Tinybird API base URL (например, https://api.tinybird.co\) |
| `query` | string | Да | SQL query to execute. Specify your desired output format (например, FORMAT JSON, FORMAT CSV, FORMAT TSV\). JSON format provides structured data, while other formats return raw text. Example: "SELECT * FROM my_datasource LIMIT 100 FORMAT JSON" |
| `pipeline` | string | Нет | Optional pipe name. When provided, enables SELECT * FROM _ syntax. Example: "my_pipe", "analytics_pipe" |
| `token` | string | Да | Tinybird API Token with PIPE:READ scope |
...
| --------- | ---- | -------- | ----------- |
| `base_url` | string | Да | Tinybird API base URL (например, https://api.tinybird.co\) |
| `pipe` | string | Да | Name of the published Pipe API Endpoint to call. Example: "top_pages" |
| `parameters` | json | Нет | Dynamic Pipe parameters as a JSON object, sent as query-string arguments. Example: \{"start_date": "2024-01-01", "limit": 10\} |
| `q` | string | Нет | Optional SQL to run on top of the Pipe result. Use "_" to reference the Pipe. Example: "SELECT count\(\) FROM _" |
| `token` | string | Да | Tinybird API Token with PIPE:READ scope |
...
| --------- | ---- | -------- | ----------- |
| `base_url` | string | Да | Tinybird API base URL (например, https://api.tinybird.co\) |
| `datasource` | string | Да | Name of the existing Data Source to append to. Example: "events_raw" |
| `url` | string | Да | Publicly accessible URL of the file to append. Example: "https://example.com/data.csv" |
| `format` | string | Нет | Format of the source file: "csv" \(default\), "ndjson", or "parquet" |
| `token` | string | Да | Tinybird API Token with DATASOURCES:CREATE scope |
...
| `status` | string | Initial job status (например, "waiting"\) |
...
Удалениеll rows from a Tinybird Data Source.
...
| --------- | ---- | -------- | ----------- |
| `base_url` | string | Да | Tinybird API base URL (например, https://api.tinybird.co\) |
| `datasource` | string | Да | Name of the Data Source to truncate. Example: "events_raw" |
| `token` | string | Да | Tinybird API Token with DATASOURCES:CREATE scope |
...
| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `truncated` | boolean | Указывает,  Data Source was truncated successfully |
...
| --------- | ---- | -------- | ----------- |
| `base_url` | string | Да | Tinybird API base URL (например, https://api.tinybird.co\) |
| `datasource` | string | Да | Name of the Data Source to delete rows from. Example: "events_raw" |
| `delete_condition` | string | Да | SQL WHERE-clause condition selecting the rows to delete. Example: "country = \'ES\'" or "event_date &lt; \'2024-01-01\'" |
| `dry_run` | boolean | Нет | When true, returns how many rows would be deleted without deleting them. Defaults to false. |
| `token` | string | Да | Tinybird API Token with DATASOURCES:CREATE scope |
...
| `status` | string | Current job status (например, "waiting", "done"\) |
