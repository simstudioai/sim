---
title: Hunter.io
description: Поиск и проверка профессиональных адресов электронной почты
---
...
| --------- | ---- | -------- | ----------- |
| `query` | string | Нет | Natural language search query for companies |
| `domain` | string | Нет | Company domain name to filter by \(e.g., "stripe.com", "company.io"\) |
| `headcount` | string | Нет | Company size filter \(e.g., "1-10", "11-50"\) |
| `company_type` | string | Нет | Type of organization |
| `technology` | string | Нет | Technology used by companies |
| `apiKey` | string | Да | Hunter.io API Key |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `domain` | string | Да | Domain name to search for email addresses \(e.g., "stripe.com", "company.io"\) |
| `limit` | number | Нет | Maximum email addresses to return \(e.g., 10, 25, 50\). Default: 10 |
| `offset` | number | Нет | Number of email addresses to skip for pagination \(e.g., 0, 10, 20\) |
| `type` | string | Нет | Filter for personal or generic emails \(e.g., "personal", "generic", "all"\) |
| `seniority` | string | Нет | Filter by seniority level \(e.g., "junior", "senior", "executive"\) |
| `department` | string | Нет | Filter by specific department \(e.g., "sales", "marketing", "engineering", "hr"\) |
| `apiKey` | string | Да | Hunter.io API Key |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `domain` | string | Да | Company domain name \(e.g., "stripe.com", "company.io"\) |
| `first_name` | string | Да | Person\'s first name \(e.g., "John", "Sarah"\) |
| `last_name` | string | Да | Person\'s last name \(e.g., "Smith", "Johnson"\) |
| `company` | string | Нет | Company name \(e.g., "Stripe", "Acme Inc"\) |
| `apiKey` | string | Да | Hunter.io API Key |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `email` | string | Да | The email address to verify |
| `apiKey` | string | Да | Hunter.io API Key |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `domain` | string | Да | Domain to find company data for \(e.g., "stripe.com", "company.io"\) |
| `apiKey` | string | Да | Hunter.io API Key |
...
| Параметр | Тип | Обязательный | Описание |
| --------- | ---- | -------- | ----------- |
| `domain` | string | Нет | Domain to count emails for \(e.g., "stripe.com"\). Required if company not provided |
| `company` | string | Нет | Company name to count emails for \(e.g., "Stripe", "Acme Inc"\). Required if domain not provided |
| `type` | string | Нет | Filter for personal or generic emails only \(e.g., "personal", "generic", "all"\) |
| `apiKey` | string | Да | Hunter.io API Key |
