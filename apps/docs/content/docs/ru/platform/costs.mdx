...
Sim автоматически рассчитывает стоимость каждого запуска рабочего процесса на основе использования ИИ-моделей и базовой платы за запуск.

## Кредиты

Sim использует **кредиты** как единицу измерения для всего использования. **1 кредит = $0.005**.

Все лимиты тарифов, счётчики использования и пороги биллинга отображаются в кредитах во всём интерфейсе Sim. Суммы в долларах в этой документации приведены для справки.

## Как рассчитывается стоимость

Каждый запуск рабочего процесса включает два компонента стоимости:

**Базовая плата за запуск**: 1 кредит ($0.005) за запуск

**Использование ИИ-моделей**: Переменная стоимость на основе потребления токенов
```javascript
...
<Callout type="info">
  Цены на ИИ-модели указаны за миллион токенов. Рабочие процессы без ИИ-блоков оплачивают только базовую плату за запуск.
</Callout>

## Разбивка по моделям в журналах

Для рабочих процессов, использующих ИИ-блоки, вы можете просмотреть детальную информацию о стоимости в журналах:

<div className="flex justify-center">
  <Image
    src="/static/logs/logs-cost.png"
    alt="Разбивка по моделям"
    width={600}
    height={400}
    className="my-6"
  />
...
Разбивка по моделям показывает:
- **Использование токенов**: Количество входных и выходных токенов для каждой модели
- **Разбивка затрат**: Индивидуальные затраты на модель и операцию
- **Распределение моделей**: Какие модели использовались и сколько раз
- **Общая стоимость**: Совокупная стоимость всего запуска рабочего процесса

## Варианты ценообразования

<Tabs items={['Hosted Models', 'Bring Your Own API Key']}>
  <Tab>
    **Hosted Models** — Sim предоставляет ключи API с множителем цены 1.1x для блоков Agent:
  ...
  <Tab>
    **Собственные ключи API** — Используйте любую модель по базовым ценам:
    ...
    *Оплачивайте провайдерам напрямую без наценки*
  </Tab>
</Tabs>

<Callout type="warning">
  Указанные цены отражают тарифы по состоянию на 10 сентября 2025 г. Проверяйте документацию провайдера для актуальных цен.
</Callout>

## Цены на размещённые инструменты

Когда рабочие процессы используют блоки инструментов с размещёнными ключами API Sim, стоимость взимается за операцию. Используйте собственные ключи через BYOK для оплаты провайдерам напрямую.
...
Используйте собственные ключи API для поддерживаемых провайдеров вместо размещённых ключей Sim, чтобы платить по базовым ценам без наценки.
...
1. Перейдите в **Настройки** → **BYOK** в вашем рабочем пространстве
2. Нажмите **Add Key** для вашего провайдера
3. Введите ваш ключ API и сохраните

<Callout type="info">
  Ключи BYOK шифруются при хранении. Только администраторы рабочего пространства могут управлять ключами.
</Callout>

При настройке рабочие процессы используют ваш ключ вместо размещённых ключей Sim. При удалении рабочие процессы автоматически возвращаются к размещённым ключам с множителем.

## Голосовой ввод

Голосовой ввод использует ElevenLabs Scribe v2 Realtime для транскрипции речи в текст. Он доступен в чате Mothership и в голосовом режиме развёрнутого чата.
