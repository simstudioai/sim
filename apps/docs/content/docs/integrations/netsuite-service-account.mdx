...
Select this credential in each NetSuite block. The private key stays encrypted in Sim; you do not need to paste it into individual blocks.
...
To rotate a certificate, create and upload the replacement certificate and create its new NetSuite mapping. Then reconnect the existing Sim credential by re-entering all four required fields: SuiteTalk URL, Client ID, the new Certificate ID, and the replacement private key. Later runs use the replacement certificate and key.
