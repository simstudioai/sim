...
Use [Microsoft Word](https://www.microsoft.com/microsoft-365/word) in Sim to create, read, and edit `.docx` files stored in OneDrive or SharePoint, fill templates, and export documents as PDF. Choose **Replace Content** to overwrite a document, **Append Content** to add paragraphs, or **Find and Replace Text** to update matching text. The sections below explain document IDs, supported formatting, and template behavior.
