...
| `runTimeout` | number | No | Timeout in seconds for the actor run. Example: 300 for 5 minutes, 3600 for 1 hour. Named runTimeout because the shared tool transport reserves `timeout` for its own request deadline in milliseconds |
...
| `runTimeout` | number | No | Timeout in seconds for the run. Example: 300 for 5 minutes. Named runTimeout because the shared tool transport reserves `timeout` for its own request deadline in milliseconds |
