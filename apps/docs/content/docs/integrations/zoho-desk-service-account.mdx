...
This guide covers Sim's Zoho Desk **Self Client** connection. It collects a Client ID, client secret, organization ID, and data center, then requests access tokens for the configured Desk organization.
...
**Get Attachment** has not been verified with this Self Client flow. If it returns a scope error, contact support with the operation and error details; the requested scope list is managed by Sim.
...
<FAQ items={[
  { question: "Where do I find the Organization ID?", answer: "In Zoho Desk, go to Setup (gear icon) → Developer Space → API. The numeric Organization ID shown there is the value to paste. If Zoho returns missing_org_info, confirm the portal ID and data center. Adding ZohoDesk. to the same ID does not change the request; Sim already applies that prefix." },
