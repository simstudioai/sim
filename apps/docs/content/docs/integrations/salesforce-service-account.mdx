...
Connect Salesforce through an External Client App and a dedicated integration user. Sim supports client credentials and JWT bearer authentication; API calls use the selected integration user's profile and permission sets.

An **API-only** integration user cannot complete interactive OAuth. Both server-to-server flows support that user type.

Sim supports both server-to-server flows. Pick one:

| Feature | **Client credentials** | **JWT bearer** |
|---|---|---|
...
## Adding the Integration User App to Sim
...
Add a Salesforce block to your workflow. In the credential dropdown, select the saved Salesforce integration user app. Select it and configure the block as you normally would.
...
<FAQ items={[
  ...
  { question: "Adding the credential fails with an authentication error even though the key and secret are right — what now?", answer: "Salesforce likely rejected the token request with invalid_grant (e.g. 'no client credentials user enabled'), which Sim reports as not being able to authenticate with those credentials. The app either doesn't have Enable Client Credentials Flow checked, or no Run As user is set. Enable Client Credentials Flow under the app's OAuth settings, then set Run As under its OAuth policies. For a legacy Connected App, use App Manager to edit the OAuth settings and Manage → Edit Policies for Run As." },
  ...
  { question: "What happens if the integration user is deactivated or frozen?", answer: "All token minting fails with invalid_grant and every workflow using the credential stops. Reactivate the user, or configure an active integration user. For client credentials, update the app's Run As policy in Salesforce. For JWT bearer, reconnect the Sim credential with the new Run as username and confirm that user is pre-authorized." },
]} />
