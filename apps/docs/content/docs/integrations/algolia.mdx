...
Use [Algolia](https://www.algolia.com/) in Sim to search indices, add or update records, run batch operations, and manage index settings. The actions below also cover browsing records, deleting by filter, copying or moving indices, and checking task status.
{/* MANUAL-CONTENT-END */}
