...
[Amazon Athena](https://aws.amazon.com/athena/) queries data in Amazon S3 with SQL. Start a query, check its execution status, then retrieve the results; cancel a query when needed. The integration also manages named queries and reads database and table metadata. Connect with an AWS access key and secret access key.
{/* MANUAL-CONTENT-END */}
