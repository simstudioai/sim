...
Connecting Jira requests these scopes.
...
| `write:servicedesk.customer:jira-service-management` | Add and remove customers from service desks |
...
| `read:servicedesk.organization:jira-service-management` | View organizations linked to service desks |
| `write:servicedesk.organization:jira-service-management` | Add and remove organizations from service desks |
...
| `write:request.participant:jira-service-management` | Add and remove participants from customer requests |
...
Jira also accepts a credential you create yourself. Sim cannot request scopes on one, so its access is whatever the credential carries: some providers let you select these permissions when you create the token, others give it the creating user's access.
