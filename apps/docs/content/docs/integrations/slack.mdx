...
Start a streaming Slack message using Markdown or structured chunks.
...
| `authMethod` | string | No | Slack authentication method |
| `botToken` | string | No | Custom Slack bot token |
| `channel` | string | Yes | Channel ID where Slack should create the streaming message |
...
| `username` | string | No | Display name used to customize the streaming agent identity |

#### Output

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `ok` | boolean | Whether Slack started the stream |
| `channel` | string | Channel containing the streaming message |
| `ts` | string | Timestamp used to append to or stop the stream |

### Slack Append Stream

Append Markdown or structured chunks to an active Slack stream.
...
| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `ok` | boolean | Whether Slack appended the stream content |
| `channel` | string | Channel containing the streaming message |
| `ts` | string | Streaming message timestamp |

### Slack Stop Stream

Finalize an active Slack stream and return the resulting message.
...
| `channel` | string | Yes | Channel containing the streaming message |
| `ts` | string | Yes | Timestamp returned by Start Stream |
| `markdownText` | string | No | Final Markdown content, mutually exclusive with chunks |
| `chunks` | json | No | Final structured Slack chunks, mutually exclusive with Markdown |
| `blocks` | json | No | Final Block Kit blocks, as an array of up to 50 block objects |
| `metadata` | json | No | Message metadata containing event_type and event_payload |
| `sessionStatus` | string | No | Final agent session state: active, processing, suspended, or closed |

#### Output

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `ok` | boolean | Whether Slack finalized the stream |
| `channel` | string | Channel containing the finalized message |
