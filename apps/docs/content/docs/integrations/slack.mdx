...
Choose a Lists action from the Slack block’s **Operation** menu.
...
The custom app needs `lists:read` for Read List Items and Get List Item, and `lists:write` for writes. Enable **Manage Lists** in the custom app setup manifest. For an existing bot, add these scopes to its Slack app, reinstall it, and update its stored credential in Sim. This integration does not add scopes to the native Sim Slack app.

### Read the schema before writing rows

1. Copy the `F…` List ID from the Slack List URL, or use `listId` returned by Create List.
2. Run **Read List Items** with **Include List Schema** enabled (the default). The `list.schema` output contains each column's `id`, `name`, `type`, and type-specific `options`. This works even when the List has no rows. **Get List Item** also returns the parent schema. Create List may return `schema`; if Slack omits it, use Read List Items.
3. Use those real `Col…` IDs as `column_id`. Use the `Rec…` IDs returned by Read List Items or Create List Item as `row_id` for updates. Select values must match the column's configured choice values.

For **Create List Item**, provide **Initial Fields** as a JSON array. For example, replace the placeholder column IDs below with IDs from your List:
...
Select a Canvas action from the Slack block’s **Operation** menu.
...
- **Get Canvas Info** returns file details and links, not the document body. **List Canvases** returns one page of metadata; increment **Page** using the returned `paging.page` and `paging.pages` to continue.
