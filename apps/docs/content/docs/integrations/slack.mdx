---
title: Slack
description: Manage Slack messages, channels, users, files, search, Lists, canvases, and Agent Sessions
---
...
Search, profile updates, and notification snooze changes require a **managed Slack user credential** authorized through your custom app in a Slack Credential Group. Use that credential’s ID in the operation. Bot tokens cannot perform these user-only actions. The native Sim Slack app keeps its existing permissions.

List and search actions return one page. Use the returned cursor or page information to continue. **Update User Group Members** replaces the complete member list. **Start List Export** returns a job ID; pass it to **Get List Export** with the same export options. Canvas access changes apply to standalone canvases and require the link to have been shared with the recipient first.

Slack’s retired reminders APIs and Enterprise administration APIs are not included. Use scheduled messages or a scheduled Sim workflow for reminders.
...
| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `profile` | json | Yes | JSON object of profile fields, including status_text, status_emoji, status_expiration, and custom fields. Empty status text and emoji clear the status. |
| `user` | string | No | Omit to update the authorized user. Updating others requires elevated Slack permissions. |
...
Set the authenticated identity’s presence to away or automatic.
...
| `sort` | string | No | Sort By Options: score, timestamp. |
| `sort_dir` | string | No | Sort Direction Options: asc, desc. |
| `page` | number | No | Page |
| `count` | number | No | Page Size |
...
| `ok` | boolean | Ok |
| `query` | string | Query |
| `messages` | object | Messagesearch |
|   ↳ `matches` | array | Matches |
|     ↳ `text` | string | Text |
|     ↳ `ts` | string | Ts |
|     ↳ `user` | string | User |
|     ↳ `permalink` | string | Permalink |
|     ↳ `channel` | object | Conversation |
|       ↳ `id` | string | Id |
|       ↳ `name` | string | Name |
|       ↳ `creator` | string | Creator |
|       ↳ `user` | string | User |
...
| `ok` | boolean | Ok |
| `query` | string | Query |
| `fileSearchResults` | object | Filesearch |
|   ↳ `matches` | array | Matches |
|     ↳ `id` | string | Id |
|     ↳ `name` | string | Name |
|     ↳ `title` | string | Title |
...
|     ↳ `per_page` | number | Per page |
|     ↳ `total_count` | number | Total count |
| `posts` | object | Legacy post search results when returned by Slack |
|   ↳ `matches` | array | Legacy post matches with provider-defined fields |
...
Snooze notifications for the authorized Slack user for a number of minutes.

#### Input

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `num_minutes` | number | Yes | Minutes |
...
### Slack End Notification Snooze

End the authorized Slack user’s current notification snooze.
...
### Slack End Do Not Disturb

End the authorized Slack user’s current Do Not Disturb session.
