...
Salesforce also accepts a credential you create yourself. Sim cannot request scopes on one, so its access is fixed when you create it, and it is generally whatever the account that created it can already do.
