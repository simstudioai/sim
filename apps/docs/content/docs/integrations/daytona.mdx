...
| `executionTimeout` | number | No | Timeout in seconds \(defaults to 10 seconds\). Named executionTimeout because the shared tool transport reserves `timeout` for its own request deadline in milliseconds |
