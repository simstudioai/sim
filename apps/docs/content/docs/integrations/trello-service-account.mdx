...
Connect Trello with an API token created for the Sim deployment's API key. The token acts as the Trello member who authorized it and can access that member's boards.
...
- **Sim's hosted cloud** — contact [Sim support](mailto:help@sim.ai) for the authorize link with the deployment's key already filled in. Do not substitute your own key.
...
Add a Trello block to your workflow. In the credential dropdown, select the saved Trello API token. Select it and configure the block as you normally would.
