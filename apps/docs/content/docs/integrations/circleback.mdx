...
|   ↳ `status` | string | The completion status, PENDING or DONE |
|   ↳ `meetings` | array | The meetings the action item is associated with \(id, name, createdAt\) |
| `nextCursor` | string | Pagination cursor for the next page |
...
|   ↳ `attendees` | array | The invited attendees, each with name, email, isOrganizer, and status \(accepted, declined, tentative, or not_available\) |
