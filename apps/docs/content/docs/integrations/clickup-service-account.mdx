...
Connect ClickUp with a personal API token. The workflow acts with the permissions of the user who created the token.
...
Add a ClickUp block to your workflow. In the credential dropdown, select the saved ClickUp API token. Select it and configure the block as you normally would.
