...
Connect Pipedrive with an API token for one user in one company. The token uses that user's data access and remains valid until it is regenerated or the user loses access.
