...
Use the Deployments block to manage other workflows in the current workspace. Deploy creates a new version; Promote Version to Live makes an existing version live without creating another. Undeploy takes a workflow offline and removes its triggers, webhooks, and schedules.

For the deployment UI and execution model, see [Workflow Deployment](/workflows/deployment).
{/* MANUAL-CONTENT-END */}
