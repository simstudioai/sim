...
This block is read-only with one narrow exception: `Cancel Query` stops a running CloudTrail Lake query. It never enables or disables logging, alters trail configuration, or deletes a trail, so it can be granted a `cloudtrail:Describe*/Get*/List*/LookupEvents/StartQuery` policy with no residual write risk. Note that `Start Query` is billed per GB scanned and consumes your account's concurrent-query quota.

`Lookup Events` is limited by AWS to two requests per second per account per Region. The block paces itself against that limit automatically, so broad searches take longer rather than failing.
{/* MANUAL-CONTENT-END */}
...
|   ↳ `name` | string | Trail name |
|   ↳ `homeRegion` | string | Region in which the trail was created |
| `nextToken` | string | Pagination token for the next page of trails |
