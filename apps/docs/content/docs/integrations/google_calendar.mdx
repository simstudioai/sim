...
Use [Google Calendar](https://calendar.google.com) to manage events and calendars, invite attendees, inspect availability, and configure sharing. Calendar event triggers can start workflows when events change.
{/* MANUAL-CONTENT-END */}
...
| `eventId` | string | Yes | Google Calendar event ID to respond to. Use a recurring-event instance ID \(from Get Recurring Instances\) to respond to a single occurrence; the series ID responds to every occurrence. |
