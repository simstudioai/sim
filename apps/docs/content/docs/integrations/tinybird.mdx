...
[Tinybird](https://www.tinybird.co/) ingests data and exposes query results through APIs. Send JSON or NDJSON events, run SQL queries, or call a published Pipe by name with its parameters. The integration also manages data sources and checks asynchronous task status.
{/* MANUAL-CONTENT-END */}
