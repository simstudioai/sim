...
| `folderPath` | string | No | Existing destination folder as a canonical percent-encoded path, e.g. "/Generated/Archives". Defaults to the workspace root. Create the folder first with file_create_folder. |
| `onConflict` | string | No | Name collision behavior: "rename" \(default\) chooses an available name; "error" refuses an existing name. Existing files are never overwritten. |
...
| `path` | string | Final workspace filesystem path, including the stored name |
