...
Connect a Shopify store with a **non-expiring offline Admin API access token** and its `.myshopify.com` domain. Sim stores the pasted token; this connection does not renew expiring tokens.
...
- **New Dev Dashboard apps** do **not** display a permanent `shpat_` token in any UI. Complete an [authorization code grant](https://shopify.dev/docs/apps/build/authentication-authorization/authenticate-standalone-apps) install and obtain a **non-expiring offline** access token. Check the token response and access mode: the `shpat_` prefix is also used by expiring tokens. The Dev Dashboard's client-credentials tokens are different — they expire after 24 hours and cannot be stored in Sim.
...
## Adding the Admin API Token to Sim
...
Add a Shopify block to your workflow. In the credential dropdown, select the saved Shopify Admin API token. Select it and configure the block as you normally would — the store domain stored with the credential is used automatically.
...
<FAQ items={[
  { question: "Does the token expire?", answer: "This connection requires a non-expiring offline Admin API token. Online tokens, expiring offline tokens, and client-credentials tokens have expiration rules even when they use a similar prefix. Sim does not refresh tokens pasted into this credential." },
