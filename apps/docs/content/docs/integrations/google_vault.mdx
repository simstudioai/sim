...
Connecting Google Vault requests these scopes.
...
| `https://www.googleapis.com/auth/ediscovery.readonly` | View Google Vault matters, holds, and saved queries |
| `https://www.googleapis.com/auth/devstorage.read_only` | Read files from Google Cloud Storage |

Google Vault also accepts a credential you create yourself. Sim cannot request scopes on one, so its access is whatever the credential carries: some providers let you select these permissions when you create the token, others give it the creating user's access.
