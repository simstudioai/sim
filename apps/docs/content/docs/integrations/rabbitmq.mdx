...
Use [RabbitMQ](https://www.rabbitmq.com/)'s Management HTTP API to publish and inspect messages, manage queue topology, and check broker health. Messages are published to exchanges and routed through bindings into queues.

The default acknowledgement mode requeues messages after reading them, so an inspection workflow can leave messages available to other consumers.
