...
Use [Wiza](https://wiza.co/) to search prospects, enrich companies, reveal contact details, and check credits. Individual Reveal starts enrichment and polls for completion within one block operation; a separate polling block is not required.
{/* MANUAL-CONTENT-END */}
...
Select **Individual Reveal** and provide a LinkedIn URL, a name plus company/domain, or an email. The operation starts the reveal and polls until it resolves, returning the contact data with `status` and `is_complete`. You do not need separate start and get operations or a callback URL.
