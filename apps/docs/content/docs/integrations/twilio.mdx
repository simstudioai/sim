...
Use Twilio triggers in Sim to start workflows from messaging events. **Message Status** reports status changes for a sent message, including its SID and any error code. **SMS Received** provides the sender, recipient, body, and media from an inbound SMS or MMS.
{/* MANUAL-CONTENT-END */}
