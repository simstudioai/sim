...
Be precise about what that protects. The value you supply to `Put Parameter` is masked in the block, is never echoed back in the result, and no parameter value ever appears in an error message. A value you read is different: once you enable decryption, the plaintext is ordinary block output. It flows to downstream blocks as intended, and it is also written to the run log and the execution trace like any other output. Enable decryption only on the steps that genuinely need the plaintext, and treat those run logs as secret material.
{/* MANUAL-CONTENT-END */}
