...
Parameter Store decryption is opt-in: `Get Parameter` and `Get Parameters` leave `WithDecryption` off unless you explicitly enable it, so a SecureString stays encrypted by default. Written and decrypted values are masked in the block and are kept out of logs, traces, and error envelopes.
{/* MANUAL-CONTENT-END */}
...
| `instanceIds` | json | No | Managed node IDs to target, as an array of strings \(e.g., \["i-0abc123"\]\). Provide instanceIds or targets |
...
| `instanceId` | string | Yes | Managed node the command ran on \(e.g., i-0abc123\) |
...
| `instanceIds` | json | No | Managed node IDs to cancel on, as an array of strings; omit to cancel on every targeted node |
...
| `accessKeyId` | string | Yes | AWS access key ID |
| `secretAccessKey` | string | Yes | AWS secret access key |
| `instanceId` | string | Yes | Managed node to report patches for \(e.g., i-0abc123\) |
