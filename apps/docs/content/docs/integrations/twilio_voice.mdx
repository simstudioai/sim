...
| `ringTimeout` | number | No | Time to wait for answer before giving up \(seconds, default: 60\). Named ringTimeout because the shared tool transport reserves `timeout` for its own request deadline in milliseconds |
