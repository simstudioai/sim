...
Follow the [Oracle NetSuite service-account setup](/integrations/netsuite-service-account) to enable SuiteTalk, configure a dedicated role and certificate mapping, and save a reusable credential in Sim. NetSuite limits certificate validity to two years, so plan renewal when creating the credential.
