...
3. Click **Add to Sim** and choose **Connect with OAuth**. For a private app token, follow the [Private App Tokens guide](/integrations/hubspot-service-account).
...
You can start from a template on the HubSpot page or configure the block directly. Skills such as *upsert-contact* provide instructions an agent can load for a task.
...
2. In **HubSpot Account**, select the connection you created.
...
Triggers run against your workflow's active [deployment](/workflows/deployment). Deploy it, allow the first poll to establish a baseline, then create or update a record to test the trigger.
