...
Register a client in the [Zoho API console](https://api-console.zoho.com/) and configure it as Sim's Zoho OAuth client. The same client serves Zoho Desk and ServiceDesk Plus — Zoho scopes are chosen per authorization request, not per registered client, so no second app is needed.
...
|   ↳ `display_id` | json | Problem number shown in the SDP UI |
...
|   ↳ `id` | string | Change ID |
|   ↳ `display_id` | json | Change number shown in the SDP UI |
...
|   ↳ `display_id` | json | Solution number shown in the SDP UI |
