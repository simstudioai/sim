...
Integra Telegram en el flujo de trabajo. Puede enviar y eliminar mensajes. Se puede usar en modo disparador para iniciar un flujo de trabajo cuando se envía un mensaje a un chat.
...
Elimina mensajes en canales o chats de Telegram a través de la API de Bot de Telegram. Requiere el ID del mensaje que se desea eliminar.
...
| `caption` | string | No | Pie de foto del video \(opcional\) |
...
| `caption` | string | No | Pie de foto del audio \(opcional\) |
...
| `botToken` | string | Sí | Tu token de API de Bot de Telegram |
| `chatId` | string | Sí | ID del chat de Telegram objetivo |
| `animation` | string | Sí | Animación a enviar. Pasa un file_id o URL HTTP |
| `caption` | string | No | Pie de la animación \(opcional\) |
