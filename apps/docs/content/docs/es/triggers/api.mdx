...
Las claves de salida en tiempo de ejecución reflejan el esquema y están disponibles bajo `<api.input>`.

Las ejecuciones manuales en el editor utilizan la columna `value` para que puedas realizar pruebas sin enviar una solicitud. Durante la ejecución, el resolvedor completa tanto `<api.fieldName>` como `<api.input.fieldName>`.
...
```json
{
  "type": "block",
  "blockType": "text",
  "text": "Este es el primer bloque de texto."
}

{
  "type": "block",
  "blockType": "text",
  "text": "Este es el segundo bloque de texto."
}
```
...
| `<api.input>` | Cuerpo completo de la solicitud estructurada |
...
```json
{
  "files": [{
    "name": "example.pdf",
    "data": "JVBERi0xLjMKJcTl8uXrp/Og0MTGCjQgMCBvYmoKPDwgL0xlbmd0aCA1IDAgUiAvRmls..."
  }]
}
```
