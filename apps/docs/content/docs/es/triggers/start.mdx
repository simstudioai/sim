...
<Tabs items={['Ejecución en editor', 'Desplegar a API', 'Desplegar a chat']}>
  <Tab>
    Cuando haces clic en <strong>Ejecutar</strong> en el editor, el bloque de inicio muestra el formato de entrada como un formulario. Los valores predeterminados facilitan volver a probar sin tener que volver a escribir los datos. Al enviar el formulario se activa el flujo de trabajo inmediatamente y los valores quedan disponibles en <code>&lt;start.fieldName&gt;</code> (por ejemplo <code>&lt;start.sampleField&gt;</code>).

    Los campos de archivo en el formulario se cargan directamente en el correspondiente{' '}
    <code>&lt;start.fieldName&gt;</code>; usa esos valores para alimentar
    herramientas posteriores o pasos de almacenamiento.
  </Tab>
  <Tab>
    Desplegar a API convierte el formato de entrada en un contrato JSON para los clientes. Cada campo se convierte en parte del cuerpo de la solicitud, y Sim convierte los tipos primitivos durante la ingesta. Los campos de archivo esperan objetos que hagan referencia a archivos cargados; usa el punto de conexión de carga de archivos de ejecución antes de invocar el flujo de trabajo.

    Los que llaman a la API pueden incluir propiedades opcionales adicionales. Se conservan
    dentro de las salidas <code>&lt;start.fieldName&gt;</code> para que puedas experimentar
    sin tener que redesplegar inmediatamente.
  </Tab>
  <Tab>
    En los despliegues de chat, el bloque de inicio se vincula a la conversación activa. El último mensaje llena <code>&lt;start.input&gt;</code>, el identificador de sesión está disponible en <code>&lt;start.conversationId&gt;</code>, y los archivos adjuntos del usuario aparecen en <code>&lt;start.files&gt;</code>, junto con cualquier campo de formato de entrada definido como <code>&lt;start.fieldName&gt;</code>.

    Si inicias el chat con contexto estructurado adicional (por ejemplo, desde una incrustación), se fusiona con las salidas <code>&lt;start.fieldName&gt;</code> correspondientes, manteniendo los bloques posteriores consistentes con la API y las ejecuciones manuales.
  </Tab>
</Tabs>

## Referenciando datos de inicio en etapas posteriores

- Conecta <code>&lt;start.fieldName&gt;</code> directamente a agentes, herramientas o funciones que esperan cargas estructuradas.
- Usa la sintaxis de plantillas como <code>&lt;start.sampleField&gt;</code> o <code>&lt;start.files[0].url&gt;</code> (solo en chat) en campos de prompt.
- Mantén <code>&lt;start.conversationId&gt;</code> a mano cuando necesites agrupar salidas, actualizar el historial de conversación o llamar de vuelta a la API de chat.
