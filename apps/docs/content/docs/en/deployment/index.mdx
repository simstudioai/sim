...
A deployment is a published version of a [workflow](/workflows) that outside callers can run. While you build, a workflow lives on your canvas as a draft only you can run. Deploying publishes a fixed copy of that draft and gives it an address: a REST endpoint, a chat page, a set of [MCP](/deployment/mcp) tools, or an [A2A](/deployment/api) agent. Each is a **surface**, a channel callers reach the same published workflow through.
...
Send an object in the request body matching the workflow's [Input Format](/triggers/start). The response is an object holding the [block outputs](/workflows#blocks) and execution metadata.
...
<Cards>
  <Card title="API" href="/deployment/api" description="A REST endpoint any application can call, with sync, streaming, and async modes." />
  <Card title="Chat" href="/deployment/chat" description="A shareable conversational page that keeps conversation history." />
  <Card title="MCP" href="/deployment/mcp" description="Expose the workflow as tools an external AI assistant can call." />
  <Card title="A2A" href="/deployment/api" description="Let other AI agents discover and call the workflow as a tool." />
</Cards>
...
<Cards>
  <Card title="Deploy as API" href="/deployment/api" description="Endpoint, auth, execution modes, and version control." />
  <Card title="Deploy as Chat" href="/deployment/chat" description="A conversational interface for your workflow." />
  <Card title="Deploy as MCP" href="/deployment/mcp" description="Make your workflow a tool for external AI assistants." />
  <Card title="Triggers" href="/triggers/start" description="Define the input a deployed workflow expects." />
