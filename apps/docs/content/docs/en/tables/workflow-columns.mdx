...
- **Company Domain** and **Company Info** are **enrichments**: built-in lookups that find each company's `domain`, then read it to fill `employee_count` and `description`.
- **Lead Score Enrichment** is a **workflow group**: it runs your own workflow against each row and writes `lead_score`, `priority`, and `score_reasoning`.
...
An **enrichment** is a built-in lookup Sim provides: find a company's domain from its name, look up company info from a domain, verify an email, find a phone number, or find a work email. Pick one from the **Enrichments** panel, bind its inputs to columns, and name the columns its outputs should fill — there's no workflow to build. Under the hood each enrichment tries a cascade of data providers in order, which is why a cell can come back **Not found**: every provider ran and none had the answer.

In the example, Company Info is an enrichment. Its required `Company domain` input is bound to the `domain` column, and its `employee count` and `description` outputs write to their columns:

<Image
  src="/static/tables/workflow-columns/column-mapping.png"
  alt="The Company Info enrichment's configuration: the required Company domain input bound to the domain column, and the employee count and description outputs bound to their column names"
  width={380}
  height={499}
/>
...
- **Add column inputs** maps columns to the workflow's inputs, which arrive at its [Start](/triggers/start) trigger.
