...
| `--description <value>` | No | New credential description. Send null to clear the stored one. (--no-description clears it). |
| `--no-description` | No | Send --description as null to clear it. |
...
```bash
sim files uploads get <uploadId> [options]
```
...
| Option | Required | Description |
| --- | --- | --- |
| `--upload-token <value>` | Yes | Signed upload control token returned when the upload session was created. |
...
| `--no-enabled` | No | Send --enabled as false. |
| `--oauth-client-id <value>` | No | Pre-registered OAuth client identifier. Changing it on update revokes the stored OAuth grant and forces reauthorization. (--no-oauth-client-id clears it). |
| `--no-oauth-client-id` | No | Send --oauth-client-id as null to clear it. |
| `--oauth-client-secret <value>` | No | Write-only pre-registered OAuth client secret. Sending it on update as null or a new value revokes the stored OAuth grant and forces reauthorization, as does switching away from OAuth authentication. (--no-oauth-client-secret clears it). |
| `--no-oauth-client-secret` | No | Send --oauth-client-secret as null to clear it. |
...
| `--max-rows <json\|@file>` | No | Cap on eligible rows to run, as an object rather than a count: &#123;"type":"rows","max":100&#125; (JSON, or @path / @- to read a file or stdin). |
...
| Option | Required | Description |
| --- | --- | --- |
| `--upload-token <value>` | No | Signed upload control token returned when an upload-backed import was created. |
...
```bash
sim tables imports get <importId> [options]
```
...
| `--description <value>` | No | Replacement table description, or null to clear it. (--no-description clears it). |
| `--no-description` | No | Send --description as null to clear it. |
| `--folder <value>` | No | Folder path as shown in the app; the leading / is optional. |
...
| `--description <value>` | No | New server description, or null to clear it. (--no-description clears it). |
| `--no-description` | No | Send --description as null to clear it. |
...
| `--description <value>` | No | Optional workflow description. (--no-description clears it). |
| `--no-description` | No | Send --description as null to clear it. |
| `--folder <value>` | No | Folder path as shown in the app; the leading / is optional. |
...
| `--description <value>` | No | Optional release note for the deployment version. (--no-description clears it). |
| `--no-description` | No | Send --description as null to clear it. |
...
| `--run-id <value>` | No | Claim this run identifier, so a retried invocation cannot start a second run; reusing a value already claimed fails with RUN_ID_CONFLICT rather than replaying the first result. |
...
| `--description <value>` | No | New release note for the deployment version, or null to clear it. (--no-description clears it). |
| `--no-description` | No | Send --description as null to clear it. |
...
| `--description <value>` | No | Replacement workflow description; null clears it. (--no-description clears it). |
| `--no-description` | No | Send --description as null to clear it. |
| `--folder <value>` | No | Folder path as shown in the app; the leading / is optional. |
