...
### sim files permanent delete

Permanently Delete File

```bash
sim files permanent delete <fileId>
```
...
```bash
sim files restore create [options]
```
...
| Option | Required | Description |
| --- | --- | --- |
| `--path <value>` | Yes | Path of the archived folder to restore, as reported by `GET /api/v2/files/folders?scope=archived`. |
