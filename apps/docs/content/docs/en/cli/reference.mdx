...
| Argument | Required | Description |
| --- | --- | --- |
| `blockId` | Yes | Block type identifier. |
...
### sim chat-deployments delete

Delete Chat Deployment

```bash
sim chat-deployments delete <chatDeploymentId>
```
...
| Option | Required | Description |
| --- | --- | --- |
| `--workflow-ids <value>` | No | Comma-separated workflow identifiers to include. An empty entry is rejected. |
| `--folder-paths <value>` | No | Comma-separated workflow folder paths to include. A path covers its whole subtree. A path that names no folder narrows the result to nothing, so the response is an empty page rather than an error. |
| `--triggers <value>` | No | Comma-separated trigger types to include. An empty entry is rejected. The vocabulary is open, so an unrecognized member selects no runs; the literal `all` disables this filter. |
| `--level <value>` | No | Severity level to include. Accepted values: `info`, `error`. |
| `--start-date <value>` | No | Only include runs started at or after this UTC ISO 8601 timestamp, e.g. `2026-08-06T00:00:00Z`. A date without a time, or a timestamp carrying a UTC offset instead of `Z`, is rejected, as is year `0000`, which names no storable instant. |
| `--end-date <value>` | No | Only include runs started at or before this UTC ISO 8601 timestamp, e.g. `2026-08-06T00:00:00Z`. A date without a time, or a timestamp carrying a UTC offset instead of `Z`, is rejected, as is year `0000`, which names no storable instant. |
| `--segment-count <value>` | No | Number of equal time buckets to divide the window into, from 1 to 500. Buckets are never narrower than one minute, so a short window yields fewer distinct instants than requested. |
...
| Option | Required | Description |
| --- | --- | --- |
| `--workflow-ids <json\|@file>` | No | Workflow identifiers to include. (JSON, or @path / @- to read a file or stdin). |
| `--folder-paths <json\|@file>` | No | Workflow folder paths to include. A path covers its whole subtree, so `/prod` also selects runs in `/prod/nested`. A path that names no folder narrows the result to nothing, so the response is an empty page rather than an error. (JSON, or @path / @- to read a file or stdin). |
| `--triggers <json\|@file>` | No | Trigger types to include. Matched exactly and case-sensitively against an open vocabulary, so an unrecognized member selects no runs rather than failing. The literal `all` is a sentinel that disables this filter. (JSON, or @path / @- to read a file or stdin). |
| `--level <value>` | No | Severity level to include. Accepted values: `info`, `error`. |
| `--status <json\|@file>` | No | Execution statuses to include, matched against the reported `status` field. (JSON, or @path / @- to read a file or stdin). |
| `--workflow-name <value>` | No | Case-insensitive substring match against the run's workflow name. Runs whose workflow has been deleted match nothing, because the name is no longer joinable. |
| `--run-id <value>` | No | Exact run identifier to match. |
| `--start-date <value>` | No | Only include runs started at or after this UTC ISO 8601 timestamp, e.g. `2026-08-06T00:00:00Z`. A date without a time, or a timestamp carrying a UTC offset instead of `Z`, is rejected, as is year `0000`, which names no storable instant. |
| `--end-date <value>` | No | Only include runs started at or before this UTC ISO 8601 timestamp, e.g. `2026-08-06T00:00:00Z`. A date without a time, or a timestamp carrying a UTC offset instead of `Z`, is rejected, as is year `0000`, which names no storable instant. |
...
| `--max-cost <value>` | No | Maximum execution cost in USD, from 0 to 1000000. A run is never charged a negative amount, so a negative bound is rejected rather than treated as a filter that matches every run. |
| `--model <value>` | No | AI model used during execution. |
| `--sort-by <value>` | No | Column to order by. `durationMs` and `cost` are null until a run settles; those runs order as though the value were below every recorded one, so they trail an ascending page and lead a descending one. Accepted values: `startedAt`, `durationMs`, `cost`, `status`. |
| `--sort-order <value>` | No | Sort direction. Accepted values: `asc`, `desc`. |
| `--limit <n>` | No | Maximum items to return (0 for everything). Defaults to `100`. |
...
### sim tables batch-update create

Batch Update Rows

```bash
sim tables batch-update create <tableId> [options]
```
...
### sim tables bulk-delete create

Bulk Delete Tables and Folders

```bash
sim tables bulk-delete create [options]
```
...
| `--folder-paths <json\|@file>` | No | Table folders to delete, by canonical path. Each cascades to everything inside it. (JSON, or @path / @- to read a file or stdin). |

</CommandTable>

### sim tables bulk-move create

Bulk Move Tables and Folders

```bash
sim tables bulk-move create [options]
```
...
| `--folder-paths <json\|@file>` | No | Table folders to re-parent, by canonical path. (JSON, or @path / @- to read a file or stdin). |
| `--target-folder-path <json\|@file>` | Yes | Destination folder path. `null` and `/` both mean the workspace root. (JSON, or @path / @- to read a file or stdin). |
...
| --- | --- | --- |
| `--limit <n>` | No | Maximum items to return (0 for everything). Defaults to `100`. |
| `--include-run-state` | No | Include per-workflow-group run state on every returned row. Off by default: run state is a separate sidecar read and its `blockErrors` are unbounded, so a full page carries it only when asked. |
| `--no-include-run-state` | No | Send --include-run-state as false. |
...
```bash
sim tables dispatches get <dispatchId>
```
...
### sim tables restore create

Restore Table

```bash
sim tables restore create <tableId>
```
...
| `--workflow-ids <json\|@file>` | No | Deployed workflows to publish as tools on the new server. (JSON, or @path / @- to read a file or stdin). |
...
```bash
sim workflow-mcp-servers delete <serverId>
```
...
| `--parameter-descriptions <json\|@file>` | No | Per-field description overrides applied to the schema generated from the deployed workflow inputs. A name matching no input field is ignored. (JSON, or @path / @- to read a file or stdin). |
...
```bash
sim workflow-mcp-servers tools delete <serverId> <workflowId>
```
...
| `--description <value>` | No | New server description, or null to clear it. |
...
### sim workflows operations create
...
| `--folder-path <value>` | No | Destination folder path. Defaults to the source workflow's folder. |
...
| Option | Required | Description |
| --- | --- | --- |
| `--workflow-ids <json\|@file>` | Yes | Workflows to move. Duplicates are collapsed. (JSON, or @path / @- to read a file or stdin). |
| `--folder-path <value>` | Yes | Destination folder path; `/` moves the workflows to the workspace root. |

</CommandTable>

### sim workflows restore create
