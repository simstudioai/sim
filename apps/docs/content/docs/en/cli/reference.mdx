...
### sim files extract create

Extract File Archive

```bash
sim files extract create <fileId>
```
...
### sim files uploads get

Get File Upload

```bash
sim files uploads get <uploadId>
```
...
| Argument | Required | Description |
| --- | --- | --- |
| `uploadId` | Yes | Upload session identifier. |
...
Permanently delete a file and its stored bytes

```bash
sim files purge <fileId> [options]
```
...
```bash
sim files text list <fileId> [options]
```
...
### sim knowledge documents tags cleanup

Remove tag definitions no document still uses

```bash
sim knowledge documents tags cleanup <knowledgeBaseId> <documentId> [options]
```
...
| Option | Required | Description |
| --- | --- | --- |
| `--action <value>` | No | Remove tag definitions no document in this knowledge base still uses. |
| `-y, --yes` | Yes | Confirm this destructive operation. |
...
### sim knowledge documents tags save

Declare the tag definitions a document’s tags need

```bash
sim knowledge documents tags save <knowledgeBaseId> <documentId> [options]
```
...
| `--field-type <value>` | No | Value type stored in the slot; it decides which slots are usable and which filter operators apply. Slot capacity per type: text 7, number 5, date 2, boolean 3. Accepted values: `text`, `number`, `date`, `boolean`. |
...
### sim knowledge archived list

List Archived Knowledge Bases

```bash
sim knowledge archived list [options]
```
...
### sim knowledge restore create

Restore Knowledge Base

```bash
sim knowledge restore create <id>
```
...
| Option | Required | Description |
| --- | --- | --- |
| `--workflow <value...>` | No | Workflow identifiers to include. (space-separated, or @path / @- with one value per line). |
| `--folder <value...>` | No | Folder path as shown in the app; the leading / is optional (space-separated, or @path / @- with one value per line). |
| `--trigger <value...>` | No | Trigger types to include. Matched exactly and case-sensitively against an open vocabulary, so an unrecognized member selects no runs rather than failing. The literal `all` is a sentinel that disables this filter. (space-separated, or @path / @- with one value per line). |
| `--level <value>` | No | Severity level to include. Accepted values: `info`, `error`. |
...
| `--folder <value...>` | No | Folder path as shown in the app; the leading / is optional (space-separated, or @path / @- with one value per line). |
| `--to <json\|@file>` | Yes | Destination folder path; omit for root (JSON, or @path / @- to read a file or stdin). |
