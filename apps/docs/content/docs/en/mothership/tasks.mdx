...
The **Superagent** handles requests that should happen right now — without building a workflow. Use it when you want to act immediately rather than automate for later.
...
The Superagent uses the credentials already connected to your workspace. If an integration isn't connected, Mothership walks you through connecting it.
...
Mothership can set environment variables for your workspace — values that workflows reference via `{{ENV_VAR}}` syntax:
...
Mothership can add and manage MCP servers connected to your workspace, making their tools available inside any workflow:
...
Mothership can create custom HTTP tools that call any external API:
