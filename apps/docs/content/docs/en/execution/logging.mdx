...
- Add the Sim Trigger block to a workflow to react to execution errors, successes, deployments, and alert conditions in real time
...
<FAQ items={[
  ...
  { question: "Can I access logs programmatically?", answer: "Yes. The External API provides endpoints to query logs with filtering by workflow, time range, trigger type, duration, cost, and model. To react to runs in real time, use the Sim Trigger block to run a workflow on execution events." },
