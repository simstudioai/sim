...
Controls which AI model providers members of this group can use. The list shows all providers available in Sim.
...
Controls which workflow blocks members can place and execute. Blocks are split into two sections: **Core Blocks** (Agent, API, Condition, Function, etc.) and **Tools** (all integration blocks).
...
Controls visibility of platform features and modules. Each checkbox maps to a specific feature; checking it hides or disables that feature for group members.
