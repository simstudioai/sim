...
<Steps>
  <Step title="Create a workflow and add an Agent">
    ...
    - **User**: insert `<start.input>` — a reference to the Start block's input, in the `<blockName.field>` syntax used everywhere in Sim — so the agent reads whatever the chat receives.
