...
<FAQ items={[
  ...
  { question: "I generated a token from my own Power-Up's key and validation fails. What now?", answer: "Discard that token and mint a new one using the authorize URL with the Sim deployment's key — the in-product link on hosted cloud, or your own TRELLO_API_KEY value if self-hosting. Tokens are free to create. (If your account is managed by a Trello Enterprise, admins can restrict token creation — ask your Enterprise admin if the authorize page won't issue a token.)" },
