...
<Steps>
  <Step>
    Log in as the service user, open Linear **Settings**, and go to **Security & access** → **API keys**
...
<Steps>
  ...
  <Step>
    Search for "Linear Service Account" and click it
  ...
  <Step>
    ...
    {/* TODO(screenshot): Add Linear Service Account dialog with the API key filled in */}
  </Step>
  <Step>
    Click **Add Service Account**. Sim verifies the key by querying Linear's `viewer` — if it fails, you'll see a specific error explaining what went wrong.
  </Step>
</Steps>
