...
<Steps>
  <Step>
    Open **Workspace settings** in Attio and go to **Developers** → **API keys**

    {/* TODO(screenshot): Attio workspace settings with Developers → API keys highlighted */}
  </Step>
  <Step>
...
<Steps>
  ...
  <Step>
    Search for "Attio Service Account" and click it
  ...
  <Step>
    ...
    {/* TODO(screenshot): Add Attio Service Account dialog with the API key filled in */}
  </Step>
  <Step>
    Click **Add Service Account**. Sim verifies the key by calling Attio's `/v2/self` endpoint — if it fails, you'll see a specific error explaining what went wrong.
  </Step>
</Steps>
