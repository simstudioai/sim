...
| `port` | number | No | Server port \(default: 1433\). Ignored when a named instance is used |
| `database` | string | Yes | Database name to connect to |
| `username` | string | Yes | Database username |
| `password` | string | Yes | Database password |
| `encrypt` | string | No | Encrypt the connection with TLS \(enabled, disabled\). Defaults to enabled |
| `trustServerCertificate` | string | No | Trust a self-signed server certificate \(enabled, disabled\). Defaults to disabled |
| `instanceName` | string | No | Named instance to connect to. Requires the SQL Server Browser service |
