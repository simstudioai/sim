...
Publish a video to TikTok by uploading a file from the workflow (uploaded file or a file from a previous block). Rate limit: 6 requests per minute per user.

#### Input

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `file` | file | Yes | MP4, MOV, or WebM video file to upload from the workflow \(maximum 250 MB\). |
...
| `disableDuet` | boolean | Yes | Whether to disable Duet for this video. Must be chosen explicitly. |
| `disableStitch` | boolean | Yes | Whether to disable Stitch for this video. Must be chosen explicitly. |
| `disableComment` | boolean | Yes | Whether to disable comments for this video. Must be chosen explicitly. |
...
| `brandContentToggle` | boolean | Yes | Set to true if the video is a paid partnership promoting a third-party business; otherwise false. Branded content cannot be posted with Only Me privacy. |
...
Send a video file from the workflow to the authenticated user's TikTok inbox for manual editing and posting. The user must open TikTok and tap the inbox notification to complete the post. Rate limit: 6 requests per minute per user.
...
| `uploadedBytes` | number | Number of video bytes TikTok has received for a FILE_UPLOAD post. |
| `downloadedBytes` | number | Number of video bytes TikTok has downloaded for a PULL_FROM_URL post. |
