...
| `total_count` | number | Number of rulesets returned |
...
|   ↳ `overrides` | json | Overrides applied to the managed ruleset, with optional ruleset-level, categories, and rules entries. Cloudflare documents action and enabled as the overridable properties |
