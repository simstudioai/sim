...
| `created` | string | Enrollment timestamp |
| `lastUpdated` | string | Last update timestamp |
| `profile` | json | Factor-specific attributes, which vary by factor type |
| `success` | boolean | Operation success status |
...
| `filter` | string | No | Okta filter expression \(e.g., status eq "ACTIVE"\) |
| `includeNonDeleted` | boolean | No | Include applications that are not deleted \(default: false\) |
| `after` | string | No | Opaque pagination cursor returned as nextCursor by a previous call |
...
| `created` | string | Creation timestamp |
| `lastUpdated` | string | Last update timestamp |
| `accessibility` | json | Accessibility settings, whose fields vary by application |
| `visibility` | json | Visibility settings, whose fields vary by application |
| `settings` | json | Application settings, whose shape varies by sign-on mode |
| `profile` | json | Application profile attributes, whose shape varies by application |
| `success` | boolean | Operation success status |
...
|   ↳ `debugData` | json | Free-form debug context published with the event |
