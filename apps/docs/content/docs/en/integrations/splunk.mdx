...
| --------- | ---- | ----------- |
| `results` | json | Search result rows |
...
| `messages` | json | Messages returned with the response |
...
| `savedSearches` | json | List of saved searches |
...
| `alerts` | json | Saved searches with currently triggered alerts |
| `firedAlerts` | json | Triggered instances of an alert |
| `indexes` | json | Indexes configured on the instance |
...
| `latestTime` | string | No | Latest \(exclusive\) time bound — relative \(e.g. now\) or absolute time |
| `execMode` | string | No | Execution mode: normal \(returns the sid immediately\) or blocking \(returns the sid once the job completes\). Defaults to normal. |
| `adhocSearchLevel` | string | No | Search mode: verbose, fast, or smart. Defaults to fast. |
...
| `count` | number | No | Maximum number of result rows to return \(e.g. 100\). 0 returns all rows. |
...
| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `search` | string | No | Filter expression matched against saved search field values \(e.g. name=Errors*, is_scheduled=1\) |
...
| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `name` | string | Yes | Name of the alerting saved search \(e.g. Errors in the last 24 hours\) |
| `count` | number | No | Maximum number of fired alert instances to return \(e.g. 50\). 0 returns all. |
| `offset` | number | No | Index of the first instance to return, for pagination |
...
| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `datatype` | string | No | Filter indexes by type: all, event, or metric |
