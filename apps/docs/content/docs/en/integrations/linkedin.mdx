...
| `visibility` | string | No | Who can see this post: "PUBLIC" \(anyone on LinkedIn\), "CONNECTIONS" \(1st-degree connections only\), or "LOGGED_IN" \(signed-in LinkedIn members only\). Default: "PUBLIC" |
...
|   ↳ `email` | string | Primary email address. Requires the `email` scope, and LinkedIn documents the claim as optional — it may be absent even when the scope was granted |
