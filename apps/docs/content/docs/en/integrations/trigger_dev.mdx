...
| `isSecret` | boolean | Whether the environment variable is a secret, meaning its value is redacted \(Get Env Var\) |
...
|   ↳ `value` | string | Value of the environment variable. Secret variables come back redacted, not as the real value; non-secret values are plaintext and appear in workflow outputs and run history |
|   ↳ `isSecret` | boolean | Whether the variable is a secret. A secret variable's value comes back redacted, so branch on this before treating the value as real |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | Yes | Trigger.dev secret API key \(starts with tr_\) |
| `tokenTimeout` | string | No | How long before the token times out, as a duration \("30s", "1m", "2h", "3d"\) or an ISO 8601 date |
