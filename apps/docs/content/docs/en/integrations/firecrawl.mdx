...
| `country` | string | No | Two-letter country code to search from. Firecrawl falls back to "us" only when `location` is also unset; setting `location` leaves the country unset instead. |
...
| `links` | array | Discovered links. Each entry is an object with `url` and optional `title`/`description` — read `&lt;firecrawl.links&gt;\[i\].url` for the address, not the entry itself. |
|   ↳ `url` | string | The discovered URL |
|   ↳ `title` | string | Page title, when Firecrawl resolved one |
|   ↳ `description` | string | Page description, when Firecrawl resolved one |
