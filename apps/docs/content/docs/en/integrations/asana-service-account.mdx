...
<Steps>
  ...
  <Step>
    Copy the token when it's shown. Asana only displays it once — if you lose it, you'll have to regenerate it, which immediately invalidates the old token.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step>
    Click **Create new token**, give it a description, and agree to the API terms
  </Step>
  <Step>
...
<Steps>
  ...
  <Step>
    Search for "Asana Service Account" and click it
  ...
  <Step>
    ...
    {/* TODO(screenshot): Add Asana Service Account dialog with the access token filled in */}
  </Step>
  <Step>
    Click **Add Service Account**. Sim verifies the token by calling Asana's `/users/me` endpoint — if it fails, you'll see a specific error explaining what went wrong.
  </Step>
</Steps>
...
<FAQ items={[
  ...
  { question: "How do I rotate the token?", answer: "Regenerating a service account token in the Admin Console rotates it immediately — the old token stops working the moment the new one is issued. Regenerate, then update the credential in Sim with the new token right away." },
