...
- **Three retrieval depths**: Mako returns the fact-carrying sentences of each source and is fast; Krill returns a smaller set of them for free under fair use, one request at a time and at low priority; Moby returns the full readable content of every page when whole-page context matters.
- **Optional written answer**: the API can also return an answer built from the sources, concise on Mako and cited on Moby. Krill returns sources only.
