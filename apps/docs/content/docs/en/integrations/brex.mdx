...
| `purchasedAtEnd` | string | No | Only include expenses purchased before this ISO 8601 timestamp |
| `cursor` | string | No | Pagination cursor from a previous response |
| `limit` | string | No | Number of expenses to return \(default 100, max 1000\) |
...
|   ↳ `category` | string | Merchant category of the expense |
...
|   ↳ `status` | string | User status \(e.g., INVITED, ACTIVE, CLOSED, DISABLED\) |
...
|   ↳ `period_recurrence_type` | string | Budget period recurrence \(e.g., MONTHLY, QUARTERLY, YEARLY, ONE_TIME\) |
...
|   ↳ `period_recurrence_type` | string | Period recurrence \(e.g., MONTHLY, QUARTERLY, YEARLY, ONE_TIME\) |
...
|   ↳ `current_period_balance` | json | Balance for the current period |
|     ↳ `amount` | number | Amount in the smallest unit of the currency \(e.g., cents for USD\) |
|     ↳ `currency` | string | ISO 4217 currency code \(e.g., USD\) |
| `nextCursor` | string | Cursor for fetching the next page of results |
...
| `currentPeriodBalance` | json | Balance for the current period |
|   ↳ `amount` | number | Amount in the smallest unit of the currency \(e.g., cents for USD\) |
|   ↳ `currency` | string | ISO 4217 currency code \(e.g., USD\) |
...
|   ↳ `payment_type` | string | Payment type \(e.g., ACH, DOMESTIC_WIRE, CHEQUE, INTERNATIONAL_WIRE\) |
...
|   ↳ `status` | string | Transfer status \(e.g., SCHEDULED, PROCESSING, COMPLETED, FAILED\) |
...
| `counterparty` | json | Transfer counterparty details |
| `description` | string | Transfer description |
| `paymentType` | string | Payment type \(ACH, DOMESTIC_WIRE, CHEQUE, INTERNATIONAL_WIRE, BOOK_TRANSFER\) |
| `amount` | json | Transfer amount |
|   ↳ `amount` | number | Amount in the smallest unit of the currency \(e.g., cents for USD\) |
|   ↳ `currency` | string | ISO 4217 currency code \(e.g., USD\) |
