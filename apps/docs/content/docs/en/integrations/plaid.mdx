...
- Get Auth returns full account and routing identifiers. Use them only with an approved non-Plaid-partner payment processor. Sim hides the `numbers` field from execution-log display; never write it to tables, files, messages, or other durable outputs.
