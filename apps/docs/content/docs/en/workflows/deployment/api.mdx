...
For long-running workflows, async mode returns an execution ID immediately so you don't need to hold the connection open. Add the `X-Execution-Mode: async` header to your request. The API returns HTTP 202 with an execution ID and status URL. Poll the execution resource until the run completes.
...
```json
{
  ...
  "statusUrl": "https://sim.ai/api/workflows/{workflow-id}/executions/c7a92e15-3f4b-4d8c-a1e6-9b0d5f2c8e74"
}
```
  </Tab>
  <Tab value="Check Status">
```bash
curl "https://sim.ai/api/workflows/{workflow-id}/executions/{executionId}?includeOutput=true" \
  -H "x-api-key: $SIM_API_KEY"
```
...
| `completed` | Finished successfully — `finalOutput` is populated when requested |
...
If an execution exceeds its time limit it is automatically marked as `failed`.
