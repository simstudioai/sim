...
A block never waits on a block it isn't downstream of. So when several blocks become ready at the same moment — all their dependencies finished — they run at the same time. Here, both agents are wired straight to Start, so they run side by side; there's nothing to configure.
