...
<Tabs items={['Approval Portal', 'API', 'Webhook']}>
  ...
  <Tab>
    ...
    - **Async mode** (`X-Execution-Mode: async` on the original execute call) — The resume dispatches execution to a background worker and returns immediately with `202`, including the resume attempt's `executionId` and `statusUrl` for polling:
    ...
    ```json
    {
      ...
      "statusUrl": "/api/workflows/<workflowId>/executions/<resumeExecutionId>"
    }
    ```
    ...
    ```bash
    GET /api/workflows/{workflowId}/executions/{resumeExecutionId}?includeOutput=true
    X-API-Key: your-api-key
    ```
