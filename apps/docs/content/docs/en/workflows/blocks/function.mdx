...
Create and edit sandboxes in **Settings → Sandboxes**. Sandboxes require an
active Max or Enterprise plan, and only workspace admins can create or edit
them.
...
runs already in flight keep using the old one.
</Callout>
...
Selecting a sandbox whose build is not **Ready** fails the run with that reason
