...
| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `channel` | object | Detailed channel information |
|   ↳ `id` | string | Channel ID \(e.g., C1234567890\) |
|   ↳ `name` | string | Channel name without # prefix |
|   ↳ `is_channel` | boolean | Whether this is a channel |
|   ↳ `is_private` | boolean | Whether channel is private |
...
| `online` | boolean | Whether user has an active client connection |
| `autoAway` | boolean | Whether user was automatically set to away due to inactivity |
| `manualAway` | boolean | Whether user manually set themselves as away |
| `connectionCount` | number | Total number of active connections for the user |
| `lastActivity` | number | Unix timestamp of last detected activity |
