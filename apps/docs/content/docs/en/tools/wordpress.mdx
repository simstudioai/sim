...
| `applicationPassword` | string | Yes | WordPress Application Password |
...
Update an existing blog post in WordPress
...
Get a single blog post from WordPress by ID
...
List blog posts from WordPress with optional filters
...
Get a single page from WordPress by ID
...
List pages from WordPress with optional filters
...
Upload a media file (image, video, document) to WordPress
...
Get a single media item from WordPress by ID
...
List comments from WordPress with optional filters
...
Update a comment in WordPress (content or status)
...
| `parent` | number | No | Filter by parent category ID |
| `hideEmpty` | boolean | No | Hide categories with no posts |
| `orderBy` | string | No | Order by field: id, name, slug, count |
| `order` | string | No | Order direction: asc or desc |
...
| `hideEmpty` | boolean | No | Hide tags with no posts |
| `orderBy` | string | No | Order by field: id, name, slug, count |
| `order` | string | No | Order direction: asc or desc |
...
List users from WordPress (requires admin privileges)
...
| `orderBy` | string | No | Order by field: id, name, slug, email, registered_date |
| `order` | string | No | Order direction: asc or desc |
...
Get a specific user from WordPress by ID
...
Search across all content types in WordPress (posts, pages, media)
