---
title: ServiceNow
description: Create, read, update, delete, and bulk import ServiceNow records
---

import { BlockInfoCard } from "@/components/ui/block-info-card"

<BlockInfoCard 
  type="servicenow"
  color="#032D42"
/>

## Usage Instructions

Integrate ServiceNow into your workflow. Can create, read, update, and delete records in any ServiceNow table (incidents, tasks, users, etc.). Supports bulk import operations for data migration and ETL. Supports OAuth 2.0 (recommended) or Basic Auth.



## Tools

### `servicenow_create`

Create a new record in a ServiceNow table

#### Input

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `instanceUrl` | string | Yes | ServiceNow instance URL \(e.g., https://instance.service-now.com\) |
| `authMethod` | string | Yes | Authentication method \(oauth or basic\) |
| `credential` | string | No | ServiceNow OAuth credential ID |
| `username` | string | No | ServiceNow username \(Basic Auth\) |
| `password` | string | No | ServiceNow password \(Basic Auth\) |
| `tableName` | string | Yes | Table name \(e.g., incident, task, sys_user\) |
| `fields` | json | Yes | Fields to set on the record \(JSON object\) |

#### Output

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `record` | json | Created ServiceNow record with sys_id and other fields |
| `metadata` | json | Operation metadata |

### `servicenow_read`

Read records from a ServiceNow table

#### Input

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `instanceUrl` | string | No | ServiceNow instance URL \(auto-detected from OAuth if not provided\) |
| `authMethod` | string | No | Authentication method \(oauth or basic\) |
| `credential` | string | No | ServiceNow OAuth credential ID |
| `username` | string | No | ServiceNow username \(for Basic Auth\) |
| `password` | string | No | ServiceNow password \(for Basic Auth\) |
| `tableName` | string | Yes | Table name |
| `sysId` | string | No | Specific record sys_id |
| `number` | string | No | Record number \(e.g., INC0010001\) |
| `query` | string | No | Encoded query string \(e.g., "active=true^priority=1"\) |
| `limit` | number | No | Maximum number of records to return |
| `fields` | string | No | Comma-separated list of fields to return |

#### Output

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `records` | array | Array of ServiceNow records |
| `metadata` | json | Operation metadata |

### `servicenow_update`

Update an existing record in a ServiceNow table

#### Input

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `instanceUrl` | string | No | ServiceNow instance URL \(auto-detected from OAuth if not provided\) |
| `authMethod` | string | No | Authentication method \(oauth or basic\) |
| `credential` | string | No | ServiceNow OAuth credential ID |
| `username` | string | No | ServiceNow username \(for Basic Auth\) |
| `password` | string | No | ServiceNow password \(for Basic Auth\) |
| `tableName` | string | Yes | Table name |
| `sysId` | string | Yes | Record sys_id to update |
| `fields` | json | Yes | Fields to update \(JSON object\) |

#### Output

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `record` | json | Updated ServiceNow record |
| `metadata` | json | Operation metadata |

### `servicenow_delete`

Delete a record from a ServiceNow table

#### Input

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `instanceUrl` | string | No | ServiceNow instance URL \(auto-detected from OAuth if not provided\) |
| `authMethod` | string | No | Authentication method \(oauth or basic\) |
| `credential` | string | No | ServiceNow OAuth credential ID |
| `username` | string | No | ServiceNow username \(for Basic Auth\) |
| `password` | string | No | ServiceNow password \(for Basic Auth\) |
| `tableName` | string | Yes | Table name |
| `sysId` | string | Yes | Record sys_id to delete |

#### Output

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `success` | boolean | Whether the deletion was successful |
| `metadata` | json | Operation metadata |

### `servicenow_import_set`

Bulk import data into ServiceNow using Import Set API

#### Input

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `instanceUrl` | string | No | ServiceNow instance URL \(auto-detected from OAuth if not provided\) |
| `authMethod` | string | Yes | Authentication method \(oauth or basic\) |
| `credential` | string | No | ServiceNow OAuth credential ID |
| `username` | string | No | ServiceNow username \(Basic Auth\) |
| `password` | string | No | ServiceNow password \(Basic Auth\) |
| `tableName` | string | Yes | Import set table name \(e.g., u_my_import_set\) or target table name for direct import |
| `records` | array | Yes | Array of records to import. Each record should be a JSON object with field values. |
| `transformMap` | string | No | Transform map sys_id to use for data transformation \(optional\) |
| `batchSize` | number | No | Number of records to import per batch \(default: all records in one batch\) |
| `importSetId` | string | No | Existing import set sys_id to add records to \(optional\) |

#### Output

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `importSetId` | string | Import set sys_id that was created or used |
| `records` | array | Array of imported records with their status |
| `metadata` | json | Import metadata including counts of inserted, updated, ignored, and error records |



## Notes

- Category: `tools`
- Type: `servicenow`
