...
Check the health status of a Grafana instance
...
| --------- | ---- | ----------- |
| `commit` | string | Git commit hash of the Grafana build |
| `database` | string | Database connection status \(ok or failing\) |
| `version` | string | Grafana version number |
...
| `baseUrl` | string | Yes | Grafana instance URL \(e.g., https://your-grafana.com\) |
| `organizationId` | string | No | Organization ID for multi-org Grafana instances |
| `dataSourceId` | string | Yes | The ID or UID of the data source to check |
