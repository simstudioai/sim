...
| `reason` | string | Yes | Cancellation reason \(CUSTOMER, FRAUD, INVENTORY, DECLINED, OTHER\) |
...
| `query` | string | No | Search query to filter customers \(e.g., "email:*@example.com" or "tag:vip" or "orders_count:&gt;5"\) |
