...
| `external_url` | string | Spotify URL |
| `image_url` | string | Cover/profile image URL |
| `popularity` | number | Popularity score \(0-100\) |
| `album` | json | Album information |
| `duration_ms` | number | Duration in milliseconds |
| `explicit` | boolean | Contains explicit content |
| `preview_url` | string | URL to 30-second preview |
| `album_type` | string | Album type \(album, single, compilation\) |
| `release_date` | string | Release date |
| `label` | string | Record label |
| `total_tracks` | number | Total tracks |
| `genres` | json | List of genres |
| `followers` | number | Number of followers |
| `description` | string | Description |
| `owner` | json | Playlist owner information |
| `public` | boolean | Whether playlist is public |
| `collaborative` | boolean | Whether playlist is collaborative |
| `snapshot_id` | string | Playlist version snapshot ID |
| `publisher` | string | Publisher name |
| `total_episodes` | number | Total episodes in show |
| `shows` | json | List of shows/podcasts |
| `languages` | json | List of languages |
| `show` | json | Parent show information |
| `episodes` | json | List of episodes |
| `authors` | json | List of authors |
| `narrators` | json | List of narrators |
| `total_chapters` | number | Total chapters |
| `audiobooks` | json | List of audiobooks |
| `chapters` | json | List of chapters |
...
| `product` | string | Subscription level \(free, premium\) |
| `is_playing` | boolean | Whether playback is active |
| `device` | json | Active device information |
| `devices` | json | Available playback devices |
| `progress_ms` | number | Current playback position in ms |
| `currently_playing_type` | string | Type of content \(track, episode, ad\) |
| `shuffle_state` | boolean | Whether shuffle is enabled |
| `repeat_state` | string | Repeat mode \(off, track, context\) |
| `track` | json | Currently playing track |
| `currently_playing` | json | Currently playing item |
| `queue` | json | Upcoming tracks in queue |
| `items` | json | List of recently played items |
| `categories` | json | List of browse categories |
| `markets` | json | List of available market codes |
| `results` | json | Check operation results \(id and saved boolean\) |
| `all_saved` | boolean | Whether all tracks are saved |
| `none_saved` | boolean | Whether no tracks are saved |
