...
| `resolution` | string | No | Video resolution: 720p, 1080p, or 4k \(default: 1080p\) |
| `visualReference` | json | No | Reference image for maintaining visual consistency \(UserFile object\) |
| `consistencyMode` | string | No | Consistency mode: character, object, style, or location |
| `stylePreset` | string | No | Style preset for consistent visual style |
...
| `prompt` | string | Yes | Text prompt describing the video to generate |
| `duration` | number | No | Video duration in seconds \(5 or 8, default: 5, max: 8\) |
| `aspectRatio` | string | No | Aspect ratio: 16:9 \(landscape\), 9:16 \(portrait\), or 1:1 \(square\) |
...
| `cameraControl` | json | No | Camera controls: \{ pan: number, zoom: number, tilt: number, truck: number, tracking: boolean \} |
| `controls` | string | No | No description |
...
Generate videos using MiniMax Hailuo 2.3 with advanced realism and camera control
...
| `apiKey` | string | Yes | Fal.ai API key \(for MiniMax Hailuo\) |
| `model` | string | No | MiniMax model: hailuo-02 \(default\) |
| `endpoint` | string | No | Endpoint quality: pro \(highest quality\), standard \(balanced\), or fast \(quickest\) |
| `prompt` | string | Yes | Text prompt describing the video to generate |
