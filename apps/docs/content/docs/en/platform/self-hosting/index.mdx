...
<Cards>
  ...
  <Card title="Backup & Restore" href="/platform/self-hosting/backup-restore">
    What to back up, and the key that makes a backup useless if lost
  </Card>
</Cards>

The sidebar covers the rest: architecture, email, Redis, authentication, background jobs, networking, security, scaling, observability, upgrades, and air-gapped deployments.
...
If your deployment has no outbound internet access, read [Air-Gapped Deployments](/platform/self-hosting/air-gapped) first — it lists what still works and what does not.
