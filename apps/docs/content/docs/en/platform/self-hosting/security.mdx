...
<Callout type="error">
  `ENCRYPTION_KEY` cannot be rotated without re-encrypting the data it protects, and cannot be recovered if lost. Changing it renders all of that data permanently unreadable. Back it up independently of the database — see [Backup & Restore](/platform/self-hosting/backup-restore).
</Callout>
