...
Integrations show as connected but fail, or provider keys error on decrypt. `ENCRYPTION_KEY` does not match the value in use when the backup was taken. There is no recovery — the key must be restored. See [Backup & Restore](/platform/self-hosting/backup-restore).
...
Either the tag does not exist in the registry (`helm get values sim` and check), or you are pulling from a private registry without `global.imagePullSecrets`. In an air-gapped setup, remember `global.useRegistryForAllImages: true` — otherwise third-party images still point at Docker Hub. See [Air-Gapped](/platform/self-hosting/air-gapped).
