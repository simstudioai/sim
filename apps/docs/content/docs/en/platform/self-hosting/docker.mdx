...
This stack needs the same `.env` as the production one — it refuses to start without `BETTER_AUTH_SECRET`, `ENCRYPTION_KEY`, and `INTERNAL_API_SECRET`. Create it as shown in [Quick Start](#quick-start) first.
