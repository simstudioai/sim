...
Sim publishes images to GHCR, tagged by release (`v0.7.44`) alongside `latest`:
...
One variable drives all three schema-coupled images, so they cannot drift apart:
...
Unset, it falls back to `latest`. Pin it in production.

The `cron` service is deliberately excluded: it only makes HTTP calls and shares no schema, so it tracks `latest` unless you pin `SIM_CRON_VERSION`.
...
See [Backup & Restore](/platform/self-hosting/backup-restore).
