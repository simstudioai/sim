...
| **Organization** | Every current and future workspace in the organization | Organization **admin** or **owner** | Enterprise on Sim Cloud |
...
If your organization's Enterprise plan lapses, organization keys stop applying and those workspaces fall back to Sim's hosted keys with the multiplier. The keys are retained, and organization admins can still delete them, but adding or updating them requires an active Enterprise plan.
