...
<Callout type="info">
Custom blocks are an Enterprise feature. Publishing requires the Enterprise plan and workspace admin access.
</Callout>
...
- **Be a workspace admin.** Publishing and managing custom blocks requires admin access on an Enterprise workspace.
...
<FAQ items={[
  {
    ...
    answer: "Workspace admins on an Enterprise-entitled workspace. On Sim Cloud this requires the Enterprise plan."
  },
  {
...
Self-hosted deployments enable custom blocks with environment variables instead of the billing/plan check.

```bash
DEPLOY_AS_BLOCK=true
NEXT_PUBLIC_CUSTOM_BLOCKS_ENABLED=true
```

Once enabled, publish and manage blocks through **Settings → Enterprise → Custom blocks** the same way.
