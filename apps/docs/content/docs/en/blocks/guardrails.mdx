...
Checks that the content parses as valid JSON. Use it before a [Function](/blocks/function) or downstream block reads a model's structured output.
...
The input to check. Usually an earlier output like `<agent.content>`, `<function.result>`, or an [API](/blocks/api) response.
...
- **Branch on the result.** Read `<guardrails.passed>` in a [Condition](/blocks/condition) to route valid and invalid content down different paths.
