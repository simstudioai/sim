...
How approvers are alerted. Send to Slack, Gmail, Microsoft Teams, SMS (via Twilio), or a custom webhook. Include the approval URL (`<blockId.url>`) in the message so they can open the portal.
