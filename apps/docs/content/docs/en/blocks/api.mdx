...
Branch on the result by reading `<api.status>` in a [Condition](/blocks/condition) block.
...
- **Handle failures.** Read `<api.status>` and branch on it with a [Condition](/blocks/condition), or connect the error path for network failures and timeouts.
