...
| ----- | ---- | -------- | ----------- |
| `contentType` | dropdown | Yes | Format GitHub will use when sending the webhook payload. |
| `webhookSecret` | short-input | No | Validates that webhook deliveries originate from GitHub. |
| `sslVerification` | dropdown | Yes | GitHub verifies SSL certificates when delivering webhooks. |
