...
| ----- | ---- | -------- | ----------- |
| `signingSecret` | short-input | Yes | The signing secret from your Slack app to validate request authenticity. |
| `botToken` | short-input | No | The bot token from your Slack app. Required for downloading files attached to messages. |
| `includeFiles` | switch | No | Download and include file attachments from messages. Requires a bot token with files:read scope. |
