...
| ----- | ---- | -------- | ----------- |
| `apiKey` | short-input | Yes | Required to create the webhook in Fathom. |
| `triggeredFor` | dropdown | No | Which recording types should trigger this webhook. |
| `includeSummary` | switch | No | Include the meeting summary in the webhook payload. |
| `includeTranscript` | switch | No | Include the full transcript in the webhook payload. |
| `includeActionItems` | switch | No | Include action items extracted from the meeting. |
| `includeCrmMatches` | switch | No | Include matched CRM contacts, companies, and deals from your linked CRM. |
