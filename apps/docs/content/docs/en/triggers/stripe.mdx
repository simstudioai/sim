...
| ----- | ---- | -------- | ----------- |
| `eventTypes` | dropdown | No | Select specific Stripe events to filter. Leave empty to receive all events from Stripe. |
| `webhookSecret` | short-input | No | Your webhook signing secret from Stripe Dashboard. Used to verify webhook authenticity. |
