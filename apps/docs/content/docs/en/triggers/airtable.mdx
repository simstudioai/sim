...
| ----- | ---- | -------- | ----------- |
| `triggerCredentials` | oauth-input | Yes | This trigger requires airtable credentials to access your account. |
| `baseId` | short-input | Yes | The ID of the Airtable Base this webhook will monitor. |
| `tableId` | short-input | Yes | The ID of the table within the Base that the webhook will monitor. |
| `includeCellValues` | switch | No | Enable to receive the complete record data in the payload, not just changes. |
