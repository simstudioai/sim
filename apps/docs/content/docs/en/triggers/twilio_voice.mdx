...
| ----- | ---- | -------- | ----------- |
| `accountSid` | short-input | Yes | Your Twilio Account SID from the Twilio Console |
| `authToken` | short-input | Yes | Your Twilio Auth Token for webhook signature verification |
| `twimlResponse` | long-input | No | TwiML instructions to return immediately to Twilio. Use square brackets instead of angle brackets \(e.g., \[Response\] instead of &lt;Response&gt;\). This controls what happens when the call comes in \(e.g., play a message, record, gather input\). Your workflow will execute in the background. |
