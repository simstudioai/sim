...
<Callout type="info">
A few constructs still can't round-trip: a `<br>` inside a table cell, a hard line break inside a heading, and non-canonical HTML entities such as `&copy;`. Files over 256 KB also open read-only. Footnotes, HTML comments, and raw HTML are preserved byte-for-byte and stay editable.
</Callout>
