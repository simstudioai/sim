...
| `content` | string | Message content from Discord webhook |
| `username` | string | Username of the sender \(if provided\) |
| `avatar_url` | string | Avatar URL of the sender \(if provided\) |
| `timestamp` | string | Timestamp when the webhook was triggered |
| `webhook_id` | string | Discord webhook identifier |
| `webhook_token` | string | Discord webhook token |
| `guild_id` | string | Discord server/guild ID |
| `channel_id` | string | Discord channel ID where the event occurred |
| `embeds` | string | Embedded content data \(if any\) |
