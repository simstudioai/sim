...
[Supabase](https://www.supabase.com/) is an open-source Firebase alternative offering a PostgreSQL database, authentication, instant APIs, real-time subscriptions, storage, and edge functions.

**In Sim Studio, you can use the Supabase block to:**
- Query, insert, update, or delete rows in your Supabase tables
- Configure operations with your Project ID, Table name, Service Role Secret, and (optionally) data or filters
- Use [PostgREST filter syntax](https://postgrest.org/en/stable/api.html#operators) for advanced queries

See the [block source code](https://github.com/your-org/simstudio/blob/main/apps/sim/blocks/blocks/supabase.ts) for details on supported operations and configuration.
{/* MANUAL-CONTENT-END */}
