...
Blocks executed within parallel instances must include a `parentId` reference to the actual parallel block ID:
...
child-block:
  ...
  parentId: parallel-count           # Actual parallel block ID (not block name)
```
