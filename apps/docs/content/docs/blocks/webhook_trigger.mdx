...
properties:
  ...
  name:
    type: string
    description: Display name for this webhook trigger
  inputs:
    type: object
    properties:
      webhookProvider:
  ...
  connections:
    ...
    properties:
      success:
        type: string
        description: Target block ID to execute when webhook is triggered
```
...
```yaml
connections:
  success: <string>                     # Target block ID to execute when webhook is triggered
```

### Examples

<Tabs items={['Generic Webhook', 'Slack Integration', 'GitHub Integration', 'Stripe Webhook']}>
  <Tab>
    ```yaml
    generic-webhook:
      type: webhook
  ...
  <Tab>
    ...
    github-webhook:
      ...
      connections:
        success: github-event-processor
    ```
  </Tab>
  <Tab>
    ```yaml
