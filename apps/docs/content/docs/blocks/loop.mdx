...
Blocks executed within a loop must include a `parentId` reference:
...
child-block:
  ...
  parentId: <loop-block-id>              # Reference to the parent loop block
```
...
<Tabs items={['Simple ForEach', 'ForEach Array', 'Nested Data']}>
  <Tab>
    ```yaml
    simple-loop:
      type: loop
    ...
    greeting-agent:
      ...
      name: "Greeting Agent"
      inputs:
        systemPrompt: "You are a friendly assistant. Create a simple greeting message that includes the current iteration number."
        userPrompt: "Create a greeting for iteration number <loop.currentItem> Make it unique and cheerful!"
        model: gpt-4o
        temperature: 0.7
