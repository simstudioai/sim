...
To reference outputs from other blocks, use the block name in lowercase with all spaces removed:
...
<Tabs items={['Simple Validation', 'Data Processing', 'API Integration', 'Calculations']}>
  <Tab>
    ```yaml
    input-validator:
      type: function
    ...
    calculator:
      ...
      inputs:
        code: |
          ...
          const discount = <agent.discount> || 0;
