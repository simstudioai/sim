...
<VideoPlaceholder eyebrow="Workflows" title="Deployment" src="https://nnjgp7vypgx4myuq.public.blob.vercel-storage.com/academy/workflows-deployments.mp4"
  captionsSrc="/captions/academy/workflows-deployments.en.vtt" className="mt-2" />
...
A deployment publishes a snapshot of the workflow. Requests use that version while you continue editing; publish a new version when the changes are ready, or roll back when needed.
...
- **Chat**, a hosted chat page with the access settings you configure; each message becomes the workflow's input.
