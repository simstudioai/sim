...
description: "Repeat the same steps over a collection: the Loop block runs them one item at a time, the Parallel block runs items in concurrent batches."
---
...
<VideoPlaceholder eyebrow="Workflows" title="Loops & Parallel" src="https://nnjgp7vypgx4myuq.public.blob.vercel-storage.com/academy/workflows-loops.mp4"
  captionsSrc="/captions/academy/workflows-loops.en.vtt" className="mt-2" />
...
<WhatYouWillLearn
  items={[
    ...
    {
      title: 'Parallel runs concurrent batches',
      body: 'The Parallel block runs independent items concurrently, using a configurable batch size.',
    },
  ]}
/>
...
<FAQ items={[
  { question: "What is the difference between Loop and Parallel?", answer: "Both repeat the blocks inside them over a collection. The Loop runs one item at a time in order; the Parallel block runs items concurrently in batches. The results collect the same way in both." },
