...
<VideoPlaceholder eyebrow="Workflows" title="Logs" src="https://nnjgp7vypgx4myuq.public.blob.vercel-storage.com/academy/workflows-logs.mp4"
  captionsSrc="/captions/academy/workflows-logs.en.vtt" className="mt-2" />
...
When a workflow runs, the run isn't a black box. Sim records the **trigger** that started it, every **block** in the order it executed, and for each block its **input**, its **output**, its **timing**, and an error message if it failed. Use these recorded fields to compare the run with what you expected.
