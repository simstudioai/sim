...
<VideoPlaceholder eyebrow="Agents" title="Agents" src="https://nnjgp7vypgx4myuq.public.blob.vercel-storage.com/academy/agents-intro.mp4"
  captionsSrc="/captions/academy/agents-intro.en.vtt" className="mt-2" />
...
The trigger, connections, and outputs work as described in [Workflows](/academy/workflows/intro). In this example, the Agent block classifies the incoming message:
...
Use separate Agent blocks when steps need different instructions, tools, or models. For example, one block can classify a request and another can draft a reply. Use a [subworkflow](/academy/workflows/subworkflows) when several workflows need the same sequence.
