...
Each teammate follows this step. Existing authorized accounts may already be connected. Its row shows your connection and indexing status.
...
After an admin approves Jira, a teammate can start from **Integrations → Jira → Connect**. To add another site or project scope later, use **More → Add projects**:
...
Teammates share the configured scope and do not choose projects again. For another site or scope, admins select **Add projects** in Jira settings; teammates use **Integrations → Jira → More → Add projects**.
