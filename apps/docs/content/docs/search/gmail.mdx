...
An admin opens **Settings → Sources**, selects **Manage** beside **Gmail**, then opens the configuration from **Advanced** or the source list. Select its **Settings** tab to change these options. Filters apply separately to each mailbox in the source. **Documents** shows indexed threads and **Sync history** shows recent runs.
