...
| --- | --- | --- |
| **Member accounts** | Each person connects their own Drive access. No domain-wide delegation is needed; your Google Workspace app policy may require administrator approval. | Connect their own Google Drive accounts. |
...
Join the Sim organization, then open **Integrations** and select **Connect** for Google Drive. Use the Google account matching your verified Sim email. Google Drive has one row even when the admin configures several folder selections; its row shows your connection and indexing status. Members do not re-enter the admin's folder filters.
...
<Image className="mx-auto h-auto w-full max-w-md" src="/static/search/google-drive-setup.png" alt="Google Drive setup with a selected service account, directory administrator email, and folder filters" width={515} height={541} />
...
Under **Service account**, choose **Add service account**, or select an existing service account. Paste the JSON key into **Add Google service account**, give it a name, and select **Add service account**. Sim returns you to the source form with that credential selected.
...
The crawl includes shared-drive files even if a selected user has never opened them. A selected user must be able to download each file and Sim must verify its permissions before showing it in Search; this also applies to shortcut targets. If a reader can download a file but cannot list its permissions, include its owner or another user who can read those permissions.
...
Use **Sync now** to request an update, **Pause syncing** to stop scheduled syncs, and **Resume syncing** to restart them. **Remove connection** deletes the configuration and its indexed documents from Sim; it does not delete Drive files. These actions belong to the admin connection page. Members use **Connect**, **Reconnect**, or **Disconnect** on their Integrations row.
