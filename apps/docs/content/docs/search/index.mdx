...
| **Administrator token / Non-admin token** (GitLab) | Connects one self-managed project. Administrator tokens refresh permissions from GitLab; non-admin tokens use uploaded CSV permissions. | Join with a verified email matching GitLab or the uploaded user mapping. No personal connection is needed. |
...
Open **Settings → Sources**, choose the integration, then select its site, project, repository, or account connection. Each row identifies its account method and shows its sync status.
