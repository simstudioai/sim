...
Accept your Sim workspace invitation and sign in. Use a verified Sim email that matches your account at the source. If your company uses SSO, follow its sign-in process; SSO provisioning alone does not grant workspace access.
...
Find the source under **Sources** and select **Connect account**. If it is missing, ask a workspace admin to add it.
...
Return to Search when the connection completes. Keep the original tab open; **Open again** resumes the connection if a popup was blocked or closed.
...
The source row shows indexing status and how many documents are available to you. Search for something you can already open in the source. The first sync may take time, especially for large accounts.
...
| Needs admin attention | Ask the workspace admin to inspect **Manage** for the source error. |

<Callout type="info">
  Your Sim workspace role does not override document access at the source. Connecting a different account or receiving a Search link does not share someone else's mailbox, private calendar, or restricted documents with you.
</Callout>
