...
GitHub Search indexes text files from a repository on `github.com`. A workspace admin chooses the repository, then each person connects their GitHub account. Installing the GitHub App alone does not connect your teammates.
...
As a Sim workspace admin, open **Search**, choose **Add source**, then **GitHub**.
...
**Document details** controls the metadata stored with results. Its defaults are suitable for most sources. Select **Create & Invite** to save the source.
