...
### Choose member accounts

An admin selects **Settings → Sources → Add source → Google Calendar**, then chooses **Sync using → Member accounts**.

</Step>
<Step>

### Choose calendars and save
...
Choose the date range and any optional filters, then select **Set up member accounts**. The default covers the previous and next 30 days. These filters apply separately to each connected member's access.
...
Each person joins the Sim organization, opens **Integrations**, selects **Connect** for Google Calendar, and authorizes the Google account matching their verified Sim email. Its row shows the connection and indexing status. Members use the calendars configured by the admin; they do not enter calendar IDs again.
