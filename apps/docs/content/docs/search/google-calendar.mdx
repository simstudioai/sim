...
An admin selects **Settings → Sources → Add source → Google Calendar**, chooses **Sync using → Member accounts**, sets any filters, and selects **Set up member accounts**. Each person joins the Sim organization, opens **Integrations**, selects **Connect** for Google Calendar, and authorizes the Google account matching their verified Sim email. Its row shows your connection and indexing status.
