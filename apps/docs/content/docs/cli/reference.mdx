...
| Option | Required | Description |
| --- | --- | --- |
| `--file-ids <value...>` | Yes | File identifiers to delete. (space-separated, or @path / @- with one value per line; @@value for a literal leading @). |
| `-y, --yes` | Yes | Confirm this operation. |
...
### sim tools files download

Download a direct tool output owned by the current user (OAuth login or personal API key required)

```bash
sim tools files download <fileId> [options]
```
...
| Argument | Required | Description |
| --- | --- | --- |
| `fileId` | Yes | The returned file.id whose file.context is "copilot", passed unchanged |
...
### sim workflows runs files download
