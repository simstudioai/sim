...
| `--expected-revision <value>` | No | Revision from a file text read, Get File Metadata, or an earlier write; the request is refused with `409` when the content moved on. |
