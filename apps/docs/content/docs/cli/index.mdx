...
By default `workflows run` executes the active deployment. Add `--manual` to run
the current saved state instead. Deploy from the editor, or with
`sim workflows deploy <id>`.
</Step>
