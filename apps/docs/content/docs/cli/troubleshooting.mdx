...
Most command errors print to stderr and exit `1`. `sim whoami` and
`sim workflows runs wait` use additional codes to distinguish outcomes; see
[Exit codes](/cli/scripting#exit-codes). The cases below explain failures whose
cause may not be obvious from the message.
...
API-key pairing sets the default workspace selected on its approval page.
After OAuth login, set one with `sim configure --set-workspace <id>`.
The default target does not restrict your credential: use `--workspace` to reach
another workspace that credential can access.
