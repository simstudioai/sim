...
sizes, reports `truncated` when detail is omitted, and omits binary fields and data
URLs. Use `--trace` or [output selectors](/cli/scripting#selecting-workflow-output)
when you need more detail.

`--summary` asks the server to omit the saved workflow snapshot, then formats the
remaining authorized log response locally; it still loads the trace. To omit
only the snapshot, use `--no-include-workflow-state`. HTTP and hosted MCP callers
can set `includeWorkflowState=false` on the log read.
