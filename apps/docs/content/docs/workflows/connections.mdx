...
<Tabs items={['Agent', 'API', 'Function', 'Condition', 'Router', 'Evaluator']}>
  ...
  <Tab>
    ```json
    {
      "content": "The original content being evaluated",
      "model": "claude-sonnet-4-6",
      "tokens": { "input": 120, "output": 85, "total": 205 },
      "metric1": 8.5,
      "metric2": 7.2
    ...
    `content` contains the original input that was evaluated. Each metric you defined appears as its own field with a numeric score.
  </Tab>
</Tabs>
