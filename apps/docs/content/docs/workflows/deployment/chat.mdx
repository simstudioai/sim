...
| **Include thinking** | When enabled, the hosted chat can show provider-exposed thinking. Requires the chat client to send `X-Sim-Stream-Protocol: agent-events-v1` (the hosted UI always does). Default is off. See [Agent stream events](/workflows/deployment/agent-events). |
...
<FAQ items={[
  ...
  { question: "Is there a message length limit?", answer: "The chat API accepts up to 1,000,000 characters in a message. The selected model's context window may impose a smaller practical limit." },
]} />
