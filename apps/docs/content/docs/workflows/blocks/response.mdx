...
<Callout type="info">
A [Webhook trigger](/workflows/triggers/webhook) acknowledges the request after queueing the run. Configure that trigger's response to customize the acknowledgement; a downstream Response block does not change it.
</Callout>
