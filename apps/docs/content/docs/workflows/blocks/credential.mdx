...
<FAQ items={[
  { question: "Does the Credential block expose my secret or token?", answer: "No. Its outputs contain credential references and account metadata. Downstream blocks resolve the authorized token during execution. The Credential block does not return OAuth tokens or MCP client secrets." },
  ...
  { question: "Can a workflow use another person's organization account?", answer: "Yes. Every authorized workflow in an allowed workspace can use every active contribution in the organization pool. The organization admin controls the workspace allowlist. Chat continues to use each person's own connections." },
