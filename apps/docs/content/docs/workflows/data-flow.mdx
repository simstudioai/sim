...
Most blocks need a value that an earlier block produced. A block gets it by referencing that block's output by name.
...
<Callout type="info">
A block's output is not the same as the **workflow's** output. Every block has its own, kept under its own name. By default, the workflow returns its last block's output. A [Response block](/workflows/blocks/response) or a deployment's selected outputs can shape what the caller receives.
</Callout>
...
See [connection tags](/workflows/connections) for reference syntax, and [Logs](/logs-debugging) to inspect the values a block received.
