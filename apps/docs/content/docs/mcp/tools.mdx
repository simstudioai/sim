...
This guide covers the hosted [Sim MCP server](/mcp). A
[workflow MCP deployment](/workflows/deployment/mcp) exposes selected workflows,
while [external MCP tools](/agents/mcp) are called from inside Sim.
...
| `body` | The JSON request body; read-only searches and queries can also require one |
...
If the tools do not appear after sign-in, use the staged
[connection checks and read-only smoke test](/mcp/troubleshooting).
