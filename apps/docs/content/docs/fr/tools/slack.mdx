...
Intégrez Slack dans le flux de travail. Peut envoyer des messages, créer des canevas et lire des messages. Nécessite un jeton Bot au lieu d'OAuth en mode avancé. Peut être utilisé en mode déclencheur pour lancer un flux de travail lorsqu'un message est envoyé à un canal.
