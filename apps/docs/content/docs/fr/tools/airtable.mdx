...
Intègre Airtable dans le flux de travail. Peut créer, obtenir, lister ou mettre à jour des enregistrements Airtable. Peut être utilisé en mode déclencheur pour lancer un flux de travail lorsqu'une mise à jour est effectuée dans une table Airtable.
