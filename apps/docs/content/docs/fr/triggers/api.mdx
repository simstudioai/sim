...
|----------|-------------|------|
| `<api.fieldName[0].url>` | URL de téléchargement signée | string |
| `<api.fieldName[0].name>` | Nom de fichier original | string |
| `<api.fieldName[0].size>` | Taille du fichier en octets | number |
| `<api.fieldName[0].type>` | Type MIME | string |
| `<api.fieldName[0].uploadedAt>` | Horodatage du téléchargement (ISO 8601) | string |
| `<api.fieldName[0].expiresAt>` | Horodatage d'expiration de l'URL (ISO 8601) | string |
