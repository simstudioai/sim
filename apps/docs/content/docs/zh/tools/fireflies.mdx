...
[Fireflies.ai](https://fireflies.ai/) 是一款会议转录与智能平台，可与 Sim 集成，让您的代理可以通过零代码自动化，直接处理会议录音、转录和洞察。

Fireflies 在 Sim 中的集成提供了以下工具：

- **列出会议转录：** 为您的团队或账户获取多个会议及其摘要信息。
- **获取完整转录详情：** 访问详细转录内容，包括摘要、行动项、主题和与会者分析。
- **上传音频或视频：** 上传音频/视频文件或提供 URL 进行转录——可选设置语言、标题、与会者，并自动接收会议笔记。
- **搜索转录：** 通过关键词、参与者、主持人或时间范围查找会议，快速定位相关讨论。
- **删除转录：** 从您的 Fireflies 工作区中移除特定会议转录。
- **创建音频片段（Bites）：** 从转录中提取并高亮关键时刻，生成音频或视频片段。
- **转录完成时触发工作流：** 利用提供的 webhook 触发器，在 Fireflies 会议转录完成后自动激活 Sim 工作流，实现基于新会议数据的实时自动化和通知。

结合这些功能，您可以简化会后操作，提取结构化洞察，自动发送通知，管理录音，并围绕组织内的通话编排自定义工作流——所有操作均可安全地使用您的 API key 和 Fireflies 凭证完成。
{/* MANUAL-CONTENT-END */}
...
| `attendees` | string | 否 | 以 JSON 格式的与会者：\[\{"displayName": "Name", "email": "email@example.com"\}\] |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | 是 | Fireflies API key |
| `transcriptId` | string | 是 | 要从中创建片段的转录文本 ID |
...
| --------- | ---- | -------- | ----------- |
| `apiKey` | string | 是 | Fireflies API key |
| `transcriptId` | string | 否 | 按特定转录文本筛选片段 |
| `mine` | boolean | 否 | 仅返回 API key 拥有者拥有的片段（默认：true） |
| `limit` | number | 否 | 返回的片段数量上限（最多 50 个） |
