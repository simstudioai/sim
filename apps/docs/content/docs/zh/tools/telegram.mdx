...
将 Telegram 集成到工作流程中。可以发送和删除消息。可以在触发模式下使用，当消息发送到聊天时触发工作流程。
...
| `botToken` | string | 是 | 您的 Telegram Bot API 令牌 |
| `chatId` | string | 是 | 目标 Telegram 聊天 ID |
| `photo` | string | 是 | 要发送的照片。传递 file_id 或 HTTP URL |
| `caption` | string | 否 | 照片标题（可选） |
...
| --------- | ---- | ----------- |
| `message` | string | 成功或错误消息 |
| `data` | object | 包括可选照片的 Telegram 消息数据 |

### `telegram_send_video`

通过 Telegram Bot API 将视频发送到 Telegram 频道或用户。
...
| `botToken` | string | 是 | 您的 Telegram Bot API 令牌 |
| `chatId` | string | 是 | 目标 Telegram 聊天 ID |
| `video` | string | 是 | 要发送的视频。传递 file_id 或 HTTP URL |
| `caption` | string | 否 | 视频标题（可选） |
...
通过 Telegram Bot API 将音频文件发送到 Telegram 频道或用户。
...
| `botToken` | string | 是 | 您的 Telegram Bot API 令牌 |
| `chatId` | string | 是 | 目标 Telegram 聊天 ID |
| `animation` | string | 是 | 要发送的动画。传递一个 file_id 或 HTTP URL |
