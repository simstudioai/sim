...
- **输入格式字段** — 每个您添加的字段都会作为 <code>&lt;start.fieldName&gt;</code> 提供。例如，`customerId` 字段会在下游模块和模板中显示为 <code>&lt;start.customerId&gt;</code>。
- **仅限聊天的字段** — 当工作流从聊天侧边栏或已部署的聊天体验中运行时，Sim 还会提供 <code>&lt;start.input&gt;</code>（最新的用户消息）、<code>&lt;start.conversationId&gt;</code>（活动会话 ID）和 <code>&lt;start.files&gt;</code>（聊天附件）。
...
根据您连接的模块，通过类似 <code>&lt;start.customerId&gt;</code> 的表达式引用下游的结构化值。
...
<Tabs items={['编辑器运行', '部署到 API', '部署到聊天']}>
  <Tab>
    <div className="space-y-3">
      <p>
        当您在编辑器中点击<strong>运行</strong>时，开始模块会将输入格式渲染为一个表单。默认值使您可以轻松地重新测试而无需重新输入数据。提交表单会立即触发工作流，这些值会在 <code>&lt;start.fieldName&gt;</code>（例如 <code>&lt;start.sampleField&gt;</code>）中可用。
      </p>
      <p>
        表单中的文件字段会直接上传到相应的{' '}
        <code>&lt;start.fieldName&gt;</code>；您可以使用这些值为下游工具或存储步骤提供数据。
      </p>
    </div>
  </Tab>
  <Tab>
    <div className="space-y-3">
      ...
      <p>
        API 调用者可以包含其他可选属性。这些属性会保留在 <code>&lt;start.fieldName&gt;</code> 输出中，因此您可以在不立即重新部署的情况下进行实验。
      </p>
    </div>
  </Tab>
  <Tab>
    <div className="space-y-3">
      <p>
        在聊天部署中，开始模块会绑定到活动会话。最新消息会填充 <code>&lt;start.input&gt;</code>，会话标识符可通过 <code>&lt;start.conversationId&gt;</code> 获取，用户附件会显示在 <code>&lt;start.files&gt;</code> 上，以及任何范围为 <code>&lt;start.fieldName&gt;</code> 的输入格式字段。
      </p>
      <p>
        如果您通过附加结构化上下文（例如嵌入）启动聊天，它会合并到相应的 <code>&lt;start.fieldName&gt;</code> 输出中，从而使下游模块与 API 和手动运行保持一致。
      </p>
    </div>
  </Tab>
</Tabs>
