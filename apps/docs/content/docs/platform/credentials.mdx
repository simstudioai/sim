...
By default, a secret's resolved value is masked everywhere Sim shows run output (see [Execution log protection](#execution-log-protection)). For values that aren't actually sensitive — a non-secret shared base URL — that masking makes your own logs harder to read.
...
| **Use in workflows and code** | Workspace admins and members with an active Credential Member or Credential Admin grant | Only you can use |
