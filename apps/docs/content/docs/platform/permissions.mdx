...
- **Read and Write members need an active credential grant** to use a shared credential. Credential Members have use access; Credential Admins can also edit, delete, and share it. You are an admin of credentials you create.
...
<FAQ items={[
  ...
  { question: "What happens when a personal environment variable has the same name as a workspace variable?", answer: "Among values the execution actor can use, the workspace variable takes precedence. An inaccessible workspace variable does not shadow an authorized personal value." },
