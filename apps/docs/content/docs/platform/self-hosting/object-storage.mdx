...
A **partially** configured backend does not fall through to the next one, and does not fall back to local disk. If no other backend is fully configured, startup fails with `File storage is partially or incorrectly configured`. `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` are checked as a pair — set both or neither.
