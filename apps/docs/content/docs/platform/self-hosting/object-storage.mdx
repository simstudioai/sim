...
Sim separates files into purpose-specific buckets. **Sim never creates buckets** — create each one yourself before configuring it. **No S3 bucket variable falls back to `S3_BUCKET_NAME`**: every one you leave unset resolves to an empty bucket name and that file type fails to store, with the single exception of `S3_EXECUTION_FILES_BUCKET_NAME`, which falls back to the literal name `sim-execution-files` that you almost certainly do not own. Set all eight.
...
`AWS_REGION` and `S3_BUCKET_NAME` are what switch Sim into S3 mode, but they do not cover the other file types — set every bucket above, because none of them falls back to the general bucket.
...
| `S3_EXECUTION_FILES_BUCKET_NAME` | Workflow execution files. Falls back to the literal name `sim-execution-files`, which you almost certainly do not own — always set this explicitly | **Yes** |
...
| `S3_OG_IMAGES_BUCKET_NAME` | OpenGraph preview images | Optional (no fallback) |
| `S3_WORKSPACE_LOGOS_BUCKET_NAME` | Workspace logos | Optional (no fallback) |
...
Because files now live in S3, nothing depends on the container's `/app/uploads` directory any more.
...
<Callout type="warning">
  Set every container name. Like the S3 variables, none of them falls back to `AZURE_STORAGE_CONTAINER_NAME` — an unset one resolves to an empty container name and that file type fails to store. The one exception is `AZURE_STORAGE_EXECUTION_FILES_CONTAINER_NAME`, which falls back to the literal name `sim-execution-files`.
</Callout>
...
`GCS_BUCKET_NAME` is what switches Sim into GCS mode, but it does not cover the other file types: no purpose-specific bucket falls back to it, so an unset one resolves to an empty bucket name and that file type fails to store. Set all eight.
...
| `GCS_KB_BUCKET_NAME` | Knowledge base documents | **Yes**, to use knowledge bases (no fallback) |
| `GCS_EXECUTION_FILES_BUCKET_NAME` | Workflow execution files | **Yes**, to store execution outputs (no fallback) |
| `GCS_CHAT_BUCKET_NAME` | Deployed chat assets | **Yes**, to use deployed chats (no fallback) |
| `GCS_COPILOT_BUCKET_NAME` | Chat attachments | **Yes**, to attach files in Chat (no fallback) |
| `GCS_PROFILE_PICTURES_BUCKET_NAME` | User avatars | **Yes**, for avatar uploads (no fallback) |
| `GCS_OG_IMAGES_BUCKET_NAME` | OpenGraph preview images | Optional (no fallback) |
| `GCS_WORKSPACE_LOGOS_BUCKET_NAME` | Workspace logos | Optional (no fallback) |
