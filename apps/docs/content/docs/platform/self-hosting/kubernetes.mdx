...
<Callout type="warn">
  ...
  `CRON_SECRET` is required whenever background jobs are enabled, which is the chart's default: the chart will not render without it. Set `cronjobs.enabled=false` instead if you deliberately want no scheduled jobs.
</Callout>
