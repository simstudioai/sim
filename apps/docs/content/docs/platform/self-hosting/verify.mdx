...
| **Schema** | Required keys are present, are not still the example placeholder, are valid URLs where they should be, and that each secret has a usable shape — `ENCRYPTION_KEY` and `API_ENCRYPTION_KEY` are checked as exactly 64 hex characters, the rest as a minimum length. `--fix` generates missing or placeholder secrets |
