...
Each table is what the OAuth flow requests, so it is what the app registration has to allow. A credential someone issues themselves instead (a service account, a pasted API token) carries its own access, which these do not govern.
...
| Connector | Provider ID | Scopes to grant |
| --- | --- | --- |
| Salesforce | `salesforce`<br />`salesforce-sandbox` | `api`, `refresh_token`, `openid` |
