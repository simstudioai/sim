...
| `lock_timeout` on DDL | 5 seconds — a statement that waits longer fails with SQLSTATE `55P03` |
| Migration retry attempts | 8, with exponential backoff and jitter |
| Connection retries at startup | 10, for transient failures only — `53300` (`too_many_connections`) and the `08xxx` connection-exception class. Anything else fails immediately |
