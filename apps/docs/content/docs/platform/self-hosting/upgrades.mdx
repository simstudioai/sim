...
| `lock_timeout` on the migration session | 5 seconds — a statement that waits longer fails with SQLSTATE `55P03`. `statement_timeout` is set to `0` on the same session, so a long migration is never cut off once it holds its locks |
