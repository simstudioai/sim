...
secret; it then authenticates with PKCE alone. The provider exposes
`offline_access`, `api:read`, and `api:write` for API authorization; it does not
expose OpenID Connect identity scopes or issue ID tokens. A confidential
client's secret is printed once and cannot be read back. Confidential clients
use `client_secret_basic`; the registration command prints the token
authentication method alongside the client ID.
