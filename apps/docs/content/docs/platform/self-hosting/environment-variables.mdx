...
`openssl rand -hex 32` prints 64 hex characters. `ENCRYPTION_KEY` and `API_ENCRYPTION_KEY` must be exactly that — the app rejects any other length. The rest are secrets of no fixed shape and only need 32 characters or more.
...
| `KB_OPENAI_MODEL_NAME` | Azure deployment name serving the knowledge base embedding model. Required on the Azure embedding path — the deployment name rarely matches the model id |
...
| Variable | Description |
|----------|-------------|
| `OCR_PROVIDER` | PDF OCR backend: `local`, `mistral`, or `azure-mistral`. Legacy installs infer it from the configured credentials |
...
Who may sign up and sign in — the allowlists, blocklists, `DISABLE_REGISTRATION`, `DISABLE_INVITATIONS`, `DISABLE_PUBLIC_API`, and `DISABLE_AUTH` — is documented in [Authentication](/platform/self-hosting/authentication#controlling-who-can-sign-up).
...
| `SANDBOXES_ENABLED` / `NEXT_PUBLIC_SANDBOXES_ENABLED` | Enable custom sandboxes on a self-hosted deployment, bypassing the hosted-plan requirement. Set both — the server-side one grants access, the `NEXT_PUBLIC_` twin renders the UI |
...
| `KB_OPENAI_MODEL_NAME` | Azure deployment name for that model, when embeddings run through Azure OpenAI |
