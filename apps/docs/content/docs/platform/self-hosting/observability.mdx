...
When it runs, OpenTelemetry traces go to `https://telemetry.simstudio.ai/v1/traces`. What is collected: feature-usage statistics, error rates, performance metrics, and AI/LLM operation traces. What is **not** collected: personal information, workflow content or outputs, API keys or tokens, and IP addresses or geolocation.

`NEXT_TELEMETRY_DISABLED=1` is a hard off switch for the server's OpenTelemetry SDK, which returns before it initializes — so it stops the app's own OTLP export to any endpoint, not just the export to Sim. It does not affect Trigger.dev task telemetry, which is configured separately below. The **Settings → Privacy → Allow anonymous telemetry** toggle is browser-side only and does not stop server spans.
...
- **Kubernetes state** — pod restarts, `CrashLoopBackOff`, OOMKills, replica count vs desired, PVC utilization (kube-state-metrics). For a point-in-time read on resource pressure, `kubectl top pods -n simstudio` shows CPU and memory against the limits — the first thing to check when a pod is being OOMKilled.
