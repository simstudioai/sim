...
Whether anonymous telemetry runs depends on how you deploy.

| Deployment | Default | Why |
|---|---|---|
| Helm chart | **Off** | `app.envDefaults` sets `NEXT_TELEMETRY_DISABLED: "1"` |
| Docker Compose | **On** | Nothing sets the variable; the runtime image does not carry it |
| From source | **On** | Same |

When it runs, OpenTelemetry traces go to `https://telemetry.simstudio.ai/v1/traces`. What is collected: feature-usage statistics, error rates, performance metrics (sampled at 10%), and AI/LLM operation traces. What is **not** collected: personal information, workflow content or outputs, API keys or tokens, and IP addresses or geolocation.

`NEXT_TELEMETRY_DISABLED=1` is a hard off switch — the OpenTelemetry SDK returns before it initializes, so it disables *all* OTLP export, not just the export to Sim. Users can also opt out individually under **Settings → Privacy → Allow anonymous telemetry**.

<Callout type="warn">
  On Helm, `NEXT_TELEMETRY_DISABLED` is what you must **clear** before any tracing works — including tracing to your own collector. Override it with `null` to remove the key entirely; setting it to `""` does not remove it.
...
The endpoint is resolved by precedence, first match wins: `OTEL_EXPORTER_OTLP_ENDPOINT`, then `TELEMETRY_ENDPOINT`, then the endpoint in `apps/sim/telemetry.config.ts`. Sampling resolves the same way: `TELEMETRY_SAMPLING_RATIO`, then `OTEL_TRACES_SAMPLER_ARG`, defaulting to 1.0. Editing `telemetry.config.ts` only changes the last fallback, so an environment variable set anywhere in your stack silently wins over it.
...
### Unintended CloudWatch metrics

<Callout type="warn">
  Sim's hosted-key metrics recorder turns itself on when `AWS_ACCESS_KEY_ID` is present — nothing else gates it. A self-hoster who sets AWS credentials for S3 object storage silently starts sending `PutMetricData` calls to CloudWatch in the `Sim/HostedKey` namespace, in `AWS_REGION` (defaulting to `us-east-1`). That is outbound traffic you did not ask for and a CloudWatch bill line you did not plan for.

  If you use S3 but not CloudWatch, deny `cloudwatch:PutMetricData` on the IAM identity behind those credentials, or use a role or instance profile rather than static keys.
</Callout>
