...
server-side entitlement, and without it a workspace offers no Shell language and the
Sandboxes settings page shows an upgrade notice even when the provider is configured
correctly. JavaScript without `import` or `require`
