...
**Workflow covers the whole run, not just the base charge.** A workflow row includes three things: the models it called on Sim's hosted keys, any hosted-key tool calls, and the per-run base charge. The base charge applies to every run, including runs with no model calls.
...
<FAQ items={[
  ...
  },
  {
    ...
    answer: "The billing API exposes billing status and usage events; the [Billing CLI](/cli/billing) provides commands for those reads. Use this page's CSV export for the organization report shown in Settings."
  },
  {
