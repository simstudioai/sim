...
People can also open Sim straight from an OIDC identity provider's app dashboard, such as the Okta tile. Set the provider's **Initiate login URL**, shown on its page under **Sign-in**, as the app's initiate login URI in your identity provider. Sim starts sign-in through that provider without asking for an email, and only when the request comes from the provider's own issuer.
