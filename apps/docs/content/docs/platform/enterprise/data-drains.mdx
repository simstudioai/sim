...
Drains select new rows by their creation cursor and deliver them at least once. A failed run can deliver a row again, so consumers should deduplicate by its `id`. Mutable fields on **Chats** (messages, title, `lastSeenAt`) are a point-in-time snapshot and won't be re-emitted if the chat is later updated. Treat the export as append-only and reconstitute current state from your own system of record if you need it.
...
- **Dataset ID / Table ID** — must already exist; Sim does not create tables. The table schema must match the source row shape, with a compatible column for each top-level field. Sim sends those fields directly and sets `ignoreUnknownValues` to `false`; it does not wrap the row in a single JSON or STRING column.
...
<FAQ items={[
  ...
  },
  {
    ...
    answer: "Object-storage and webhook destinations receive NDJSON, one row per line. BigQuery, Snowflake, and Datadog use the destination-specific payloads described above."
  }
]} />
