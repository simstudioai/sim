/**
 * i18n HALF: Process only FIRST 1000 files
...
async function getFiles() {
  const result = execSync(
    `find apps/sim/app apps/sim/components -type f \\( -name "*.ts" -o -name "*.tsx" \\) ! -path "*/node_modules/*" | head -1000`,
...
async function main() {
  console.log('🚀 i18n HALF (first 1000 files)\n')
  ...
  let totalNew = 0
  const newTranslations: { key: string; ru: string; de: string }[] = []
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    for (const str of strings) {
      ...
      if (!catalog.ru[key]) {
        ...
        catalog.ru[key] = ru
        catalog.de[key] = de
        newTranslations.push({ key, ru, de })
        totalNew++

        if (totalNew % 5 === 0) {
          console.log(`✓ +${totalNew} new: "${str.substring(0, 50)}"`)
        }
      }
      totalExtracted++
    }
    ...
    if ((i + 1) % 100 === 0) {
      console.log(`   [${i + 1}/${files.length}] ${totalNew} new translations so far\n`)
    }
  }
  ...
  if (totalNew > 0) {
    ...
    newTranslations.slice(0, 5).forEach(t => {
      console.log(`   ${t.key}: RU="${t.ru}" DE="${t.de}"`)
    })
  }
}
