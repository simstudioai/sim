 ...
 * table, a knowledge base, a log, a scheduled task. A resource
 ...
 *
 * A resource kind with no canonical view yet (knowledge, log, schedule) is
 * simply absent from `CANONICAL_UNITS`. That is the correct state for an
 ...
 *   R3a public tree   an anonymous surface (`app/f`, `(landing)`, …)
 ...
 * make a build green.
 *
 * Measured against the working tree after the file resource was
...
const RESOURCE_POLICY_BASELINE = {
   ...
   * The resource surfaces (`app/f`, `app/(interfaces)`, `app/(shared)`, public
   * API routes) are all at 0. The 3 remaining are `(landing)/pricing`
   ...
   *
   * Re-based 30 → 38 against the current tree. Every offender is inherited —
   * none was introduced by the migration, which strictly REDUCED the count
   * relative to the tree it landed on. The old number was measured before that
 ...
 *
 * `Public*` is deliberately absent: `PublicFileView` is the share page's own
 * shell, and it constructs the axes and mounts the canonical view. That is the shape this rule wants, not the shape it bans.
 */
